...
class Mensaje0200_Consulta_Pagos(MessageDefinition):
    ...
    def createRequest(self):
        ...
        # numero de operacion
        operRequest.setOperationNumber(7595)
        ...
        #se comtrola que venga el campo 105
        if self.posMessage.getImpuestosData() != None:
            #impuestos
            impPerc = self.posMessage.getImpPerc()
            impuestoCambio = self.posMessage.getImpuestoCambio()
            impuestoPorcent = self.posMessage.getImpuestoPorcent()
            operRequest.addParameter("IMPUESTO_PERCEPCION", impPerc)
            operRequest.addParameter("TIPO_CAMBIO", impuestoCambio)
            operRequest.addParameter("PORCENTAJE_TOTAL", impuestoPorcent)
        ...
        operRequest.addParameter("IMPORTE_COMPRA", Double(importe)) # importe
