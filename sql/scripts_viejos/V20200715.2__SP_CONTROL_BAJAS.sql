DROP PROC SP_CONTROL_BAJAS
go


CREATE  PROC SP_CONTROL_BAJAS @campoIN AS VARCHAR(20), @valor AS VARCHAR(20), 
...
BEGIN
	...
	DECLARE cursor_diccionario cursor FOR
		...
		AND TABLA<>0 AND TABLA <> (SELECT tabla FROM DICCIONARIO WHERE NUMERODECAMPO=@campoIN)); 
