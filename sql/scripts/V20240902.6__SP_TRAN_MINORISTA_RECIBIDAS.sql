...
--		DECLARE @totContrAux VARCHAR(12);
	...
	BEGIN
      ...
      BEGIN
			...
			SELECT TOP 1 @nombreeo=substring(nombre_completo,1,70) FROM ITF_BCRA_PADFYJ WHERE CUIT=@cuit_EO AND TZ_LOCK=0
...
--	      PRINT @TipoCuenta
		  ...
		  IF @CODREGISTRO_IN='CCD' AND (SELECT TOP 1 PRODUCTO FROM SALDOS NOLOCK  WHERE CUENTA=@NUMcUENTA AND SUCURSAL=try_convert(NUMERIC(5,0),@sucursal_ri) AND TZ_LOCK=0) NOT IN (1,3,4,6,9,10,16,23)--(SELECT C6250 FROM productos NOLOCK WHERE TIPO_CUENTA_VISTA IN (7,11,5,8) AND TZ_LOCK=0)
			BEGIN 
	      		INSERT INTO dbo.VTA_TRANSFERENCIAS (OP_NUMERO, NUMERO_ASIENTO,  OP_FECHA_PRES, OP_CLASE_TRANS, ESTADO, OP_MONEDA, OP_TIPO, BEN_CBU, OP_IMPORTE, BEN_JTS_OID_CUENTA, BEN_NRO_DOC, MOTIVO_RECHAZO, OP_FORMA_ING, FECHA_ESTADO, CONTABILIZADA, OP_INFO_REF,OPERATORIA_BCRA, FCH_DEV_REC, TRACENUMBER 
	      													, ORD_NRO_DOC
											, ORD_NOMBRE
