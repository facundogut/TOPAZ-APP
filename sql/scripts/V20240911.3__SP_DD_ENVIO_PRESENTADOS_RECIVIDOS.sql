...
--	DECLARE @T_CUENTA NUMERIC(12,0);
	...
	SELECT ID_DEBITO
			...
			, identif_cliente
			, CUIT_EO
			, convert(VARCHAR(3),MOTIVO_RECHAZO)
			, PRESTACION
...
--		END
		...
		IF(@Excedente=0 OR @Excedente<99999999)
   		BEGIN
			...
			SET @RA_INFO_ADICIONAL = LEFT(CONCAT((SELECT TOP 1 descripcion FROM SNP_MOTIVOS_RECHAZO WHERE FUNCIONALIDAD='DD'AND CODIGO_NACHA=@T_MOTIVO_RECHAZO),REPLICATE(' ',44)),44);
