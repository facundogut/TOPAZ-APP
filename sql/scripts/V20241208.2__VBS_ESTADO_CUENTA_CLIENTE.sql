...
SELECT DISTINCT 
	...
	CASE 
	WHEN (PP.C2309+PP.C2310)>0 THEN pp.C2309 
	WHEN PPO.CAPITAL IS NULL THEN pp.C2309 
    ...
    CASE 
    	WHEN PP.TZ_LOCK <> 0 THEN PP.C2313 
    	WHEN (PP.C2309+PP.C2310) <> (PP.C2304+PP.C2305) AND (PP.C2309+PP.C2310) >0  THEN S.C1625 
    	WHEN (PP.C2309+PP.C2310) = 0  THEN PP.C2313 END AS FECHAVALOR,  
	CASE WHEN PP.TZ_LOCK <> 0 THEN 0 
		 ELSE
	...
	CASE 
 		 WHEN (PP.C2309+PP.C2310) <> (PP.C2304+PP.C2305) AND (PP.C2309+PP.C2310) >0
			THEN ''Pago Parcial''
		 WHEN PP.TZ_LOCK <> 0 OR ((PP.C2309+PP.C2310)=0 AND PP.C2301 > (SELECT FECHAPROCESO FROM PARAMETROS WITH (nolock))) THEN ''Pago Adelantada'' 
