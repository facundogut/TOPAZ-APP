...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE cursor1 CURSOR FOR 
			...
			WHERE C.TZ_LOCK = 0 AND VTA.TZ_LOCK = 0 AND S.TZ_LOCK=0
			...
			WHERE C.TZ_LOCK = 0 AND CRE.TZ_LOCK = 0 AND S.TZ_LOCK=0 AND P.TZ_LOCK=0
			UNION ALL -- Tarjetas
			SELECT  C.CODIGOCLIENTE,  C.CATEGORIA_COMERCIAL,
					CASE P.C6825
					WHEN ''C'' THEN 1
			...
			WHERE C.TZ_LOCK = 0 AND S.TZ_LOCK=0 AND P.TZ_LOCK=0
			UNION ALL -- Cuentas Corrientes Sobregiradas sin Acuerdos
			SELECT  C.CODIGOCLIENTE,  C.CATEGORIA_COMERCIAL,
					0 AS CATEGORIA 
			FROM CLI_CLIENTES C WITH (nolock)
			JOIN SALDOS S WITH (nolock) ON C.CODIGOCLIENTE = S.C1803 AND S.C1785=2
			JOIN PRODUCTOS P WITH (nolock) ON P.C6250=S.PRODUCTO
			WHERE S.C1604<0 AND S.C1683=0 AND C.TZ_LOCK = 0 AND S.TZ_LOCK=0 AND P.TZ_LOCK=0
