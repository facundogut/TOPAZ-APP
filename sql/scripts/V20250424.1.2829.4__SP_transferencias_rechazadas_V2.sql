EXECUTE('
CREATE OR ALTER   PROCEDURE [dbo].[SP_ITF_COELSA_TRANS_RECH_ENV_V2]
	@TICKET NUMERIC(16)
AS
...
--s	DECLARE @FL_SUMA_TOTAL_CREDITO_LOTE VARCHAR(12)=replicate(''0'',12); --cambio el largo a 12
	DECLARE @FL_SUMA_TOTAL_DEBITO_LOTE VARCHAR(20)=replicate(''0'',20); --cambio el largo a 20
	DECLARE @FL_SUMA_TOTAL_CREDITO_LOTE VARCHAR(20)=replicate(''0'',20); --cambio el largo a 20
...
--s		IF (@SumaImportes > 9999999999.99 OR @SumaEntidad > 999999 OR @FA_SUMA_ACUMULADA > 999999999999.99)
		...
		BEGIN
			...
			SET @FL_SUMA_TOTAL_DEBITO_LOTE  = RIGHT(replicate(''0'', 20), 20);
		   	SET @FL_SUMA_TOTAL_CREDITO_LOTE = RIGHT(concat(replicate(''0'', 20), (replace((SELECT sum(convert(bigint, substring(linea,26,14))) FROM VTA_TRANSFERENCIAS_TEMP WHERE LINEA LIKE ''6%'' AND id>(SELECT max(id) FROM VTA_TRANSFERENCIAS_TEMP WHERE LINEA LIKE ''5%'')), ''.'', ''''))), 20);  
			...
			SET @FA_SUMA_TOTAL_CREDITOS = RIGHT(concat(replicate(''0'', 20), (replace((SELECT sum(convert(bigint, substring(linea,26,14))) FROM VTA_TRANSFERENCIAS_TEMP WHERE LINEA LIKE ''6%'' AND id>(SELECT max(id) FROM VTA_TRANSFERENCIAS_TEMP WHERE LINEA LIKE ''1%'')), ''.'', ''''))), 20);  
