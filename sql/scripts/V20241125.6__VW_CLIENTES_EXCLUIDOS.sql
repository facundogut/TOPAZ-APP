...
WHEN GAR.GRADUACION IS NOT NULL THEN ''Tipo garantía''
ELSE ''No'' 
...
LEFT JOIN (SELECT GC.JTSOID_SALDO, ETG.GRADUACION
FROM CRE_GARANTIACREDITO GC WITH(NOLOCK) 
INNER JOIN CRE_GARANTIASRECIBIDAS GR WITH(NOLOCK) ON GR.NUM_GARANTIA = GC.NUM_GARANTIA AND GR.TZ_LOCK = 0
INNER JOIN CRE_EXCLUSIONES_TIPOSGARANTIAS ETG WITH(NOLOCK) ON ETG.TIPO_GARANTIA = GR.TIPOGARANTIA AND ETG.TZ_LOCK = 0
WHERE GC.DESVINCULADO = ''N'' AND GC.TZ_LOCK=0) GAR ON GAR.JTSOID_SALDO = A.JTS_OID
