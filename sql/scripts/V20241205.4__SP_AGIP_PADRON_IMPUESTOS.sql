...
BEGIN	
	...
	SET IIBB_CABA_PERCEPCION_COND = CASE WHEN convert(NUMERIC(11,7),replace(A.Alicuota_Percepcion,',','.'))=0 THEN 'EX' ELSE 'AC' END
		, IIBB_CABA_PERCEPCION_ALI = convert(NUMERIC(11,7),replace(A.Alicuota_Percepcion,',','.'))/100  
		, IIBB_CABA_RETENCION_COND = CASE WHEN convert(NUMERIC(11,7),replace(A.Alicuota_RETENCION,',','.'))=0 THEN 'EX' ELSE 'AC' END
		, IIBB_CABA_RETENCION_ALI = convert(NUMERIC(11,7),replace(A.Alicuota_Retencion,',','.'))/100
	FROM CLI_Personasfisicas PF
...
--	UPDATE clientes percepcion
	...
	JOIN (
    	...
    	FROM (
        	...
        	INNER JOIN VW_CLIENTES_PERSONAS v ON cf.NUMEROPERSONAfisica = v.numeropersona
        	--INNER JOIN VW_CLI_PERSONAS p ON cj.NUMEROPERSONAjuridica = p.[Numero de Persona]
			INNER JOIN ITF_AGIP_PADRON_IMPUESTOS_AUX A WITH (NOLOCK) ON v.numerodoc=a.CUIT
        	WHERE CF.TZ_LOCK = 0
        	        	AND v.estadopersona=0
        	AND v.estadocliente=0
        	AND (A.MSj_ERROR='' OR a.msj_error IS NULL)
          	AND (CF.IIBB_CABA_PERCEPCION_COND != 'NA')
          	AND CF.NUMEROPERSONAFISICA = (
              								...
              								WHERE x.codigocliente = v.codigocliente
              	              				...
              	              				AND x.NUMERODOC IN (SELECT cuit FROM ITF_AGIP_PADRON_IMPUESTOS_AUX)
        	...
        	INNER JOIN VW_CLIENTES_PERSONAS v ON cj.NUMEROPERSONAjuridica = v.numeropersona
        	--INNER JOIN VW_CLI_PERSONAS p ON cj.NUMEROPERSONAjuridica = p.[Numero de Persona]
			INNER JOIN ITF_AGIP_PADRON_IMPUESTOS_AUX A WITH (NOLOCK) ON v.numerodoc=a.CUIT
        	WHERE CJ.TZ_LOCK = 0
        	        	AND v.estadopersona=0
        	AND v.estadocliente=0
        	AND (A.MSj_ERROR='' OR a.msj_error IS NULL)
          	AND (CJ.IIBB_CABA_PERCEPCION_COND != 'NA' )
...
--	UPDATE clientes retencion
	...
	JOIN (
    	...
    	FROM (
        	...
        	AND v.estadocliente=0
        	AND (A.MSj_ERROR='' OR a.msj_error IS NULL)
          	AND (cf.IIBB_CABA_RETENCION_COND!='NA')
          	AND CF.NUMEROPERSONAFISICA = (
              							SELECT TOP 1 xx.numeropersonafisica
              							FROM CLI_PERSONASFISICAS xx
              							INNER JOIN VW_CLIENTES_PERSONAS x ON xx.numeropersonafisica = x.NUMEROPERSONA
        	...
        	AND v.estadocliente=0
        	AND (A.MSj_ERROR='' OR a.msj_error IS NULL)
          	AND (cj.IIBB_CABA_RETENCION_COND!='NA')
          	AND CJ.NUMEROPERSONAJURIDICA = (
              								SELECT TOP 1 xx.numeropersonajuridica
              								FROM CLI_PERSONASJURIDICAS xx
              								INNER JOIN VW_CLIENTES_PERSONAS x ON xx.numeropersonajuridica = x.NUMEROPERSONA
...
--								, t.letra_alicuota 
								...
								, TRY_CONVERT(NUMERIC(11,7),REPLACE(T.alicuota_percepcion,',','.'))/100
			FROM 
			(SELECT  DISTINCT p.numeropersona AS numeroPersona
								...
								, P.suc_alta
				FROM ITF_AGIP_PADRON_IMPUESTOS_AUX c WITH (NOLOCK)
...
--								, t.letra_alicuota 
								...
								, TRY_CONVERT(DATE,SUBSTRING(T.fecha_vig_desde, 5, 4) + '-' + SUBSTRING(T.fecha_vig_desde, 3, 2) + '-' + SUBSTRING(T.fecha_vig_desde, 1, 2), 120)
								, TRY_CONVERT(DATE,SUBSTRING(T.fecha_vig_HASTA, 5, 4) + '-' + SUBSTRING(T.fecha_vig_HASTA, 3, 2) + '-' + SUBSTRING(T.fecha_vig_HASTA, 1, 2), 120)
								, TRY_CONVERT(NUMERIC(11,7),REPLACE(T.alicuota_RETENCION,',','.'))/100
			FROM 
			(SELECT  DISTINCT p.numeropersona AS numeroPersona
								, c.cuit
								--, C.letra_alicuota
		...
		SELECT ISNULL((SELECT MAX(JTS_NOVEDAD) FROM CON_BITACORA_IMPUESTOS),0)+ (ROW_NUMBER() OVER (ORDER BY T.Persona))
	    	...
	    	FROM (SELECT DISTINCT CF.IIBB_CABA_PERCEPCION_ALI, CF.NUMEROPERSONAFISICA AS "PERSONA", CF.IIBB_CABA_RETENCION_ALI, v.codigocliente, A.fecha_vig_desde, A.fecha_vig_hasta,a.alicuota_percepcion,a.cuit, cf.suc_alta
					...
					INNER JOIN ITF_AGIP_PADRON_IMPUESTOS_AUX A WITH (NOLOCK) ON p.[Numero de Documento]=a.CUIT
	...
	--- ACTUALIZO PADRON-----
	DELETE FROM ITF_AGIP_PADRON WHERE Fecha_VIGENCIA_DESDE=(SELECT TOP 1 CONVERT(DATE, SUBSTRING(fecha_vig_desde,5,4)+'-' +SUBSTRING(fecha_vig_desde,3,2)+'-'+SUBSTRING(fecha_vig_desde,1,2))FROM ITF_AGIP_PADRON_IMPUESTOS_AUX WITH (NOLOCK) WHERE msj_error='' OR msj_error IS NULL)
   	...
   	SELECT try_CONVERT(DATE, SUBSTRING(fecha_vig_desde,5,4)+'-' +SUBSTRING(fecha_vig_desde,3,2)+'-'+SUBSTRING(fecha_vig_desde,1,2))
   			...
   			, Tipo_Contr_Insc
   			, Marca_alta_sujeto
   			, convert(NUMERIC(6,2),replace(Alicuota_Percepcion,',','.'))/100
   			, convert(NUMERIC(6,2),replace(Alicuota_Retencion,',','.'))/100   
   			, Nro_Grupo_Percepcion	
   			, Marca_alicuota
   			, Nro_Grupo_Retencion
   	FROM ITF_AGIP_PADRON_IMPUESTOS_AUX WITH (NOLOCK)
   	WHERE msj_error='' OR msj_error IS NULL
   	
   	IF (SELECT count(1) FROM ITF_AGIP_PADRON_IMPUESTOS_AUX WITH (NOLOCK) WHERE msj_error='' OR msj_error IS NULL)>0
   	BEGIN 
   		DECLARE @fechaLimite DATE =(SELECT TOP 1 DATEADD(MONTH, -6,CONVERT(DATE, SUBSTRING(fecha_public,5,4)+'-' +SUBSTRING(fecha_public,3,2)+'-'+SUBSTRING(fecha_public,1,2)))FROM ITF_AGIP_PADRON_IMPUESTOS_AUX WITH (NOLOCK))
