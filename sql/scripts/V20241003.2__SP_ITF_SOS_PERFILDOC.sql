...
FROM dbo.VW_CLIENTE_PFPJ T--dbo.CLI_CLIENTES  AS R WITH(NOLOCK)
  ...
  INNER JOIN dbo.CLI_PERSONASFISICAS U WITH(NOLOCK) ON U.NUMEROPERSONAFISICA = T.NUMEROPERSONA AND T.TIPO = ''F''
...
WHERE (A.TZ_LOCK < 100000000000000 OR A.TZ_LOCK >= 200000000000000) AND 
      ...
      NOT EXISTS(SELECT 1 FROM CLI_CLIENTES cli WITH(NOLOCK)
  				 ...
  				 WHERE cli.CODIGOCLIENTE = T.CODIGOCLIENTE
  				   ...
  				   AND p.TZ_LOCK = 0
  				   AND nat.TZ_LOCK = 0)
      AND (U.FALLECIDO = ''N'' OR (U.FALLECIDO = ''S''
          AND NOT EXISTS(SELECT 1 FROM CLI_DOCUMENTOSPFPJ V WITH(NOLOCK)
                         WHERE V.NUMERODOCUMENTO = T.NUMERODOCUMENTO AND V.TIPOPERSONA = ''J'')))
...
FROM dbo.VW_CLIENTE_PFPJ T --dbo.CLI_CLIENTES  AS R WITH(NOLOCK)
  ...
  INNER JOIN dbo.CLI_PERSONASJURIDICAS  AS S WITH(NOLOCK) ON S.NUMEROPERSONAJURIDICA = T.NUMEROPERSONA AND T.TIPO = ''J''
