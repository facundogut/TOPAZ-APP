...
PRINT @CL_NUMERO_LOTE
		...
		BEGIN	
			...
			IF (@t_ord_tipo_doc NOT IN (SELECT tipodocumento FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA=''F'')) OR @CL_RESERVADO_CL in (''002'',''007'')
			BEGIN
				SET @CL_NOM_EMPRESA =  LEFT(concat(@T_NOMBRE_ORDENANTE,replicate('' '', 16)),16);
				SET @CL_ID_EMPRESA = right(concat(replicate(''0'', 10),LEFT(@T_CUIT_ORD,10)),10)
			END
...
BEGIN
	...
	SELECT A.ORD_NRO_DOC, 
		   A.BEN_BANCO,
	   		...
	   		(CASE WHEN A.ORD_TIPO_DOC NOT IN (SELECT TIPODOCUMENTO FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA=''F'') or OP_TIPO=3  THEN SUBSTRING(CAST(A.ORD_NRO_DOC AS VARCHAR(11)),1,10) ELSE REPLICATE(''0'',10) END) AS CL_IDENTIFICACION_EMP_IND,
   			(CASE WHEN A.ORD_TIPO_DOC NOT IN (SELECT TIPODOCUMENTO FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA=''F'') or OP_TIPO=3  THEN SUBSTRING(CAST(A.ORD_NRO_DOC AS VARCHAR(11)),11,1) ELSE ''0'' END) AS CL_DIGITO_VERIFICADOR,
   			LEFT((CASE WHEN A.ORD_TIPO_DOC NOT IN (SELECT TIPODOCUMENTO FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA=''F'') or OP_TIPO=3 THEN ORD_NOMBRE ELSE ''PARTICULARES'' END) + REPLICATE('' '',16),16) AS CL_NOMBRE_EMPRESA_INDIVIDUO,	   		
...
Execute('IF NOT EXISTS (SELECT 1 
    FROM sys.indexes 
    WHERE name = ''ix_itf_agip_padron_impuestos_aux_CUIT'' 
    AND object_id = OBJECT_ID(''dbo.ITF_AGIP_PADRON_IMPUESTOS_AUX''))
