...
BEGIN
	...
	WHERE
		CE.LEGAL = ''''
		AND CE.PERIODO = ''P''
		AND CE.JTSOID = @SALDO_JTS_OID
		AND CD.FECHA_REAL_MOV BETWEEN @FECHA_DESDE AND @FECHA_HASTA
		
		
---- Elimino datos existentes de la tabla

DELETE FROM GRL_ECC_CREDEB WHERE SALDO_JTS_OID = @SALDO_JTS_OID AND FECHA_DESDE = @FECHA_DESDE AND FECHA_HASTA = @FECHA_HASTA

---- Grabo tabla

INSERT INTO GRL_ECC_CREDEB (SALDO_JTS_OID,FECHA_DESDE,FECHA_HASTA,NUMERICO,ALICUOTA,TASA_CREDITO_FISCAL,TOTAL_PERCIBIDO,CREDITO_COMPUTABLE)
SELECT
	SALDO_JTS_OID,
	FECHA_DESDE,
	FECHA_HASTA,
...
GROUP BY
	TASA,
	TASA_CREDITO_FISCAL, SALDO_JTS_OID, FECHA_DESDE, FECHA_HASTA
