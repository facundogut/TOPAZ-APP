...
PRINT 'ENTRA ADHESIONES'
		  		...
		  		SET 
									...
									, APELLIDO_NOMBRE=(SELECT TOP 1 p.NOMBRE FROM CLI_ClientePersona cli JOIN vw_personas_f_y_j p ON cli.NUMEROPERSONA=p.NUMEROPERSONA WHERE TITULARIDAD='T' AND CODIGOCLIENTE=@Cod_Cliente AND cli.TZ_LOCK=0) 
									--, PRODUCTO
   									--, SUCURSAL
									, CUENTA=@NumCuenta
									, MONEDA=@Moneda
...
PRINT @IdClientePegador
		  		...
		  		INSERT INTO dbo.SNP_ADHESIONES
							...
							VALUES
									...
									, (SELECT TOP 1 p.NOMBRE FROM CLI_ClientePersona cli JOIN vw_personas_f_y_j p ON cli.NUMEROPERSONA=p.NUMEROPERSONA WHERE TITULARIDAD='T' AND CODIGOCLIENTE=@Cod_Cliente AND cli.TZ_LOCK=0) 
									--, 52
									--, @VCod_Suc_Originante
									, @NumCuenta
									, @Moneda
