EXECUTE('
ALTER PROCEDURE dbo.SP_PA_REVISION_DE_TASAS_PREPARA
...
BEGIN
	SET @CANTIDAD_REGISTROS = (SELECT COUNT(*)
			...
			WHERE s.TZ_LOCK = 0  AND s.C1785 = 5
			  ...
			  AND NOT EXISTS (SELECT 1 FROM REVISION_TASAS WITH (nolock) WHERE SALDO_JTS_OID=s.JTS_OID AND ESTADO=''P'' AND 
...
--Sacar periodo
	...
	--De donde salen NUEVOS PUNTOS TEA
		UPDATE s 
		SET s.PUNTOSTASA= CASE WHEN (RTA.TEA_FINAL-RTA.TEA_BASE) > 0 THEN RTA.TEA_FINAL-RTA.TEA_BASE ELSE 0 END,
			s.C1631= (SELECT FECHAPROCESOANTERIOR FROM PARAMETROS WITH (nolock))
		FROM SALDOS s
