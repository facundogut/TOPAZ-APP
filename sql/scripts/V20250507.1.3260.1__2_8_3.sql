...
--s	DECLARE @PuntoIntercambio VARCHAR(16);
	...
	SELECT 
		@sumaDebitos_RI = ISNULL(sum(CAST(substring(LINEA, 61, 16) AS NUMERIC)), 0),
		@sumaEntidades_RI = ISNULL(sum(CAST(substring(LINEA, 4, 4) AS NUMERIC)), 0),
		@sumaSucursales_RI = ISNULL(sum(CAST(substring(LINEA, 8, 4) AS NUMERIC)), 0)
	FROM ITF_OTROS_CHEQUES_RESPUESTA_AUX
	WHERE LINEA LIKE ''626%'';
 
	SELECT 
		@sumaCreditos_RI = ISNULL(sum(CAST(substring(LINEA, 61, 16) AS NUMERIC)), 0),
		@sumaEntidades_RIaux = ISNULL(sum(CAST(substring(LINEA, 4, 4) AS NUMERIC)), 0),
		@sumaSucursales_RIaux = ISNULL(sum(CAST(substring(LINEA, 8, 4) AS NUMERIC)), 0)
	FROM ITF_OTROS_CHEQUES_RESPUESTA_AUX
...
--HASTA ACA--
					...
					END
					--Se comenta la actualizacion e inserción de la RRII_CHE_RECHAZADOS a pedido de Vives
					--Por el ticket CDS-3260 - A.E - 7/5/2025
