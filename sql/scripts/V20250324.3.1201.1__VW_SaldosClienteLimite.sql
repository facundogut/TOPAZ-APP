...
UNION
select	s.C1803 AS cliente, 
		((C1604*-1)+C1608) IMPORTE,
		s.C1600 AS CAPITAL_ORIGINAL,
		s.c1608 AS INTERESES,
		s.moneda as MONEDA,
		s.jts_oid AS SALDO_JTS_OID,
		p.c6283 as FAMILIAPROD,
...
INNER JOIN PLANCTAS pc WITH (NOLOCK) ON pc.C6326 = s.C1730
									and pc.tz_lock = 0									
WHERE s.C1785=6 AND S.C1604 < 0 AND (s.tz_lock = 0 	OR (s.TZ_LOCK > 099999999999999 AND s.TZ_LOCK < 199999999999999 ))
UNION 												
select	s.C1803 AS cliente, 
		s.C1604 IMPORTE,
		s.C1600 AS CAPITAL_ORIGINAL,
...
INNER JOIN PLANCTAS pc WITH (NOLOCK) ON pc.C6326 = s.C1730
									and pc.tz_lock = 0									
WHERE (s.C1785 = 1 AND p.C6800 = ''GO'') AND S.C1604 > 0 AND (s.tz_lock = 0 	OR (s.TZ_LOCK > 099999999999999 AND s.TZ_LOCK < 199999999999999 ))
UNION
SELECT	s.C1803 AS CLIENTE, 
		sum(v.IMPORTE) AS IMPORTE,
		sum(v.IMPORTE) AS  CAPITAL_ORIGINAL, 
		0 AS INTERES,
		s.MONEDA AS MONEDA,
		s.JTS_OID AS SALDO_JTS_OID, 
		p.C6283 as FAMILIAPROD, 
