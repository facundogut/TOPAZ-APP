...
--		END
		...
		IF(@Excedente=0 OR @Excedente<99999999)
   		BEGIN
			...
			SET @RA_INFO_ADICIONAL = LEFT(CONCAT((SELECT TOP 1 upper(descripcion) FROM SNP_MOTIVOS_RECHAZO WHERE FUNCIONALIDAD='DD'AND CODIGO_NACHA=@CODIGO_RECHAZO),REPLICATE(' ',44)),44);
