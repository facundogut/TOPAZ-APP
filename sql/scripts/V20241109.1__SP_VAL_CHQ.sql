...
BEGIN
    ...
    /* Se comenta hasta que la solucion de digitalizacion de cheque este pronta
      ...
      END
            ...
            ---R79---
       /* Validación 1: Saldo inexistente DPF */
	  IF (@Tipo=''D'')
	  BEGIN
	      DECLARE @V79 NUMERIC(3);

	      	SET @V79 = (SELECT COUNT(*) FROM SALDOS  S WHERE S.OPERACION=@NroCheque AND S.MONEDA=@Moneda AND S.c1785=4);
      ...
      BEGIN
      	...
      	BEGIN TRY
	      	INSERT INTO dbo.CLE_CHEQUES_CLEARING (CLIENTE
	      											, MONEDA
	      											, ORDINAL_LISTA
	      											, PRODUCTO
	      											, NUMERO_BANCO
	        ...
	        VALUES (@NroCLiente
	        		...
	        		, @CuentaDebitar
	        		, 1
	        		, (SELECT CONCAT( @Entidad, RIGHT(@EntidadDebitar, 3),RIGHT(@CodigoPostal,4),RIGHT(CONCAT(REPLICATE(''0'',8),RIGHT(@NroCheque, 8)),8), RIGHT(CONCAT(''00000000000'',RIGHT(@CuentaDebitar,11)),11) ))
	        		, @TraceNumber
	        		, 0
	        		) 
      	...
      	END TRY 
      	BEGIN CATCH
      		UPDATE dbo.ITF_COELSA_CHEQUES_PROPIOS SET ESTADO = ''E'' WHERE ID = @Id;
      	END CATCH
      END
      IF (@Tipo = ''D'')
      BEGIN
      	...
      	SELECT TOP 1 @DNroCLiente=S.C1803
      				, @DOrdinal=S.ORDINAL
      				, @DProducto=S.PRODUCTO 
      	FROM SALDOS  S 
      	WHERE S.OPERACION=@NroCheque AND S.MONEDA=@Moneda AND S.c1785=4
      	
      	BEGIN TRY 
	      	INSERT INTO dbo.CLE_DPF_RECIBIDO (TZ_LOCK
	      										, MONEDA
	      										, NUMERO_DPF
	      										, CLIENTE
	      										, NUMERICO_CUENTA
      ...
      IF (@Tipo = ''A'')
      BEGIN
      	--Obtendremos el numerador con un SP y se deja de
      	--usar la simulación - AE - 11/2024
      	DECLARE @Aordinal NUMERIC(10);
      	EXEC SP_GET_NUMERADOR_TOPAZ 35171,@Aordinal OUTPUT;      	
        /*
