...
(CASE WHEN mov.MONEDA = 1 THEN
        ...
        /*(CASE WHEN sa.C1785 = 4 AND mov.OPERACION = 5001 THEN 
				(SELECT CAPITALORIGINAL 
				 FROM BS_HISTORIA_PLAZO BS WITH (NOLOCK) 
				 WHERE BS.FECHAPROCESOMOV = mov.FECHAPROCESO 
				   AND BS.SUCURSALMOV = mov.SUCURSAL 
				   AND BS.NROASIENTOMOV = mov.ASIENTO
				   AND BS.TZ_LOCK = 0)
			  WHEN sa.C1785 = 4 AND mov.OPERACION IN (5017,8620) THEN
			    (SELECT SUM(CAPITALORIGINAL) - (SUM(CAPITALPAGADO) + SUM(INTERESPAGADO)) 
			     FROM BS_HISTORIA_PLAZO BS WITH (NOLOCK) 
			     WHERE BS.FECHAPROCESOMOV = mov.FECHAPROCESO 
			       AND BS.SUCURSALMOV = mov.SUCURSAL 
			       AND BS.NROASIENTOMOV = mov.ASIENTO
...
(CASE WHEN mov.MONEDA <> 1 THEN 
   mov.CAPITALREALIZADO
  /*(CASE WHEN sa.C1785 = 4 AND mov.OPERACION = 5001 THEN 
				(SELECT CAPITALORIGINAL FROM BS_HISTORIA_PLAZO BS WITH (NOLOCK) 
				 WHERE BS.FECHAPROCESOMOV = mov.FECHAPROCESO
				   AND BS.SUCURSALMOV = mov.SUCURSAL 
				   AND BS.NROASIENTOMOV = mov.ASIENTO
				   AND BS.TZ_LOCK = 0)
...
CASE WHEN sos.Subsistema IN (''CC'',''AC'') AND sos.ind_rte = ''S'' AND ((sos.MtoMinimo < mov.CAPITALREALIZADO AND sos.MtoMinimo <> 0) OR sos.MtoMinimo = 0)
     ...
     WHEN sos.TipOperacion IN (101,103) THEN (SELECT x.NUMERODOCUMENTO FROM VW_CLIENTE_PFPJ x WHERE x.CODIGOCLIENTE = mov.CLIENTE)
	ELSE '''' END
...
FROM MOVIMIENTOS_CONTABLES mov WITH (NOLOCK)
  ...
  INNER JOIN SALDOS sa WITH (NOLOCK) ON sa.C1785 in (''5'',''8'') 
										AND EXISTS(SELECT 1 FROM MOVIMIENTOS_CONTABLES movs WITH (NOLOCK)
															WHERE movs.FECHAPROCESO = mov.FECHAPROCESO
															  AND movs.SUCURSAL = mov.SUCURSAL
															  AND movs.ASIENTO = mov.ASIENTO
															  AND movs.OPERACION_CUENTA <> 0
															  AND movs.OPERACION_CUENTA = sa.OPERACION)
									    AND EXISTS(SELECT 1 FROM MOVIMIENTOS_CONTABLES movs WITH (NOLOCK)
															WHERE movs.FECHAPROCESO = mov.FECHAPROCESO
															  AND movs.SUCURSAL = mov.SUCURSAL
															  AND movs.ASIENTO = mov.ASIENTO
															  AND movs.OPERACION_CUENTA <> 0
															  AND movs.SALDO_JTS_OID = sa.JTS_OID)
...
FROM ITF_UNTD_IMPUESTOS mov WITH (NOLOCK)  
  ...
  INNER JOIN SALDOS SA WITH (NOLOCK) ON SA.CUENTA = cta.CUENTA AND SA.TZ_LOCK = 0
