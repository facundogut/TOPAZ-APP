...
SELECT 
		...
		cl.CATEGORIARESULTANTE AS Situacion,
		CASE WHEN ln.CUIT IS NOT NULL THEN ''S'' 
		ELSE ''N'' 
		END AS ListaNegra
...
INNER JOIN SALDOS s WITH (NOLOCK) ON vc.JTS_CV = s.JTS_OID 
									AND s.TZ_LOCK = 0 
									AND s.C1785 IN (2,3)
INNER JOIN CLI_CLIENTES cl WITH (NOLOCK) ON cl.CODIGOCLIENTE=s.c1803
...
LEFT JOIN CRE_LISTANEGRA ln WITH(NOLOCK) 	ON p.C6250=ln.PRODUCTO
											AND vc.ID_CONVENIO = ln.CONVENIO
											AND vc.ID_JURISDICCION = ln.JURISDICCION
											AND vc.DOCUMENTO = ln.CUIT
WHERE vc.TZ_LOCK = 0
