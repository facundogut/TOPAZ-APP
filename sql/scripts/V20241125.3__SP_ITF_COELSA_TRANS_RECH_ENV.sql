...
BEGIN
	...
	DECLARE @FA_TotalesCreditos NUMERIC(15) = 0;
	...
	SELECT 
			convert(VARCHAR,A.OP_FECHA_PRES, 12),
			A.ORD_NRO_DOC, 
			A.BEN_BANCO, 
			A.OP_CLASE_TRANS,
		...
		FROM 
		(
			SELECT DISTINCT VT1.OP_FECHA_PRES, VT1.ORD_NRO_DOC, VT1.BEN_BANCO,VT1.OP_CLASE_TRANS,VT1.ORD_NOMBRE
			FROM VTA_TRANSFERENCIAS VT1
			WHERE VT1.OP_CLASE_TRANS = 'R' and VT1.ESTADO='RC' and VT1.OP_MONEDA = 2 
				AND VT1.CONTABILIZADA<> 'R' AND VT1.TZ_LOCK=0
		) A
	OPEN CursorLote
	FETCH NEXT FROM CursorLote INTO @CL_FECHA_PRESENTACION,@CL_ORD_NRO_DOC, @CL_BEN_BANCO, @CL_OP_CLASE_TRANS, @CL_NUM_LOTE_CAB, @CL_IDENTIFICACION_EMP_IND, @CL_DIGITO_VERIFICADOR, @CL_NOMBRE_EMPRESA_INDIVIDUO 
