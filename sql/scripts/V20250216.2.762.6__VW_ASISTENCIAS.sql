...
WHERE ((s.C1785 IN (5,6) AND ext.tz_lock=0) OR (s.C1785 = 1 AND p.C6800 = ''GO'')) AND ( (s.tz_lock < 300000000000000 OR s.tz_lock >= 400000000000000) AND (s.tz_lock < 100000000000000 OR s.tz_lock >= 200000000000000)  )	
UNION
SELECT	s.SUCURSAL AS SUCURSAL, 
		suc.NOMBRESUCURSAL AS NOMBRESUCURSAL, 
