...
PRINT 'ENTRA ADHESIONES'
		  		...
		  		SET 
									DNI_CLIENTE_ADHERIDO=(SELECT TOP 1 NUMDOCFISICO FROM VW_PERSONAS_F_Y_J WHERE try_convert(NUMERIC, NroDocIdent)=@CUIT_EO)
									, APELLIDO_NOMBRE=(SELECT TOP 1 NOMBRE FROM VW_PERSONAS_F_Y_J WHERE try_convert(NUMERIC, NroDocIdent)=@CUIT_EO) 
									--, PRODUCTO
   									--, SUCURSAL
									, CUENTA=@NumCuenta
									, MONEDA=@Moneda
...
PRINT @IdClientePegador
		  		...
		  		INSERT INTO dbo.SNP_ADHESIONES
							...
							VALUES
									...
									0
									,  @Cod_Cliente
									, (SELECT TOP 1 NUMDOCFISICO FROM VW_PERSONAS_F_Y_J WHERE try_convert(NUMERIC, NroDocIdent)=@CUIT_EO)
									, (SELECT TOP 1 NOMBRE FROM VW_PERSONAS_F_Y_J WHERE try_convert(NUMERIC, NroDocIdent)=@CUIT_EO) 
									--, 52
									--, @VCod_Suc_Originante
									, @NumCuenta
									, @Moneda
