   ...
   BEGIN
      ...
      INSERT INTO @RegistrosBancarizados
		   	...
		   	JOIN GRL_ACREDITACIONES_MASIVAS am WITH (NOLOCK)
														ON a.ID=ISNULL(CAST(REPLACE(am.REFERENCIA_EXTERNA,'' '',0) AS numeric(15)), 0) 
		   WHERE am.CANAL = ''LOTE'' AND a.ESTADO=''R'' AND a.TZ_LOCK = 0 AND am.TZ_LOCK = 0;
      ...
      BEGIN
			...
			Lista_Cod_Err= (CASE WHEN R.ESTADO = ''P'' 
				...
				WHEN R.ESTADO = ''E'' AND R.COMENTARIO = ''SALDO DEBITO SIN DISPONIBLE'' AND R.Lista_Cod_Err<>''0''
					THEN concat(R.Lista_Cod_Err,'' '',''012'')
				WHEN R.ESTADO = ''E'' AND R.COMENTARIO = ''SALDO DEBITO SIN DISPONIBLE'' AND R.Lista_Cod_Err=''0''
					THEN ''012''					
					 ELSE '''' END)		
