	...
	------------Fin Primera Validacion ----------------
			...
			BEGIN	
				...
				BEGIN
					...
					AND OP_NUMERO=TRY_CONVERT(NUMERIC(12,0),LTRIM(RIGHT(@ReferenciaUnivoca,12)))
		...
		END 
	      ...
	      BEGIN
			...
			SET ESTADO = 'RC' 
				, FECHA_ESTADO=@FECHADATE 
				, NUMERO_ASIENTO=@TICKET
				, MOTIVO_RECHAZO=(SELECT TOP 1 id_motivo FROM snp_motivos_rechazo WHERE CODIGO_NACHA= (SELECT try_convert(NUMERIC(15),substring(linea,33,3)) FROM ITF_TRAN_MINORISTA_RECIBIDAS_AUX WHERE ID=@id+1) AND FUNCIONALIDAD='TR' AND TZ_LOCK=0)
				, OPERATORIA_BCRA=@Operatoria_BCRA
				, FCH_DEV_REC=@FECHACOMPENSACION
