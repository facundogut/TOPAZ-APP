...
SELECT DISTINCT mov.NROPERSFISICAJURIDICA, cli.TIPODOCUMENTO, cli.NUMERODOCUMENTO, cli.CODIGOCLIENTE, cli.TIPO
  INTO #TMP_CLIENTES
FROM SOS_VINCULACION mov 
  INNER JOIN VW_CLIENTE_PFPJ cli ON cli.NUMEROPERSONA = mov.NROPERSFISICAJURIDICA
  INNER JOIN CLI_PERSONASFISICAS nat ON nat.NUMEROPERSONAFISICA = cli.NUMEROPERSONA
...
AND (nat.FALLECIDO = ''N'' OR (nat.FALLECIDO = ''S''
          AND NOT EXISTS(SELECT 1 FROM CLI_DOCUMENTOSPFPJ V WITH(NOLOCK)
                         WHERE V.NUMERODOCUMENTO = cli.NUMERODOCUMENTO AND V.TIPOPERSONA = ''J'')))
UNION
SELECT DISTINCT mov.NROPERSFISICAJURIDICA, cli.TIPODOCUMENTO, cli.NUMERODOCUMENTO, cli.CODIGOCLIENTE, cli.TIPO
FROM SOS_VINCULACION mov 
  INNER JOIN VW_CLIENTE_PFPJ cli ON cli.NUMEROPERSONA = mov.NROPERSFISICAJURIDICA
  INNER JOIN CLI_PERSONASJURIDICAS jur ON jur.NUMEROPERSONAJURIDICA = cli.NUMEROPERSONA
WHERE MOV.FECHADEINFORMACION = @Fecha
AND cli.TIPO = ''J''
AND jur.TZ_LOCK = 0
...
WHERE cliact.CODIGO_PERSONA = x.NROPERSFISICAJURIDICA
...
--SELECT dir.PAIS, *
FROM #TMP_CLIENTES x
  INNER JOIN CLI_PERSONASFISICAS nat (NOLOCK) ON nat.NUMEROPERSONAFISICA = x.NROPERSFISICAJURIDICA
...
WHERE nat.TZ_LOCK = 0
  ...
  AND (nat.FALLECIDO = ''N'' OR (nat.FALLECIDO = ''S''
          AND NOT EXISTS(SELECT 1 FROM CLI_DOCUMENTOSPFPJ V WITH(NOLOCK)
                         WHERE V.NUMERODOCUMENTO = x.NUMERODOCUMENTO AND V.TIPOPERSONA = ''J'')))
...
--SELECT dir.PAIS, *
FROM #TMP_CLIENTES x
  INNER JOIN CLI_PERSONASJURIDICAS jur (NOLOCK) ON jur.NUMEROPERSONAJURIDICA = x.NROPERSFISICAJURIDICA
