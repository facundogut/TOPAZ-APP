...
BEGIN
	...
	AND C1803 = @PCliente AND C1785 = 4 AND C1604 <> 0 AND @JTS_OID=C1665
	...
	DECLARE @CantRegistroEnPagina INT
	
	IF(@TotalPaginas>@PPagina)
	BEGIN
		SET @CantRegistroEnPagina=@CERTIFICADO_X_PAG+1;
	END
	ELSE
	BEGIN 
		SET @CantRegistroEnPagina=@resto +@cantidadElementoPagina+1;
	END
	SET @CABECERA126 = '''';
	----------------------------------------------------------------------------------------------------------------------
	
	--REMPLAZAR CON RESTO+1, ANTES ERA @CANT_CERTIFICADOS
	IF(@TotalPaginas=1 OR @TotalPaginas=0)
	BEGIN
		SET @CABECERA126 = @CABECERA126 + REPLICATE(''0'',2-LEN(@CANT_CERTIFICADOS)) + CONVERT(VARCHAR,@CANT_CERTIFICADOS) + ''S'';
	END
	
	ELSE
	BEGIN
	SET @CABECERA126 = @CABECERA126 + REPLICATE(''0'',2-LEN(@CantRegistroEnPagina)) + CONVERT(VARCHAR,@CantRegistroEnPagina);
	END
	...
	BEGIN
		...
		SET @PAGINA = ''PAGINA 1'';
		SET @PAGINA = @PAGINA + REPLICATE('' '',28-DATALENGTH(@PAGINA));
	END
	...
	---------Se actualiza para que cuando no tenga Plazo Fijo, el 126=00
	...
	BEGIN
		...
		FROM SALDOS WITH(NOLOCK) WHERE C1800 = @CANAL AND C1803 = @PCliente AND @JTS_OID=C1665 AND C1785 = 4 
		...
		SET @Resultado126 = @Resultado126 + @LINEAS + @PAGINA + REPLICATE('' '',28); 
	END
	...
	BEGIN
		...
		WHERE C1800 = @CANAL AND C1803 = @PCliente AND C1785 = 4 AND @JTS_OID=C1665
		AND C1604 <> 0 AND C1627 >= (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK))
	    GROUP BY C1627, OPERACION, C1604
		ORDER BY C1627, OPERACION, C1604
		...
		SELECT @LINEAS= STRING_AGG(DATOS,'''') FROM @AUX_LINEAS
		SET @Resultado126 = @Resultado126 + @LINEAS + REPLICATE('' '',56);
	END
	...
	BEGIN
		...
		CONVERT(VARCHAR,OPERACION) +  REPLICATE('' '',14-LEN(REPLACE(FORMAT(C1604, ''C'', ''es-ar''),''$'',''''))) +
		REPLACE(FORMAT(C1604, ''C'', ''es-ar''),''$'',''''),'''')
		FROM SALDOS WITH(NOLOCK) WHERE C1800 = @CANAL AND C1803 = @PCliente AND C1785 = 4 AND @JTS_OID=C1665
		AND C1604 <> 0 AND C1627 >= (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK))
		...
		WHERE S.C1800 = @CANAL AND S.C1803 = @PCliente AND S.C1785 = 4 AND @JTS_OID=C1665
		...
		SET @Resultado125 = @CABECERA125 + @LINEA1 + @LINEA2 + @NOMBRECLIENTE + @CAPITAL + @INTERES + @MONTO + @PLAZO 
...
BEGIN
	...
	BEGIN
		...
		@TEM=CASE
		WHEN PR.C6253 = ''E'' --TASA EFECTIVA
			THEN (POWER((1 + (S.C1632 / 100.0)), (30.0 / 365.0)) - 1) * 100
			--CONVERT(NUMERIC(11,7),S.C1632/12)
		-- TASA
		WHEN PR.C6253 = ''M'' --TASA EFECTIVA MENSUAL
			THEN S.C1632			
		--CONVENCIÓN DEL BCRA
		...
		AND S.C1604 <> 0 AND S.C1627 >= (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK))
		AND S.OPERACION = @POperacion AND @JTS_OID=C1665
