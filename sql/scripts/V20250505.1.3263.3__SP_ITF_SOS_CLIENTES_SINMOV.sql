execute('
ALTER       PROCEDURE [dbo].[SP_ITF_SOS_CLIENTES_SINMOV]
	@Fecha VARCHAR(8),
...
--4
try_CONVERT(NUMERIC,doc.NUMERODOCUMENTO) AS IDENTTRIBUTARIANUMERO,
--5
...
--6
try_CONVERT(NUMERIC,doc.NUM_DOC_FISICO) AS DOCIDENTIFICATORIANUMERO,
--7
...
--11
LEFT(dir.APARTAMENTO,10) AS DOMICILIODPTOOFICINA,
--12
...
--16
(SELECT TOP 1 cliact.CODIGO_BCRA FROM #TMP_SOS_ACT_ECO cliact	
WHERE cliact.CODIGO_PERSONA = nat.NUMEROPERSONAFISICA
...
/*(CASE WHEN (SELECT COUNT(1) FROM PRODUCTOS pro 
            ...
            WHERE pro.TIPO_CUENTA_VISTA = 7
              AND sa.C1803 = vfj.CODIGOCLIENTE
              AND sa.C1651 <> ''1''
              AND sa.TZ_LOCK = 0
...
--6
try_CONVERT(NUMERIC,doc.NUMERODOCUMENTO) AS DOCIDENTIFICATORIANUMERO,
--7
...
--16
(SELECT TOP 1 cliact.CODIGO_BCRA FROM #TMP_SOS_ACT_ECO cliact	
  WHERE cliact.CODIGO_PERSONA = jur.NUMEROPERSONAJURIDICA
    AND cliact.tipo_actividad = ''Primaria'') AS CODIGOACTIVIDAD,
--17
(SELECT TOP 1 cliact.CODIGO_BCRA FROM #TMP_SOS_ACT_ECO cliact	
WHERE cliact.CODIGO_PERSONA = jur.NUMEROPERSONAJURIDICA
...
FROM /*CLI_CLIENTES cli (NOLOCK)
  ...
  LEFT JOIN VW_CLIENTE_PFPJ vfj (NOLOCK) ON vfj.NUMEROPERSONA = jur.NUMEROPERSONAJURIDICA
  LEFT JOIN (SELECT A.*
