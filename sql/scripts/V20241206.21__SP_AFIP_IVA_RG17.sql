ALTER    PROCEDURE dbo.[SP_AFIP_IVA_RG17]
...
BEGIN 
	--inserto en bitacora las bajas de los registgros que no vinieron en el nuevo padron
			...
			INSERT INTO dbo.CON_BITACORA_IMPUESTOS (JTS_NOVEDAD, 
						...
						AND C.TZ_LOCK=0
						AND car.TZ_LOCK=0
						GROUP BY vv.TIPOCARGO,a.ID_CLIENTE,C.NUMEROPERSONA,V.NUMERODOC,a.PERIODO_DESDE,a.PERIODO_HASTA,VV.SEGMENTO,a.porcentaje
 ...
 --cargo tabla con nuevo padron
	...
	--doy de alta los registros que vienen en el nuevo padron
					...
					AND a.ID_CLIENTE NOT IN (SELECT id_cliente FROM CI_CARGOS_TARIFAS WHERE RANGO_HASTA=9999999999998)
					AND c.ESTADO=0	
					AND c.TZ_LOCK=0
   ...
   --inserto en bitacora Altas de cargos y de impuestos	
						...
						INNER JOIN VW_CARGOS_IMPUESTOS VV ON vv.id_cargo= CAr.id_cargo							
						WHERE C.TZ_LOCK=0
						AND CAR.RANGO_HASTA!=9999999999998
						AND A.ID_CLIENTE NOT IN (SELECT id_cliente FROM CI_CARGOS_TARIFAS WHERE RANGO_HASTA=9999999999998)
						AND car.TZ_LOCK=0
						AND cli.estado=0
						AND CLI.tz_lock=0
