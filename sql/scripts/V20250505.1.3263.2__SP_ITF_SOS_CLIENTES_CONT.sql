execute('
ALTER        PROCEDURE [dbo].[SP_ITF_SOS_CLIENTES_CONT]
	@FechaIni VARCHAR(8),
	@FechaFin VARCHAR(8)
AS
...
BEGIN
  ...
  SET @FechaDesde = CONVERT(VARCHAR, DATEADD(year, -1,@FechaFin),112);
  SET @FechaHasta = @FechaFin;
	...
	--16
	(SELECT cliact.CODIGO_BCRA FROM #TMP_SOS_ACT_ECO cliact (NOLOCK)
	...
	'''' AS FECHABAJA,
	--30
	(CASE WHEN nat.SUC_ALTA>30 THEN 30 ELSE nat.SUC_ALTA END) AS SUC_ALTA,
	--31
  ...
  (CASE WHEN (SELECT count(1) FROM CRE_SOL_ACREDITACIONES_SUELDOS A (NOLOCK)
	...
	'''' AS FECHABAJA,
	--30
	(CASE WHEN jur.SUC_ALTA>30 THEN 30 ELSE jur.SUC_ALTA END) AS SUC_ALTA,
	--31
