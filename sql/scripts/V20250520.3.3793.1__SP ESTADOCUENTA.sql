...
BEGIN
	...
	CREATE TABLE #TMPIntegrantesCliente (
		...
		DocTitular1 varchar(20),
		NomTitular1 varchar(70),
	...
	*/
    ...
    BEGIN TRY
		...
		SELECT
			...
			(CASE WHEN cp.TIPOPERSONA=''F'' THEN c.NOMBRECLIENTE ELSE vs.NOMBRE_CUENTA END) AS nomcli,
	...
	BEGIN
		...
		*/
		...
		/* Obtiene datos de las personas que integran el cliente/firmantes del saldo */
		/* Se mueve posterior al cálculo de fechas para utilizar en búsqueda de firmantes */
		BEGIN TRY
			
			; WITH ORDEN_PODERES AS (
				SELECT 
					(CASE 
						WHEN TIPO_PODER = 4 THEN 1
						WHEN TIPO_PODER = 2 THEN 2
						WHEN TIPO_PODER = 5 THEN 3
						ELSE 4 
					END) AS ORDEN,
					TIPO_PODER,
					DESCRIPCION
				FROM PYF_TIPOPODERES with (nolock)
				--ORDER BY ORDEN
			),
			
			DATOS AS (
				SELECT
					ROW_NUMBER() OVER(PARTITION BY S.JTSOID ORDER BY S.JTSOID, OP.ORDEN, A.ID_PERSONA) AS FILA,
					ROW_NUMBER() OVER(PARTITION BY S.JTSOID, A.ID_PERSONA ORDER BY S.JTSOID, OP.ORDEN, A.ID_PERSONA) AS FILA_PERSONA,
					S.CLIENTE AS CLIENTE,
					A.TIPO_PODER,
					A.ID_PERSONA,
					ISNULL(PF.PRIMERNOMBRE, '''') + '' '' + ISNULL(PF.APELLIDOPATERNO, '''')  AS NOMBRE,
					D.TIPODOCUMENTO,
					D.NUMERODOCUMENTO,
					S.JTSOID
				FROM PYF_APODERADOS AS A with (nolock)
				INNER JOIN ORDEN_PODERES AS OP ON
					OP.TIPO_PODER = A.TIPO_PODER
				INNER JOIN #TMPSaldos AS S ON
					S.JTSOID = A.ID_ENTIDAD2
				INNER JOIN CLI_PERSONASFISICAS AS PF with (nolock) ON
					PF.NUMEROPERSONAFISICA = A.ID_PERSONA
				...
				WHERE
					A.TIPO_ENTIDAD = 2
					AND A.FECHA_INI_VIGENCIA <= @vFechaFin
					AND (A.FECHA_VENCIMIENTO >= @vFechaFin
						OR A.FECHA_VENCIMIENTO IS NULL	
					)
					AND	(@vFechaFin NOT BETWEEN 
						ISNULL(A.FECHA_INI_SUSPENSION,''18000101'') AND ISNULL(A.FECHA_FIN_SUSPENSION, ''18000101'')
					)
		    ...
		    SELECT
				...
				COUNT(DISTINCT D.ID_PERSONA) AS CantTitulares,
				MAX(CASE WHEN D.ORDEN = 1 THEN D.NUMERODOCUMENTO END) AS DocTitular1,
				MAX(CASE WHEN D.ORDEN = 1 THEN D.NOMBRE END) AS NomTitular1,
				MAX(CASE WHEN D.ORDEN = 2 THEN D.NUMERODOCUMENTO END) AS DocTitular2,
				MAX(CASE WHEN D.ORDEN = 2 THEN D.NOMBRE END) AS NomTitular2,
				MAX(CASE WHEN D.ORDEN = 3 THEN D.NUMERODOCUMENTO END) AS DocTitular3,
				MAX(CASE WHEN D.ORDEN = 3 THEN D.NOMBRE END) AS NomTitular3,
				D.JTSOID
			FROM 
				(SELECT 
					ROW_NUMBER() OVER(ORDER BY FILA) AS ORDEN,
					D.*
				FROM DATOS AS D
				WHERE
					D.FILA_PERSONA = 1) AS D
			GROUP BY
				CLIENTE,
				JTSOID
		...
		BEGIN TRY
			...
			SELECT
				...
				sd.SALDO_AJUSTADO AS SalDiaIni,
	...
	BEGIN
		...
		BEGIN TRY
			...
			UPDATE sd
			SET sd.SaldoINI = g.SALDO_AJUSTADO
	...
	SELECT
		...
		/* INFO EXTENDIDA */
		ISNULL(CT.INFO_EXTENDIDA, ''N'') AS InfoExtendida,
		CASE WHEN ISNULL(CT.INFO_EXTENDIDA, ''N'') = ''S''
			THEN
				CASE WHEN M.COD_TRANSACCION IN (21,22,23,30,34,152,153,156,157,158,159,160,161,162,421,422,423)
				THEN
					CASE WHEN M.DEBITOCREDITO = ''C'' THEN
					  	CONCAT(
							''Origen: '', ''Doc.:'', JSON_VALUE(HM.infoExtendidaMeta,  ''$.origen.cuit''),
							''- Denom.:'' , JSON_VALUE(HM.infoExtendidaMeta, ''$.origen.nombre''),
							''- Ref.: '',  JSON_VALUE(HM.infoExtendidaMeta, ''$.idOperacion'')
						)
					ELSE
					  	CONCAT(
							''Destino: '', ''Doc.:'', JSON_VALUE(HM.infoExtendidaMeta,  ''$.destino.cuit''),
							''- Denom.:'' , JSON_VALUE(HM.infoExtendidaMeta, ''$.destino.nombre''),
							''- Ref.: '',  JSON_VALUE(HM.infoExtendidaMeta, ''$.idOperacion'')
						)
					END
				ELSE
	...
	INNER JOIN MONEDAS AS MON WITH(nolock) ON
		MON.C6399 = M.MONEDA
	LEFT JOIN HISTORICO_MOVIMIENTOS AS HM WITH(NOLOCK) ON
		hm.fechaAsiento = m.FECHAPROCESO
		and hm.fechaReloj = convert(date, a.horainicio)
		and hm.movJtsOid = m.JTS_OID
	LEFT JOIN TTR_CODIGO_TRANSACCION_DEF AS CT WITH(NOLOCK) ON
		CT.CODIGO_TRANSACCION = M.COD_TRANSACCION
