     ...
     ---DECLARE @FECHA_PROCESO DATETIME 
      ...
      DELETE FROM MEMO_CABECERA WHERE Fecha_Info = @FECHA_PROCESO
                ...
                SELECT DISTINCT
                 ...
                 CASE WHEN V.ID_VINCULO IS NULL THEN ''N'' ELSE ''S'' END AS VINCULADO, ---Vinculado
                ...
                CASE WHEN C.CLIENTE_DEVENGA_SUSPENSO = 1 THEN ''S'' ELSE ''N'' END AS SIT_JUD_EXT
                 ...
                 LEFT JOIN CRE_VINCULACIONES V WITH(NOLOCK) ON V.ID_PERSONA_VINCULADA = CP.NUMEROPERSONA AND V.FECHA_INICIO <= @FECHA_PROCESO AND V.TZ_LOCK = 0
                 LEFT JOIN OPCIONES OP WITH(NOLOCK) ON OP.NUMERODECAMPO = 1357 AND OP.IDIOMA = ''E'' AND OP.OPCIONINTERNA = C.IVA 
                 
                 LEFT JOIN (SELECT SUM(ISNULL(HDG.PREVISION,0)) AS PREVISION, S.C1803 AS CLIENTE 
                 FROM CRE_HISTORICO_DEUDA_GARANTIAS HDG WITH(NOLOCK)
                 LEFT JOIN VTA_SOBREGIROS VS WITH(NOLOCK) ON HDG.SALDO_JTS_OID = VS.NRO_AUTORIZACION AND VS.ESTADO = 1
                 AND VS.TZ_LOCK = 0
                 LEFT JOIN SALDOS S WITH(NOLOCK) ON S.JTS_OID = 
                 (CASE WHEN HDG.TIPO_ASISTENCIA = ''A'' THEN VS.JTS_OID_SALDO ELSE HDG.SALDO_JTS_OID END)
                 WHERE HDG.FECHA_PROCESO = (SELECT TOP 1 FECHA_PROCESO 
                 FROM CRE_HISTORICO_DEUDA_GARANTIAS WITH(NOLOCK) WHERE SALDO_JTS_OID = HDG.SALDO_JTS_OID
                 AND FECHA_PROCESO <= @FECHA_PROCESO ORDER BY FECHA_PROCESO DESC)
                 GROUP BY S.C1803) HPC ON HPC.CLIENTE = C.CODIGOCLIENTE
