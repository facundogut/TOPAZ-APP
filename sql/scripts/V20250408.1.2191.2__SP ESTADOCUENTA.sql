EXECUTE('
----
CREATE OR ALTER PROCEDURE [dbo].[ESTADOCUENTA]
...
BEGIN
	...
	-- Limpieza previa de tablas temporales si existen
	DROP TABLE IF EXISTS #TMPSaldos;
	DROP TABLE IF EXISTS #TMPSaldosDiarios;
	DROP TABLE IF EXISTS #TMPDatosTitular;
	DROP TABLE IF EXISTS #TMPDireccionTitular;
	DROP TABLE IF EXISTS #TMPIntegrantesCliente;
	DROP TABLE IF EXISTS #TMPMovimientosContables;
	DROP TABLE IF EXISTS #TMPMovimientosContablesAux;
	DROP TABLE IF EXISTS #TMPInfoExt;
	...
	CREATE TABLE #TMPDireccionTitular (
		Cliente float(53),
		Calle varchar(60),
	...
	CREATE TABLE #TMPIntegrantesCliente (
		Cliente float(53),
	...
	CREATE TABLE #TMPMovimientosContables (
		...
		Cliente float(53),
		RegistroxJTS BIGINT,
		JTSRegistro Numeric(10, 0)
	);
	
	CREATE TABLE #TMPMovimientosContablesAux (
		JTSOID float(53),
		RegistroxJTS BIGINT,
		SaldoCalcLineaAux NUMERIC (15,2)
	);
	
	CREATE TABLE #TMPInfoExt (
		JTSOID float(53),
		RegistroxJTS BIGINT,
		Asiento float(53),
	...
	*/
	...
	WHERE
		...
		AND m.COD_TRANSACCION <> 99002 --Código transacción movimientos descartados.
	ORDER BY
		FechaRealMov,
		M.FECHAPROCESO,
		A.HORAFIN,
	...
	BEGIN
		...
		;WITH MovsOrdenados AS (
			SELECT
				...
				SignoSal,
				SUM(
					Monto * CASE WHEN SignoSal = SignoMov THEN 1 ELSE -1 END
				) OVER (PARTITION BY JTSOID ORDER BY RegistroxJTS ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS SaldoCalcLineaAux
			FROM #TMPMovimientosContables
		)
		
		UPDATE m
		SET m.SaldoCalcLineaAux = o.SaldoCalcLineaAux
		FROM #TMPMovimientosContables m
		INNER JOIN MovsOrdenados o ON 
			m.JTSOID = o.JTSOID
			AND m.RegistroxJTS = o.RegistroxJTS
		...
		SELECT
			...
			m.Asiento AS Asiento,
			m.FechaProcesado AS Fecha_Proceso,
        	m.SucursalOrigen AS Sucursal,
			m.CodTransaccion as CodTransaccion,
			--mc.COD_TRANSACCION AS CodTransaccion,
			m.Cliente AS Cliente,
			(CASE
				/* Transferencias Recibidas (créditos) */
				WHEN mc.COD_TRANSACCION IN (28,29,30,31,32,33,34)
					THEN
						--Origen (nombre) cuit/cuil (nro) ref: transferencia 0000000000000
						''Origen: '' + ISNULL(ISNULL(TR.ORD_NOMBRE, CT.NOMBRE),'''') +
						'' cuit/cuil: '' + CONVERT(VARCHAR(20),ISNULL(ISNULL(TR.ORD_NRO_DOC, CT.NUMERODOC),0)) +
						'' ref : '' + mc.CONCEPTO
				/* Transferencias Enviadas (débitos) */
				WHEN mc.COD_TRANSACCION IN (21,22,23,24,25,26)
					THEN
						--Destino (nombre) cuit/cuil (nro) ref: transferencia 0000000000000
						''Destino: '' +  ISNULL(ISNULL(TE.BEN_NOMBRE, CT.NOMBRE),'''') +
						'' cuit/cuil: '' +  CONVERT(VARCHAR(20),ISNULL(ISNULL(TE.BEN_NRO_DOC, CT.NUMERODOC),0)) + 
						'' ref.: '' + mc.CONCEPTO
				/* Plazo fijo */
				WHEN mc.COD_TRANSACCION IN (48,49,50,106,107)
					THEN
						''Suc.: '' + CONVERT(VARCHAR(5), SD.SUCURSAL) +
						'' Cert.: '' + str((
										SELECT ISNULL(NULLIF(ps.CERTIFICADO_DPF, 0), SD.OPERACION)
										FROM PZO_SALDOS PS
										WHERE
											PS.JTS_OID_SALDO = SD.JTS_OID
										)) +
						'' Cap.: '' + 
							CASE WHEN HP.TIPOMOV = ''P''
								THEN convert(VARCHAR(20), HP.CAPITALPAGADO)
								ELSE convert(VARCHAR(20), HP.CAPITALORIGINAL)
							END +
						'' Int.: '' + 
							CASE WHEN HP.TIPOMOV = ''P''
								THEN convert(VARCHAR(20), HP.INTERESPAGADO)
								ELSE convert(VARCHAR(20), HP.INTALVTO)
							END
				/* Pago de Cheques */
				WHEN mc.COD_TRANSACCION IN (5,6,7,8,9,11,12,40,41,42,43,44,85,87,88,89,90,91,98,99,3018,3019,3020)
					THEN
						NULLIF(concat(''Cheque número '', mc.REFERENCIA), ''Cheque número 0'')
				/* Préstamos */
				WHEN mc.COD_TRANSACCION IN (3016)
					THEN
						''Préstamo: '' +  convert(VARCHAR(max),SP.OPERACION) +
						'' Línea: '' + convert(VARCHAR(12),SP.PRODUCTO) +
						'' Cuota: '' + convert(VARCHAR(20), isnull((
									SELECT TOP 1 CUOTA
									FROM BS_PAYS_DETAIL
									WHERE
										SALDOS_JTS_OID IN (SP.JTS_OID)
									ORDER BY CUOTA DESC
								), 0)
							) + 
		...
		INNER JOIN MOVIMIENTOS_CONTABLES mc with (nolock) ON
			...
			AND mc.JTS_OID = m.JTSRegistro
		LEFT JOIN VTA_TRANSFERENCIAS AS TE with (nolock) ON --TRANSFERENCIAS ENVIADAS A OTRO BANCO
			TE.OP_CLASE_TRANS = ''E''
			AND TE.NUMERO_ASIENTO = m.Asiento
			AND TE.SUCURSAL_ASIENTO = m.SucursalOrigen
			AND TE.FECHA_ASIENTO = m.FechaProcesado
		LEFT JOIN VTA_TRANSFERENCIAS AS TR with (nolock) ON --TRANSFERENCIAS RECIBIDAS DE OTRO BANCO
			TR.OP_CLASE_TRANS = ''R''
			AND TR.NUMERO_ASIENTO = m.Asiento
			AND TR.SUCURSAL_ASIENTO = m.SucursalOrigen
			AND TR.FECHA_ASIENTO = m.FechaProcesado
		LEFT JOIN SALDOS AS ST with (nolock) ON --SALDO DE TRANSFERENCIAS MISMO BANCO
			ST.JTS_OID = (	SELECT TOP 1 SALDO_JTS_OID
							FROM MOVIMIENTOS_CONTABLES WITH (nolock)
							WHERE
								ASIENTO = mc.ASIENTO
								AND SUCURSAL = mc.SUCURSAL
								AND FECHAPROCESO = mc.FECHAPROCESO
								AND SALDO_JTS_OID NOT IN (m.JTSOID)
								AND SALDO_JTS_OID IN (
									SELECT JTS_OID 
									FROM SALDOS 
									WHERE C1785 IN (2,3)	
								)
						)
		LEFT JOIN VW_CLIENTES_PERSONAS CT with (nolock) ON --CLIENTE DE TRANSFERENCIAS MISMO BANCO
			CT.CODIGOCLIENTE = ST.C1803
			AND CT.TITULARIDAD = ''T''
		LEFT JOIN SALDOS AS SD with (nolock) ON --SALDO DE DPF EN EL ASIENTO
			SD.JTS_OID = (	SELECT TOP 1 SALDO_JTS_OID
							FROM MOVIMIENTOS_CONTABLES WITH (nolock)
							WHERE
								ASIENTO = mc.ASIENTO
								AND SUCURSAL = mc.SUCURSAL
		...
		LEFT JOIN SALDOS AS SP with (nolock) ON --SALDO DE PRÉSTAMO O DESCUENTO EN EL ASIENTO
			SP.JTS_OID = (	SELECT TOP 1 SALDO_JTS_OID
							FROM MOVIMIENTOS_CONTABLES WITH (nolock)
							WHERE
								ASIENTO = mc.ASIENTO
								AND SUCURSAL = mc.SUCURSAL
								AND FECHAPROCESO = mc.FECHAPROCESO
								AND CONCEPTO = mc.CONCEPTO
								AND SALDO_JTS_OID IN (
									SELECT JTS_OID 
									FROM SALDOS 
									WHERE C1785 IN (5,6)
		...
		LEFT JOIN VW_CLIENTES_PERSONAS CP with (nolock) ON --CLIENTE DEL PROPIO SALDO O MOVIMIENTO
			CP.CODIGOCLIENTE = MC.CLIENTE
			AND CP.TITULARIDAD = ''T''
		LEFT JOIN BS_HISTORIA_PLAZO HP with (nolock) ON
			HP.NROASIENTOMOV = m.Asiento
			AND HP.FECHAPROCESOMOV = m.FechaProcesado
			AND HP.SUCURSALMOV = m.SucursalOrigen
			AND HP.JTS_OID = (
				SELECT TOP 1 JTS_OID
				FROM BS_HISTORIA_PLAZO
				WHERE
					NROASIENTOMOV = HP.NROASIENTOMOV
					AND FECHAPROCESOMOV = HP.FECHAPROCESOMOV
					AND SUCURSALMOV = HP.SUCURSALMOV
			)
		WHERE
			m.InfoExtendida = ''S''
			/*AND m.CodTransaccion IN (28,29,30,31,32,33,34, -- Crédito
									   21,22,23,24,25,26,27, -- Débito
									   48,49,50,106,107, 	 -- DPF
									   5,6,7,8,9,11,12,40,41,42,43,44,85,87,88,89,90,91,98,99,3018,3019,3020, -- CHEQUES
									   3016                 )-- Préstamos */
		...
		INNER JOIN #TMPInfoExt i ON
			i.Asiento = m.Asiento
        	AND i.Sucursal = m.SucursalOrigen
			AND i.Fecha_Proceso = m.FechaProcesado
			AND i.RegistroxJTS = m.RegistroxJTS
        
        /*Obtengo Saldo al útimo día*/
        BEGIN TRY
	...
	-- Limpieza posterior de tablas temporales
	DROP TABLE IF EXISTS #TMPSaldos;
	DROP TABLE IF EXISTS #TMPSaldosDiarios;
	DROP TABLE IF EXISTS #TMPDatosTitular;
	DROP TABLE IF EXISTS #TMPDireccionTitular;
