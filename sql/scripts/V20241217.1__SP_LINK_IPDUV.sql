...
OPEN cursorOk
    ...
    BEGIN
		...
		SELECT @idConv = id_convrec, @nomConv = nomconvrec FROM CONV_REL_ENTECONV (nolock) WHERE COD_ENTE = @ente AND ID_PROCESO = 136 AND TIPO_CONCEPTO = @idRef;
					
		SET @idCabezal = (SELECT ISNULL(Max(ID),0) + 1 FROM REC_CAB_RECAUDOS_CANAL (nolock));
