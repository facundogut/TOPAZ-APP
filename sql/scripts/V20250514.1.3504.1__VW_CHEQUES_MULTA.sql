...
CREATE OR ALTER VIEW [dbo].[VW_CHEQUES_MULTA] (
	...
	INNER JOIN CLE_CHEQUES_CLEARING_RECIBIDO CC WITH (NOLOCK)
	ON CC.MONEDA = B.MONEDA
		AND B.PRODUCTO = CC.PRODUCTO
		AND B.CUENTA = CC.CUENTA
		AND B.SUCURSAL = CC.SUCURSAL
		AND B.NRO_CHEQUE = CC.NUMERO_CHEQUE
		AND B.SERIE_CHEQUE = CC.NUMERO_SERIE 
		AND CC.TZ_LOCK = 0
	...
	INNER JOIN CLE_TIPO_CAUSAL T WITH (NOLOCK) ON 
		T.CODIGO_DE_CAUSAL = CC.CODIGO_CAUSAL_DEVOLUCION
