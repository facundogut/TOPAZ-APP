...
DELETE FROM SOS_VINCULACION WHERE convert(VARCHAR,FECHADEINFORMACION) = @FECHAPROCESO;
...
FROM #TMP_CLIENTE B
  ...
  INNER JOIN CLI_PERSONASFISICAS nat (NOLOCK) ON nat.NUMEROPERSONAFISICA = d.NUMEROPERSONA AND nat.TZ_LOCK = 0
  INNER JOIN CLI_DocumentosPFPJ doc (NOLOCK) ON doc.NUMEROPERSONAFJ = D.NUMEROPERSONA AND doc.TIPOPERSONA = ''F'' AND doc.TZ_LOCK = 0
...
FROM #TMP_CLIENTE B
  INNER JOIN CLI_ClientePersona D (NOLOCK) ON D.CODIGOCLIENTE = B.CODCLIENTE AND D.TZ_LOCK = 0
  INNER JOIN CLI_PERSONASJURIDICAS jur (NOLOCK) ON jur.NUMEROPERSONAJURIDICA = d.NUMEROPERSONA AND jur.TZ_LOCK = 0
  INNER JOIN CLI_DocumentosPFPJ doc (NOLOCK) ON doc.NUMEROPERSONAFJ = D.NUMEROPERSONA AND doc.TIPOPERSONA = ''J'' AND doc.TZ_LOCK = 0
--UNION ALL

--Se evita los registros duplicados en PYF_APODERADOS
SELECT AP.ID_PERSONA, AP.ID_ENTIDAD
  INTO #T_APODERADOS
FROM PYF_APODERADOS AP (NOLOCK)
  INNER JOIN #TMP_CLIENTE B ON AP.ID_ENTIDAD = B.SALDO_JTS_OID AND AP.TZ_LOCK = 0
	AND AP.TIPO_ENTIDAD = 2 AND AP.APODERAMIENTO = 2
GROUP BY AP.ID_PERSONA, AP.ID_ENTIDAD
...
FROM #TMP_CLIENTE B
  ...
  INNER JOIN #T_APODERADOS AP ON AP.ID_PERSONA = I.NUMEROPERSONAFISICA AND AP.ID_ENTIDAD = B.SALDO_JTS_OID
  INNER JOIN CLI_DocumentosPFPJ doc (NOLOCK) ON doc.NUMEROPERSONAFJ = I.NUMEROPERSONAFISICA AND doc.TIPOPERSONA = ''F'' AND doc.TZ_LOCK = 0
