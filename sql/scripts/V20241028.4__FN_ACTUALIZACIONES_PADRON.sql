...
SELECT
		...
		CASE 
			WHEN (PF.CODIGODERESIDENCIA = 3) THEN 
				RIGHT(Ltrim(Rtrim(CP.CODIGOSWIFT)) + ''00'',4)
			ELSE 
				DIR.CPA_NUEVO	
...
LEFT JOIN PYF_APODERADOS APO WITH(nolock) ON 
...
LEFT JOIN PYF_APODERADOS APO WITH(nolock) ON 
	PF.NUMEROPERSONAFISICA = APO.ID_PERSONA  
	AND APO.TZ_LOCK = 0
...
INNER JOIN CLI_INTEGRANTESPJ IPJ WITH(nolock) ON 
	PJ.NUMEROPERSONAJURIDICA = IPJ.NUMEROPERSONAFISICA
	AND IPJ.CODIGOCARGO IN (''SDE'',''FCO'',''APO'')
	AND IPJ.TZ_LOCK = 0
