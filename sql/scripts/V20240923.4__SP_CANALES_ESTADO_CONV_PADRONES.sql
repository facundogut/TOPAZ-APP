...
BEGIN
 ...
 -- Crear una tabla temporal para almacenar los IDs de liquidación
    DECLARE @ID_LIQUIDACION_TABLE TABLE (ID_LIQUIDACION INT);

 -- Insertar los valores de la cadena en la tabla temporal
    INSERT INTO @ID_LIQUIDACION_TABLE (ID_LIQUIDACION)
    SELECT value
    FROM STRING_SPLIT(REVERSE(STUFF(REVERSE(@P_ID_LIQUIDACION), 1, 1, '''')) , '','');
	...
	BEGIN
		...
		SELECT DISTINCT L.ID_CABEZAL, RD.ID_LINEA, VW.NRO_COMPROBANTE_CLIENTE, VW.LETRA,
		 ...
		 INNER JOIN VW_CANAL_REND_CARGO_ESPECIFICO VW ON VW.ID_CABEZAL = l.ID_CABEZAL
		                                         ...
		                                         AND VW.ESTADO = ''P''
		 WHERE l.ID_LIQUIDACION IN (SELECT ID_LIQUIDACION FROM @ID_LIQUIDACION_TABLE)
		   AND l.TZ_LOCK = 0;
