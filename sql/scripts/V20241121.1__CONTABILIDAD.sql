...
IF (SELECT DB_NAME()) = @BD
  BEGIN
	...
	print ''Comprobación OK.Tabla MOVIMIENTOS_CONTABLES con SALDO ''
	...
	if exists (	select a.ID_ASIENTO
			from CON_ASIENTOS_TIPO_CV a with(nolock)
			where a.ID_ASIENTO  not in (select b.ASIENTO
									from ASIENTOS b with(nolock))
			)	
			select     ''Tabla CON_ASIENTOS_TIPO_CV no se ajusta a la integridad referencial con ASIENTOS '',*
			from CON_ASIENTOS_TIPO_CV a with(nolock)
			where a.ID_ASIENTO  not in (select b.ASIENTO
									from ASIENTOS b with(nolock))
	else
	print ''Comprobación OK. Tabla CON_ASIENTOS_TIPO_CV con ASIENTOS ''
	...
	if exists (	select a.ASIENTO
				from MOVIMIENTOS a with(nolock)
				inner join MOVIMIENTOS_CONTABLES b with(nolock) on
				...
				where 
						...
						or
						(b.sucursal is null
						and b.fechaproceso  is null
						and b.tipo  is null
						and b.ordinal  is null
						and b.trreal  is null
	...
	if exists (	select a.ASIENTO
			...
			where 
				...
				(b.sucursal is null
				and b.fechaproceso  is null
				and b.TRTIPO  is null
				and b.ordinal  is null
				and b.trreal  is null
				and b.asiento is null
				and b.SUCURSAL is null
				and b.trvirtual  is null))
	...
	print ''Comprobación OK. Tabla MOVIMIENTOS_A_REAPLICAR con MOVIMIENTOS_CONTABLES ''
		...
		if exists (	select a.ASIENTO
			...
			where 
				...
				and a.[ORDINAL]  is null)
			or
				(b.[FECHAPROCESO] is null
				and b.[SUCURSAL]  is null
				and b.[ASIENTO]  is null
				and b.[SUB_ASIENTO]  is null
				and b.[TRREAL]  is null
				and b.[ORDINAL]  is null))
	...
	if exists (	select a.tipo_cargo_impositivo
				...
				right join CON_REGIMEN_CERTIF_RET b with(nolock) on
		...
		select ''Tabla CON_REGIMEN_CERTIF_RET no se ajusta a la integridad referencial con CON_CERTIFICADOS_RETENCION '',*
				from CON_CERTIFICADOS_RETENCION a with(nolock)
				right join CON_REGIMEN_CERTIF_RET b with(nolock) on
					a.tipo_cargo_impositivo = b.tipo_cargo_impositivo
					and a.codigo_regimen =b.codigo_regimen
		...
		print ''Comprobación OK. Tabla CON_REGIMEN_CERTIF_RET con CON_CERTIFICADOS_RETENCION ''
	...
	if exists (	select a.jts_oid_asiento
			from BANDEJA_MOVS_IN_EXTENDIDO a with(nolock)
			where a.jts_oid_asiento  not in (select b.jts_oid
									from BANDEJA_ASIENTOS_IN_EXTENDIDO b with(nolock))
			)	
			select     ''Tabla BANDEJA_MOVS_IN_EXTENDIDO no se ajusta a la integridad referencial con BANDEJA_ASIENTOS_IN_EXTENDIDO '',*
			from BANDEJA_MOVS_IN_EXTENDIDO a with(nolock)
			where a.jts_oid_asiento  not in (select b.jts_oid
									from BANDEJA_ASIENTOS_IN_EXTENDIDO b with(nolock))
	else
	print ''Comprobación OK. Tabla BANDEJA_MOVS_IN_EXTENDIDO con BANDEJA_ASIENTOS_IN_EXTENDIDO ''

	if exists (	select a.rubrocontable
			from MOVIMIENTOS_CONTABLES a with(nolock)
			where a.rubrocontable  not in (select b.C6326
									from PLANCTAS b with(nolock))
			)	
			select     ''Tabla MOVIMIENTOS_CONTABLES no se ajusta a la integridad referencial con PLANCTAS '',*
			from MOVIMIENTOS_CONTABLES a with(nolock)
			where a.rubrocontable  not in (select b.C6326
									from PLANCTAS b with(nolock))
	else
	print ''Comprobación OK. Tabla MOVIMIENTOS_CONTABLES con PLANCTAS ''
