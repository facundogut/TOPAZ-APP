		...
		*/
		...
		BEGIN TRY
		    ...
		    SELECT
				...
				MAX(CASE WHEN D.FILA = 1 THEN D.NUMERODOCUMENTO END) AS DocTitular1,
				MAX(CASE WHEN D.FILA = 1 THEN D.NOMBRE END) AS NomTitular1,
				MAX(CASE WHEN D.FILA = 2 THEN D.NUMERODOCUMENTO END) AS DocTitular2,
				MAX(CASE WHEN D.FILA = 2 THEN D.NOMBRE END) AS NomTitular2,
				MAX(CASE WHEN D.FILA = 3 THEN D.NUMERODOCUMENTO END) AS DocTitular3,
				MAX(CASE WHEN D.FILA = 3 THEN D.NOMBRE END) AS NomTitular3,
				D.JTSOID
			FROM DATOS AS D
			WHERE
				D.FILA_PERSONA = 1
