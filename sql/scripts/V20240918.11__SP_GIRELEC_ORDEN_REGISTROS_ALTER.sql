...
--det	
	...
	INSERT INTO ITF_GIRELEC_AUX  (ID_CABEZAL, CAMPO, TIPO) 
		SELECT 
			...
			Concat('T',
				LEFT(CONCAT(isnull(
				CASE 
					WHEN LEFT(codigo_barras, 3) = '316' AND RIGHT(trim(codigo_barras), 10) = '0000000000'
						THEN 
							dbo.getCodBarraVerificado(
							concat(
								LEFT(codigo_barras, 14),
								'1',
								RIGHT(concat(replicate('0',12),CAST(cast(IMPORTE*100 AS INT) AS VARCHAR(12))),12),
								REPLACE(CONVERT(VARCHAR(10), DET.FECHA_COBRANZA, 105), '-','')))
					WHEN len(codigo_barras_rendido) = 0 OR RIGHT(trim(codigo_barras_rendido), 10) = '0000000000'
						THEN codigo_barras						
					ELSE codigo_barras_rendido
				END 			
				,' '),REPLICATE(' ',100)),100),
		...
		GROUP BY DET.CODIGO_BARRAS,REN.SUCURSAL_RENDICION, cab.ID,DET.IMPORTE, DET.FECHA_COBRANZA,DET.codigo_barras_rendido
	...
	INSERT INTO ITF_GIRELEC_AUX (ID_CABEZAL, CAMPO, TIPO)
		...
		GROUP BY DET.CODIGO_BARRAS,REN.SUCURSAL_RENDICION, CAB.ID,DET.IMPORTE, DET.CODIGO_BARRAS_RENDIDO
