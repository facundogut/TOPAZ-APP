      	...
      	SELECT
		    ...
		    round((	SELECT SUM(CAPITALREALIZADO)
		    	...
		    	WHERE
		    		...
		    		AND FECHAPROCESO = D.MOV_FECHA_PROCESO
		    		AND DEBITOCREDITO = ''D''
		    )/(SELECT TCVENTACOMUN FROM MONEDAS WHERE C6399=M.moneda) * HTC.TIPO_CAMBIO_VENTA,2)	    
		   ...
		   ELSE  
		    ...
		    CASE WHEN D.MOV_MONEDA <>1 AND MON.C6403 <> ''I'' THEN FL.MONTO_IMPONIBLE *CB.TC_VENTA 
		         ...
		         WHEN D.MOV_MONEDA =1 AND MON.C6403  = ''N'' AND M.moneda IN (SELECT C6399 FROM MONEDAS WHERE TZ_LOCK= 0 AND C6403 =''I'')
		              THEN round(FL.MONTO_IMPONIBLE/(SELECT TCVENTACOMUN FROM MONEDAS WHERE C6399=M.moneda) * HTC.TIPO_CAMBIO_VENTA,2)
		    ...
		    CASE WHEN D.MOV_MONEDA <>1 AND MON.C6403 <> ''I'' THEN FL.MONTO_FACTURABLE*CB.TC_VENTA 
		         WHEN (D.MOV_MONEDA <>1 AND MON.C6403  = ''I'') 
		          ...
		          WHEN D.MOV_MONEDA =1 AND MON.C6403  = ''N'' AND M.moneda IN (SELECT C6399 FROM MONEDAS WHERE TZ_LOCK= 0 AND C6403 =''I'')
		              THEN round(FL.MONTO_FACTURABLE/(SELECT TCVENTACOMUN FROM MONEDAS WHERE C6399=M.moneda) * HTC.TIPO_CAMBIO_VENTA,2)
