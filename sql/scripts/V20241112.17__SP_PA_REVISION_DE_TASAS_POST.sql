EXECUTE('
ALTER PROCEDURE SP_PA_REVISION_DE_TASAS_POST
...
AS
BEGIN
DECLARE @CANTIDAD_REGISTROS INT = 0;
...
BEGIN
	SET @CANTIDAD_REGISTROS = (SELECT COUNT(*)
			...
			WHERE s.TZ_LOCK = 0  AND s.C1785 = 5
			  ...
			  AND EXISTS (SELECT 1 FROM REVISION_TASAS WITH (nolock) WHERE SALDO_JTS_OID=s.JTS_OID AND ESTADO=''P'' AND 
...
IF @CANTIDAD_REGISTROS>0 
	...
	-- Actalizar nueva tasa convertida TNA, post proceso de revision de tasas
	    UPDATE cs 
		SET cs.TASA_CONVERTIDA = RTA.NUEVA_TNA_BASE
		FROM CRE_SALDOS cs
		INNER JOIN @REVISION_TASAS_AUX RTA ON cs.SALDOS_JTS_OID=RTA.JTS_OID
