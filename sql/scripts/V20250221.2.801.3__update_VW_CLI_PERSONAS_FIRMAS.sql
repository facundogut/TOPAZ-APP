execute('
CREATE OR ALTER VIEW [dbo].[VW_CLI_PERSONAS_FIRMAS]
													(TIPO_DOC,
													NRO_DOC,
													NRO_PERSONA,
													NOMBRE_APELLIDO,
...
SELECT	D.TIPODOCUMENTO, 
		...
		F.AUTORIZADOR,
		CASE 
			WHEN F.FIRMA IS NULL OR DATALENGTH(F.FIRMA) = 0 THEN 0
    	 ELSE 1
     	END		
FROM CLI_PERSONAS_FIRMAS AS F WITH (NOLOCK)
INNER JOIN CLI_PERSONASFISICAS AS P WITH (NOLOCK) ON F.NUMEROPERSONAFISICA = P.NUMEROPERSONAFISICA 
													AND P.TZ_LOCK = 0
