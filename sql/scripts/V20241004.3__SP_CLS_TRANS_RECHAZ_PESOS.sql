...
BEGIN
	...
	-- Cerrar el cursor si est� abierto
	...
	/******** Variables Registro adicional de rechazos de �rdenes de transferencia *************/
...
--				SET @CodRechazo = (SELECT TOP 1 SUBSTRING(LINEA,4,3) FROM ITF_TRANS_PESOS_RECHAZ_AUX WHERE ID>@ID AND LINEA LIKE '799%');
			...
			BEGIN 
				...
				SET ESTADO = 'RC' , FECHA_ESTADO=@FECHADATE , NUMERO_ASIENTO=@TICKET, MOTIVO_RECHAZO=@CodRechazo,fch_dev_rec=@FechaPresentacion
