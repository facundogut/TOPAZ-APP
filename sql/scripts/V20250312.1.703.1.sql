Execute('CREATE OR ALTER  PROCEDURE [dbo].[SP_COELSA_ENVIO_CHEQUES_PROPIOS_RECHAZADOS]
   @TICKET NUMERIC(16)
--2.8.5
--   ,
...
--PRINT @SumaImportes		
		...
		PRINT ''Trace: '' + CAST(@CheTraceNumber AS varchar)
		
		SET @entidad = Substring(RIGHT(REPLICATE(''0'', 3) + CAST(@CheTraceNumber AS VARCHAR(15)), 15),1,8);
...
--PRINT @RI_CUENTA_DEBITAR
		...
		SET @entidad = Substring(RIGHT(REPLICATE(''0'', 3) + CAST(@CheTraceNumberCD AS VARCHAR(15)), 15),1,8);
...
--seba
	...
	SET ESTADO = ''P''
		, ESTADO_AJUSTE = ''R''
	WHERE ESTADO = ''I'' AND ENVIADO_RECIBIDO = ''R'' AND ESTADO_AJUSTE IS NULL

	IF(0<(SELECT COUNT(1) FROM CLE_CHEQUES_AJUSTE WHERE ESTADO = ''P'' AND ENVIADO_RECIBIDO = ''R'' AND ESTADO_AJUSTE = ''R''))
--	IF(0<(SELECT COUNT(1) FROM CLE_CHEQUES_AJUSTE WHERE ESTADO = ''P'' AND ENVIADO_RECIBIDO = ''R'' AND ESTADO_AJUSTE = ''R'') AND (@FL_CONTADOR_AL_INICIAR<>(SELECT NUMERICO_1 FROM ITF_MASTER_PARAMETROS WHERE CODIGO = 138)))
	...
	DECLARE @V_CodErrorAjusteParam NUMERIC(20,0) = (SELECT p.numerico_1 FROM ITF_MASTER_PARAMETROS p WITH (nolock) WHERE p.codigo = 540);
	-----------------------------------
...
--				SET @FL_NUMERO_LOTE = RIGHT(concat(replicate(''0'', 7), @FL_NUMERO_LOTE), 7);
			...
			SET @entidad = Substring(RIGHT(REPLICATE(''0'', 3) + CAST(@CheTrackNumberAJ AS VARCHAR(15)), 15),1,8);
			...
			BEGIN
				SET @CODIGO_RECHAZO = ''R'' + RIGHT(CONCAT(''00'',@V_CodErrorAjusteParam),2);
		...
		IF(0<(SELECT count(*) FROM CLE_RECEPCION_CHEQUES_DEV CD WHERE CD.ESTADO_DEVOLUCION=0 AND CD.TZ_LOCK=0))
--		IF(0<(SELECT count(*) FROM CLE_RECEPCION_CHEQUES_DEV CD WHERE CD.ESTADO_DEVOLUCION=0 AND CD.TZ_LOCK=0) AND (@FL_CONTADOR_AL_INICIAR<>(SELECT NUMERICO_1 FROM ITF_MASTER_PARAMETROS WHERE CODIGO = 138)))	 
		BEGIN		
	...
	DECLARE @TRACENROCHEQDEV VARCHAR(15);
	-----------------------------------
	...
	DECLARE cursor_che_rechazados CURSOR FOR
		...
		SELECT CD.NUMERO_CHEQUE, 
			...
			(SELECT TOP 1 CPA_VIEJO FROM CLI_DIRECCIONES WHERE ID= CD.BANCO_GIRADO AND TZ_LOCK=0), -- poner CP de itf_coelsa_cheques_otros
			CD.CODIGO_RECHAZO 
		...
		FROM CLE_RECEPCION_CHEQUES_DEV CD 
			JOIN ITF_COELSA_CHEQUES_OTROS CC
				ON CD.BANCO_GIRADO = CC.BANCO AND CD.SUCURSAL_BANCO_GIRADO = CC.SUCURSAL
					AND CD.NUMERO_CHEQUE = CC.NRO_CHEQUE AND CD.NUMERO_CUENTA_GIRADORA = CC.CUENTA
					AND CD.FECHA_ALTA = CC.FECHA_VENCIMIENTO
...
--			SET @FL_NUMERO_LOTE = RIGHT(concat(replicate(''0'', 7), @FL_NUMERO_LOTE), 7);
		...
		SET @RI_CONTADOR_REGISTRO = @TRACENROCHEQDEV; --concat(''0311'', RIGHT(concat(replicate(''0'', 4), (''0097'')), 4),RIGHT(concat(replicate(''0'', 7), (SELECT NUMERICO_1 FROM ITF_MASTER_PARAMETROS WHERE CODIGO = 138)), 7));
...
PRINT ''aca''
	...
	IF(0=(SELECT COUNT(*) FROM ITF_COELSA_CHEQUES_RECHAZO WHERE TIPO IN (''C'', ''A'') AND ESTADO = ''P'' AND FECHAPROCESO = (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)))
		+(SELECT COUNT(*) FROM CLE_CHEQUES_CLEARING CC,CLE_CHEQUES_CLEARING_RECIBIDO CR WHERE CC.CMC7 = CR.MICRLINE AND CR.FECHA = CC.FECHA_VALOR AND (CR.CANJE_INTERNO=''N'' OR CR.CANJE_INTERNO IS NULL) AND CC.FECHA_VALOR = (SELECT FECHAPROCESO FROM PARAMETROS) AND CR.ESTADO_DEVOLUCION<>0 AND CC.TZ_LOCK = 0 AND CR.TZ_LOCK = 0)
		+(SELECT COUNT(1) FROM CLE_CHEQUES_AJUSTE WHERE ESTADO = ''P'' AND ENVIADO_RECIBIDO = ''R'' AND ESTADO_AJUSTE = ''R'')
		+(SELECT count(*) FROM CLE_RECEPCION_CHEQUES_DEV CD WHERE CD.ESTADO_DEVOLUCION=0 AND CD.TZ_LOCK=0))
	BEGIN
		SET @CL_NUMERO_LOTE =RIGHT(concat(replicate(''0'', 7), (SELECT COUNT(*) + 1 FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE ''5%'' AND ID>(SELECT max(id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE ''1%''))), 7);
		SET @CL_CABECERA = concat(@CL_ID_REG, @CL_CODIGO_CLASE_TRANSAC, @CL_RESERVADO, @CL_TIPO_REGISTRO, @CL_DESCRIP_TRANSAC, @CL_FECHA_PRESENTACION, @CL_FECHA_VENCIMIENTO, @CL_RESERVADO_CL, @CL_CODIGO_ORIGEN, @CL_ID_ENTIDAD_ORIGEN, @CL_NUMERO_LOTE);
		INSERT INTO dbo.ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX (LINEA) VALUES (@CL_CABECERA);
