...
--cab	
	...
	BEGIN 
		...
		SELECT
			...
			convert(VARCHAR,CAB.FECHA_COBRANZA,12),
			RIGHT(CONCAT(REPLICATE(''0'',10), COUNT(DET.IMPORTE)),10), 
			RIGHT(CONCAT(REPLICATE(''0'',12),CAST(Round(SUM(DET.IMPORTE)*100,0) AS NUMERIC(12))),12),
		...
		FROM CONV_CONVENIOS_REC (nolock) CONV 
            ...
            JOIN REC_det_RECAUDOS_CANAL (NOLOCK) DET ON DET.ID_CABEZAL = CAB.ID
		...
		GROUP BY SUCFIC.SUCURSAL, CAB.FECHA_COBRANZA, /*CAB.FECHACARGA,*/ CAB.ID --, CAB.TOTALREGISTROS, CAB.TOTALIMPORTE	
	END 
	...
	BEGIN 	
		...
		SELECT 
			...
			Concat(''C'',
				...
				convert(VARCHAR,CAB.FECHACARGA,12),
				RIGHT(CONCAT(REPLICATE(''0'',10),COUNT(DET.IMPORTE)),10),
				RIGHT(CONCAT(REPLICATE(''0'',10),CAST(Round(SUM(DET.IMPORTE)*100,0) AS NUMERIC(12))),12),
				REPLICATE('' '',107)) as Registro
				, 2
