   ...
   BEGIN
      ...
      FROM CRE_HISTORICO_DEUDA_GARANTIAS hd
    	WHERE
        	...
        	hd.NRO_GARANTIA = 0 AND
        	hd.TIPO_ASISTENCIA = ''P'' AND
			(hd.CAPITAL + hd.INTERES + hd.IVA_FINANCIADO + hd.IVA_PERCEPCION_FINANCIADO + hd.ACCESORIOS)<>hd.MONTO_GARANTIZADO AND
        	EXISTS (SELECT * 
                FROM CRE_HISTORICO_DEUDA_GARANTIAS dg WITH(NOLOCK)
                WHERE 
                    hd.SALDO_JTS_OID = dg.SALDO_JTS_OID AND 
                    hd.TIPO_ASISTENCIA = ''P'' AND
                    dg.FECHA_PROCESO = @FECHA_PROCESO AND 
                    dg.NRO_GARANTIA <> 0);
      ...
      SET @COTIZACION = (SELECT TOP 1 TIPO_CAMBIO_OFICIAL FROM HISTORICOTIPOSCAMBIO WITH(NOLOCK) WHERE MONEDA=2 AND FECHA_COTIZACION <= @FECHA_PROCESO ORDER BY FECHA_COTIZACION DESC)
			...
			UNION ALL
				...
				SELECT DISTINCT
                 ...
                 ---CASE WHEN S.C1785 = 6 THEN VA.FECHA_VENCIMIENTO WHEN S.C1785 = 1 THEN TMU. ELSE S.C1627 END AS FECHA_VENCIMIENTO,
                 0 AS CANT_CUOTAS,
                 S.C1806, --LIMITE EN CUOTAS Y AL CONTADO
                 C.CATEGORIARESULTANTE,
                 0 AS ID_CONVENIO,
                 NULL NomConvPago,
                 0 AS ID_DOMINIO,
				 ...
				 LEFT JOIN TJC_ADINTAR_DEUDASPLAS TAD WITH(NOLOCK) ON TAD.NRO_CUENTA = TMU.USUARIO AND TAD.ADMINISTRADORA = TMU.ADMINISTRADORA AND TAD.TZ_LOCK = 0
				 LEFT JOIN TJC_SALDOS_RRII TSR WITH(NOLOCK) ON TSR.USUARIO = TMU.USUARIO AND TSR.ADMINISTRADORA = TMU.ADMINISTRADORA AND TSR.TZ_LOCK = 0
			...
			UNION ALL
               ...
               --CC con acuerdo sin sobregiro o con acuerdo no utilizado
                ...
                FROM @INFO_GENERAL I
                WHERE EXISTS (SELECT * FROM CRE_HISTORICO_DEUDA_GARANTIAS HD where HD.SALDO_JTS_OID = I.SALDO_JTS_OID
