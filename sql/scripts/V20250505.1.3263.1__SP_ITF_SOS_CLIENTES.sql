execute('
ALTER           PROCEDURE [dbo].[SP_ITF_SOS_CLIENTES]
	@Fecha VARCHAR(8)
AS
...
SET @FechaDesde = CONVERT(VARCHAR, DATEADD(year, -1,@Fecha),112);
SET @FechaHasta = @Fecha;

--Limpiamos por si hay reproceso
DELETE FROM SOS_CLIENTES WHERE convert(VARCHAR,FECHADEINFORMACION) = @FECHAPROCESO;
...
SELECT DISTINCT b.* 
  INTO #TMP_SOS_ACT_ECO
FROM #TMP_CLIENTES a
  INNER JOIN VW_SOS_ACT_ECO b
ON a.NROPERSFISICAJURIDICA = b.CODIGO_PERSONA
...
--4
CONVERT(VARCHAR,doc.NUMERODOCUMENTO) AS IDENTTRIBUTARIANUMERO,
--5
...
--6
CONVERT(VARCHAR,doc.NUM_DOC_FISICO) AS DOCIDENTIFICATORIANUMERO,
--7
...
--16
(SELECT TOP 1 cliact.CODIGO_BCRA FROM #TMP_SOS_ACT_ECO cliact (NOLOCK)
...
'''' AS FECHABAJA,
--30
CASE WHEN nat.SUC_ALTA>30 THEN 30 ELSE nat.SUC_ALTA END,
--31
...
--37
(SELECT TOP 1 cliact.CODIGO_ACT_AFIP FROM #TMP_SOS_ACT_ECO cliact	
  WHERE cliact.CODIGO_PERSONA = xcli.NROPERSFISICAJURIDICA
...
/*(CASE WHEN (SELECT COUNT(1) FROM PRODUCTOS pro (NOLOCK)
  ...
  (CASE WHEN (SELECT count(1) FROM CRE_SOL_ACREDITACIONES_SUELDOS A (NOLOCK)
			  ...
			  INNER JOIN VW_CLI_X_DOC c (NOLOCK) ON c.CODIGOCLIENTE = B.C1803
			    AND c.NUMERODOC = doc.NUMERODOCUMENTO AND c.TIPODOC = doc.TIPODOCUMENTO) > 0 THEN ''Y'' ELSE ''N'' END)  AS MARCASUELDO,
--48
0 AS TAMANIOEMPRESA,
--49
...
--6
CONVERT(VARCHAR,doc.NUMERODOCUMENTO) AS DOCIDENTIFICATORIANUMERO,
--7
...
'''' AS FECHABAJA,
--30
CASE WHEN jur.SUC_ALTA>30 THEN 30 ELSE jur.SUC_ALTA END,
--31
...
--37
(SELECT TOP 1 cliact.CODIGO_ACT_AFIP FROM #TMP_SOS_ACT_ECO cliact	(NOLOCK)
  WHERE cliact.CODIGO_PERSONA = xcli.NROPERSFISICAJURIDICA
    AND cliact.tipo_actividad = ''Primaria'') AS CODIGOACTIVIDADAFIP,
--38
(SELECT TOP 1 cliact.CODIGO_ACT_AFIP FROM #TMP_SOS_ACT_ECO cliact	(NOLOCK)
...
isnull(bal.PERSONAL_OCUPADO,0) AS TAMANIOEMPRESA,
--49
(SELECT mono.MONOTRIBUTO_CATEGORIA FROM ITF_AFIP_PUCA mono (NOLOCK) WHERE mono.CUIT = doc.NUMERODOCUMENTO) AS CLIMONTRIBIVA,
--
  ...
  LEFT JOIN (SELECT A.*
			...
			WHERE a.F_INICIO = (SELECT MAX(F_INICIO) 
								...
								WHERE T.ID_PERSONA = A.ID_PERSONA 
								  ...
								  AND T.TZ_LOCK = 0)
			  AND A.TZ_LOCK = 0) bal ON bal.DOCUMENTO = doc.NUMERODOCUMENTO AND bal.TIPO_DOCUMENTO = doc.TIPODOCUMENTO
WHERE jur.TZ_LOCK = 0
  AND dir.TZ_LOCK = 0
  AND doc.TZ_LOCK = 0
