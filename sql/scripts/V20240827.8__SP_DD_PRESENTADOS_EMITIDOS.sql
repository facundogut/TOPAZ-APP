...
--							AND TZ_LOCK=0) 
--				AND @t_trace2pre IS NULL 
--				AND @t_trace1pre IS NOT NULL)
--caso 1
--PRESENTACIÒN 1ER VTO < FECHA PROCESO < FECHA VENCIEMIENTO
--TRACE_N_1PRE<>null
--TRACE_N_2PRE=null
			
			IF (( @T_PRES_1ER_VTO<=@fecha_proceso
				 AND @T_FECHA_VTO>@fecha_proceso
				 AND @t_estado='PR'
				AND @t_trace2pre IS NULL 
				AND @t_trace1pre IS NOT NULL)
				OR
				(@T_PRES_2DO_VTO<=@fecha_proceso
				AND @T_SEGUNDO_VTO>@fecha_proceso
				AND @t_estado='PR'
				AND @t_trace2pre IS NOT NULL 
				AND @t_trace1pre IS NOT NULL)
				)
			BEGIN 
