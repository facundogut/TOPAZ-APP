...
WITH AUX AS ( 
-- ALTAS PERSONAS FISICAS 
SELECT DISTINCT
		@fch_proceso as FECHAPROCESO,
		D.NUMERODOCUMENTO, 
...
WHERE  C.ESTADO=1 
	...
	AND PF.NUMEROPERSONAFISICA NOT IN (SELECT NUMEROPERSONAFISICA 
										FROM BITACORA_PERSONAS_FISICAS 
										WHERE TIPO_TRAZA=''A'' AND 
											TZ_LOCK=0 AND 
											FECHA BETWEEN @fch_proceso AND EOMONTH(@fch_proceso) AND 
											NUMEROPERSONAFISICA = PF.NUMEROPERSONAFISICA)
