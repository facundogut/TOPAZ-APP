EXECUTE('
ALTER           PROCEDURE [dbo].[SP_ITF_SOS_TRANSFERENCIAS]
    @FechaIni VARCHAR(8),
	@FechaFin VARCHAR(8)
AS
...
BEGIN
  ...
  SET @PERIODO = SUBSTRING(CONVERT(VARCHAR, @FechaIni, 112),1,6);
	...
	--7
	 CONVERT(VARCHAR,(SELECT SUCURSALBCRA FROM SOS_SUCURSALES sucx WHERE sucx.Sucursal = sa.Sucursal)) AS SUCURSALSISCEN,
	--8
	CONVERT(VARCHAR,sa.SUCURSAL) AS SUCURSALINTERNA,
	--9
	...
	--16
	(CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.TIPOPERACION NOT IN (101, 103) THEN '''' 
		 WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.TIPOPERACION IN (101, 103) THEN 
		    ISNULL(vt.BEN_MISMO_TITULAR, ISNULL((CASE WHEN JSON_VALUE(infoExtendidaMeta,''$.mismotitular'') = ''C'' THEN ''S''
		                   ...
		                   /*(CASE WHEN (SELECT COUNT(DISTINCT CLIENTE) 
		                    ...
		                    ELSE ''NO'' END)*/) END)AS DATOSADICIONALES,
	--17
	...
	1.1 Fin */
	(CASE WHEN ASI.EXTORNO = 1 THEN ''SI''ELSE ''NO'' END) AS REVERSADA, --1.1
	--21
	'''' AS OPER_ID_BIC_SWIFT,
	--22
	LEFT((CASE WHEN sos.TipOperacion = 101 THEN
