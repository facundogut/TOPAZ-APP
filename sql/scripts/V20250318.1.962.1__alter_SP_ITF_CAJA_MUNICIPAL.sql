execute('
CREATE OR ALTER   PROCEDURE [dbo].[SP_ITF_CAJA_MUNICIPAL] 
  @formato AS VARCHAR(20), 
  @fch_proceso AS VARCHAR(8), 
  @nomArchivo AS VARCHAR(15), 
  @ticket AS VARCHAR(15)
AS
BEGIN
	--------------- DECLARO VRIABLES --------------- 						
	...
	DECLARE @hayError NUMERIC(12) = (SELECT count(id_cabezal) FROM REC_EXT_RECAUDOS_CANAL_AUX (NOLOCK) WHERE id_cabezal = @ticket AND nro_11 = 1) 
	------------------------------------------------		
	
	IF @formato = ''CM01''
	BEGIN
	--Seteo error en los registros que no tienen ente asociado
	UPDATE REC_EXT_RECAUDOS_CANAL_AUX SET cadena_15 = ''No existe ente.'', nro_11 = 1 WHERE ID_CABEZAL = @ticket AND CADENA_03 NOT IN (SELECT cod_ente FROM CONV_REL_ENTECONV (nolock) WHERE TZ_LOCK = 0)

	IF (SELECT COUNT(1) FROM REC_EXT_RECAUDOS_CANAL_AUX ext WHERE ext.ID_CABEZAL = @ticket AND ext.NRO_11 = 1) = 0 --no hay registros con error
	BEGIN
	
		DECLARE cursor_ayuda cursor FOR
			SELECT DISTINCT cadena_03 FROM REC_EXT_RECAUDOS_CANAL_AUX ext 
		...
		BEGIN		
			...
			DECLARE cursor_convenios cursor FOR
				...
				FROM REC_EXT_RECAUDOS_CANAL_AUX WHERE FORMATO= @formato AND FECHA= @fch_proceso AND CADENA_03= @codEnte  AND ID_CABEZAL = @ticket AND NRO_11 = 0 ;	
 ...
 ----------------------------------------------------------------------
 ----------------------------------------------------------------------
 IF @formato = ''CM'' AND @hayError = 0
	BEGIN
		SET @convenio = (SELECT NUMERICO FROM PARAMETROSGENERALES WHERE CODIGO = 782 AND TZ_LOCK = 0)
		SET @descrEnte = (SELECT NomConvRec FROM CONV_CONVENIOS_REC WHERE Id_ConvRec = @convenio AND TZ_LOCK = 0)
		SET @ID_CABEZAL = ( SELECT (ISNULL(max(ID),0)+1) AS MAX_ID FROM REC_CAB_RECAUDOS_CANAL );
		SET @CANT_REG = ( SELECT count(1) FROM REC_EXT_RECAUDOS_CANAL_AUX WHERE FORMATO= @formato AND FECHA= @fch_proceso );
		SET @TOT_IMPORTE = ( SELECT sum(isnull(nro_02,0) + isnull(nro_04, 0)) FROM REC_EXT_RECAUDOS_CANAL_AUX WHERE FORMATO= @formato AND FECHA= @fch_proceso );
		SET @FechaCarga = (SELECT fechaproceso FROM PARAMETROS);
		
		INSERT INTO dbo.REC_CAB_RECAUDOS_CANAL (ID, ESTADO, ARCHIVO, CONVENIO, FECHACARGA, ID_LIQUIDACION, MONEDA, TOTALREGISTROS, TOTALIMPORTE, 
		    TZ_LOCK, TOTAL_CARGO_ESPECIFICO, NOMBRE, FECHA_COBRANZA)
			VALUES (@ID_CABEZAL, ''I'', @nomArchivo, @convenio, @FechaCarga, 0, 1, @CANT_REG, @TOT_IMPORTE, 
			@TZ_LOCK, 0, @descrEnte, @fch_proceso);
			
		DECLARE cursor_convenios_3 cursor FOR
			SELECT NRO_01, NRO_02, NRO_03, NRO_04, NRO_05, NRO_06, NRO_07, NRO_08, NRO_09, NRO_10, NRO_11, CADENA_01, CADENA_02, 
			CADENA_13, CADENA_14, CADENA_15, CADENA_03, CADENA_04, CADENA_05, CADENA_06, CADENA_07, CADENA_08, CADENA_09, 
			CADENA_10, CADENA_11, CADENA_12, FECHA_01
			FROM REC_EXT_RECAUDOS_CANAL_AUX WHERE FORMATO= @formato AND FECHA= @fch_proceso;
		
		
		OPEN cursor_convenios_3;
		FETCH NEXT FROM cursor_convenios_3 INTO @NRO_01,@NRO_02,@NRO_03,@NRO_04,@NRO_05,@NRO_06,@NRO_07,@NRO_08,@NRO_09,@NRO_10,
		@NRO_11,@CADENA_01,@CADENA_02,@CADENA_13,@CADENA_14,@CADENA_15,@CADENA_03,@CADENA_04,@CADENA_05,@CADENA_06,@CADENA_07,@CADENA_08,
		@CADENA_09,@CADENA_10,@CADENA_11,@CADENA_12, @FECHA_01
		...
		WHILE @@fetch_status = 0 
			BEGIN
				...
				(ID_CABEZAL, ID_LINEA, MONEDA, IMPORTE, CODIGO_BARRAS, CODIGO_BARRAS_RENDIDO, ESTADO, DETALLE_ESTADO, 
				TOTAL_CARGO_ESPECIFICO, TZ_LOCK, FECHA_COBRANZA, COMPROBANTE)
				VALUES (@ID_CABEZAL, @ID_LINEA, CAST(@CADENA_05 AS NUMERIC(4,0)), (isnull(@NRO_02, 0) + isnull(@NRO_04, 0)),  @CADENA_08, '''', ''I'', ''Ingresado'', 
				0, @TZ_LOCK, @FECHA_01, @CADENA_01);
