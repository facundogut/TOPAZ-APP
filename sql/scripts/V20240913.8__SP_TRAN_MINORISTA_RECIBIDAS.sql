...
--		DECLARE @totContrAux VARCHAR(12);
		DECLARE @valMandFA NUMERIC(1);	
			...
			DECLARE @BenBanco NUMERIC(6,0);
				DECLARE @nombreEO VARCHAR(70)
				DECLARE @nombreBEN VARCHAR(70)
	...
	BEGIN
      ...
      BEGIN
			...
			SELECT TOP 1 @nombreeo=substring(isnull(nombre_completo,''),1,70) FROM ITF_BCRA_PADFYJ WHERE CUIT=@cuit_EO AND TZ_LOCK=0
...
--			PRINT @CUIT_BEN
			SELECT TOP 1 @nombreBEN=substring(isnull(nombre_completo,''),1,70) FROM ITF_BCRA_PADFYJ WHERE CUIT=@cuit_ben AND TZ_LOCK=0
...
--			SET @CUIT_BEN = TRY_CONVERT(NUMERIC(11),(SELECT TOP 1 SUBSTRING(LINEA,4,11) FROM ITF_TRAN_MINORISTA_RECIBIDAS_AUX WHERE ID>@ID AND LINEA LIKE '7%'));
...
IF @contadorregistros LIKE '%4400993003318%'PRINT @ContadorRegistros
				...
				BEGIN TRY
IF ( UPPER(SUBSTRING(@ARCHIVO,1,2))!='SP' AND (@cuit_EO IS NULL OR @cuit_EO=0) AND @RegistrosAdicionales='1')
BEGIN 
...
--		  SET @FECHADATE = (SELECT top 1 FECHAPROCESO FROM PARAMETROS);
	      ...
	      BEGIN
			...
			SET ESTADO = 'RC' 
				, FECHA_ESTADO=@FECHADATE 
				, NUMERO_ASIENTO=@TICKET
				, MOTIVO_RECHAZO=(SELECT try_convert(NUMERIC(15),substring(linea,34,2)) FROM ITF_TRAN_MINORISTA_RECIBIDAS_AUX WHERE ID=@id+1)
				, OPERATORIA_BCRA=@Operatoria_BCRA
				, FCH_DEV_REC=@FECHACOMPENSACION
			...
			isnumeric(ltrim(rtrim(@ContadorRegistros)))=0 
		  	...
		  	INSERT INTO dbo.VTA_TRANSFERENCIAS (OP_NUMERO, NUMERO_ASIENTO,  OP_FECHA_PRES, OP_CLASE_TRANS, ESTADO, OP_MONEDA, OP_TIPO, BEN_CBU, OP_IMPORTE, BEN_JTS_OID_CUENTA, BEN_NRO_DOC, MOTIVO_RECHAZO, OP_FORMA_ING, FECHA_ESTADO, CONTABILIZADA, OP_INFO_REF,OPERATORIA_BCRA, FCH_DEV_REC, TRACENUMBER 
				...
				VALUES ((SELECT ISNULL(MAX(OP_NUMERO),0)+1 FROM VTA_TRANSFERENCIAS WHERE OP_CLASE_TRANS='R')
											...
											, 'R'
											, 'RC'
											, @Moneda, @Tipo_transferencia, @CBU, @VImporte, @JTS_OID, @CUIT_BEN
...
--	      BEGIN
--	      		INSERT INTO dbo.VTA_TRANSFERENCIAS (OP_NUMERO, NUMERO_ASIENTO,  OP_FECHA_PRES, OP_CLASE_TRANS, ESTADO, OP_MONEDA, OP_TIPO, BEN_CBU, OP_IMPORTE, BEN_JTS_OID_CUENTA, BEN_NRO_DOC, MOTIVO_RECHAZO, OP_FORMA_ING, FECHA_ESTADO, CONTABILIZADA , OP_INFO_REF,OPERATORIA_BCRA, FCH_DEV_REC, TRACENUMBER )
--				VALUES ((SELECT ISNULL(MAX(OP_NUMERO),0)+1 FROM VTA_TRANSFERENCIAS WHERE OP_CLASE_TRANS='R'), @TICKET, @FECHADATE, 'R', 'RC', @Moneda, @Tipo_transferencia, @CBU, @Importe, @JTS_OID, @CUIT_BEN,(SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 4), 'C' , @FECHADATE, 'N', RTRIM(LTRIM(@ReferenciaUnivoca)),@OPERATORIA_BCRA, @FECHACOMPENSACION, TRY_CONVERT(NUMERIC(15),@ContadorRegistros));
--
--	      	GOTO Final
--	      END
	      ...
	      BEGIN
				...
				VALUES ((SELECT ISNULL(MAX(OP_NUMERO),0)+1 FROM VTA_TRANSFERENCIAS WHERE OP_CLASE_TRANS='R')
						...
						, @JTS_OID
						, @CUIT_BEN
						, (SELECT try_convert(INT,substring(CODIGO_NACHA,2,2))  FROM SNP_MOTIVOS_RECHAZO nolock WHERE id_motivo=1 AND tZ_LOCK=0)
						, 'C' 
						, @FECHADATE
						, 'N'
						, RTRIM(LTRIM(@ReferenciaUnivoca))
...
--					AND FJ.NUMERODOCUMENTO IN (SELECT NUMERODOC
--												FROM VW_CLIENTES_PERSONAS
--												WHERE CODIGOCLIENTE IN (SELECT C1803
--																		FROM SALDOS
--																		WHERE JTS_OID IN (SELECT JTS_OID_SALDO
--																							FROM VTA_SALDOS
--																							WHERE CTA_CBU=@CBU)																		
--																		)
--												AND TITULARIDAD='T'
    ...
    WHERE cp.TITULARIDAD = 'T'
      ...
      AND fj.TZ_LOCK = 0
	      ...
	      BEGIN
				...
				VALUES ((SELECT MAX(OP_NUMERO)+1 FROM VTA_TRANSFERENCIAS), @TICKET, @FECHADATE, 'R', 'RC', 1, @Tipo_transferencia, @CBU, @VImporte, @JTS_OID, @CUIT_BEN,(SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 92), 'C' , @FECHADATE, 'N', RTRIM(LTRIM(@ReferenciaUnivoca)));
...
--	      PRINT @TipoCuenta
				...
				VALUES ((SELECT ISNULL(MAX(OP_NUMERO),0)+1 FROM VTA_TRANSFERENCIAS WHERE OP_CLASE_TRANS='R'), @TICKET, @FECHADATE, 'R', 'RC', @Moneda, @Tipo_transferencia, @CBU, @Importe, @JTS_OID, @CUIT_BEN,(SELECT try_convert(INT,substring(CODIGO_NACHA,2,2))  FROM SNP_MOTIVOS_RECHAZO nolock WHERE id_motivo=6 AND tZ_LOCK=0), 'C' , @FECHADATE, 'N', RTRIM(LTRIM(@ReferenciaUnivoca)),@OPERATORIA_BCRA, @FECHACOMPENSACION, TRY_CONVERT(NUMERIC(15),@ContadorRegistros)
										 , @cuit_eo
						 , @nombreEO
						 , @BenBanco
						 , CASE WHEN substring (@CUIT_BEN,1,1)='2' THEN 'CUIL' WHEN substring (CONVERT(VARCHAR,@CUIT_BEN),1,1)='3' THEN 'CUIT' END
		  ...
		  IF @CODREGISTRO_IN='CCD' AND (SELECT TOP 1 PRODUCTO FROM SALDOS NOLOCK  WHERE CUENTA=@NUMcUENTA AND jts_oid=@JTS_OID AND TZ_LOCK=0) NOT IN (1,3,4,6,9,10,16,23)--(SELECT C6250 FROM productos NOLOCK WHERE TIPO_CUENTA_VISTA IN (7,11,5,8) AND TZ_LOCK=0)
			BEGIN 
	      		INSERT INTO dbo.VTA_TRANSFERENCIAS (OP_NUMERO, NUMERO_ASIENTO,  OP_FECHA_PRES, OP_CLASE_TRANS, ESTADO, OP_MONEDA, OP_TIPO, BEN_CBU, OP_IMPORTE, BEN_JTS_OID_CUENTA, BEN_NRO_DOC, MOTIVO_RECHAZO, OP_FORMA_ING, FECHA_ESTADO, CONTABILIZADA, OP_INFO_REF,OPERATORIA_BCRA, FCH_DEV_REC, TRACENUMBER 
	      													, ORD_NRO_DOC
											, ORD_NOMBRE
				...
				VALUES ((SELECT ISNULL(MAX(OP_NUMERO),0)+1 FROM VTA_TRANSFERENCIAS WHERE OP_CLASE_TRANS='R'), @TICKET, @FECHADATE, 'R', 'RC', @Moneda, @Tipo_transferencia, @CBU, @Importe, @JTS_OID, @CUIT_BEN,(SELECT try_convert(INT,substring(CODIGO_NACHA,2,2))  FROM SNP_MOTIVOS_RECHAZO nolock WHERE id_motivo=15 AND tZ_LOCK=0), 'C' , @FECHADATE, 'N', RTRIM(LTRIM(@ReferenciaUnivoca)),@OPERATORIA_BCRA, @FECHACOMPENSACION, TRY_CONVERT(NUMERIC(15),@ContadorRegistros)
										 , @cuit_eo
						 , @nombreEO
						 , @BenBanco
						 , CASE WHEN substring (@CUIT_BEN,1,1)='2' THEN 'CUIL' WHEN substring (CONVERT(VARCHAR,@CUIT_BEN),1,1)='3' THEN 'CUIT' END
	      ...
	      BEGIN
				...
				VALUES ((SELECT ISNULL(MAX(OP_NUMERO),0)+1 FROM VTA_TRANSFERENCIAS WHERE OP_CLASE_TRANS='R'),@TICKET, @FECHADATE, 'R', 'RC', @Moneda, @Tipo_transferencia, @CBU, @Importe, @JTS_OID, @CUIT_BEN,(SELECT try_convert(INT,substring(CODIGO_NACHA,2,2))  FROM SNP_MOTIVOS_RECHAZO nolock WHERE id_motivo=25 AND tZ_LOCK=0), 'C' , @FECHADATE, 'N', RTRIM(LTRIM(@ReferenciaUnivoca)),@OPERATORIA_BCRA, @FECHACOMPENSACION, TRY_CONVERT(NUMERIC(15),@ContadorRegistros)
										 , @IdEmpresa
						 , @nombreEO
						 , @BenBanco
						 , CASE WHEN substring (@CUIT_BEN,1,1)='2' THEN 'CUIL' WHEN substring (CONVERT(VARCHAR,@CUIT_BEN),1,1)='3' THEN 'CUIT' END
...
--		  IF (@CodigoOrigen!=0 AND (@cuit_eo NOT IN (SELECT CUIT_EO FROM SNP_PRESTACIONES_EMPRESAS WHERE ENTIDAD = @IdEntidadOrigen)))
	      ...
	      BEGIN
				...
				VALUES ((SELECT ISNULL(MAX(OP_NUMERO),0)+1 FROM VTA_TRANSFERENCIAS WHERE OP_CLASE_TRANS='R'),@TICKET, @FECHADATE, 'R', 'RC', @Moneda, @Tipo_transferencia, @CBU, @Importe, @JTS_OID, @CUIT_BEN,(SELECT try_convert(INT,substring(CODIGO_NACHA,2,2))  FROM SNP_MOTIVOS_RECHAZO nolock WHERE id_motivo=17 AND tZ_LOCK=0), 'C' , @FECHADATE, 'N', RTRIM(LTRIM(@ReferenciaUnivoca)),@OPERATORIA_BCRA, @FECHACOMPENSACION, TRY_CONVERT(NUMERIC(15),@ContadorRegistros)
										 , @cuit_eo
						 , @nombreEO
						 , @BenBanco
						 , CASE WHEN substring (@CUIT_BEN,1,1)='2' THEN 'CUIL' WHEN substring (CONVERT(VARCHAR,@CUIT_BEN),1,1)='3' THEN 'CUIT' END
	      ...
	      BEGIN
	      		INSERT INTO dbo.VTA_TRANSFERENCIAS (OP_NUMERO
	      									...
	      									, BEN_CBU
	      									, OP_IMPORTE
	      									, BEN_JTS_OID_CUENTA, BEN_NRO_DOC, MOTIVO_RECHAZO, OP_FORMA_ING, FECHA_ESTADO, CONTABILIZADA, OP_INFO_REF,OPERATORIA_BCRA, FCH_DEV_REC, TRACENUMBER
	      													, ORD_NRO_DOC
											, ORD_NOMBRE
											, BEN_BANCO
											, BEN_TIPO_DOC 
				...
				VALUES ((SELECT ISNULL(MAX(OP_NUMERO),0)+1 FROM VTA_TRANSFERENCIAS WHERE OP_CLASE_TRANS='R'),@TICKET, @FECHADATE, 'R', 'RC', @Moneda, @Tipo_transferencia, @CBU, @Importe, @JTS_OID, @CUIT_BEN,(SELECT try_convert(INT,substring(CODIGO_NACHA,2,2))  FROM SNP_MOTIVOS_RECHAZO nolock WHERE id_motivo=9 AND tZ_LOCK=0), 'C' , @FECHADATE, 'N', RTRIM(LTRIM(@ReferenciaUnivoca)),@OPERATORIA_BCRA, @FECHACOMPENSACION
						, TRY_CONVERT(NUMERIC(15),@ContadorRegistros)
												 , @cuit_eo
						 , @nombreEO
						 , @BenBanco
