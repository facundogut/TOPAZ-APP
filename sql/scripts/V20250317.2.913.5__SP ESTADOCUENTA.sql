	...
	BEGIN
		...
		SELECT
			...
			(CASE
				...
				WHEN m.CodTransaccion IN (28,29,30,31,32,33,34)
					THEN
						''Originante: '' + cp.NOMBRE +
						'' CUIT/CUIL/DNI: '' + cp.NUMERODOC +
						'' Ref.: '' + mc.CONCEPTO
				/* Transferencias para débito */
				WHEN m.CodTransaccion IN (21,22,23,24,25,26,27)
					THEN
						''CUIT Destino: '' +  cp.NUMERODOC +
						'' Descripción: '' +  cp.NOMBRE +
						'' Ref.: '' + mc.CONCEPTO
				/* Plazo fijo */
				WHEN m.CodTransaccion IN (48,49,50,106,107)
					THEN
						''Suc.: '' + convert(VARCHAR(8), h.SUCURSALMOV) + 
							'' Cert.: '' + str((
											SELECT ps.CERTIFICADO_DPF 
											FROM PZO_SALDOS ps 
				...
				/* Pago de Cheques */
				WHEN m.CodTransaccion IN (5,6,7,8,9,11,12,40,41,42,43,44,85,87,88,89,90,91,98,99,3018,3019,3020)
					THEN
