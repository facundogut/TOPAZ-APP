...
BEGIN
	...
	DECLARE @CA_ORIGEN_INMEDIATO VARCHAR(10) = (SELECT '' 031100''+CAST((SELECT NUMERICO FROM PARAMETROSGENERALES WITH (NOLOCK) WHERE CODIGO=40) AS VARCHAR)+''0''); 
	...
	DECLARE @CL_ID_ENTIDAD_ORIGEN VARCHAR(8) = (SELECT ''031100''+CAST((SELECT NUMERICO FROM PARAMETROSGENERALES WITH (NOLOCK) WHERE CODIGO=40) AS VARCHAR)); 
	...
	DECLARE @FL_REG_ENTIDAD_ORIGEN VARCHAR(8) = (SELECT ''0311''+ RIGHT(concat(replicate(''0'', 4), CAST((SELECT NUMERICO FROM PARAMETROSGENERALES WITH (NOLOCK) WHERE CODIGO=40) AS VARCHAR)), 4)); 
	...
	DECLARE @fecha_proceso DATETIME =(SELECT fechaproceso FROM parametros WITH (NOLOCK))
	...
	SELECT @T_CONTADOR_1 = m.NUMERICO_1,@T_CONTADOR_2 = m.NUMERICO_1
	FROM ITF_MASTER_PARAMETROS m WITH (NOLOCK)
	CROSS JOIN PARAMETROS p WITH (NOLOCK)
	WHERE m.CODIGO = 300 AND m.FECHA = p.FECHAPROCESO
	

	IF((SELECT COUNT(1) 
		FROM SNP_MSG_ORDENES o WITH (NOLOCK)
		LEFT JOIN SNP_MSG_CABEZAL c WITH (NOLOCK) ON o.CUIT_EO=c.CUIT_EO AND o.ID_ARCHIVO=c.ID_ARCHIVO AND o.NRO_ARCHIVO=c.NRO_ARCHIVO AND o.TZ_LOCK=0 AND c.TZ_LOCK=0 
		LEFT JOIN SNP_CUENTAS_RELACIONADAS r WITH (NOLOCK) ON r.CUIT_EO=o.CUIT_EO AND r.PRESTACION=o.PRESTACION AND r.RELACION_CUENTA=1 and r.TZ_LOCK=0
 		WHERE ((o.ESTADO=''PP'' AND o.PRESENTACION_PRIMER_VTO=(SELECT fechaproceso FROM PARAMETROS WITH (NOLOCK))) OR (o.ESTADO=''RC'' AND o.PRESENTACION_SEGUNDO_VTO=(SELECT fechaproceso FROM PARAMETROS WITH (NOLOCK))))
		AND o.TZ_LOCK=0)  
		+
	    (SELECT COUNT(1) 
		FROM SNP_ADHESIONES o WITH (NOLOCK)
		LEFT JOIN SNP_CUENTAS_RELACIONADAS r WITH (NOLOCK) ON r.CUIT_EO=o.CUIT_EO AND r.PRESTACION=o.PRESTACION AND r.RELACION_CUENTA=1 and r.TZ_LOCK=0
		WHERE o.TZ_LOCK=0 
	...
	IF ISNULL((SELECT a.FECHA FROM ITF_MASTER_PARAMETROS a WITH (NOLOCK) WHERE a.CODIGO = 148), CAST(''01-01-1800'' AS DATETIME)) <> (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK))
	...
	SELECT o.CORRELATIVO AS CORRELATIVO,
			...
			o.MONEDA AS MONEDA,
			Case When o.ESTADO=''PP'' Then o.IMPORTE ELSE o.segundo_importe END AS IMPORTE, --Para Secheep o ATP,etc , el imp de 2 presentación es distinto al de primera -- AE 31/3/25 - CDS 
	...
	BEGIN
        --PRINT ''Prestacion: '' + @T_PRESTACION
        
		--SET @CL_DESCRIP_TRANSAC= LEFT(concat(replicate('' '', 10), @T_PRESTACION), 10);
        SET @CL_DESCRIP_TRANSAC = LEFT(CONCAT(TRIM(@T_PRESTACION), REPLICATE('' '', 10)), 10);

        --Print ''Desc: '' + @CL_DESCRIP_TRANSAC
		IF @T_TIPO=''O''
		BEGIN	
		...
		BEGIN 
			...
			IF NOT EXISTS (SELECT 1 FROM snp_debitos WITH (NOLOCK) WHERE ID_DEBITO=try_convert(NUMERIC(15,0),@t_id_archivo_reversado) AND TZ_LOCK=0)
			BEGIN 
		...
		IF(@T_MONEDA=(SELECT C6399 FROM MONEDAS WITH (NOLOCK) WHERE C6403=''N''))
			SET @CL_ID_ENTIDAD_ORIGEN  = (SELECT ''031100''+CAST((SELECT NUMERICO FROM PARAMETROSGENERALES WITH (NOLOCK) WHERE CODIGO=40) AS VARCHAR));
		ELSE
			SET @CL_ID_ENTIDAD_ORIGEN  = (SELECT ''081100''+CAST((SELECT NUMERICO FROM PARAMETROSGENERALES WITH (NOLOCK) WHERE CODIGO=40) AS VARCHAR));
		...
		BEGIN
			...
			SET @CL_NOM_EMPRESA =  RIGHT(concat(replicate('' '', 16),(SELECT NOMBRE_EMPRESA FROM SNP_PRESTACIONES_EMPRESAS WITH (NOLOCK) WHERE CUIT_EO=@T_CUIT AND PRESTACION=@T_PRESTACION)),16);
			SET @CL_CRITERIO_EMPRESA = replicate('' '', 20);
			SET @CL_ID_EMPRESA = LEFT(concat(@T_CUIT,replicate('' '', 10)),10);
			SET @CL_CABECERA = concat(@CL_ID_REG
										, @CL_CODIGO_CLASE_TRANSAC
			...
			IF(@T_MONEDA=(SELECT C6399 FROM MONEDAS WITH (NOLOCK) WHERE C6403=''N''))	
				SET @FL_REG_ENTIDAD_ORIGEN = (SELECT ''0311''+ RIGHT(concat(replicate(''0'', 4), CAST((SELECT NUMERICO FROM PARAMETROSGENERALES WITH (NOLOCK) WHERE CODIGO=40) AS VARCHAR)), 4)); 
			ELSE
				SET @FL_REG_ENTIDAD_ORIGEN = (SELECT ''0811''+ RIGHT(concat(replicate(''0'', 4), CAST((SELECT NUMERICO FROM PARAMETROSGENERALES WITH (NOLOCK) WHERE CODIGO=40) AS VARCHAR)), 4)); 
		END
		
		
		
		SELECT @T_SUCURSAL=SUCURSAL, @T_CUENTA=CUENTA FROM SALDOS WITH (NOLOCK) WHERE JTS_OID=@T_JTS_OID;
...
--			SET @T_TRACENUMBER = concat(''0311'', RIGHT(concat(replicate(''0'', 4), (@T_SUCURSAL_TN)), 4), RIGHT(concat(replicate(''0'', 7), @T_CONTADOR_2), 7)); 
        ...
        --PRINT ''Referencia UNI: '' + @RI_REFERENCIA_UNIVOCA
        ...
        --PRINT ''Cliente Pag'' + @RI_CLIENTE_PAGADOR
		...
		END
   		...
   		IF @T_CODIGO_TRANSACCION=''37'' AND (@t_id_archivo_reversado IS NOT NULL AND EXISTS (SELECT 1 FROM snp_debitos WITH (NOLOCK) WHERE ID_DEBITO=try_convert(NUMERIC(15,0),@t_id_archivo_reversado) AND TZ_LOCK=0))
			BEGIN 
				SELECT TOP 1 @tracenumber_original=TRacknumber					
				FROM SNP_debitos WITH (NOLOCK)
				WHERE ID_DEBITO=try_convert(NUMERIC(15,0), @t_id_archivo_reversado)
				AND TZ_LOCK=0
				SET @RI_ENTIDAD_DEBITAR = LEFT(@tracenumber_original,8);
			END 
		...
		IF @t_tipo=''O''
		BEGIN 
			IF @t_id_archivo_reversado IS NOT NULL AND EXISTS (SELECT 1 FROM snp_debitos WITH (NOLOCK) WHERE ID_DEBITO=try_convert(NUMERIC(15,0),@t_id_archivo_reversado) AND TZ_LOCK=0)
			BEGIN
	    		SET @T_CODIGO_TRANSACCION=''37'';
		...
		IF(@T_CODIGO_TRANSACCION=''32'' OR (@T_CODIGO_TRANSACCION=''38'' AND @RI_INFO_ADICIONAL IN (''05'',''06'')) OR (@T_CODIGO_TRANSACCION=''37'' AND @RI_INFO_ADICIONAL IN (''01'')))
   		...
   		IF(@MONEDA=(SELECT C6399 FROM MONEDAS WITH (NOLOCK) WHERE C6403=''N''))
   			SET @RI_CONTADOR_REGISTRO = concat(''0311'', RIGHT(concat(replicate(''0'', 4), (''97'')), 4), RIGHT(concat(replicate(''0'', 7), (SELECT NUMERICO_1 FROM ITF_MASTER_PARAMETROS WITH (NOLOCK) WHERE CODIGO = 148)), 7)); 
   		ELSE
   			SET @RI_CONTADOR_REGISTRO = concat(''0811'', RIGHT(concat(replicate(''0'', 4), (''97'')), 4), RIGHT(concat(replicate(''0'', 7), (SELECT NUMERICO_1 FROM ITF_MASTER_PARAMETROS WITH (NOLOCK) WHERE CODIGO = 148)), 7)); 
		...
		BEGIN 
			...
			BEGIN
				SET @RI_ENTIDAD_DEBITAR =RIGHT(concat(replicate(''0'', 8), (SELECT entidad FROM SNP_PRESTACIONES_EMPRESAS WITH (NOLOCK) WHERE CUIT_EO=@t_cuit AND PRESTACION=@T_PRESTACION)),8) ;
	...
	IF @T_CONTADOR_1 <> @T_CONTADOR_2
	BEGIN
		UPDATE dbo.ITF_MASTER_PARAMETROS
		SET NUMERICO_1 = @T_CONTADOR_1
			, FECHA = (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK))
		WHERE CODIGO = 300
	END 
...
Execute('
CREATE OR ALTER     PROCEDURE [dbo].[SP_DD_PRESENTADOS_EMITIDOS_V2]
   @TICKET NUMERIC(16)
AS 
BEGIN
	...
	DECLARE @RI_ID_REG VARCHAR(1) = ''6''; -- fijo  
	DECLARE @RI_ENTIDAD_DEBITAR VARCHAR(8);
--s	DECLARE @RI_RESERVADO VARCHAR(1) = ''0''; -- fijo 
--s	DECLARE @RI_CUENTA_DEBITAR VARCHAR(17); 
	DECLARE @RI_CUENTA_DEBITAR VARCHAR(14); 
--s	DECLARE @RI_IMPORTE VARCHAR(11); 
	...
	DECLARE @FL_CANT_REG_INDIVIDUAL_ADICIONAL VARCHAR(6) = 0; --registros individuales y adicionales que existen en el lote
	DECLARE @FL_TOTALES_DE_CONTROL VARCHAR(10) = 0;
--s	DECLARE @FL_SUMA_TOTAL_DEBITO_LOTE VARCHAR(12); 
--s	DECLARE @FL_SUMA_TOTAL_CREDITO_LOTE VARCHAR(12); 
	DECLARE @FL_SUMA_TOTAL_DEBITO_LOTE VARCHAR(20); 
	DECLARE @FL_SUMA_TOTAL_CREDITO_LOTE VARCHAR(20); 
	DECLARE @FL_RESERVADO1 VARCHAR(10) = replicate('' '', 10); -- fijo
--s	DECLARE @FL_RESERVADO2 VARCHAR(19) = ''                   ''; -- fijo
	DECLARE @FL_RESERVADO2 VARCHAR(3) = replicate('' '', 3); -- fijo
	DECLARE @FL_RESERVADO3 VARCHAR(6) = replicate('' '', 6); -- fijo
...
--s	DECLARE @FA_RESERVADO  VARCHAR(100) = replicate('' '', 39); -- fijo
	...
	SELECT o.CORRELATIVO AS CORRELATIVO,
			r.SALDO_JTS_OID AS SALDO_JTS_OID, 
			o.MONEDA AS MONEDA,
			Case When o.ESTADO=''PP'' Then o.IMPORTE ELSE o.segundo_importe END AS IMPORTE, --Para Secheep, ATP,etc , el imp de 2 presentación es distinto al de primera -- AE 31/3/25 - CDS 
			(CASE WHEN O.estado=''RC'' THEN o.SEGUNDO_VTO ELSE o.FECHA_VENCIMIENTO END) AS FECHA_VENCIMIENTO,
			o.SEGUNDO_VTO AS SEGUNDO_VTO, 
			dbo.diaHabil( dateadd(dd,1, (CASE WHEN O.estado=''RC'' THEN o.SEGUNDO_VTO ELSE o.FECHA_VENCIMIENTO END)),''A'') AS FECHA_COMPENSACION, 
			o.CBU AS CBU, 
	...
	WHILE @@FETCH_STATUS = 0
	BEGIN
		SET @CL_DESCRIP_TRANSAC = RIGHT(CONCAT(@T_PRESTACION, REPLICATE('' '', 10 - LEN(TRIM(@T_PRESTACION)))), 10);
		IF @T_TIPO=''O''
		BEGIN	
			SET @CL_FECHA_VENCIMIENTO = CONVERT(VARCHAR, @T_FECHA_VTO, 12);--revisar
			SET @CL_FECHA_COMPENSACION = convert(VARCHAR,dbo.diaHabil( dateadd(dd,1, @T_FECHA_VTO),''A''), 12); 
		...
		Start:
--s		IF (@SumaImportes > 9999999999.99 OR @SumaEntidad > 999999) -- 99 millones
		IF (@SumaImportes > 999999999999.99 OR @SumaEntidad > 999999) -- 99 millones
		BEGIN
			...
			END
	   		...
	   		SET @FL_TOTALES_DE_CONTROL = concat((RIGHT(concat(replicate(''0'', 6), @SumaEntidad), 6)), (RIGHT(concat(replicate(''0'', 4), @SumaSucursal), 4)));
--s			SET @FL_SUMA_TOTAL_DEBITO_LOTE = RIGHT(concat(replicate(''0'', 12), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,30,10)))/100) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''6%'' AND substring(linea,1,3)!=''632'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''5%'') ), ''.'', ''''))), 12);	   		
--s			SET @FL_SUMA_TOTAL_CREDITO_LOTE = RIGHT(concat(replicate(''0'', 12), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,30,10)))/100) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''632%'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''5%'') ), ''.'', ''''))), 12); 
			SET @FL_SUMA_TOTAL_DEBITO_LOTE = RIGHT(concat(replicate(''0'', 20), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,26,14)))/100) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''6%'' AND substring(linea,1,3)!=''632'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''5%'') ), ''.'', ''''))), 20);	   		
			SET @FL_SUMA_TOTAL_CREDITO_LOTE = RIGHT(concat(replicate(''0'', 20), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,26,14)))/100) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''632%'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''5%'') ), ''.'', ''''))), 20); 
	   		SET @TotalesDebitos += @SumaImportes;
	   		SET @FL_CANT_REG_INDIVIDUAL_ADICIONAL = RIGHT(concat(replicate(''0'', 6), (SELECT COUNT(*) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE (LINEA LIKE ''6%'' or linea like ''7%'') and id>(select max(id) from ITF_DD_PRESENTADOS_EMITIDOS_AUX where linea like ''5%''))), 6); 
...
--s			SET @FA_SUMA_TOTAL_DEBITOS = RIGHT(concat(replicate(''0'', 12), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,30,10)))/100) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''6%'' AND substring(linea,1,3)!=''632'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''1%'') ), ''.'', ''''))), 12);
            SET @FA_SUMA_TOTAL_DEBITOS = RIGHT(concat(replicate(''0'', 20), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,26,14)))/100) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''6%'' AND substring(linea,1,3)!=''632'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''1%'') ), ''.'', ''''))), 20);
			
--s			SET @FA_SUMA_TOTAL_CREDITOS = RIGHT(concat(replicate(''0'', 12), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,30,10)))/100) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''632%'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''1%'') ), ''.'', ''''))), 12);
			SET @FA_SUMA_TOTAL_CREDITOS = RIGHT(concat(replicate(''0'', 20), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,26,14)))/100) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''632%'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_EMITIDOS_AUX WHERE LINEA LIKE ''1%'') ), ''.'', ''''))), 20);
...
--			SET @T_TRACENUMBER = concat(''0311'', RIGHT(concat(replicate(''0'', 4), (@T_SUCURSAL_TN)), 4), RIGHT(concat(replicate(''0'', 7), @T_CONTADOR_2), 7)); 
		...
		BEGIN
	 	...
	 	BEGIN
	 		...
	 		SET @CountExcedente += 1;
	 	END
--s	 	IF	(@T_IMPORTE>99999999.99)
	 	IF	(@T_IMPORTE>999999999999.99)
	 	BEGIN
--s	 		SET @Excedente = (@T_IMPORTE - 99999999.99);
--s	 		SET @T_IMPORTE = 99999999.99;
	 	...
	 	---------------------------- Grabar Registro Individual -----------------------------------------------------------------------------------------------------------------------------------------
	 	SET @RI_ENTIDAD_DEBITAR = CONCAT(RIGHT(concat(replicate(''0'', 4), @T_COD_BANCO),4) , RIGHT(concat(replicate(''0'', 4), @T_SUCURSAL),4));
--s 	SET @RI_CUENTA_DEBITAR = RIGHT(concat(replicate(''0'', 17), SUBSTRING(@t_cbu, 9, 14)), 17);
     	SET @RI_CUENTA_DEBITAR = RIGHT(concat(replicate(''0'', 14), SUBSTRING(@t_cbu, 9, 14)), 14);
     	--SET @RI_REFERENCIA_UNIVOCA = RIGHT(concat(replicate(''0'', 15), @T_REFERENCIA), 15);
         SET @RI_REFERENCIA_UNIVOCA = LEFT(CONCAT(@T_REFERENCIA, REPLICATE('' '', 15)), 15);
     	--SET @RI_CLIENTE_PAGADOR = RIGHT(concat(replicate(''0'', 22), @T_CLIENTE_PAGADOR), 22);
        SET @RI_CLIENTE_PAGADOR = LEFT(CONCAT(@T_CLIENTE_PAGADOR, REPLICATE('' '', 22)), 22);
	 	...
	 	BEGIN
       		...
       		SET @SumaImportes += @T_IMPORTE;
       		SET @RI_ENTIDAD_DEBITAR = concat(replicate(''0'', 4), @T_COD_BANCO, ''0888'');
--s    		SET @RI_CUENTA_DEBITAR = RIGHT(concat(replicate(''0'', 17), ''88888888888''), 17);
       		SET @RI_CUENTA_DEBITAR = RIGHT(concat(replicate(''0'', 14), ''88888888888''), 14);
       		SET @RI_REFERENCIA_UNIVOCA = ''000088888888888'';
...
--s 	SET @RI_IMPORTE = RIGHT(concat(replicate(''0'', 10), replace(CAST(@T_IMPORTE AS VARCHAR),''.'','''')), 10);
		...
		BEGIN 
			...
			IF @RI_INFO_ADICIONAL =''06''
			BEGIN
--s				SET @RI_CUENTA_DEBITAR = RIGHT(concat(replicate(''0'', 17), SUBSTRING(@t_cbu_original, 9, 14)), 17);
				SET @RI_CUENTA_DEBITAR = RIGHT(concat(replicate(''0'', 14), SUBSTRING(@t_cbu_original, 9, 14)), 14);
			END		
		END 
...
--s											, @RI_RESERVADO
											...
											, @RI_INFO_ADICIONAL
											, @RI_REGISTRO_ADICIONAL
											, @T_TRACENUMBER); --antes el ultimo campo era:@RI_CONTADOR_REGISTRO
