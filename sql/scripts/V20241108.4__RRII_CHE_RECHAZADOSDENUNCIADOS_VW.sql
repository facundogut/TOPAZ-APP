...
replicate(''0'', 2 - len(Cast(MONTH(FECHA_RECHAZO_O_PRES_COBRO) AS varchar))) + CAST(MONTH(CR.FECHA_RECHAZO_O_PRES_COBRO) AS varchar) as [FechaPagoCheque_Mes], -- Se usa campo [FECHA_PAGO_CHEQUE]
replicate(''0'', 2 - len(Cast(DAY(FECHA_RECHAZO_O_PRES_COBRO) AS varchar))) + CAST(DAY(CR.FECHA_RECHAZO_O_PRES_COBRO) AS varchar) as [FechaPagoCheque_Dia], -- Se usa campo [FECHA_PAGO_CHEQUE]
--16
''00000000'' as SinUso,
--17
