...
SELECT TOP 9223372036854775807 WITH TIES 
      ...
      convert(DATETIME,(dbo.diaHabil(convert(DATETIME,(concat(
      ...
      )),103)+1, ''A'')),103) AS "Vto Opción a Compra",
      S.JTS_OID AS "JTS OID",
	  S.C1803 AS Cliente
FROM SALDOS  AS S	WITH(NOLOCK) 
INNER JOIN SUCURSALES AS SUC WITH(NOLOCK) ON S.SUCURSAL=SUC.SUCURSAL
INNER JOIN CRE_BIENES_LEASING AS B WITH(NOLOCK) ON S.JTS_OID=B.SALDOS_JTS_OID AND B.ESTADO=6
...
WHERE S.C1604=0 AND S.TZ_LOCK=0 AND B.NUMERO_BIEN NOT IN (SELECT NUMERO_BIEN FROM CRE_SOLICITUDCREDITO WHERE NUMERO_BIEN IS NOT NULL AND TZ_LOCK=0)
