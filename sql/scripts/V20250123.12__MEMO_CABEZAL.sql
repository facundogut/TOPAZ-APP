     ...
     ---DECLARE @FECHA_PROCESO DATETIME 
      ...
      DELETE FROM MEMO_CABECERA WHERE Fecha_Info = @FECHA_PROCESO
               ...
               FROM MEMO_DETALLE WITH(NOLOCK) WHERE Fecha_Info = @FECHA_PROCESO GROUP BY Num_Doc) M ON M.Num_Doc = IC.NUMERODOC
				...
				SUM(CASE WHEN (MD.Tipo_Gtia_BCRA IS NULL OR MD.Tipo_Gtia_BCRA = 0) THEN (MD.Saldo_Capital+MD.Acuerdo_No_Utiliz) ELSE 0 END) 
				+SUM(CASE WHEN (MD.Tipo_Gtia_BCRA = 1) THEN ((MD.Int_Deven_Cobrar_Cont-MD.Int_Adelantados) + MD.IVA_Financiado + MD.RG_Financiado + MD.Seguro_Financ + MD.Otros_Accesorios + MD.Acuerdo_No_Utiliz) ELSE 0 END)
				+SUM(CASE WHEN MD.Tipo_Gtia_BCRA = 2 THEN (MD.IVA_Financiado + MD.RG_Financiado + MD.Seguro_Financ + MD.Otros_Accesorios + MD.Acuerdo_No_Utiliz) ELSE 0 END)
				AS DEUDA_SIN_GARANTIA,
