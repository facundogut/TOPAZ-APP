...
BEGIN
  ...
  INSERT INTO @TMPIntegrantesCliente		
	...
	BEGIN
		...
		SELECT
			...
			(CASE
				...
				WHEN mc.COD_TRANSACCION IN (48,49,50,106,107)
					THEN
						''Suc.: '' + convert(VARCHAR(8), h.SUCURSALMOV) + 
						'' Cert.: '' + str((
										SELECT ISNULL(NULLIF(ps.CERTIFICADO_DPF, 0), S.OPERACION)
										FROM PZO_SALDOS ps
										...
										)) + -- convert(VARCHAR(20),m.Cuenta) +
						'' Cap.: '' + 
							CASE WHEN h.TIPOMOV = ''P''
								THEN convert(VARCHAR(20), h.CAPITALPAGADO)
								ELSE convert(VARCHAR(20), h.CAPITALORIGINAL)
							END +
						'' Int.: '' + 
							CASE WHEN h.TIPOMOV = ''P''
								THEN convert(VARCHAR(20), h.INTERESPAGADO)
								ELSE convert(VARCHAR(20), h.INTALVTO)
							END
				/* Pago de Cheques */
				...
				WHEN mc.COD_TRANSACCION IN (3016)
					THEN
						''Préstamo: '' +  convert(VARCHAR(max),(
								SELECT OPERACION
								FROM SALDOS
								WHERE
									JTS_OID IN (
										SELECT TOP 1 SALDO_JTS_OID
										FROM MOVIMIENTOS_CONTABLES
										WHERE
											ASIENTO = m.Asiento
											AND SUCURSAL = m.SucursalOrigen
		...
		LEFT JOIN VW_CLIENTES_PERSONAS cp with (nolock) ON
			cp.CODIGOCLIENTE = mc.CLIENTE
			AND TITULARIDAD = ''T''
		LEFT JOIN BS_HISTORIA_PLAZO h with (nolock) ON
			h.NROASIENTOMOV = m.Asiento
			AND h.FECHAPROCESOMOV = m.FechaProcesado
			AND h.SUCURSALMOV = m.SucursalOrigen
		WHERE
			m.InfoExtendida = ''S''
		...
		INNER JOIN @TMPInfoExt i ON 
			...
			AND i.CodTransaccion = m.CodTransaccion
