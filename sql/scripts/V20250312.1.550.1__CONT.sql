...
Execute('CREATE OR ALTER   PROCEDURE SP_ITF_UNTD_ASIENTOS_CONTABLES
...
FROM ITF_ASIENTOS_EXTERNOS (nolock) WHERE ID=@idCount;
	...
	BEGIN	
		...
		fecha_valor, moneda_monto, RUBRO_OPERATIVO,MARCA_AJUSTE)
		SELECT  (SELECT MAX(JTS_OID) FROM BANDEJA_ASIENTOS_IN WHERE ESTADO=''S'' AND ORIGEN=''UNT''), s.SUCURSAL, s.PRODUCTO, s.CUENTA, a.MONEDA,
		...
		Case When a.fecha_asiento < (select fechaproceso from PARAMETROS) Then ''A'' Else null End
		FROM ITF_ASIENTOS_EXTERNOS a INNER JOIN SALDOS s ON s.JTS_OID = (SELECT TOP 1 JTS_OID FROM SALDOS WHERE C1730=(CONCAT(a.CAPITULO_CONT,a.RUBRO_CONT,a.MONEDA_CONT,a.CUENTA_CONT,a.SUCUENTA_CONT)) AND SUCURSAL=(SELECT NUMERICO FROM PARAMETROSGENERALES WHERE CODIGO=21))
...
Execute('CREATE OR ALTER  PROCEDURE SP_ITF_I2000_ASIENTOS_CONTABLES
...
FROM ITF_ASIENTOS_EXTERNOS (nolock) WHERE ID=@idCount;
	...
	CUENTA, MONEDA, OPERACION, ORDINAL, CLIENTE, SIGNO, MONTO, CONCEPTO, 
	RUBRO_CONTABLE, fecha_valor, moneda_monto,RUBRO_OPERATIVO,MARCA_AJUSTE)
	SELECT  (SELECT MAX(JTS_OID) FROM BANDEJA_ASIENTOS_IN WHERE ESTADO=''S'' AND ORIGEN=''DOS''), s.SUCURSAL, s.PRODUCTO, 
	s.CUENTA, a.MONEDA, s.OPERACION, s.ORDINAL, 0, a.COD_MOVIMIENTO, (a.IMPORTE_DEBE + a.IMPORTE_HABER), /*CONCAT(''1.11.8 '',a.CONCEPTO_ASIENTO)*/ a.CONCEPTO_ASIENTO, 
	CONCAT(a.CAPITULO_CONT,a.RUBRO_CONT,a.MONEDA_CONT,a.CUENTA_CONT,a.SUCUENTA_CONT), a.FECHA_ASIENTO/*(SELECT FECHAPROCESO FROM PARAMETROS)*/, a.MONEDA,
...
Execute('CREATE OR ALTER  PROCEDURE  SP_ITF_AD_ASIENTOS_CONTABLES
...
FROM ITF_ASIENTOS_EXTERNOS (nolock) WHERE ID=@idCount;
	...
	SELECT  (SELECT MAX(JTS_OID) FROM BANDEJA_ASIENTOS_IN WHERE ESTADO=''S'' AND ORIGEN=''AAC''), 
			 ...
			 (select C6340 
			  ...
			  Case When a.fecha_asiento < (select fechaproceso from PARAMETROS)
