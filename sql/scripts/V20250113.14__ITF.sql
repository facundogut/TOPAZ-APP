EXEC('
	delete from TTR_CODIGO_TRANSACCION_INFO_EXTENDIDA where codigoTransaccion in (152,153);
');

EXEC('
	INSERT INTO TTR_CODIGO_TRANSACCION_INFO_EXTENDIDA (codigoTransaccion,infoExtendidaTipo,descripcion,TZ_LOCK) VALUES
		(152,1,''NBCH24: Credito - Transferencias Monobanco'',0),
		(153,1,''NBCH24: Debito - Transferencias Monobanco'',0);
');
...
Execute('
DELETE FROM dbo.ITF_MASTER_PARAMETROS WHERE CODIGO IN (541,542);
...
VALUES (541, 0, ''Asunto Email Anses'', ''Asunto Email ANSES'', ''Proceso Apertura de Cuentas de Anses '', '' '', 0, 0, NULL, 0, 0, 0)
INSERT INTO dbo.ITF_MASTER_PARAMETROS (CODIGO, CODIGO_INTERFACE, FUNCIONALIDAD, ALFA_1, ALFA_2, ALFA_3, NUMERICO_1, NUMERICO_2, FECHA, IMPORTE_1, IMPORTE_2, TZ_LOCK)
VALUES (542, 0, ''Email Anses'', ''Direccion Email ANSES'', ''nbch@nbch.com.ar '', '' '', 0, 0, NULL, 0, 0, 0)')
