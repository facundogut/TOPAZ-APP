EXECUTE('
ALTER PROCEDURE [dbo].[SP_BUSCA_SEGURO_SALDO]
...
BEGIN
    ...
    DECLARE @DEUDA_ACUERDOS_SV NUMERIC(15,2)
    ...
    IF(@P_EDAD>=@EDAD_MIN)
    	BEGIN
    		---- Obtengo deuda total
    		SET @DEUDA_TOTAL = (SELECT ISNULL(SUM(CASE WHEN CLASIFICACION=''A'' THEN SALDO ELSE SALDO_DEUDA END),0)
    		...
    		--- Obtengo deuda de acuerdos con seguro de vida - Cao NBCHINT-517
    		
    		SET @DEUDA_ACUERDOS_SV = (SELECT ISNULL(SUM(DC.SALDO),0)
    		FROM CRE_SEGURO_VIDA_VOLUNTARIO SSD WITH(NOLOCK)
    		INNER JOIN VW_DEUDA_CLIENTES DC WITH(NOLOCK) ON DC.JTS_OID = SSD.SALDO_JTS_OID
    		INNER JOIN CRE_SOLICITUDCREDITO SC WITH(NOLOCK) ON SSD.NRO_SOLICITUD = SC.NUMEROSOLICITUD
    		AND DC.JTS_OID = SC.JTS_OID_ACUERDO_CUENTA AND DC.OPERACION = SC.OPERACION_RENOVACION
    		WHERE  SSD.TZ_LOCK=0 AND DC.CLASIFICACION = ''A''
    		AND DC.CLIENTE = @P_CLIENTE -- AND (DC.PRODUCTO = @P_PRODUCTO OR DC.FAMILIA = @P_FAMILIA)
    		)
			...
			SET @IMPORTE_DISPONIBLE = (@IMPORTE_MAXIMO-(@DEUDA_TOTAL-ISNULL(@DEUDA_ACUERDOS_SV,0)))
