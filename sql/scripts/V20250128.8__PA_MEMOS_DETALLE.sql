   ...
   BEGIN
      ...
      SET @COTIZACION = (SELECT TOP 1 TIPO_CAMBIO_OFICIAL FROM HISTORICOTIPOSCAMBIO WITH(NOLOCK) WHERE MONEDA=2 AND FECHA_COTIZACION <= @FECHA_PROCESO ORDER BY FECHA_COTIZACION DESC)
			   ...
			   SELECT DISTINCT
                 ...
                 S.C1604<>0 AND S.C1785=5 AND S.PRODUCTO NOT IN (select c6250 from productos WITH(NOLOCK) where c6800 in (''CF'', ''CR''))
            ...
            UNION ALL
			   ...
			   SELECT DISTINCT
                 ...
                 C.CODIGOCLIENTE,
                 CASE WHEN CLD.TIPO = ''CHEQUE'' THEN CLD.numero ELSE 0 END AS NUMERO_CHEQUE,
                 CCS.NUMERO_DEPOSITO AS NUMERO_BOLETA,
                 0 as NUM_TJC,
                 '''' AS ADMINISTRADORA,
                 ABS(S.C1604), --- AS Capital adeudado
                 CASE WHEN S.C1785 IN (5,6) AND S.C1628 < @FECHA_PROCESO THEN DATEDIFF(DD,S.C1628,@FECHA_PROCESO) WHEN S.C1785 = 1 THEN S.C1652 ELSE 0 END AS DIAS_ATRASO,
                 ...
                 LEFT JOIN CRE_SOL_RECIBO_SUELDO SRS WITH(NOLOCK) ON SRS.SOLICITUD = S.C1704 AND SRS.CONVENIO = CS.ID_CONVENIO 
                 AND CONVERT(VARCHAR,SRS.JURIDICCION) = CS.JURISDICCION
				 LEFT JOIN CLE_CHEQUES_SALIENTE CCS ON CCS.JTS_OID_DESCUENTO = S.JTS_OID
				 LEFT JOIN (SELECT SALDO_JTS_OID, SUM(ISNULL(PREVISION,0)) AS PREVISION 
                 FROM CRE_HISTORICO_DEUDA_GARANTIAS WITH(NOLOCK) 
...
SELECT DISTINCT
                 ...
                 NULL AS ADMINISTRADORA,
                 HDP.MONTO_GARANTIZADO, --- AS Capital adeudado
                 DATEDIFF(DD,S.ULT_FECHA_VIGENTE,@FECHA_PROCESO) AS DIAS_ATRASO,
                 S.C1621,
                 NULL AS PRIMER_VENC,
                 NULL AS VENCIMIENTO,
                 ---CASE WHEN S.C1785 = 6 THEN VA.FECHA_VENCIMIENTO WHEN S.C1785 = 1 THEN TMU. ELSE S.C1627 END AS FECHA_VENCIMIENTO,
               ...
               --CC con acuerdo sin sobregiro
                ...
                DELETE I
                FROM @INFO_GENERAL I
                INNER JOIN SALDOS S ON S.JTS_OID = I.SALDO_JTS_OID
                LEFT JOIN CRE_HISTORICO_DEUDA_GARANTIAS HD ON HD.SALDO_JTS_OID = I.SALDO_JTS_OID AND HD.NRO_GARANTIA = I.NUM_GARANTIA
                WHERE S.C1785 = 5 AND I.NUM_GARANTIA <> 0 AND HD.SALDO_JTS_OID IS NULL
                AND I.SALDO_JTS_OID IN (SELECT SALDO_JTS_OID FROM @INFO_GENERAL GROUP BY SALDO_JTS_OID HAVING COUNT(1)>1)
                 ...
                 SALDO_JTS_OID
                 FROM @INFO_GENERAL 
                 WHERE (SALDO_DEUDA+ACUERDO_NO_UTILIZADO) > 0
                 )
