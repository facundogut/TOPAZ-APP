EXECUTE('
IF OBJECT_ID (''dbo.VW_DEUDA_CLIENTES_POSICION'') IS NOT NULL
	DROP VIEW dbo.VW_DEUDA_CLIENTES_POSICION
')
EXECUTE('
CREATE VIEW [dbo].[VW_DEUDA_CLIENTES_POSICION] (
									SUCURSAL,    
									NOMBRESUCURSAL, 
									PRODUCTO, 
									NOMBREPRODUCTO, 
									CUENTA, 
									OPERACION,
									DESGLOSE, 
									MONEDA, 
									MONTOORIGEN,
									SALDO,
									SALDO_DEUDA,
									VENCIMIENTO,
									CLIENTE,
									TIPO,
									JTS_OID,
									ORDINAL,
									PORCENTAJE_PAGO,
									TASA,
									GARANTIA_DESEMBOLSO,
									SITUACION,
									FAMILIA,
									CLASIFICACION,
									CREDITO_ADICIONAL,
									ESTADO,
									TIPODOC, 
									NUMERODOC,
									PAISDOCUMENTO,
									NUMEROPERSONA)									
AS
-- Prestamos, Descuentos y Garantias Otorgadas
SELECT	s.SUCURSAL AS SUCURSAL, 
		suc.NOMBRESUCURSAL AS NOMBRESUCURSAL, 
		s.PRODUCTO AS PRODUCTO, 
		p.C6251 AS NOMBREPRODUCTO,
		s.CUENTA AS CUENTA, 
		s.OPERACION AS OPERACION, 
		s.ORDINAL AS DESGLOSE, 
		m.C6401 AS MONEDA, 
		CASE WHEN C1785=6 THEN s.C1600 ELSE s.C1601 END*m.C6440 AS MONTOORIGEN, 
		CASE WHEN C1785 = 1 THEN s.C1604 
			 WHEN C1785 = 6 THEN ((C1604*-1)+C1608) 
			 ELSE s.C1604*-1 END*m.C6440 AS SALDO,
		CASE WHEN C1785 = 6 THEN ((C1604*-1)+C1608) 
			 ELSE ISNULL(ad.SALDO_A_FECHA,0) END*m.C6440 AS SALDO_DEUDA ,	   
		s.C1627 AS VENCIMIENTO, 
		s.C1803 AS CLIENTE, 
		s.C1785 AS TIPO, 
		s.JTS_OID AS JTS_OID, 
		0 AS ORDINAL,
		CASE WHEN s.C1644 > 0 AND s.C1785 = 5 THEN round(s.C1645 * 100 / s.C1644,2) ELSE NULL END AS PORCENTAJE_PAGO,
		s.C1632 AS TASA,
		ext.GARANTIA_DESEMBOLSO,
		c.CATEGORIARESULTANTE AS SITUACION,
		f.FAMILIAPROD AS FAMILIA,
		p.C6800 AS CLASIFICACION,
		CASE WHEN S.C1695 = ''S'' THEN ''S'' 
			 WHEN S.C1785=5 AND S.C1621=(SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK)) THEN '' ''
			 ELSE ''N'' END AS CREDITO_ADICIONAL,
		CASE WHEN s.C1728 = ''Z'' THEN ''Cedido'' WHEN s.C1728 = ''C'' THEN ''Castigado'' ELSE ''Normal'' END AS ESTADO,
		d.TIPODOC, 
		d.NUMERODOC,
		d.PAISDOCUMENTO,
		d.NUMEROPERSONA			
FROM SALDOS s WITH(NOLOCK)
INNER JOIN PRODUCTOS p WITH(NOLOCK) ON s.PRODUCTO = p.C6250 
						AND ( (p.tz_lock < 300000000000000 OR p.tz_lock >= 400000000000000) AND (p.tz_lock < 100000000000000 OR p.tz_lock >= 200000000000000)  )
INNER JOIN CRE_FAMILIAS f ON p.C6283 = f.FAMILIAPROD AND ( (f.tz_lock < 300000000000000 OR f.tz_lock >= 400000000000000) AND (f.tz_lock < 100000000000000 OR f.tz_lock >= 200000000000000)  )					
INNER JOIN MONEDAS m WITH(NOLOCK) ON s.MONEDA = m.C6399 
						AND ( (m.tz_lock < 300000000000000 OR m.tz_lock >= 400000000000000) AND (m.tz_lock < 100000000000000 OR m.tz_lock >= 200000000000000)  )
INNER JOIN SUCURSALES suc WITH(NOLOCK) ON s.SUCURSAL = suc.SUCURSAL 
							AND ( (suc.tz_lock < 300000000000000 OR suc.tz_lock >= 400000000000000) AND (suc.tz_lock < 100000000000000 OR suc.tz_lock >= 200000000000000)  )
LEFT JOIN CRE_SALDOS ext WITH(NOLOCK) ON s.JTS_OID = ext.SALDOS_JTS_OID AND ( (ext.tz_lock < 300000000000000 OR ext.tz_lock >= 400000000000000) AND (ext.tz_lock < 100000000000000 OR ext.tz_lock >= 200000000000000)  )						
INNER JOIN CLI_CLIENTES c WITH(NOLOCK) ON s.C1803 = c.CODIGOCLIENTE AND ( (c.tz_lock < 300000000000000 OR c.tz_lock >= 400000000000000) AND (c.tz_lock < 100000000000000 OR c.tz_lock >= 200000000000000)  )	
INNER JOIN VW_CLI_X_DOC d ON d.CODIGOCLIENTE=s.C1803
LEFT JOIN VW_ASISTENCIAS_D_EXIGIBLE_POSICION ad ON ad.JTS_OID=s.JTS_OID
WHERE (s.C1785 IN (5,6) OR (s.C1785 = 1 AND p.C6800 = ''GO'')) AND ( (s.tz_lock < 300000000000000 OR s.tz_lock >= 400000000000000) AND (s.tz_lock < 100000000000000 OR s.tz_lock >= 200000000000000)  )	
UNION
-- Acuerdos Vigentes
SELECT	s.SUCURSAL AS SUCURSAL, 
		suc.NOMBRESUCURSAL AS NOMBRESUCURSAL, 
		v.NROLINEA AS PRODUCTO, 
		p.C6251 AS NOMBREPRODUCTO,
		s.CUENTA AS CUENTA, 
		v.NRO_AUTORIZACION AS OPERACION, 
		s.ORDINAL AS DESGLOSE, 
		m.C6401 AS MONEDA, 
		v.IMPORTE AS MONTOORIGEN, 
		CASE WHEN C1604 < 0 THEN 0 ELSE v.IMPORTE - v.IMPORTE_CONSUMIDO END AS SALDO, -- Seteamos en 0 porque lo gestionamos en el otro select de CC
		0 AS SALDO_DEUDA,-- Seteamos en 0 porque lo gestionamos en el otro select de CC
		v.FECHA_VENCIMIENTO AS VENCIMIENTO, 
		s.C1803 AS CLIENTE, 
		1 AS TIPO, 
		s.JTS_OID AS JTS_OID, 
		v.ORDINAL AS ORDINAL,
		NULL AS PORCENTAJE_PAGO,
		NULL AS TASA,
		NULL AS GARANTIA_DESEMBOLSO,
		NULL AS SITUACION,
		NULL AS FAMILIA,
		p.C6800 AS CLASIFICACION,
		''N'' AS CREDITO_ADICIONAL,
		''Normal'' AS ESTADO,
		d.TIPODOC, 
		d.NUMERODOC,
		d.PAISDOCUMENTO,
		d.NUMEROPERSONA	
FROM VTA_SOBREGIROS v WITH(NOLOCK)
INNER JOIN SALDOS s WITH(NOLOCK) ON v.JTS_OID_SALDO = s.JTS_OID 
					AND ( (s.tz_lock < 300000000000000 OR s.tz_lock >= 400000000000000) AND (s.tz_lock < 100000000000000 OR s.tz_lock >= 200000000000000)  )	
INNER JOIN PRODUCTOS p WITH(NOLOCK) ON v.NROLINEA = p.C6250 
						AND ( (p.tz_lock < 300000000000000 OR p.tz_lock >= 400000000000000) AND (p.tz_lock < 100000000000000 OR p.tz_lock >= 200000000000000)  )						
INNER JOIN MONEDAS m WITH(NOLOCK) ON s.MONEDA = m.C6399 
					AND ( (m.tz_lock < 300000000000000 OR m.tz_lock >= 400000000000000) AND (m.tz_lock < 100000000000000 OR m.tz_lock >= 200000000000000)  )	
INNER JOIN SUCURSALES suc WITH(NOLOCK) ON v.SUCURSAL_ACUERDO = suc.SUCURSAL 
						AND ( (suc.tz_lock < 300000000000000 OR suc.tz_lock >= 400000000000000) AND (suc.tz_lock < 100000000000000 OR suc.tz_lock >= 200000000000000)  )	
INNER JOIN VW_CLI_X_DOC d ON d.CODIGOCLIENTE=s.C1803						
WHERE v.ESTADO = 1 AND ( (v.tz_lock < 300000000000000 OR v.tz_lock >= 400000000000000) AND (v.tz_lock < 100000000000000 OR v.tz_lock >= 200000000000000)  )	

UNION 
-- Sobregiros de CC sin acuerdo
SELECT	s.SUCURSAL AS SUCURSAL, 
		suc.NOMBRESUCURSAL AS NOMBRESUCURSAL, 
		s.PRODUCTO AS PRODUCTO, 
		p.C6251 AS NOMBREPRODUCTO,
		s.CUENTA AS CUENTA, 
		s.OPERACION AS OPERACION, 
		s.ORDINAL AS DESGLOSE, 
		m.C6401 AS MONEDA, 
		0 AS MONTOORIGEN, 
		CASE WHEN abs(C1604)>C1683 THEN 0 -- Saldo CC por encima del acuerdo
		     ELSE (C1683 - (abs(C1604)))*m.C6440 -- Saldo CC dentro del acuerdo
	    END AS SALDO, -- Deuda no exigible
		((C1604)*-1)*m.C6440 AS SALDO_DEUDA, -- Deuda Exigible
		NULL AS VENCIMIENTO, 
		s.C1803 AS CLIENTE, 
		1 AS TIPO, 
		s.JTS_OID AS JTS_OID, 
		s.ORDINAL AS ORDINAL,
		NULL AS PORCENTAJE_PAGO,
		NULL AS TASA,
		NULL AS GARANTIA_DESEMBOLSO,
		NULL AS SITUACION,
		NULL AS FAMILIA,
		p.C6800 AS CLASIFICACION,
		''N'' AS CREDITO_ADICIONAL,
		CASE WHEN s.C1728 = ''Z'' THEN ''Cedido'' WHEN s.C1728 = ''C'' THEN ''Castigado'' ELSE ''Normal'' END AS ESTADO,
		d.TIPODOC, 
		d.NUMERODOC,
		d.PAISDOCUMENTO,
		d.NUMEROPERSONA	
FROM SALDOS s WITH(NOLOCK)
					
INNER JOIN PRODUCTOS p WITH(NOLOCK) ON s.PRODUCTO = p.C6250 
						AND ( (p.tz_lock < 300000000000000 OR p.tz_lock >= 400000000000000) AND (p.tz_lock < 100000000000000 OR p.tz_lock >= 200000000000000)  )						
INNER JOIN MONEDAS m WITH(NOLOCK) ON s.MONEDA = m.C6399 
					AND ( (m.tz_lock < 300000000000000 OR m.tz_lock >= 400000000000000) AND (m.tz_lock < 100000000000000 OR m.tz_lock >= 200000000000000)  )	
INNER JOIN SUCURSALES suc WITH(NOLOCK) ON s.SUCURSAL = suc.SUCURSAL 
						AND ( (suc.tz_lock < 300000000000000 OR suc.tz_lock >= 400000000000000) AND (suc.tz_lock < 100000000000000 OR suc.tz_lock >= 200000000000000)  )	
INNER JOIN VW_CLI_X_DOC d ON d.CODIGOCLIENTE=s.C1803						
WHERE C1785=2 AND C1604<0
AND ( (s.tz_lock < 300000000000000 OR s.tz_lock >= 400000000000000) AND (s.tz_lock < 100000000000000 OR s.tz_lock >= 200000000000000)  )	
UNION
-- Tarjetas de Credito
SELECT	s.SUCURSAL AS SUCURSAL, 
		suc.NOMBRESUCURSAL AS NOMBRESUCURSAL, 
		s.PRODUCTO AS PRODUCTO, 
		p.C6251 AS NOMBREPRODUCTO,
		s.CUENTA AS CUENTA, 
		s.OPERACION AS OPERACION, 
		s.ORDINAL AS DESGLOSE, 
		m.C6401 AS MONEDA, 
		s.C1832 AS MONTOORIGEN, 
		s.AJUSTEINFLACION AS SALDO,  
		s.AJUSTEINFLACION AS SALDO_DEUDA,	
		s.C1761 AS VENCIMIENTO, 
		s.C1803 AS CLIENTE, 
		s.C1785 AS TIPO, 
		s.JTS_OID AS JTS_OID, 
		0 AS ORDINAL,
		NULL AS PORCENTAJE_PAGO,
		NULL AS TASA,
		NULL AS GARANTIA_DESEMBOLSO,
		NULL AS SITUACION,
		f.FAMILIAPROD AS FAMILIA,
		p.C6800 AS CLASIFICACION,
	    ''N'' AS CREDITO_ADICIONAL,
		''Normal'' AS ESTADO,
		d.TIPODOC, 
		d.NUMERODOC,
		d.PAISDOCUMENTO,
		d.NUMEROPERSONA			
FROM SALDOS s WITH(NOLOCK)
INNER JOIN PRODUCTOS p WITH(NOLOCK) ON s.PRODUCTO = p.C6250 
						AND  ( (p.tz_lock < 300000000000000 OR p.tz_lock >= 400000000000000) AND (p.tz_lock < 100000000000000 OR p.tz_lock >= 200000000000000)  )	
						AND p.C6800 = ''T'' 
INNER JOIN CRE_FAMILIAS f ON p.C6283 = f.FAMILIAPROD AND ( (f.tz_lock < 300000000000000 OR f.tz_lock >= 400000000000000) AND (f.tz_lock < 100000000000000 OR f.tz_lock >= 200000000000000)  )						
INNER JOIN MONEDAS m WITH(NOLOCK) ON s.MONEDA = m.C6399 
						AND ( (m.tz_lock < 300000000000000 OR m.tz_lock >= 400000000000000) AND (m.tz_lock < 100000000000000 OR m.tz_lock >= 200000000000000)  )	
INNER JOIN SUCURSALES suc WITH(NOLOCK) ON s.SUCURSAL = suc.SUCURSAL 
							AND ( (suc.tz_lock < 300000000000000 OR suc.tz_lock >= 400000000000000) AND (suc.tz_lock < 100000000000000 OR suc.tz_lock >= 200000000000000)  )						
INNER JOIN CLI_CLIENTES c WITH(NOLOCK) ON s.C1803 = c.CODIGOCLIENTE AND ( (c.tz_lock < 300000000000000 OR c.tz_lock >= 400000000000000) AND (c.tz_lock < 100000000000000 OR c.tz_lock >= 200000000000000)  )	
INNER JOIN VW_CLI_X_DOC d ON d.CODIGOCLIENTE=s.C1803
WHERE s.C1785 = 1 
	AND  ( (s.tz_lock < 300000000000000 OR s.tz_lock >= 400000000000000) AND (s.tz_lock < 100000000000000 OR s.tz_lock >= 200000000000000)  )
UNION
-- SGR y FGR
SELECT  g.SUCURSAL_GARANTIA AS SUCURSAL, 
		s.NOMBRESUCURSAL AS NOMBRESUCURSAL, 
		c.TIPO_GARANTIA AS PRODUCTO, 
		c.DSC_TIPO_GARANTIA AS NOMBREPRODUCTO,
		0 AS CUENTA, 
		g.NUM_GARANTIA AS OPERACION, 
		0 AS DESGLOSE, 
		m.C6401 AS MONEDA, 
		g.IMPORTECONTRACTUAL*m.C6440 AS MONTOORIGEN, 
		round(g.IMPORTECONTRACTUAL - (SELECT ISNULL(SUM(MONTO_GARANTIZADO_MG), 0)
					FROM CRE_HISTORICO_DEUDA_GARANTIAS WITH (NOLOCK)
					WHERE NRO_GARANTIA = g.NUM_GARANTIA
					AND FECHA_PROCESO = (SELECT FECHAPROCESOANTERIOR FROM PARAMETROS WITH (NOLOCK))),2) * m.C6440 AS SALDO, 
		round((SELECT ISNULL(SUM(MONTO_GARANTIZADO_MG), 0)
			FROM CRE_HISTORICO_DEUDA_GARANTIAS WITH (NOLOCK)
			WHERE NRO_GARANTIA = g.NUM_GARANTIA
			AND FECHA_PROCESO = (SELECT FECHAPROCESOANTERIOR FROM PARAMETROS WITH (NOLOCK))),2)*m.C6440 AS SALDO_DEUDA,	
		g.FCHVTO_GARANTIA AS VENCIMIENTO, 
		0 AS CLIENTE, 
		1 AS TIPO, 
		0 AS JTS_OID, 
		0 AS ORDINAL,
		NULL AS PORCENTAJE_PAGO,
		NULL AS TASA,
		NULL AS GARANTIA_DESEMBOLSO,
		NULL AS SITUACION,
		NULL AS FAMILIA,
		''SGR'' AS CLASIFICACION,
		''N'' AS CREDITO_ADICIONAL,
		''Normal'' AS ESTADO,
		g.TIPODOCUMENTO AS TIPODOCUMENTO, 
		g.NUMERODOCUMENTO AS NUMERODOCUMENTO,
		0 AS PAISDOCUMENTO,
		v.NUM_PERSONA AS NUMEROPERSONA
FROM CRE_GarantiaPersonas v WITH (nolock)
INNER JOIN CRE_GARANTIASRECIBIDAS g WITH (nolock) ON g.NUM_GARANTIA=v.NUM_GARANTIA AND g.TZ_LOCK=0 
						AND ( (g.tz_lock < 300000000000000 OR g.tz_lock >= 400000000000000) AND (g.tz_lock < 100000000000000 OR g.tz_lock >= 200000000000000)  )	

INNER JOIN MONEDAS m WITH(NOLOCK) ON g.MONEDA_GARANTIA = m.C6399 
						AND ( (m.tz_lock < 300000000000000 OR m.tz_lock >= 400000000000000) AND (m.tz_lock < 100000000000000 OR m.tz_lock >= 200000000000000)  )	
INNER JOIN SUCURSALES s WITH (nolock) ON g.SUCURSAL_GARANTIA = s.SUCURSAL AND s.TZ_LOCK = 0 
INNER JOIN CRE_TIPOSGARANTIAS c WITH (nolock) ON g.TIPOGARANTIA=c.TIPO_GARANTIA
						AND ( (c.tz_lock < 300000000000000 OR c.tz_lock >= 400000000000000) AND (c.tz_lock < 100000000000000 OR c.tz_lock >= 200000000000000)  )	
WHERE v.TPO_PERSONA = ''B'' AND v.TZ_LOCK = 0 AND c.TIPO_GARANTIA_INTERNO=4 AND c.TIPO_BCRA=2
')