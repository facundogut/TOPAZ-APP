...
AS 
   BEGIN
      ...
      -- ACTUALIZO EL CAPITAL e INTERES DE LA TABLA CRE_HISTORICO_DEUDA_GARANTIAS cuando las ASISTENCIAS TIPO P cuando hay INTERES NEGATIVO
      
      UPDATE hd
      SET hd.CAPITAL=0, hd.INTERES=0
      FROM CRE_HISTORICO_DEUDA_GARANTIAS hd
    	WHERE
			hd.FECHA_PROCESO = @FECHA_PROCESO AND 
        	hd.TIPO_ASISTENCIA = ''P'' AND
			(hd.CAPITAL + hd.INTERES)=0 AND
			hd.INTERES<0;
      ...
      SET @COTIZACION = (SELECT TOP 1 TIPO_CAMBIO_OFICIAL FROM HISTORICOTIPOSCAMBIO WITH(NOLOCK) WHERE MONEDA=2 AND FECHA_COTIZACION <= @FECHA_PROCESO ORDER BY FECHA_COTIZACION DESC)
			   ...
			   SELECT DISTINCT
                 ...
                 ISNULL(VAD.INTERES+VAD.IVAINTERES+VAD.MORA+VAD.IVA_MORA+VAD.PUNITORIO+VAD.IVA_PUNITORIO+VAD.IVA_PERCEPCION_MORA+VAD.IVA_PERCEPCION_INTCOMP_MORA,0), ---INT DEVEN VENCIDO
			...
			UNION ALL
			   ...
			   SELECT DISTINCT
                 ...
                 OPG.DESCRIPCION, ---CTG.DSC_TIPO_GARANTIA,
                 GAR.TIPO_BCRA,
                 ISNULL(GAR.NUM_GARANTIA,0) AS NRO_GARANTIA,
                 ISNULL(GAR.IMPORTECONTRACTUAL,0) AS IMPORTE_GARANTIZADO,
                 GAR.TIPO_GARANTIA,
                 0 as NRO_AUTORIZACION,
                 0 as SALDO_ACUERDO,
                 0 AS IMPORTE_NOUTILIZADO,
                 ...
                 ISNULL(VAD.INTERES+VAD.IVAINTERES+VAD.MORA+VAD.IVA_MORA+VAD.PUNITORIO+VAD.IVA_PUNITORIO+VAD.IVA_PERCEPCION_MORA+VAD.IVA_PERCEPCION_INTCOMP_MORA,0), ---INT DEVEN VENCIDO
                 (SELECT SUM(C2309) FROM PLANPAGOS WITH(NOLOCK) WHERE SALDO_JTS_OID = S.JTS_OID AND C2302 <= @FECHA_PROCESO), ---CAPITAL VENCIDO
                 ISNULL(GAR.IMPORTE_REAL,0) AS IMPORTE_VALOR_TASACION,
                 0 AS INTERESES_ADELANTADOS,
                 --HPD.INTERES AS INT_VIGENTES,
                 CASE WHEN (S.C1610 + S.C1893) <= S.C1814 THEN S.C1814 - (S.C1610 + S.C1893) ELSE 0 END AS INT_VIGENTES,
                 S.C1632 AS TNA,
                 ...
                 THEN GAR.PORCENTAJEAFECTACION ELSE GAR.PORCENTAJEAFECTACION_1_ANIO END AS AFORO,
                 (SELECT SUM(DEVENGADO) FROM BS_COMISIONDESEMBOLSO WITH(NOLOCK) WHERE SALDOS_JTS_OID=S.JTS_OID),
                 0 AS TIENE_GARANTIA
                 ...
                 LEFT JOIN CLI_PERSONASFISICAS PF WITH(NOLOCK) ON PF.NUMEROPERSONAFISICA = CP.NUMEROPERSONA AND PF.TZ_LOCK = 0
                 LEFT JOIN CLI_PERSONASJURIDICAS PJ WITH(NOLOCK) ON PJ.NUMEROPERSONAJURIDICA = CP.NUMEROPERSONA AND PJ.TZ_LOCK = 0
                 LEFT JOIN (SELECT GC.JTSOID_SALDO, GC.NUM_GARANTIA, GR.IMPORTECONTRACTUAL, GR.IMPORTE_REAL, TG.TIPO_BCRA, TG.TIPO_GARANTIA, CG.PORCENTAJEAFECTACION, CG.PORCENTAJEAFECTACION_1_ANIO
                 FROM CRE_GARANTIACREDITO GC WITH(NOLOCK), CRE_GARANTIASRECIBIDAS GR WITH(NOLOCK), CRE_TIPOSGARANTIAS TG WITH(NOLOCK), CRE_CLASGARANTIAS CG WITH(NOLOCK)
                 WHERE GC.DESVINCULADO = ''N'' AND GC.TZ_LOCK = 0
                 AND GC.NUM_GARANTIA = GR.NUM_GARANTIA AND GR.TZ_LOCK = 0 AND GR.FCHVTO_GARANTIA >= @FECHA_PROCESO
                 ...
                 AND CG.TIPOGARANTIA = GR.TIPOGARANTIA AND CG.SUBTIPOGTIA = GR.SUBTIPOGARANTIA) GAR
                 ON GAR.JTSOID_SALDO = S.JTS_OID
				 LEFT JOIN OPCIONES OPG WITH(NOLOCK) ON OPG.IDIOMA = ''E'' AND OPG.OPCIONINTERNA = GAR.TIPO_BCRA AND OPG.NUMERODECAMPO = 44448
				 --LEFT JOIN VW_CRE_DOCUMENTOS_DESC_O_AL_COBRO_SL CLD WITH(NOLOCK) ON CLD.jts_oid = S.JTS_OID AND ISNUMERIC(CLD.numero) = 1
				 --LEFT JOIN HISTORICO_CALIF_X_SALDO HCS WITH(NOLOCK) ON HCS.JTS_PRESTAMO = S.JTS_OID 
				 --AND HCS.FECHA = (SELECT TOP 1 FECHA FROM HISTORICO_CALIF_X_SALDO WITH(NOLOCK) WHERE JTS_PRESTAMO = S.JTS_OID AND FECHA <= @FECHA_PROCESO ORDER BY FECHA DESC)
                 LEFT JOIN CONV_CONVENIOS_PAG CCP WITH(NOLOCK) ON CCP.ID_ConvPago = CS.ID_CONVENIO AND CCP.TZ_LOCK = 0
            ...
            UNION ALL
			   ...
			   SELECT DISTINCT
                 ...
                 C.SUCURSALVINCULADA,
                 S.C1803,
                 CASE WHEN S.C1728 = ''C'' AND S.C1785 = 6 THEN ''S'' ELSE ''N'' END AS CUENTAORDEN,
                 P.C6283,
                 S.PRODUCTO,
                 P.C6251,
                 S.MONEDA,
			...
			UNION ALL
				...
				SELECT DISTINCT
                 ...
                 C.SUCURSALVINCULADA,
                 S.CUENTA,
                 CASE WHEN S.C1728 = ''C'' THEN ''S'' ELSE ''N'' END AS CUENTAORDEN,
                 P.C6283,
                 S.PRODUCTO,
                 P.C6251,
                 S.MONEDA,
                 ...
                 TMU.FECHA_ALTA_USU, --fecha alta usuario
            ...
            ---Garantias otorgadas
                ...
                0 as NUM_TJC,
                '''' AS ADMINISTRADORA,
                ABS(S.C1604), ---Capital adeudado
                S.C1652, ---Dias de atraso
                S.C1621,
                NULL AS PRIMER_VENC,
                S.C1627 AS VENCIMIENTO,
                S.C1644 AS CANT_CUOTAS,
				...
				LEFT JOIN OPCIONES OPG WITH(NOLOCK) ON OPG.IDIOMA = ''E'' AND OPG.OPCIONINTERNA = CTG.TIPO_BCRA AND OPG.NUMERODECAMPO = 44448
                WHERE S.TZ_LOCK = 0 AND
                (S.C1785 = 1 AND S.PRODUCTO IN (SELECT C6250 FROM PRODUCTOS WITH(NOLOCK) WHERE TZ_LOCK = 0 AND C6800=''GO''))
                AND S.C1621 <= @FECHA_PROCESO
			UNION ALL 
			...
			UNION ALL
               ...
               --CC con acuerdo sin sobregiro o con acuerdo no utilizado
				SELECT DISTINCT
                 ...
                 0 AS CANT_CUOTAS,
                 S.C1601,
                 ''1'' AS CATEGORIARESULTANTE, --Campo Clasificacion del MEMO
                 0 AS ID_CONVENIO,
                 NULL AS  NomConvPago,
                 0 AS ID_DOMINIO,
                 NULL DESCRIPCION,
				...
				--Se ajusta el campo Int_Deven_Cobrar_Cont para los Préstamos sin Garantía
                UPDATE I
                SET I.INT_DEVEN_NOPAGADOS=hd.INTERES_DEVENG_VIGENTE_CONT+hd.ICMORA_DEVEN_VIGENTE_CONT
                FROM @INFO_GENERAL I
                INNER JOIN HISTORICO_DEVENGAMIENTO hd on I.SALDO_JTS_OID=hd.SALDO_JTS_OID and hd.FECHA=@FECHA_PROCESO and hd.TZ_LOCK=0
                WHERE I.TIENE_GARANTIA=''0'';
				...
				--Si hay mas de una garantia otorgada dejo 1 de ellos según el tipo BCRA y el IMP Valotr Tasacion
              	WITH GOConMultiplesRegistros AS
				(select t.*, ROW_NUMBER() OVER (PARTITION BY t.SALDO_JTS_OID order by t.GARANTIA_BCRA desc, t.IMP_VALOR_TASACION desc) AS NUMERO 
				FROM (SELECT md.* FROM @INFO_GENERAL md WHERE md.familia=15000) AS t)
				DELETE FROM GOConMultiplesRegistros	WHERE NUMERO > 1;
