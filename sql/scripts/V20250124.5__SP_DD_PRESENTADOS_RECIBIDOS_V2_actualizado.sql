execute('
CREATE OR ALTER      PROCEDURE [SP_DD_PRESENTADOS_RECIBIDOS_V2]
	@TICKET NUMERIC(16),
...
--s	DECLARE @ReservadoFA VARCHAR(39);
	...
	IF(0=(SELECT COUNT(1) FROM ITF_DD_PRESENTADOS_RECIBIDOS_AUX
	WHERE LINEA LIKE ''1%''))
	BEGIN
...
--s			SET @ReservadoFA  = substring(@LINEA, 56, 39); -- Se vuelve a formato anterior
			...
			--#validacion9
			IF(@ExisteRI = 1 AND (SELECT COUNT(1) FROM ITF_DD_PRESENTADOS_RECIBIDOS_AUX WHERE LINEA LIKE ''5%'') <> @CantLotesFA)
			BEGIN
			...
			--#validacion10
			IF((SELECT COUNT(1) FROM ITF_DD_PRESENTADOS_RECIBIDOS_AUX WHERE LINEA LIKE ''6%'' OR LINEA LIKE ''7%'') <> @CantRegAdFA)
			BEGIN
...
--s		  SET @Importe =  CONVERT(NUMERIC(15,2),@VImporte)/100;
	      ...
	      SET @V08 = (SELECT COUNT(1) FROM SNP_STOP_DEBIT WHERE FECHA_DESDE<=@FECHADATE AND FECHA_VENCIMIENTO>@FECHADATE AND SALDO_JTS_OID = @saldo_jts_oid );
...
PRINT ''ENTRA ADHESIONES''
		  		...
		  		SET 
									DNI_CLIENTE_ADHERIDO=(SELECT TOP 1 p.NumDocFisico FROM CLI_ClientePersona cli JOIN vw_personas_f_y_j p ON cli.NUMEROPERSONA=p.NUMEROPERSONA WHERE TITULARIDAD=''T'' AND CODIGOCLIENTE=@Cod_Cliente AND cli.TZ_LOCK=0)
									, APELLIDO_NOMBRE=(SELECT TOP 1 replace(p.NOMBRE,'','','' '') FROM CLI_ClientePersona cli JOIN vw_personas_f_y_j p ON cli.NUMEROPERSONA=p.NUMEROPERSONA WHERE TITULARIDAD=''T'' AND CODIGOCLIENTE=@Cod_Cliente AND cli.TZ_LOCK=0) 
									, CUENTA=@NumCuenta
									, MONEDA=@Moneda
									...
									, fecha_recepcion=(SELECT fechaproceso FROM PARAMETROS)
...
PRINT @IdClientePegador
		  		INSERT INTO dbo.SNP_ADHESIONES
							...
							VALUES
									...
									0
									,  @Cod_Cliente
									, (SELECT TOP 1 p.NumDocFisico FROM CLI_ClientePersona cli JOIN vw_personas_f_y_j p ON cli.NUMEROPERSONA=p.NUMEROPERSONA WHERE TITULARIDAD=''T'' AND CODIGOCLIENTE=@Cod_Cliente AND cli.TZ_LOCK=0)									
									, (SELECT TOP 1 replace(p.NOMBRE,'','','' '') FROM CLI_ClientePersona cli JOIN vw_personas_f_y_j p ON cli.NUMEROPERSONA=p.NUMEROPERSONA WHERE TITULARIDAD=''T'' AND CODIGOCLIENTE=@Cod_Cliente AND cli.TZ_LOCK=0) 
									, @NumCuenta
									, @Moneda
									, @CUIT_EO
									, @VPRESTACION
		...
		BEGIN 
 			...
 			IF(
   			...
   			BEGIN
 				...
 				SELECT (SELECT isnull(max(correlativo),0)+1 FROM SNP_MSG_ORDENES) AS correlativo, 
 						''N'' AS contabilizada, 
 						''N'' AS rendida, 
