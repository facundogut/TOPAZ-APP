EXECUTE('
ALTER     PROCEDURE [DBO].[SP_ITF_SOS_OPERACIONES]
	@FECHA VARCHAR(8),
...
BEGIN
	...
	**********************************/
	BEGIN 
		...
		--4
		LEFT(CONCAT(MOV.ASIENTO, CONVERT(VARCHAR,MOV.FECHAPROCESO,112), MOV.SUCURSAL, MOV.ORDINAL, MOV.JTS_OID),30) AS NUMERODEOPERACION,
		--5
		LEFT(SOS.SUBSISTEMA,3) AS CLASEDECUENTA,
		--6
		--SOS.TIPOPERACION  AS TIPODECUENTAUOPERACION,
		(CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.TIPOPERACION = 101
              ...
              WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.TIPOPERACION = 103
		         AND ISNULL(VT.BEN_MISMO_TITULAR, ISNULL((CASE WHEN JSON_VALUE(infoExtendidaMeta,''$.mismotitular'') = ''C'' THEN ''S''
		...
		(CASE WHEN SOS.SUBSISTEMA = ''ME'' THEN ''0'' 
		      ELSE CONVERT(VARCHAR,(SELECT SUCURSALBCRA FROM SOS_SUCURSALES sucx WHERE sucx.Sucursal = sa.Sucursal)) END) AS SUCURSALSISCEN,
		--8
		--(CASE WHEN SOS.SUBSISTEMA = ''ME'' THEN ''0'' ELSE CONVERT(VARCHAR,MOV.SUCURSAL) END) AS SUCURSALINTERNA,
		(CASE WHEN SOS.SUBSISTEMA = ''ME'' THEN ''0'' ELSE CONVERT(VARCHAR,SA.SUCURSAL) END) AS SUCURSALINTERNA,
		--9
		...
		(CASE WHEN SOS.SUBSISTEMA IN (''PF'',''CE'',''ME'') THEN '''' 
		      ...
		      WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.TIPOPERACION IN (101, 103) 
		         THEN ISNULL(VT.BEN_MISMO_TITULAR, ISNULL((CASE WHEN JSON_VALUE(infoExtendidaMeta,''$.mismotitular'') = ''C'' THEN ''S''
		...
		CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.IND_RTE = ''S'' AND ((SOS.MTOMINIMO < MOV.CAPITALREALIZADO AND SOS.MTOMINIMO <> 0) OR SOS.MTOMINIMO = 0)
				  THEN (CASE WHEN SOS.OPERACION = 3510 THEN CONVERT(VARCHAR,CHELAV.NRO_DOC_PORTADOR) ELSE CONVERT(VARCHAR,VMOV.NRO_DOC_I) END)
		     WHEN SOS.TIPOPERACION = 101 THEN 
		     								ISNULL(CONVERT(VARCHAR,VT.BEN_NRO_DOC), JSON_VALUE(infoExtendidaMeta,''$.destino.cuit''))
		     WHEN SOS.TIPOPERACION = 103 THEN 
		     								ISNULL(CONVERT(VARCHAR,VT.BEN_NRO_DOC), JSON_VALUE(infoExtendidaMeta,''$.origen.cuit'')
													     /*(SELECT DISTINCT (CLIX.NUMERODOC) FROM MOVIMIENTOS_CONTABLES MOVX WITH (NOLOCK)
													      INNER JOIN SALDOS SAX WITH (NOLOCK) ON SAX.JTS_OID = MOVX.SALDO_JTS_OID
														...
														WHERE MOVX.FECHAPROCESO = MOV.FECHAPROCESO
														  ...
														  AND MOVX.CLIENTE <> MOV.CLIENTE
														  AND MOVX.SUB_ASIENTO = MOV.SUB_ASIENTO
														  AND MOVX.CLIENTE <> 0
														  AND MOVX.ORDINAL = 0
														  --AND MOVX.COD_TRANSACCION <> MOV.COD_TRANSACCION
														  AND MOVX.DEBITOCREDITO <> MOV.DEBITOCREDITO)*/)
			ELSE '''' END
		),11) AS RTE_OPE_FON_CUIT_CUIL_CDI,
		--23
		LEFT((
		CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.IND_RTE = ''S'' AND ((SOS.MTOMINIMO < MOV.CAPITALREALIZADO AND SOS.MTOMINIMO <> 0) OR SOS.MTOMINIMO = 0)
		          THEN (CASE WHEN SOS.OPERACION = 3510 THEN (CASE WHEN CHELAV.TIPO_PERSONA_PORTADOR = ''F'' THEN ''PF'' WHEN CHELAV.TIPO_PERSONA_PORTADOR = ''J'' THEN ''PJ'' END)
		...
		LEFT((
		CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.IND_RTE = ''S'' AND ((SOS.MTOMINIMO < MOV.CAPITALREALIZADO AND SOS.MTOMINIMO <> 0) OR SOS.MTOMINIMO = 0)
		           THEN (CASE WHEN SOS.OPERACION = 3510 THEN (SELECT PFJ.NOMBRE FROM VW_PERSONAS_FYJ PFJ WHERE PFJ.NroDocIdent = CONVERT(VARCHAR,CHELAV.NRO_DOC_PORTADOR))
		...
		LEFT((
		CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.IND_RTE = ''S'' AND ((SOS.MTOMINIMO < MOV.CAPITALREALIZADO AND SOS.MTOMINIMO <> 0) OR SOS.MTOMINIMO = 0)
		           THEN (CASE WHEN SOS.OPERACION = 3510 THEN (SELECT PFJ.APELLIDOS FROM VW_PERSONAS_FYJ PFJ WHERE PFJ.NroDocIdent = CONVERT(VARCHAR,CHELAV.NRO_DOC_PORTADOR))
		...
		LEFT((
		CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.IND_RTE = ''S'' AND ((SOS.MTOMINIMO < MOV.CAPITALREALIZADO AND SOS.MTOMINIMO <> 0) OR SOS.MTOMINIMO = 0)
		           THEN (CASE WHEN SOS.OPERACION = 3510 THEN (SELECT PFJ.PRIMERNOMBRE FROM VW_PERSONAS_FYJ PFJ WHERE PFJ.NroDocIdent = CONVERT(VARCHAR,CHELAV.NRO_DOC_PORTADOR))
		...
		LEFT((
		CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.IND_RTE = ''S'' AND ((SOS.MTOMINIMO < MOV.CAPITALREALIZADO AND SOS.MTOMINIMO <> 0) OR SOS.MTOMINIMO = 0)
				  THEN (CASE WHEN SOS.OPERACION = 3510 THEN SUBSTRING(CONVERT(VARCHAR,CHELAV.NRO_DOC_PORTADOR),3,8)
		...
		LEFT((
		CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.IND_RTE = ''S'' AND ((SOS.MTOMINIMO < MOV.CAPITALREALIZADO AND SOS.MTOMINIMO <> 0) OR SOS.MTOMINIMO = 0)
		          THEN (CASE WHEN SOS.OPERACION = 3510 THEN (CASE WHEN CHELAV.TIPO_PERSONA_TITULAR = ''F'' THEN ''PF'' ELSE/*WHEN CHELAV.TIPO_PERSONA_TITULAR = ''J'' THEN*/ ''PJ'' END) ELSE (CASE WHEN VMOV.TIPO_PERSONA_M = ''F'' THEN ''PF'' WHEN VMOV.TIPO_PERSONA_M = ''J'' THEN ''PJ'' END) END)
			ELSE '''' END
		...
		(
		CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.IND_RTE = ''S'' AND ((SOS.MTOMINIMO < MOV.CAPITALREALIZADO AND SOS.MTOMINIMO <> 0) OR SOS.MTOMINIMO = 0)
		           THEN (CASE WHEN SOS.OPERACION = 3510 THEN (CASE WHEN CHELAV.TIPO_PERSONA_TITULAR = ''F'' THEN ''96'' /*WHEN CHELAV.TIPO_PERSONA_TITULAR = ''J'' THEN*/ELSE ''80'' END) ELSE
		...
		CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.IND_RTE = ''S'' AND ((SOS.MTOMINIMO < MOV.CAPITALREALIZADO AND SOS.MTOMINIMO <> 0) OR SOS.MTOMINIMO = 0)
		           THEN (CASE WHEN SOS.OPERACION = 3510 THEN (CASE WHEN CHELAV.TIPO_PERSONA_TITULAR = ''F'' THEN SUBSTRING(CONVERT(VARCHAR,CHELAV.NRO_DOC_TITULAR),3,8)
								      /*WHEN CHELAV.TIPO_PERSONA_TITULAR = ''J''  THEN*/ ELSE SUBSTRING(CONVERT(VARCHAR,CHELAV.NRO_DOC_TITULAR),1,10) END)
		...
		--39
		LEFT((CASE WHEN SOS.IND_RTE = ''N'' THEN '''' ELSE (CASE WHEN SOS.OPERACION = 3510 THEN 
		                                                          (CASE WHEN (SELECT COUNT(1) FROM VW_CLIENTE_PERSONAS_INTEGRANTES V WITH (NOLOCK)
																   WHERE V.TIPODOCUMENTO = CHELAV.TIPO_DOC_TITULAR
																     AND V.NUMERODOCUMENTO= CHELAV.NRO_DOC_TITULAR
																     AND V.CODIGOCLIENTE = SA.C1803) > 0 THEN ''SI'' ELSE ''NO'' END)
                                                          	 ELSE (CASE WHEN (SELECT COUNT(1) FROM VW_CLIENTE_PERSONAS_INTEGRANTES V WITH (NOLOCK)
																   WHERE V.TIPODOCUMENTO = VMOV.TIPO_DOC_M 
																     AND V.NUMERODOCUMENTO= VMOV.NRO_DOC_M
																     AND V.CODIGOCLIENTE = SA.C1803) > 0 THEN ''SI'' ELSE ''NO'' END) END) END), 30) AS REL_PRO_TIT,
		--40
		LEFT((CASE WHEN SOS.IND_RTE = ''N'' THEN '''' ELSE ''OPERADOR'' END), 30) AS REL_FON_OPER,
		--41
		LEFT((CASE WHEN SOS.IND_RTE = ''N'' THEN '''' ELSE (CASE WHEN SOS.OPERACION = 3510 THEN 
		                                                          (CASE WHEN (SELECT COUNT(1) FROM VW_CLIENTE_PERSONAS_INTEGRANTES V WITH (NOLOCK)
																   WHERE V.TIPODOCUMENTO = CHELAV.TIPO_DOC_PORTADOR 
																     AND V.NUMERODOCUMENTO= CHELAV.NRO_DOC_PORTADOR
																     AND V.CODIGOCLIENTE = SA.C1803) > 0 THEN ''SI'' ELSE ''NO'' END)
															ELSE (CASE WHEN (SELECT COUNT(1) FROM VW_CLIENTE_PERSONAS_INTEGRANTES V WITH (NOLOCK)
																   WHERE V.TIPODOCUMENTO = VMOV.TIPO_DOC_I 
																     AND V.NUMERODOCUMENTO= VMOV.NRO_DOC_I
																     AND V.CODIGOCLIENTE = SA.C1803) > 0 THEN ''SI'' ELSE ''NO'' END) END) END), 30) AS REL_PRO_OPER,
		--
		SA.JTS_OID
		--SELECT MOV.*
		FROM MOVIMIENTOS_CONTABLES MOV WITH (NOLOCK)
		  ...
		  LEFT JOIN VTA_CUMPLIMIENTO VMOV WITH (NOLOCK) ON VMOV.SUCURSAL = MOV.SUCURSAL /*MOV.SUCURSAL_CUENTA*/ AND VMOV.ASIENTO = MOV.ASIENTO AND VMOV.FECHA_PROCESO = MOV.FECHAPROCESO
		  LEFT JOIN VTA_TRANSFERENCIAS VT WITH (NOLOCK) ON VT.FECHA_ASIENTO = MOV.FECHAPROCESO AND VT.NUMERO_ASIENTO = MOV.ASIENTO AND VT.SUCURSAL_ASIENTO = MOV.SUCURSAL
		  LEFT JOIN CHE_LAVADO CHELAV WITH (NOLOCK) ON CHELAV.JTS_OID_CUENTA = MOV.SALDO_JTS_OID AND CHELAV.FECHA = MOV.FECHAPROCESO AND CHELAV.NRO_CHEQUE = MOV.REFERENCIA
		  LEFT JOIN HISTORICO_MOVIMIENTOS HM WITH (NOLOCK) ON HM.fechaAsiento = MOV.FECHAPROCESO AND HM.movJtsOid = MOV.JTS_OID AND HM.ASIENTO = MOV.ASIENTO
	...
	**********************************/
	BEGIN 
      ...
      ELSE CONVERT(VARCHAR,(SELECT SUCURSALBCRA FROM SOS_SUCURSALES sucx WHERE sucx.Sucursal = sa.Sucursal)) END) AS SUCURSALSISCEN,
		...
		LEFT((
		CASE WHEN SOS.SUBSISTEMA IN (''CC'',''AC'') AND SOS.IND_RTE = ''S'' AND ((SOS.MTOMINIMO < MOV.CAPITALREALIZADO AND SOS.MTOMINIMO <> 0) OR SOS.MTOMINIMO = 0)
		          THEN (CASE WHEN VMOV.TIPO_PERSONA_M = ''F'' THEN ''PF'' WHEN VMOV.TIPO_PERSONA_M = ''J'' THEN ''PJ'' END)
			ELSE '''' END
		),2) AS RTE_TIT_FON_TIPO_PERSONA,
		--31
		LEFT((
		...
		LEFT((CASE WHEN SOS.IND_RTE = ''N'' THEN '''' ELSE ''TITULAR'' END), 30) AS REL_FON_TIT,
		--39
		LEFT((CASE WHEN SOS.IND_RTE = ''N'' THEN '''' ELSE (CASE WHEN (SELECT COUNT(1) FROM VW_CLIENTE_PERSONAS_INTEGRANTES V WITH (NOLOCK)
																   WHERE V.TIPODOCUMENTO = VMOV.TIPO_DOC_M 
																     AND V.NUMERODOCUMENTO= VMOV.NRO_DOC_M
	...
	**********************************/
	...
	FROM MOVIMIENTOS_CONTABLES MOV WITH (NOLOCK)
	  ...
	  INNER JOIN SOS_SUCURSALES SUC WITH (NOLOCK) ON SUC.SUCURSAL = MOV.SUCURSAL
	  --
      INNER JOIN MOVIMIENTOS_CONTABLES movx WITH (NOLOCK) ON movx.FECHAPROCESO = mov.FECHAPROCESO 
        AND movx.SUCURSAL = mov.SUCURSAL
        AND movx.ASIENTO = mov.ASIENTO 
        --AND CONVERT(VARCHAR,movx.OPERACION_CUENTA) = (CASE WHEN CHARINDEX('':'', MOV.CONCEPTO) > 0 THEN SUBSTRING(MOV.CONCEPTO, CHARINDEX('':'', MOV.CONCEPTO)+2, 12) ELSE CONVERT(VARCHAR,MOV.OPERACION_CUENTA) END)
        AND CONVERT(VARCHAR, movx.OPERACION_CUENTA) = (CASE WHEN CHARINDEX('':'', MOV.CONCEPTO) > 0 THEN RTRIM(LTRIM(SUBSTRING(LTRIM(RTRIM(SUBSTRING(MOV.CONCEPTO, CHARINDEX('':'', MOV.CONCEPTO)+1, 20))), 1,
																					                     (CASE WHEN CHARINDEX('' '',LTRIM(RTRIM(SUBSTRING(MOV.CONCEPTO, CHARINDEX('':'', MOV.CONCEPTO)+1, 20)))) > 0 THEN 
																					             CHARINDEX('' '',LTRIM(RTRIM(SUBSTRING(MOV.CONCEPTO, CHARINDEX('':'', MOV.CONCEPTO)+1, 20))))
																					             ELSE 20 END))))
             												ELSE CONVERT(VARCHAR,MOV.OPERACION_CUENTA) END)
        AND movx.CLIENTE = mov.CLIENTE
        AND movx.PRODUCTO <> 0
        AND movx.SUB_ASIENTO = MOV.SUB_ASIENTO
        --AND movx.SUCURSAL_CUENTA = mov.SUCURSAL_CUENTA
      --
	  INNER JOIN SALDOS SA WITH (NOLOCK) ON SA.sucursal = movx.SUCURSAL_CUENTA 
	    AND SA.OPERACION = movx.OPERACION_CUENTA
		AND SA.PRODUCTO = movx.PRODUCTO
		AND SA.C1803 = movx.CLIENTE
		AND SA.C1785 IN (''5'',''8'')
	...
	**********************************/
	...
	@FECHAPROCESO AS FECHADEINFORMACION,
	--3
	CONCAT(RIGHT(''0''+CONVERT(VARCHAR, B.ADMINISTRADORA),2), TRA.SUBSISTEMA, CONVERT(VARCHAR, B.TIPO_TJC), CONVERT(VARCHAR,MOV.USUARIO)) AS NUMERODECUENTA,
	--4
	(LEFT(CONCAT(MOVC.ASIENTO, CONVERT(VARCHAR,MOVC.FECHAPROCESO,112), MOVC.SUCURSAL, MOVC.ORDINAL, MOVC.JTS_OID), 30)) AS NUMERODEOPERACION,
	--5
	...
	CONVERT(VARCHAR,(SELECT SUCURSALBCRA FROM SOS_SUCURSALES sucx 
					 WHERE sucx.Sucursal = (CASE WHEN b.SUC_CUENTA_COBRO = 0 THEN (CASE WHEN b.SUC_USUARIO BETWEEN 1 AND 30 THEN b.SUC_USUARIO ELSE 30 END) 
	                                             ELSE b.SUC_CUENTA_COBRO END))) AS SUCURSALSISCEN,
	--8
	CONVERT(VARCHAR,(CASE WHEN b.SUC_CUENTA_COBRO = 0 THEN (CASE WHEN b.SUC_USUARIO BETWEEN 1 AND 30 THEN b.SUC_USUARIO ELSE 30 END) 
	                      ELSE b.SUC_CUENTA_COBRO END)) AS SUCURSALINTERNA,
	--9
	...
	--SUCURSAL BCRA
	(SELECT SUCX.SUCURSALBCRA FROM SOS_SUCURSALES SUCX WITH (NOLOCK) 
	 WHERE SUCX.SUCURSAL = (CASE WHEN b.SUC_CUENTA_COBRO = 0 THEN (CASE WHEN b.SUC_USUARIO BETWEEN 1 AND 30 THEN b.SUC_USUARIO ELSE 30 END) 
	                        ELSE b.SUC_CUENTA_COBRO END) 
	   AND SUCX.TZ_LOCK = 0) AS SUCURSALSISCENCTA,
	--38
	'''' AS REL_FON_TIT,
	--39
	''NO'' AS REL_PRO_TIT,
	...
	FROM TJC_HISTORICO_PAGOS MOV WITH (NOLOCK)
	  INNER JOIN MOVIMIENTOS_CONTABLES MOVC WITH (NOLOCK) ON MOVC.FECHAPROCESO = MOV.FECHA 
	    AND MOVC.ASIENTO = MOV.ASIENTO
	    AND MOVC.INICIALESUSUARIO = MOV.USUARIO_MOD
	  	/*AND MOVC.OPERACION = 2854*/ 
	  	AND MOVC.SUCURSAL_CUENTA = (SELECT NUMERICO FROM PARAMETROSGENERALES WITH (NOLOCK) WHERE CODIGO = 400)
	...
	**********************************/
	...
	--4
	CONCAT(MOV.NUMBOLETO, MOV.FECINGRESO, MOV.CODSUCURSAL, MOV.OPERACION) AS NUMERODEOPERACION,
	--5
	''ME'' AS CLASEDECUENTA,
	--6
	...
	**********************************/
	...
	SUBSTRING(CONCAT(MOV.FECHA_INFORMACION, MOV.NUMERO_MINUTA, MOV.DESCRIPCION_TIPO_OPERACION),1,30) AS NUMERODEOPERACION,
	--5
	''TT'' AS CLASEDECUENTA,
	--6
	...
	(SELECT SUCURSALBCRA FROM SOS_SUCURSALES sucx WHERE sucx.Sucursal = mov.SUCURSAL_CUENTA_PARTE) AS SUCURSALSISCEN,
	--8
	CONVERT(VARCHAR,MOV.SUCURSAL_CUENTA_PARTE) AS SUCURSALINTERNA,
	--9
	...
	FROM ITF_UNTD_IMPUESTOS MOV WITH (NOLOCK)  
	  ...
	  INNER JOIN SALDOS SA WITH (NOLOCK) ON  SA.CUENTA = MOV.CUENTA_VISTA AND SA.sucursal = mov.SUCURSAL_CUENTA_PARTE 
	    --AND sa.c1785 in (2,3)
	    AND sa.c1785 = (CASE WHEN MOV.ENTIDAD_CUSTODIA_CUENTA_VISTA = ''CA$'' THEN 3
	                         WHEN MOV.ENTIDAD_CUSTODIA_CUENTA_VISTA = ''CC$'' THEN 2 END)
	    AND ((SA.TZ_LOCK < 300000000000000 OR SA.TZ_LOCK >= 400000000000000) 
			AND (SA.TZ_LOCK < 100000000000000 OR SA.TZ_LOCK >= 200000000000000)) 
