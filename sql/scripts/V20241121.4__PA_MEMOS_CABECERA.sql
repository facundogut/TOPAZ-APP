     ...
     ---DECLARE @FECHA_PROCESO DATETIME 
      ...
      DELETE FROM MEMO_CABECERA WHERE Fecha_Info = @FECHA_PROCESO
                ...
                CASE WHEN C.CLIENTE_DEVENGA_SUSPENSO = 1 THEN ''S'' ELSE ''N'' END AS SIT_JUD_EXT
                 ...
                 WHERE C.TZ_LOCK = 0 AND C.CODIGOCLIENTE IN (SELECT C1803 FROM SALDOS WITH(NOLOCK) WHERE ((C1604<>0 AND C1785 IN (5,6)) OR (C1785 = 2 AND C1651<>''1'' AND C1604<0)
                 OR (C1785 = 1 AND AJUSTEINFLACION <> 0 AND PRODUCTO IN (SELECT C6250 FROM PRODUCTOS WITH(NOLOCK) WHERE TZ_LOCK = 0 AND C6800=''T''))) AND TZ_LOCK = 0)
                ...
                SELECT 
				SUM(AD.SALDO_DEUDA),
				SUM(CASE WHEN (CG.JTSOID_SALDO IS NULL OR CTG.TIPO_BCRA = 0 OR CTG.TIPO_GARANTIA IS NULL) THEN AD.SALDO_DEUDA ELSE 0 END),
				...
				INNER JOIN VW_DEUDA_CLIENTES AD WITH(NOLOCK) ON AD.CLIENTE = IC.CLIENTE
				...
				WHERE AD.SALDO_DEUDA <> 0 AND S.C1785 IN (1,2,5,6)
				GROUP BY AD.CLIENTE
