...
PRINT @SumaImportes
			...
			END
	   		...
	   		SET @FL_TOTALES_DE_CONTROL =RIGHT(concat(replicate('0', 10), (SELECT sum(convert(INT,substring(t.LINEA,4,8))) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX t WHERE t.ID>(SELECT max(tt.id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX tt WHERE tt.LINEA LIKE '5%') AND substring(t.LINEA,1,1) IN ('6'))), 10);
	   		SET @FL_SUMA_TOTAL_DEBITO_LOTE =  RIGHT(concat(replicate('0', 12), (replace((SELECT sum(convert(bigint, substring(linea,30,10))) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '6%' AND LINEA NOT LIKE '622%' AND id>(SELECT max(id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '5%')), '.', ''))), 12); 
			...
			SET @FA_TOTALES_DE_CONTROL =  RIGHT(concat(replicate('0', 10), (SELECT sum(convert(INT,substring(t.LINEA,4,8))) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX t WHERE t.ID>(SELECT max(tt.id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX tt WHERE tt.LINEA LIKE '1%') AND substring(t.LINEA,1,1) IN ('6'))), 10);
		
			SET @FA_SUMA_TOTAL_DEBITOS = RIGHT(concat(replicate('0', 12), (replace((SELECT sum(convert(bigint, substring(linea,30,10))) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '6%' AND LINEA NOT LIKE '622%' AND id>(SELECT max(id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '1%')), '.', ''))), 12);
		
			SET @FA_SUMA_TOTAL_CREDITOS = RIGHT(concat(replicate('0', 12), (replace((SELECT sum(convert(bigint, substring(linea,30,10))) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '622%' AND id>(SELECT max(id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '1%')), '.', ''))), 12);
...
--			SET @FL_NUMERO_LOTE = RIGHT(concat(replicate('0', 7), @FL_NUMERO_LOTE), 7);
 ...
 --@ID, @CheFechaProceso, @CheCodigoTransaccion, @CheEntidadDebitar, @CheCuentaDebitar, @CheImporte, @CheFechaPresentado, @CheCodigoPostal, @CheFechaVencimiento, @CheNroCheque, @ChePuntoIntercambio, @CheTraceNumber, @CheEstado, @CheTipo, @CheCodRechazo
		...
		END
		IF	(@CheImporte>99999999.99)
		BEGIN            
			SET @Excedente = (@CheImporte - 99999999.99);
			SET @CheImporte = 99999999.99;
			SET @CountExcedente += 1;
		END
		ELSE
    	BEGIN
...
PRINT 'hola'	
		...
		IF(@Excedente=0 OR @Excedente<99999999.99)
		BEGIN
...
--			SET @FL_NUMERO_LOTE = RIGHT(concat(replicate('0', 7), @FL_NUMERO_LOTE), 7);
		...
		BEGIN
			...
			SET @CountExcedente += 1;
		END
		IF	(@CheImporteCD>99999999.99)
		BEGIN
			SET @Excedente = (@CheImporteCD - 99999999.99);
			SET @CheImporteCD = 99999999.99;
			SET @CountExcedente += 1;
		END
		ELSE
    	BEGIN
...
--				SET @FL_NUMERO_LOTE = RIGHT(concat(replicate('0', 7), @FL_NUMERO_LOTE), 7);
			...
			BEGIN
				...
				SET @CountExcedente += 1;
			END
			IF	(@CheImporteAJ>99999999.99)
			BEGIN
				SET @Excedente = (@CheImporteAJ - 99999999.99);
				SET @CheImporteAJ = 99999999.99;
				SET @CountExcedente += 1;
			END
			ELSE
    		BEGIN
		...
		--SET @TotalesControl += @SumaEntidad + @SumaSucursal;
			SET @FL_TOTALES_DE_CONTROL =RIGHT(concat(replicate('0', 10), (SELECT sum(convert(INT,substring(t.LINEA,4,8))) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX t WHERE t.ID>(SELECT max(tt.id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX tt WHERE tt.LINEA LIKE '5%') AND substring(t.LINEA,1,1) IN ('6'))), 10);
		SET @FL_SUMA_TOTAL_DEBITO_LOTE =  RIGHT(concat(replicate('0', 12), (replace((SELECT sum(convert(bigint, substring(linea,30,10))) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '6%' AND LINEA NOT LIKE '622%' AND id>(SELECT max(id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '5%')), '.', ''))), 12); 
		SET @FL_SUMA_TOTAL_DEBITO_LOTE =  RIGHT(concat(replicate('0', 12), (replace((SELECT sum(convert(bigint, substring(linea,30,10))) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '622%' AND id>(SELECT max(id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '5%')), '.', ''))), 12);
		SET @TotalesDebitos += @SumaImportes;
		SET @FL_CANT_REG_INDIVIDUAL_ADICIONAL = RIGHT(concat(replicate('0', 6), (SELECT count(id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX t WHERE t.ID>(SELECT max(tt.id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX tt WHERE tt.LINEA LIKE '5%') AND substring(t.LINEA,1,1) IN ('6','7'))), 6); 
...
--			SET @FL_NUMERO_LOTE = RIGHT(concat(replicate('0', 7), @FL_NUMERO_LOTE), 7);
		...
		BEGIN
			...
			SET @CountExcedente += 1;
		END
		IF	(@CheImporteRCD>99999999.99)
		BEGIN
			SET @Excedente = (@CheImporteRCD - 99999999.99);
			SET @CheImporteRCD = 99999999.99;
			SET @CountExcedente += 1;
		END
		ELSE
    	BEGIN
		...
		IF @SumaImportes>9999999999.99 GOTO Start_4
