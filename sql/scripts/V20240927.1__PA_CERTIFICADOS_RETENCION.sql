	...
	BEGIN
		...
		WITH DET_CONTABILIDAD_ORDENADO AS (
      	...
      	SELECT
		    ...
		    CASE WHEN m.MONEDA <>1 AND MON.C6403 <> ''I'' THEN
		    ...
		    WHEN m.MONEDA <>1 AND MON.C6403 = ''I'' THEN
		    (	SELECT SUM(CAPITALREALIZADO)
		    	FROM MOVIMIENTOS_CONTABLES WITH(nolock)
		    	WHERE
		    		ASIENTO = D.MOV_ASIENTO
		    ...
		    CASE WHEN m.MONEDA <>1 AND MON.C6403 <> ''I'' THEN FL.MONTO_IMPONIBLE *CB.TC_VENTA 
		         WHEN m.MONEDA <>1 AND MON.C6403  = ''I'' THEN FL.MONTO_IMPONIBLE *HTC.TIPO_CAMBIO_VENTA
		    ELSE FL.MONTO_IMPONIBLE END AS MONTO_IMPONIBLE,
		    
		    
		    CASE WHEN m.MONEDA <>1 AND MON.C6403 <> ''I'' THEN FL.IMPORTEMN*CB.TC_VENTA 
		         WHEN m.MONEDA <>1 AND MON.C6403  = ''I'' THEN FL.IMPORTEMN*HTC.TIPO_CAMBIO_VENTA
		    ELSE FL.IMPORTEMN END AS IMPORTEMN,
		...
		INNER JOIN DET_CONTABILIDAD_ORDENADO AS DCO WITH (nolock) ON
			...
			AND DCO.NRO_FILA = 1
		INNER JOIN CON_COTIZACIONES_BNA	CB ON CB.Moneda = m.MONEDA
		                                     AND cb.TZ_LOCK = 0 
		                                     AND Fecha_Cotizacion = CASE WHEN (SELECT Fecha_Cotizacion FROM CON_COTIZACIONES_BNA CB2
