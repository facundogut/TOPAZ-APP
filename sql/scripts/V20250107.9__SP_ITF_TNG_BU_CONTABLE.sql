EXECUTE('
CREATE OR ALTER PROCEDURE dbo.SP_ITF_TNG_BU_CONTABLE
...
FROM ITF_ASIENTOS_EXTERNOS (nolock) WHERE ID=@idCount;
	...
	SELECT
		(SELECT MAX(JTS_OID) FROM BANDEJA_ASIENTOS_IN WHERE ESTADO=''S'' AND ORIGEN=''TNB''),
		s.SUCURSAL,
		s.PRODUCTO,
		s.CUENTA,
		s.MONEDA,
		...
		CONCAT(''1.18.7 '',a.CONCEPTO_ASIENTO),
	...
	IF(1<(SELECT COUNT(1) FROM BANDEJA_MOVS_IN WHERE JTS_OID_ASIENTO=(SELECT MAX(JTS_OID) FROM BANDEJA_ASIENTOS_IN WHERE ESTADO=''S'' AND ORIGEN=''TNB'')))
	BEGIN
