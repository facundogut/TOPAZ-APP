EXECUTE('
ALTER PROCEDURE [dbo].[SP_PA_GRADUACION_FRACCIONAMIENTO]
...
DECLARE @MAX_REG BIGINT
--- Retornos de sp
 
DECLARE @TOTAL_DEUDA NUMERIC (38, 2)
DECLARE @TRAMO_SIN_G NUMERIC (38, 2)
DECLARE @TRAMO_CON_G NUMERIC (38, 2)
DECLARE @TRAMO_SIN_G_B NUMERIC (38, 2)
DECLARE @TRAMO_CON_G_B NUMERIC (38, 2)
DECLARE @RPC_BANCO NUMERIC (38, 2)
DECLARE @EXCLUSIONES NUMERIC (38, 2)
DECLARE @TOT_CONTROL_GRAD NUMERIC (38, 2)
DECLARE @RPC_CLIENTE NUMERIC (38, 2)
DECLARE @RPC_CLIENTE_MAX NUMERIC (38, 2)
DECLARE @RPC_NETO_BANCO NUMERIC (38, 2)
DECLARE @MARGEN_RESGUAR NUMERIC (38, 2)
DECLARE @LIMITE_GRAD NUMERIC (38, 2)
DECLARE @RESULTADO_GRADUACION VARCHAR (200)
DECLARE @RESULTADO_FRACCIONAMIENTO VARCHAR (200)
DECLARE @IMPORTE_SING_GRUPO NUMERIC (38, 2)
DECLARE @IMPORTE_CONG_GRUPO NUMERIC (38, 2)
...
DECLARE @RPC_BANCO_IMP NUMERIC (38, 2)
...
DECLARE @IMPORTE_LIMITEVINC_O NUMERIC(25,2)
    ...
    SET @fechaproceso = (SELECT FECHAPROCESOANTERIOR FROM PARAMETROS WITH(NOLOCK))
    ...
    BEGIN
		...
		GROUP BY VF.CLIENTE
		SET @MAX_REG = ISNULL((SELECT MAX(NROREGISTRO) FROM @CLIENTES),0)
		INSERT INTO @CLIENTES
		SELECT DISTINCT CLI.CODIGOCLIENTE, MAX(CLI.NUMEROPERSONA) AS NUMEROPERSONA,
		@MAX_REG + ROW_NUMBER() OVER (ORDER BY CLI.CODIGOCLIENTE)
		FROM CRE_VINCULACIONES VINC WITH(NOLOCK)
		INNER JOIN VW_CLI_X_DOC CLI WITH(NOLOCK) ON CLI.NUMEROPERSONA = VINC.ID_PERSONA_VINCULADA AND CLI.TIPOPERSONA = VINC.TIPO_PERSONA_VINCULADA
		WHERE VINC.TZ_LOCK = 0 AND VINC.FECHA_INICIO <= @fechaproceso AND (VINC.FECHA_FIN IS NULL OR VINC.FECHA_FIN >= @fechaproceso)
		AND CLI.CODIGOCLIENTE NOT IN (SELECT CLIENTE FROM @CLIENTES)
		...
		BEGIN
			...
			EXECUTE dbo.SP_CALCULA_GRADUACION @CLIENTE, @PERSONA, @IMPORTE_SOLICITUD, @GRADUACION, @CON_GTIA, @TOTAL_DEUDA OUT, @TRAMO_SIN_G OUT, @TRAMO_CON_G OUT, 
			@TRAMO_SIN_G_B OUT, @TRAMO_CON_G_B OUT, @RPC_BANCO OUT, @EXCLUSIONES OUT, @TOT_CONTROL_GRAD OUT, @RPC_CLIENTE OUT, @RPC_CLIENTE_MAX OUT, @RPC_NETO_BANCO OUT, 
			@MARGEN_RESGUAR OUT, @LIMITE_GRAD OUT, @RESULTADO_GRADUACION OUT, @RESULTADO_FRACCIONAMIENTO OUT, @IMPORTE_SING_GRUPO OUT, @IMPORTE_CONG_GRUPO OUT, @PERIODO OUT, 
			@CN1_BANCO OUT, @MARGEN_BASICO OUT, @RPC_CLIENTE_O OUT, @RPC_BANCO_O OUT, @RPC_CLIENTE_SGR_FGP OUT, @RPC_BANCO_SGR_FGP OUT, @PORC_TRAMO_SING OUT, 
			@PORC_TRAMO_CONG OUT, @MARGEN_RESGUARDO OUT, @RPC_BANCO_IMP OUT, @VINCULO OUT, @DESCRIPCION_GRUPO OUT, @MENSAJE_VINCULADOS OUT, @EXCEDE_GRADUACION_O OUT, 
			@EXCEDE_FRACCIONAMIENTO_O OUT, @TIPO_LIMITE_O OUT, @IMPORTE_LIMITEVINC_O OUT
			...
			DESCRIPCION_GRUPO, DESCRIPCION_VINCULO, TOPE_VINCULADOS, TIPO_TOPE)
	    	...
	    	@TOTAL_DEUDA, @PERIODO, @RESULTADO_GRADUACION, @RESULTADO_FRACCIONAMIENTO, @DESCRIPCION_GRUPO,
	    	@VINCULO, @IMPORTE_LIMITEVINC_O, @TIPO_LIMITE_O)
			SET @PROCESANDO += 1;
		END
		BEGIN
	...
	BEGIN CATCH
	  BEGIN
		    ...
		    DECLARE @PKG_CONSTANTES$C_LOG_TIPO_INFORMACION$4 varchar(1)
			SET @PKG_CONSTANTES$C_LOG_TIPO_INFORMACION$4 = ''E''
			  EXECUTE dbo.PKG_LOG_PROCESO$PROC_INS_LOG_PROCESO 
			   @P_ID_PROCESO = @P_ID_PROCESO, 
			   @P_FCH_PROCESO = @P_DT_PROCESO, 
			   @P_NOM_PACKAGE = ''SP_PA_GRADUACION_FRACCIONAMIENTO - Error'', 
