...
EXECUTE('
	...
	SELECT
		...
		CASE 
			...
			THEN SUBSTRING(O.CBU, 4, 4)  
			ELSE '''' 
		END AS SUCURSAL,
		CASE 
			WHEN (SELECT NUMERICO FROM PARAMETROSGENERALES WHERE CODIGO = 2) = LEFT(O.CBU, 3)
			THEN SUBSTRING(O.CBU, 9, 13)  
			ELSE '''' 
		END AS CUENTA,
		OT.DESCRIPCION AS TIPO_CUENTA_DEBITADA,
		M.C6400 AS NOMBRE_MONEDA,
		B.NOMBRE AS NOMBRE_BANCO,
		O.CLIENTE_PAGADOR AS ID_CLIENTE_PAGADOR,
		O.REFERENCIA,
	...
	LEFT JOIN OPCIONES AS OT WITH (nolock) ON
		OE.NUMERODECAMPO = 1785
		AND OE.IDIOMA = ''E''
		AND OE.OPCIONINTERNA = S.C1785
	WHERE
		O.TIPO_ORDEN = ''DEBENV''
		AND O.TZ_LOCK = 0
')
...
EXECUTE('
	...
	LEFT JOIN CLI_BANCOSPLAZA AS B WITH (nolock) ON
		CONVERT(VARCHAR, B.CODIGOBC) = LEFT(D.CBU, 3)
		AND B.SUCURSAL = 0
		AND ((B.TZ_LOCK < 100000000000000 OR B.TZ_LOCK >= 200000000000000)
			AND (B.TZ_LOCK < 300000000000000 OR B.TZ_LOCK >= 400000000000000)  
		)
