Execute('ALTER TABLE VTA_TRANSFERENCIAS ADD REJECTADO VARCHAR(1), ORD_BANCO VARCHAR(4), ORD_SUCURSAL VARCHAR(4);');

Execute('Create OR ALTER   PROCEDURE [dbo].[SP_CLS_TRANS_RECHAZ_PESOS]
	@TICKET NUMERIC(16),
...
Execute('CREATE OR ALTER   PROCEDURE [dbo].[SP_ITF_COELSA_TRANS_RECH_ENV]
	@TICKET NUMERIC(16)
AS
...
BEGIN
	...
	DECLARE @FL_SUMA_TOTAL_DEBITO_LOTE VARCHAR(12)=replicate(''0'',12); --cambio el largo a 12
	DECLARE @FL_SUMA_TOTAL_CREDITO_LOTE VARCHAR(12)=replicate(''0'',12); --cambio el largo a 12
...
EXECUTE('ALTER     PROCEDURE [dbo].[SP_TRAN_MINORISTA_RECIBIDAS]
	@TICKET NUMERIC(16),
	@MONEDA_IN NUMERIC(1),
...
BEGIN TRANSACTION;
	...
	------------Fin Primera Validacion ----------------
	    ...
	    WHERE cp.TITULARIDAD = ''T''
		  ...
		  IF @CODREGISTRO_IN=''CCD'' AND (SELECT TOP 1 PRODUCTO FROM SALDOS NOLOCK  WHERE CUENTA=@NUMcUENTA AND jts_oid=@JTS_OID AND TZ_LOCK=0) NOT IN (1,2,3,4,6,7,16,23,24,53)--(SELECT C6250 FROM productos NOLOCK WHERE TIPO_CUENTA_VISTA IN (7,11,5,8) AND TZ_LOCK=0)
			BEGIN 
	      		INSERT INTO dbo.VTA_TRANSFERENCIAS (OP_NUMERO, NUMERO_ASIENTO,  OP_FECHA_PRES, OP_CLASE_TRANS, ESTADO, OP_MONEDA, OP_TIPO, BEN_CBU, OP_IMPORTE, BEN_JTS_OID_CUENTA, BEN_NRO_DOC, MOTIVO_RECHAZO, OP_FORMA_ING, FECHA_ESTADO, CONTABILIZADA, OP_INFO_REF,OPERATORIA_BCRA, FCH_DEV_REC, TRACENUMBER 
	      													, ORD_NRO_DOC
											, ORD_NOMBRE
