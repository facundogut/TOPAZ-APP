...
BEGIN
	...
	BEGIN
		...
		SET @CL_FECHA_COMPENSACION = convert(VARCHAR,(SELECT FECHAPROXIMOPROCESO FROM PARAMETROS WITH(NOLOCK)), 12); 
		SET @CL_FECHA_VENCIMIENTO = convert(VARCHAR,(SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)), 12);
