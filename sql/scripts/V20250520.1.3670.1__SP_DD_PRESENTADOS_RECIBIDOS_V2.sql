EXECUTE('CREATE OR ALTER   PROCEDURE [dbo].[SP_DD_PRESENTADOS_RECIBIDOS_V2]
	@TICKET NUMERIC(16),
...
--s		@totalDebitos_FA = CAST(substring(linea, 32, 12) AS NUMERIC), -- Se vuelve a formato anterior A.E 10/10/2024
--s		@totalCreditos_FA = CAST(substring(linea, 44, 12) AS NUMERIC) -- Se vuelve a formato anterior A.E 10/10/2024
...
--s		@controlDebitos_FL = sum(CAST(substring(LINEA, 21, 12) AS NUMERIC)), -- Se vuelve a formato anterior A.E 10/10/2024
--s		@controlCreditos_FL = sum(CAST(substring(LINEA, 33, 12) AS NUMERIC)) -- Se vuelve a formato anterior A.E 10/10/2024
	...
	BEGIN
      ...
      BEGIN
			SET @ClaseTransaccion = substring(@LINEA, 2, 3);
--s			SET @ReservadoFL = substring(@LINEA, 45, 35); -- Se vuelve a formato anterior
...
--s			SET @ReservadoFA  = substring(@LINEA, 56, 39); -- Se vuelve a formato anterior
...
--s			SET @VImporte = substring(@LINEA, 30, 10); -- Se vuelve a formato anterior
...
--	      SET @TipoCuenta = SUBSTRING(@CuentaDebitar,4,2);
	      ...
	      --SELECT @TipoCuenta AS ''TipoCuenta''
...
-- Calcular @E01 de manera que siempre esté entre 0 y 9
	SET @E01 = (10 - @Temp) % 10;
		SET @CTA_CBU=concat(RIGHT(@EntidadDebitar,7),convert(VARCHAR(1),@E01),RIGHT(@CuentaDebitar,14))

	--En caso de que no exista el CBU, seteo el JTS_OID en null y vamos a insertar la inconsistencia 
	IF ((select count(*) from vta_saldos where CTA_CBU = @CTA_CBU)=0)
		SET @saldo_jts_oid = null;
...
--s		  IF ISNUMERIC(@CodTransaccion) = 0 or ISNUMERIC(@EntidadDebitar) = 0  OR ISNUMERIC(@ReservadoRI) = 0 OR ISNUMERIC(@CuentaDebitar) = 0
          IF ISNUMERIC(@CodTransaccion) = 0 or ISNUMERIC(@EntidadDebitar) = 0  OR ISNUMERIC(@CuentaDebitar) = 0
...
--	      SET @TipoCuenta = SUBSTRING(@CuentaDebitar,4,2);
	      ...
	      SET @V08 = 0 --se quita validacion por jira NBCHINT-655 (SELECT COUNT(1) FROM SNP_STOP_DEBIT WHERE FECHA_DESDE<=@FECHADATE AND FECHA_VENCIMIENTO>@FECHADATE AND SALDO_JTS_OID = @saldo_jts_oid );
		...
		IF (@CodTransaccion = ''32'' AND @InfoAdicional = ''00'') OR (@CodTransaccion = ''37'' AND @InfoAdicional = ''01'')
		BEGIN
		...
		END
		-- tipo 38 e info adicional in 4 baja -> ESTADO=
		--								5,6 cambio 6 cambio: ESTADO=
		IF @codtransaccion!=38
		BEGIN
			--NBCHINT-850 - A.E 10/3/2025
			IF @codtransaccion = 37 AND @RegistrosAdicionales = ''0''
			BEGIN
       			SET @TraceNumberRA = NULL;
				SET @FechaVencimientoRA = NULL;
			END

	/**		IF (@codtransaccion = 37 AND @InfoAdicional = ''01'' AND LEFT(@EntidadDebitar,4)=''0311'' AND LEFT(@ContadorRegistros,4) = ''0311'')
				SELECT TOP 1 @CTA_CBU=CBU ,@saldo_jts_oid=SALDO_JTS_OID, @Cod_Cliente=CODIGO_CLIENTE
					from SNP_DEBITOS WITH (NOLOCK)
					WHERE REFERENCIA= @ReferenciaUnivoca 
					AND FECHA_VTO=@FechaVencimientoRA
					AND IDENTIF_CLIENTE= @IdClientePegador**/

			IF (@codtransaccion = 37 AND @InfoAdicional = ''01'' AND LEFT(@EntidadDebitar,4)=''0311'' AND LEFT(@ContadorRegistros,4) = ''0311'')
			BEGIN
				-- Inicializar variables temporales
				DECLARE @tmpCBU		  VARCHAR(22),
						@tmpSaldoOid  NUMERIC(15),
						@tmpCliente   NUMERIC(20);
 
				SET @tmpCBU = NULL;
				SET @tmpSaldoOid = NULL;
				SET @tmpCliente = NULL;
 
				-- Intentamos obtener los datos desde SNP_DEBITOS
				SELECT TOP 1 
					@tmpCBU = d.CBU,
					@tmpSaldoOid = d.SALDO_JTS_OID,
					@tmpCliente = d.CODIGO_CLIENTE
				FROM SNP_DEBITOS d WITH (NOLOCK)
				WHERE d.REFERENCIA = @ReferenciaUnivoca
				  AND d.FECHA_VTO = @FechaVencimientoRA
				  AND d.IDENTIF_CLIENTE = @IdClientePegador;
 
				-- Si encontramos datos, sobrescribimos el valor por defecto
				IF @tmpCBU IS NOT NULL
				BEGIN
					SET @CTA_CBU = @tmpCBU;
					SET @saldo_jts_oid = @tmpSaldoOid;
					SET @Cod_Cliente = @tmpCliente;
				END
				ELSE
				BEGIN
					-- Buscar en ITF_DD_PRESENT_RECIB_RECHAZOS
					SELECT TOP 1 
						@tmpCBU = t.CBU,
						@tmpSaldoOid = t.SALDO_JTS_OID
					FROM ITF_DD_PRESENT_RECIB_RECHAZOS t WITH (NOLOCK)
					WHERE t.REFERENCIA = @ReferenciaUnivoca
					  AND t.FECHA_VTO = @FechaVencimientoRA
					  AND t.ID_CLIENTE_PAGADOR = @IdClientePegador;
 
					-- Si encontramos en ITF, también sobreescribimos
					IF @tmpCBU IS NOT NULL
					BEGIN
						SET @CTA_CBU = @tmpCBU;
						SET @saldo_jts_oid = @tmpSaldoOid;
			...
			IF NOT (@codtransaccion = 37 AND @InfoAdicional = ''01'' AND LEFT(@EntidadDebitar,4)=''0311'' AND LEFT(@ContadorRegistros,4) <> ''0311'')
...
PRINT @RegistrosAdicionales
		...
		BEGIN 
 			...
 			IF(
 				...
 				/**AND FECHA_VENCIMIENTO=@FechaVencimientoRA
 				AND (TRACE_N_1PRE=@TraceNumberRA OR TRACE_N_2PRE=@TraceNumberRA)**/
				AND ((FECHA_VENCIMIENTO=@FechaVencimientoRA AND TRACE_N_1PRE=@TraceNumberRA)
 				OR (SEGUNDO_VTO=@FechaVencimientoRA AND TRACE_N_2PRE=@TraceNumberRA)))=''S''
   			BEGIN
 				...
 				AND (TRACE_N_1PRE=@TraceNumberRA OR TRACE_N_2PRE=@TraceNumberRA)**/
				AND ((FECHA_VENCIMIENTO=@FechaVencimientoRA AND TRACE_N_1PRE=@TraceNumberRA)
 				OR (SEGUNDO_VTO=@FechaVencimientoRA AND TRACE_N_2PRE=@TraceNumberRA))
