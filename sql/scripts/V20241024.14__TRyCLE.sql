...
BEGIN
    ...
    -- CONSTANTES máximos de importes
    DECLARE @MAX_SUMA_IMPORTES NUMERIC(23, 2) = 999999999999999999.99
    DECLARE @MAX_IMPORTE_INDIVIDUAL NUMERIC(16, 2) = 9999999999999.99
    DECLARE @MAX_SUMA_ENTIDAD INT = 999999
    DECLARE @MAX_SUMA_SUCURSAL INT = 9999
	...
	DECLARE @FL_SUMA_TOTAL_DEBITO_LOTE VARCHAR(21); 
	DECLARE @FL_SUMA_TOTAL_CREDITO_LOTE VARCHAR(21); 
	...
	DECLARE @FA_SUMA_TOTAL_DEBITOS VARCHAR(21);
	DECLARE @FA_SUMA_TOTAL_CREDITOS VARCHAR(21);
	...
	------- Variables generales ------------
	DECLARE @SumaImportes NUMERIC(23,2) = 0;
	DECLARE @TotalesControl NUMERIC(10) = 0;
	DECLARE @TotalesDebitos NUMERIC(23,2) = 0;
	DECLARE @TotalesCreditos NUMERIC(23,2) = 0;
	...
	DECLARE @Excedente NUMERIC(23,2) = 0;
	...
	BEGIN
		...
		SET @FL_SUMA_TOTAL_DEBITO_LOTE = (RIGHT(concat(replicate(''0'', 20), (SELECT sum(convert(numeric,substring(linea,61,16)))
																	FROM ITF_CHEQUES_SALIDA_AUX
																	WHERE LINEA LIKE ''6%'' 
																	AND ID>(SELECT max(id) 
																			FROM ITF_CHEQUES_SALIDA_AUX 
																			WHERE LINEA LIKE ''5%''))), 20))
   	
		SET @FL_SUMA_TOTAL_CREDITO_LOTE = RIGHT(replicate(''0'', 20), 20);
		...
		SET @FA_SUMA_TOTAL_DEBITOS = (RIGHT(concat(replicate(''0'', 20), (SELECT sum(convert(numeric,substring(linea,21,20)))
																	FROM ITF_CHEQUES_SALIDA_AUX
																	WHERE LINEA LIKE ''8%'' 
																	AND ID>(SELECT max(id) 
																			FROM ITF_CHEQUES_SALIDA_AUX 
																			WHERE LINEA LIKE ''1%''))), 20))



		SET @FA_SUMA_TOTAL_CREDITOS = RIGHT(concat(replicate(''0'', 20), (replace(@TotalesCreditos, ''.'', ''''))),20);
	...
	BEGIN
		...
		BEGIN
			IF @T_SERIE_DEL_CHEQUE!=''E''
				SET @ri_info_adicional=''00'';
			ELSE 
				SET @ri_info_adicional=''02'';
			Start_1:
			IF (@SumaImportes > @MAX_SUMA_IMPORTES OR @SumaEntidad > @MAX_SUMA_ENTIDAD) 
				BEGIN
			
					IF @SumaSucursal > @MAX_SUMA_SUCURSAL
					BEGIN
			...
			END
			IF	(@T_IMPORTE>@MAX_IMPORTE_INDIVIDUAL)
			BEGIN
				SET @Excedente = (@T_IMPORTE - @MAX_IMPORTE_INDIVIDUAL);
				SET @T_IMPORTE = @MAX_IMPORTE_INDIVIDUAL;
				SET @CountExcedente += 1;
			...
			IF @SumaImportes>@MAX_SUMA_IMPORTES GOTO Start_1
			...
			SET @RI_REGISTRO_INDIVIDUAL = concat(@RI_ID_REG, @RI_CODIGO_TRANSAC, @RI_ENTIDAD_DEBITAR, @RI_RESERVADO,
                                                @RI_CUENTA_DEBITAR, @RI_PUNTO_INTERCAMBIO, @RI_NUMERO_CHEQUE, @RI_CODIGO_POSTAL,
                                                @RI_IMPORTE, @RI_INFO_ADICIONAL, @RI_REGISTRO_ADICIONAL,@RI_CONTADOR_REGISTRO);
		...
		-- ¿Será acá?	
		SET @FL_FIN_LOTE = concat(@FL_ID_REG, @FL_CODIGO_CLASE_TRANSAC, @FL_CANT_REG_INDIVIDUAL_ADICIONAL,
                                @FL_TOTALES_DE_CONTROL, @FL_SUMA_TOTAL_DEBITO_LOTE, @FL_SUMA_TOTAL_CREDITO_LOTE,
                                @FL_RESERVADO1, @FL_RESERVADO2, @FL_RESERVADO3, @FL_REG_ENTIDAD_ORIGEN, @FL_NUMERO_LOTE);
		...
		BEGIN
			...
			BEGIN
				...
				SET @CountExcedente += 1;
			END
			IF	(@A_IMPORTE>@MAX_IMPORTE_INDIVIDUAL)
			BEGIN
				SET @Excedente = (@A_IMPORTE - @MAX_IMPORTE_INDIVIDUAL);
				SET @A_IMPORTE = @MAX_IMPORTE_INDIVIDUAL;
				SET @CountExcedente += 1;
				--SET @RA_CONTADOR_REGISTRO_ORIGEN = @RI_CONTADOR_REGISTRO;
			END
			ELSE
...
--***************************************************************************************--	
			...
			IF @SumaImportes>@MAX_SUMA_IMPORTES GOTO Start_2

			SET @RI_IMPORTE = RIGHT(concat(replicate(''0'', 16), replace(CAST(@A_IMPORTE AS VARCHAR),''.'','''')), 16);
		...
		BEGIN
			SET @ri_info_adicional=''00''
			Start_3:
			IF (@SumaImportes > @MAX_SUMA_IMPORTES OR @SumaEntidad > @MAX_SUMA_ENTIDAD) -- 99 millones
			BEGIN
			...
			BEGIN
				...
				SET @CountExcedente += 1;
			END
			IF	(@D_IMPORTE>@MAX_IMPORTE_INDIVIDUAL)
			BEGIN
				SET @Excedente = (@D_IMPORTE - @MAX_IMPORTE_INDIVIDUAL);
				SET @D_IMPORTE = @MAX_IMPORTE_INDIVIDUAL;
				SET @CountExcedente += 1;
				--SET @RA_CONTADOR_REGISTRO_ORIGEN = @RI_CONTADOR_REGISTRO;
			END
			ELSE
			...
			IF @SumaImportes>@MAX_SUMA_IMPORTES GOTO Start_3
			
			SET @RI_IMPORTE = RIGHT(concat(replicate(''0'', 16), replace(CAST(@D_IMPORTE AS VARCHAR),''.'','''')), 16);
...
PRINT @RI_CODIGO_POSTAL
		...
		BEGIN
			...
			--SET @FL_SUMA_TOTAL_DEBiTO_LOTE = RIGHT(concat(replicate(''0'', 12), (replace(@SumaImportes, ''.'', ''''))), 12); 
					   	   	...
					   	   	--nuevo
	   		SET @FL_SUMA_TOTAL_DEBITO_LOTE = (RIGHT(concat(replicate(''0'', 20), (SELECT sum(convert(numeric,substring(linea,61,16))) FROM ITF_CHEQUES_SALIDA_AUX WHERE LINEA LIKE ''6%'' AND ID>(SELECT max(id) FROM ITF_CHEQUES_SALIDA_AUX WHERE LINEA LIKE ''5%''))), 20))
			SET @FL_SUMA_TOTAL_CREDITO_LOTE = RIGHT(replicate(''0'', 20), 20);
			SET @FL_NUMERO_LOTE = RIGHT(concat(replicate(''0'', 7), (SELECT count(ID) FROM ITF_CHEQUES_SALIDA_AUX WHERE LINEA LIKE ''5%'' AND id>(SELECT max(id) FROM ITF_CHEQUES_SALIDA_AUX WHERE LINEA LIKE ''1%''))), 7); 		
			...
			SET @FA_SUMA_TOTAL_DEBITOS = (RIGHT(concat(replicate(''0'', 20), (SELECT sum(convert(numeric,substring(linea,21,20)))	FROM ITF_CHEQUES_SALIDA_AUX	WHERE LINEA LIKE ''8%'' AND ID>(SELECT max(id) FROM ITF_CHEQUES_SALIDA_AUX WHERE LINEA LIKE ''1%''))), 20))
			SET @FA_SUMA_TOTAL_CREDITOS = RIGHT(concat(replicate(''0'', 20), (replace(@TotalesCreditos, ''.'', ''''))),20);				
			SET @FA_FIN_ARCHIVO = concat(@FA_ID_REG, @FA_CANT_LOTES, @FA_NUMERO_BLOQUES, @FA_CANT_REG_INDIVIDUAL_ADICIONAL, @FA_TOTALES_DE_CONTROL, @FA_SUMA_TOTAL_DEBITOS, @FA_SUMA_TOTAL_CREDITOS, @FA_RESERVADO);
...
Execute('CREATE OR ALTER PROCEDURE [dbo].[SP_ITF_COELSA_TRANS_MPD]
@TICKET NUMERIC(16)
AS
...
BEGIN
	...
	SELECT A.ORD_NRO_DOC, 
		   A.BEN_BANCO,
	   		...
	   		(CASE WHEN A.ORD_TIPO_DOC NOT IN (SELECT TIPODOCUMENTO FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA=''F'') or OP_TIPO=8  THEN SUBSTRING(CAST(A.ORD_NRO_DOC AS VARCHAR(11)),1,10) ELSE REPLICATE(''0'',10) END) AS CL_IDENTIFICACION_EMP_IND,
			(CASE WHEN A.ORD_TIPO_DOC NOT IN (SELECT TIPODOCUMENTO FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA=''F'') or OP_TIPO=8  THEN SUBSTRING(CAST(A.ORD_NRO_DOC AS VARCHAR(11)),11,1) ELSE ''0'' END) AS CL_DIGITO_VERIFICADOR,
			LEFT((CASE WHEN A.ORD_TIPO_DOC NOT IN (SELECT TIPODOCUMENTO FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA=''F'') or OP_TIPO=8 THEN ORD_NOMBRE ELSE ''PARTICULARES'' END) + REPLICATE('' '',16),16) AS CL_NOMBRE_EMPRESA_INDIVIDUO,	   		
	...
	BEGIN
	--*******--
PRINT @CL_OP_CLASE_TRANS
PRINT @CL_OP_TIPO
PRINT @CL_BEN_BANCO
PRINT @CL_ORD_NRO_DOC
	--*******--
		...
		BEGIN
	 	...
	 	Start:
			...
			BEGIN
				PRINT @SumaImportes
				--SELECT ''FA_SUMA_ACUMAULADA '', @FA_SUMA_ACUMULADA AS FA_SUMA_ACUMULADA;
				IF @SumaSucursal > 9999
				BEGIN
					SET @SobranteSucursal = CAST(substring(CAST(@SumaSucursal AS VARCHAR), 1, (len(@SumaSucursal) - 4)) AS NUMERIC);
					--SELECT ''SOBRANTE SUCURSAL '', @SobranteSucursal
					SET @SumaEntidad += @SobranteSucursal;
					SET @SumaSucursal = CAST(RIGHT(@SumaSucursal, 4) AS NUMERIC);
				END
				...
				SET @FL_CANT_REG_INDIVIDUAL_ADICIONAL = (SELECT count(1) FROM ITF_COELSA_MPD_TEMP WHERE substring(linea,1,1) IN (''6'',''7'') AND ID>(SELECT max(id) FROM ITF_COELSA_MPD_TEMP WHERE LINEA LIKE ''5%'')) 
	   			--SELECT ''FL MITAD'', @SumaEntidad AS SumaEntidad, @SumaSucursal AS SumaSucursal
				...
				-------------------------------------------------------------------
				-------------Grabamos el Fin de Archivo ----------------------------------------------------------------------------------------------------------------------
				--SELECT ''FA MITAD'', @FA_SumaEntidad AS FA_SumaEntidad, @FA_SumaSucursal AS FA_SumaSucursal
				...
				--SELECT ''FA FINAL_NEW'', @FA_TotalesCreditos AS FA_TotalesCreditos, @SumaImportes AS SumaImportes
				--OPH FIN 07112023
			...
			--SELECT ''SUM_ACUMU_INDIVIDUAL'', @FA_SUMA_ACUMULADA AS FA_SUMA_ACUMULADA;
    		...
    		BEGIN
				...
				------ Registro Individual -------
				--SET @RI_ENTIDAD_AACREDITAR = concat(RIGHT(concat(replicate(''0'', 4), @DP_BANCO+500), 4), RIGHT(concat(replicate(''0'', 4), @DP_SUCURSAL), 4)); 
			...
			--***editado por j izquierdo 06/05/2024***--
    		...
    		SET ESTADO=''PR'' 
    			...
    			--, FCH_DEV_REC=(SELECT FECHAPROCESO FROM PARAMETROS)
			...
			INSERT INTO ITF_COELSA_MPD_TEMP (LINEA) VALUES (replace(@RA_REGISTRO_ADICIONAL, ''.'', ''''));
		...
		--SELECT ''FL FINAL'', @SumaEntidad AS SumaEntidad, @SumaSucursal AS SumaSucursal
		...
		--SELECT ''cursor entidad'', @SumaEntidad AS SumaEntidad
		SET @FA_SumaEntidad = @FA_SumaEntidad + @SumaEntidad
		--SELECT ''cursor FA entiedad'', @FA_SumaEntidad AS FA_SumaEntidad
