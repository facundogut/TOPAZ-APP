EXECUTE('
IF OBJECT_ID (''dbo.VW_POSICION_CLIENTES_TOTALES'') IS NOT NULL
	DROP VIEW dbo.VW_POSICION_CLIENTES_TOTALES
')

EXECUTE('
CREATE VIEW [dbo].[VW_POSICION_CLIENTES_TOTALES] AS 
SELECT 
TIPODOC AS ''Tipo'',
...
CASE WHEN SUM(DEUDA_TOTAL) < SUM(DEUDA_CONTINGENTE) THEN SUM(DEUDA_CONTINGENTE)+SUM(OPCION_COMPRA) 
	 ELSE SUM(DEUDA_TOTAL)+SUM(OPCION_COMPRA) END AS ''Riesgo Total''
FROM ( 
SELECT  	 
	 CASE 
	 WHEN vw.CLASIFICACION=''L'' THEN ''Leasing''
	 ...
	 SUM(ISNULL(bl.OPCION_COMPRA,0)) AS OPCION_COMPRA,
...
WHERE (CASE WHEN CLASIFICACION IN (''S'',''A'',''O'') THEN (vw.SALDO+vw.SALDO_DEUDA+vw.MONTOORIGEN) ELSE vw.SALDO END) >0
GROUP BY vw.TIPODOC,vw.NUMERODOC, vw.PAISDOCUMENTO, vw.NUMEROPERSONA,CLASIFICACION, prod.MONTO
) T
GROUP BY TIPODOC,NUMERODOC, PAISDOCUMENTO, NUMEROPERSONA
')
