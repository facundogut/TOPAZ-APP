...
BEGIN
	...
	-----------------------------------	
	@contador NUMERIC(10),
	@contadorEmergencia NUMERIC(10)
	SET @contador = 0;
	SET @contadorEmergencia = 0;
	------- Campos para el LOG --------
	...
	BEGIN TRY
		...
		-- Si cliente esta en emergencia se congela clasificacion, siempre y cuando no tenga categoria subjetiva cargada.				
		SET @contador = (SELECT count(1) FROM CLI_CLIENTES WHERE NIVEL_APERTURA IN (3,4) 
						 AND CODIGOCLIENTE NOT IN (SELECT c.nro_cliente 
									FROM CRE_EMERGENCIA_AGRO a WITH (NOLOCK)
									INNER JOIN CRE_CERTIFICADOS_EMERGENCIA c WITH (NOLOCK) ON a.id=c.id_emergencia AND c.TZ_LOCK = 0
									INNER JOIN CLI_CLIENTES cl WITH (NOLOCK) ON cl.CODIGOCLIENTE=c.nro_cliente AND cl.TZ_LOCK=0
									WHERE    									 
									a.fecha_vig_desde <= (SELECT fechaproceso FROM PARAMETROS WITH (NOLOCK))
									AND a.fecha_vig_hasta >= (SELECT fechaproceso FROM PARAMETROS WITH (NOLOCK))
									AND c.fecha_alta <= (SELECT fechaproceso FROM PARAMETROS WITH (NOLOCK))
									AND a.TZ_LOCK =0
									AND cl.CATEGORIASUBJETIVA = '' '' AND CATEGORIASUBJETIVA IS NOT NULL
									)  AND TZ_LOCK = 0
						)
	    SET @contadorEmergencia = (SELECT count(1) FROM CLI_CLIENTES WHERE NIVEL_APERTURA IN (3,4) 
						 			AND CODIGOCLIENTE IN (SELECT c.nro_cliente 
									FROM CRE_EMERGENCIA_AGRO a WITH (NOLOCK)
									INNER JOIN CRE_CERTIFICADOS_EMERGENCIA c WITH (NOLOCK) ON a.id=c.id_emergencia AND c.TZ_LOCK = 0
									INNER JOIN CLI_CLIENTES cl WITH (NOLOCK) ON cl.CODIGOCLIENTE=c.nro_cliente AND cl.TZ_LOCK=0
									WHERE    									 
		...
		BEGIN
			...
			CODIGOCLIENTE 
			FROM CLI_CLIENTES WITH (NOLOCK) WHERE NIVEL_APERTURA IN (3,4) AND TZ_LOCK = 0) a
			ON a.CODIGOCLIENTE = c.CODIGOCLIENTE
			-- Agregamos condicion de emergencia sanitaria y sin subjetiva cargada
			AND c.CODIGOCLIENTE NOT IN (SELECT c.nro_cliente 
									FROM CRE_EMERGENCIA_AGRO a WITH (NOLOCK)
									INNER JOIN CRE_CERTIFICADOS_EMERGENCIA c WITH (NOLOCK) ON a.id=c.id_emergencia AND c.TZ_LOCK = 0
									INNER JOIN CLI_CLIENTES cl WITH (NOLOCK) ON cl.CODIGOCLIENTE=c.nro_cliente AND cl.TZ_LOCK=0
									WHERE    									 
			...
			-- Agregamos condicion de emergencia sanitaria y sin subjetiva cargada
			DELETE HISTORICO_CALIF_X_CLIENTE WHERE CLIENTE IN 
			(SELECT CODIGOCLIENTE FROM CLI_CLIENTES WITH (NOLOCK) WHERE NIVEL_APERTURA IN (3,4) AND TZ_LOCK = 0
			AND CODIGOCLIENTE NOT IN (SELECT c.nro_cliente 
									FROM CRE_EMERGENCIA_AGRO a WITH (NOLOCK)
									INNER JOIN CRE_CERTIFICADOS_EMERGENCIA c WITH (NOLOCK) ON a.id=c.id_emergencia AND c.TZ_LOCK = 0
									INNER JOIN CLI_CLIENTES cl WITH (NOLOCK) ON cl.CODIGOCLIENTE=c.nro_cliente AND cl.TZ_LOCK=0
			...
			SELECT CODIGOCLIENTE, (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK)), CATEGORIAOBJETIVA, OBJETIVA_REFINANCIADO, CATEGORIASUBJETIVA, CATEGORIA_OFICIAL, 
			...
			WHEN OBJETIVA_REFINANCIADO = '' '' AND CATEGORIAOBJETIVA = '' '' THEN 7
			...
			FROM CLI_CLIENTES WITH (NOLOCK) WHERE NIVEL_APERTURA IN (3,4) AND TZ_LOCK = 0  
			-- Agregamos condicion de emergencia sanitaria y sin subjetiva cargada
			AND CODIGOCLIENTE NOT IN (SELECT c.nro_cliente 
									FROM CRE_EMERGENCIA_AGRO a WITH (NOLOCK)
									INNER JOIN CRE_CERTIFICADOS_EMERGENCIA c WITH (NOLOCK) ON a.id=c.id_emergencia AND c.TZ_LOCK = 0
		...
		-- Clientes que presentaron hoy el certificado solo generan HISTORICO_CALIF_X_CLIENTE
		...
		BEGIN	  
			...
			INSERT INTO dbo.HISTORICO_CALIF_X_CLIENTE (CLIENTE, FECHA, CALIFICACION_OBJETIVA, CALIFICACION_REESTRUCTURADA, CALIFICACION_SUBJETIVA, CALIFICACION_SF, CALIFICACION_RESULTANTE, TIPO_RIESGO, DIAS_ATRASO, TZ_LOCK, SITUACION_JUDICIAL, Sit_MorExEnt, DISCREPANCIA, TIPO_CALIFICACION)
			SELECT CODIGOCLIENTE, (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK)), 
			CATEGORIAOBJETIVA, 
			OBJETIVA_REFINANCIADO, 
			CATEGORIASUBJETIVA, 
			CATEGORIA_OFICIAL, 
			CATEGORIARESULTANTE AS CALIFICACION_RESULTANTE,
			CATEGORIA_COMERCIAL, 0, 0, SITUACION_JUDICIAL, Sit_MorExEnt, DISCREPANCIA,
			CASE WHEN CATEGORIASUBJETIVA <> '' '' AND CATEGORIASUBJETIVA IS NOT NULL THEN 1 
			WHEN Sit_MorExEnt <> '' '' AND Sit_MorExEnt IS NOT NULL THEN 2 
			WHEN SITUACION_JUDICIAL <> '' '' AND SITUACION_JUDICIAL IS NOT NULL THEN 3
