...
BEGIN
	...
	BEGIN TRY
		...
		WHERE (c.CATEGORIAOBJETIVA <> '' '' or c.OBJETIVA_REFINANCIADO <> '' '') AND c.TZ_LOCK=0 AND b.TZ_LOCK=0  
		...
		LEFT JOIN (SELECT CODIGOCLIENTE, sum(DEUDA_TOTAL) AS DEUDA 
		...
		round(( d.DEUDA / (dt.DEUDA + ISNULL(dm.DEUDA,0)) ),2) * 100  >= (SELECT IMPORTE FROM CRE_PARAMETROS WITH (NOLOCK) WHERE CODIGO = 168))				
...
-- Si existen registros candidatos actualizamos
		...
		BEGIN
			...
			INNER JOIN (SELECT c.CODIGOCLIENTE, p.CATEGORIASUB
				...
				WHERE c.TZ_LOCK = 0 AND
				round((d.DEUDA / (dt.DEUDA + ISNULL(dm.DEUDA,0))),2) * 100 >= (SELECT IMPORTE FROM CRE_PARAMETROS WITH (NOLOCK) WHERE CODIGO = 168)) a
