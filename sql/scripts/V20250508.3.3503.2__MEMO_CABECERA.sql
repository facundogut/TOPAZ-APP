...
AS 
   BEGIN
      ...
      */
      ...
      NUM_CATEGORIA NUMERIC(5), NUMERO BIGINT
      )
     ...
     ---DECLARE @FECHA_PROCESO DATETIME 
      ...
      DELETE FROM MEMO_CABECERA WHERE Fecha_Info = @FECHA_PROCESO
                ...
                ---- Casos con deuda en memo detalle
                
                INSERT INTO @INFO_CLIENTES
                select t.*, ROW_NUMBER() OVER (PARTITION BY t.Num_Doc order by t.Num_Doc) AS NUMERO FROM (
                SELECT DISTINCT
                 ...
                 C.IVA, ---IVA
                 OP.DESCRIPCION AS DESC_IVA, ---Descripcion iva
                 C.TIPO, ---Tipo persona
                 ...
                 CASE WHEN c.OBJETIVA_REFINANCIADO <> '' '' OR c.OBJETIVA_REFINANCIADO IS NULL THEN ''S'' ELSE ''N'' END AS REFINACIADO, ---Refinanciado
                 CASE WHEN
                ...
                ---0 AS NUMERADOR,
                CASE 
                WHEN C.CATEGORIA_COMERCIAL = ''M'' THEN 3
                WHEN C.CATEGORIA_COMERCIAL = ''S'' THEN 2
                WHEN C.CATEGORIA_COMERCIAL = ''C'' THEN 1
                ELSE 0 END AS NUM_CATEGORIA
                ...
                INNER JOIN MEMO_DETALLE MD WITH(NOLOCK) ON C.CODIGOCLIENTE = MD.Cliente
                 ...
                 WHERE C.TZ_LOCK = 0 AND MD.Fecha_Info = @FECHA_PROCESO) AS t
                 
                 
                 
                 --- Caso sin deuda en memo detalle
                 
                 --- Tomo clientes que no tienen deuda en el día pero si en el mes
                 
                 IF(@DIA_FECHA > 1)
                 BEGIN
                 	DECLARE @CLIENTES_SINDEUDA TABLE (NUMERODOC VARCHAR(20), NOMBRE VARCHAR(70), ULT_FECHA DATETIME)
                 	
                 	INSERT INTO @CLIENTES_SINDEUDA
                 	SELECT CUIT_CUIL_CDI, Denominacion, MAX(Fecha_Info)
                 	FROM MEMO_CABECERA WITH(NOLOCK)
                 	WHERE MONTH(Fecha_Info) = MONTH(@FECHA_PROCESO) AND YEAR(Fecha_Info) = YEAR(@FECHA_PROCESO)
                 	AND CUIT_CUIL_CDI NOT IN (SELECT NUMERODOC FROM @INFO_CLIENTES)
                 	GROUP BY CUIT_CUIL_CDI, Denominacion
                 	...
                 	M.Cod_Activ_BCRA,
                 	M.Cod_Activ_AFIP,
                 	M.Desc_Cod_Activ_AFIP,
                 	M.Suc_Cliente,
                 	M.Categoria_Cliente AS CATEGORIA_CLIENTE,
                 	M.Cat_IVA,
                 	M.Desc_Cat_IVA,
                 	'''' AS SIT_PROD_BANCO,
                 	M.Sector_Persona,
                 	...
                 	''N'' AS EMERGENCIA,
                 	CASE 
                 	WHEN M.Categoria_Cliente = 3 THEN ''Comercial''
                 	WHEN M.Categoria_Cliente = 2 THEN ''Comercial asimilable a consumo''
                 	WHEN M.Categoria_Cliente = 1 THEN ''Consumo''
                 	ELSE '''' END AS DESC_CATEGORIA,
                 	0 AS TZ_LOCK
                 	FROM MEMO_CABECERA M WITH(NOLOCK) 
                 	INNER JOIN @CLIENTES_SINDEUDA C ON M.CUIT_CUIL_CDI = C.NUMERODOC AND C.ULT_FECHA = M.Fecha_Info
                 	
                 END
                
                /*
                --- Agrupo registros de más de un cliente
                
                ---Clasificacion
                
                UPDATE IC
                SET IC.CLASIFICACION = (SELECT MAX(IC2.CLASIFICACION) FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                ----INNER JOIN @INFO_CLIENTES IC2 ON IC.NUMERODOC = IC2.NUMERODOC
                WHERE IC.NUMERODOC IN (SELECT NUMERODOC FROM @INFO_CLIENTES GROUP BY NUMERODOC HAVING COUNT(1) > 1)
                
                ---Sector persona
                
                UPDATE IC
                SET IC.SECTOR_PERSONA = (SELECT MAX(IC2.SECTOR_PERSONA) FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                ---INNER JOIN @INFO_CLIENTES IC2 ON IC.NUMERODOC = IC2.NUMERODOC
                WHERE IC.NUMERODOC IN (SELECT NUMERODOC FROM @INFO_CLIENTES GROUP BY NUMERODOC HAVING COUNT(1) > 1)
                
                ---Categoria comercial - Ver sp categorización
               
                UPDATE IC
                SET IC.CAT_CLIENTE = (SELECT CASE WHEN MAX(IC2.NUM_CATEGORIA) = 0 THEN ''S''
                WHEN MAX(IC2.NUM_CATEGORIA) = 1 THEN ''C''
                WHEN MAX(IC2.NUM_CATEGORIA) = 2 THEN ''S''
                WHEN MAX(IC2.NUM_CATEGORIA) = 3 THEN ''M''
                ELSE ''C'' END 
                FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                ----INNER JOIN @INFO_CLIENTES IC2 ON IC.NUMERODOC = IC2.NUMERODOC
                WHERE IC.NUMERODOC IN (SELECT NUMERODOC FROM @INFO_CLIENTES GROUP BY NUMERODOC HAVING COUNT(1) > 1)
                
                UPDATE IC
                SET IC.DESC_CATEGORIA = (SELECT TOP 1 CASE 
                WHEN IC2.CAT_CLIENTE = ''C'' THEN ''Consumo''
                WHEN IC2.CAT_CLIENTE = ''S'' THEN ''Comercial asimilable a consumo''
                WHEN IC2.CAT_CLIENTE = ''M'' THEN ''Comercial''
                ELSE '''' END
                FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                ----INNER JOIN @INFO_CLIENTES IC2 ON IC.NUMERODOC = IC2.NUMERODOC
                ...
                UPDATE IC
                SET IC.CAT_IVA = (SELECT MAX(CAT_IVA) FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                WHERE IC.NUMERODOC IN (SELECT NUMERODOC FROM @INFO_CLIENTES GROUP BY NUMERODOC HAVING COUNT(1) > 1)
                ...
                UPDATE IC
                SET IC.SEGMENTO = (SELECT MAX(SEGMENTO) FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                WHERE IC.NUMERODOC IN (SELECT NUMERODOC FROM @INFO_CLIENTES GROUP BY NUMERODOC HAVING COUNT(1) > 1)
                ...
                UPDATE IC
                SET IC.SUBSEGMENTO = (SELECT MAX(SUBSEGMENTO) FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                WHERE IC.NUMERODOC IN (SELECT NUMERODOC FROM @INFO_CLIENTES GROUP BY NUMERODOC HAVING COUNT(1) > 1)
                ...
                UPDATE IC
                SET IC.SUCURSAL_CLIENTE = (SELECT MAX(SUCURSAL_CLIENTE) FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                WHERE IC.NUMERODOC IN (SELECT NUMERODOC FROM @INFO_CLIENTES GROUP BY NUMERODOC HAVING COUNT(1) > 1)
                
                ---Objetiva refinanciada
                
                UPDATE IC
                SET IC.REFINANCIADO = (SELECT MAX(REFINANCIADO) FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                WHERE IC.NUMERODOC IN (SELECT NUMERODOC FROM @INFO_CLIENTES GROUP BY NUMERODOC HAVING COUNT(1) > 1)
                ...
                UPDATE IC
                SET IC.SIT_ATRASO = (SELECT MAX(SIT_ATRASO) FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                WHERE IC.NUMERODOC IN (SELECT NUMERODOC FROM @INFO_CLIENTES GROUP BY NUMERODOC HAVING COUNT(1) > 1)
                
                ---Cliente devenga suspenso   --- SIT_JUD_EXT
                
                UPDATE IC
                SET IC.SIT_JUD_EXT = (SELECT MAX(SIT_JUD_EXT) FROM @INFO_CLIENTES IC2 WHERE IC.NUMERODOC = IC2.NUMERODOC)
                FROM @INFO_CLIENTES IC
                WHERE IC.NUMERODOC IN (SELECT NUMERODOC FROM @INFO_CLIENTES GROUP BY NUMERODOC HAVING COUNT(1) > 1)
                */
               ...
               FROM MEMO_DETALLE WITH(NOLOCK) WHERE Fecha_Info = @FECHA_PROCESO GROUP BY Num_Doc) M ON M.Num_Doc = IC.NUMERODOC
                ...
                DEUDA_CON_GARANTIA_B NUMERIC(18,2),
                DEUDA_CON_GARANTIA_A NUMERIC(18,2), NUMERODOC VARCHAR(20))
                
                INSERT INTO @INFO_ASISTENCIAS
                SELECT SUM(ISNULL(DEUDA_SIN_GARANTIA,0))+SUM(ISNULL(DEUDA_CON_GARANTIA_A,0))+SUM(ISNULL(DEUDA_CON_GARANTIA_B,0)) AS DEUDA_TOTAL,
                SUM(ISNULL(DEUDA_SIN_GARANTIA,0)) AS DEUDA_SIN_GARANTIA,
                SUM(ISNULL(DEUDA_CON_GARANTIA_B,0)) AS DEUDA_CON_GARANTIA_B,
                SUM(ISNULL(DEUDA_CON_GARANTIA_A,0)) AS DEUDA_CON_GARANTIA_A,
                NUMERODOC
                FROM (
                SELECT 
				...
				SUM(CASE WHEN (MD.Tipo_Gtia_BCRA IS NULL OR MD.Tipo_Gtia_BCRA = 0) THEN (MD.Saldo_Capital + (MD.Int_Deven_Cobrar_Cont) + MD.IVA_Financiado + MD.RG_Financiado + MD.Seguro_Financ + MD.Otros_Accesorios + MD.Acuerdo_No_Utiliz) END) AS DEUDA_SIN_GARANTIA,
				SUM(CASE WHEN MD.Tipo_Gtia_BCRA = 1 THEN (MD.Saldo_Capital+MD.Acuerdo_No_Utiliz) ELSE 0 END) AS DEUDA_CON_GARANTIA_B,
				SUM(CASE WHEN MD.Tipo_Gtia_BCRA = 2 THEN (MD.Saldo_Capital + (MD.Int_Deven_Cobrar_Cont)+MD.Acuerdo_No_Utiliz) ELSE 0 END) AS DEUDA_CON_GARANTIA_A,
				IC.NUMERODOC
				FROM @INFO_CLIENTES IC
				INNER JOIN MEMO_DETALLE MD WITH(NOLOCK) ON IC.NUMERODOC = MD.Num_Doc
				WHERE MD.Fecha_Info = @FECHA_PROCESO
				GROUP BY IC.NUMERODOC) T
				GROUP BY NUMERODOC
                ...
                IC.SEGMENTO,
                IC.SUBSEGMENTO,
                CASE WHEN ISNULL(IAT.DIAS_ATRASO,0) < 1825 THEN ''N'' ELSE ''S'' END AS DEUDOR_ENCUADRADO,
                IC.REFINANCIADO,
                IC.FALLECIDO,
