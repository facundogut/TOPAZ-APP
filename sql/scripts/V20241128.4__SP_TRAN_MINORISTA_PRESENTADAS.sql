...
--			IF @t_ord_tipo_doc NOT IN (SELECT tipodocumento FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA='F') 
			IF (@t_ord_tipo_doc NOT IN (SELECT tipodocumento FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA='F') AND @CL_RESERVADO_CL not in ('00C','00D')) OR @CL_RESERVADO_CL in ('002','007') 
			BEGIN
