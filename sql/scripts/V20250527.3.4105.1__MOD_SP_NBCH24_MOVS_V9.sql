EXECUTE('
	CREATE OR ALTER PROCEDURE [dbo].[SP_NBCH24_MOVIMIENTOS]
		...
		@P_fv char(1),
		@P_filter char(10) -- aumentado para admitir ''*VALOR''
	AS
	BEGIN
		SET NOCOUNT ON;
		...
		AND h.MONTO > 0 
		AND (
				(@P_filter = ''*PROC'' AND h.SALDO_JTS_OID = @P_jtsoid AND CAST(h.FECHA_PROCESADO AS Date) BETWEEN @P_fechaDesde AND @P_fechaHasta)
			 OR (@P_filter = ''*TIME'' AND h.SALDO_JTS_OID = @P_jtsoid AND CAST(a.HORAFIN AS Date) BETWEEN @P_fechaDesde AND @P_fechaHasta)
			 OR (@P_filter = ''*VALOR'' AND h.SALDO_JTS_OID = @P_jtsoid AND CAST(h.FECHA_VALOR AS Date) BETWEEN @P_fechaDesde AND @P_fechaHasta)
			)
