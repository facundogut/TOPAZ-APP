...
CREATE OR ALTER PROCEDURE PA_AGENCIEROS_PASAJE_CREDITOS
...
AS 
   ...
   BEGIN
      ...
      SELECT @IdConv_CR= NUMERICO FROM PARAMETROSGENERALES WHERE CODIGO = 300 AND TZ_LOCK = 0;
      SELECT @TRANS_CREDITO= CTA_TRANSITORIA FROM CONV_CONVENIOS_REC WHERE Id_ConvRec=@IdConv_CR AND Estado = ''A'' AND TZ_LOCK = 0;
	...
	INSERT INTO @RegistrosAgencieros 
		SELECT ID,JTS_OID,IMPORTE_LIQUIDO,SIG_IMP_LIQUIDO,SUCURSAL FROM REC_Agencieros  
           WHERE ESTADO = ''V'' AND CAJA_DEBITO = ''D'' AND SIG_IMP_LIQUIDO = 1;
      ...
      BEGIN
     			...
     			INSERT INTO GRL_ACREDITACIONES_MASIVAS (TIPO_CTA_DEB, CTA_DEB, SUC_DEB, ESTADO, FCHA_PROCESAR, TIPO_CTA_CRED, CTA_CRED, SUC_CRE, IMPORTE, MON_IMPORTE, APL_EVEN_CRE_DEB, 
     			                                        TZ_LOCK, REFERENCIA_EXTERNA,TIPO_ACREDITACION,
     			                                        EVENTO, EVEN_PAG_COB, CANAL,CODIGO_TRANSACCION)--SE agrega a S NBCHSEG-4189 
				(SELECT
				''V'' AS TIPO_CTA_DEB, 
				@TRANS_CREDITO, 
				SUCURSAL, 
				...
				IMPORTE_LIQUIDO, 
				1 AS MON_IMPORTE, 
				''C'' AS APL_EVEN_CRE_DEB,--SE cambia NBCHSEG-4189 
				0 AS TZ_LOCK, 
				ID AS REFERENCIA_EXTERNA, 
				''AGEN'' AS TIPO_ACREDITACION,
				853 AS EVENTO,--SE agrega NBCHSEG-4189 
      ...
      SET @P_RET_PROCESO = 1
	  SET @P_MSG_PROCESO = ''Pasaje Créditos Funcionó correctamente. Se pasaron ''+ ISNULL(CAST(@CANT AS nvarchar(max)), '''') + '' registros.''
