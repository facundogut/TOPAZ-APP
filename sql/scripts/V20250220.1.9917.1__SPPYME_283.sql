...
Execute('CREATE OR ALTER  PROCEDURE [dbo].[SP_ITF_MIPYME_PADRON]
...
--s	  --Actualizo fecha proceso registros
  ...
  END
	...
	WHERE A.TIPO_CARGO_IMPOSITIVO = 6 AND FECHA = (SELECT MAX(FECHA) FROM RRII_BITACORA_CREDEB B (NOLOCK) WHERE B.JTS_OID_SALDOS = A.JTS_OID_SALDOS AND b.TIPO_CARGO_IMPOSITIVO = 6)
	...
	MERGE RRII_BITACORA_CREDEB AS TARGET
	USING(
			SELECT RI.TZ_LOCK, RI.JTS_OID_SALDOS, @FECHAPROCESO AS FECHA, RI.SEGMENTO, RI.TIPO_CARGO_IMPOSITIVO, RI.PADRON_BENEF_FISCAL, isnull(CB.PADRON,c.CREDEB_MIPYME) AS PADRON
			FROM #TMP_RRII_BITACORA_CREDEB RI
			INNER JOIN SALDOS S (NOLOCK) ON S.JTS_OID = RI.JTS_OID_SALDOS
			LEFT JOIN #TMP_CLIENTE_CAMBIADO CB (NOLOCK) ON CB.CODIGOCLIENTE = S.C1803
			JOIN CLI_CLIENTES c WITH (NOLOCK) ON c.CODIGOCLIENTE = s.c1803
	) AS SOURCE ON TARGET.FECHA = SOURCE.FECHA AND TARGET.JTS_OID_SALDOS = SOURCE.JTS_OID_SALDOS AND TARGET.TIPO_CARGO_IMPOSITIVO = SOURCE.TIPO_CARGO_IMPOSITIVO
	WHEN MATCHED THEN UPDATE SET PADRON_MiPyme = SOURCE.PADRON
	WHEN NOT MATCHED THEN INSERT (TZ_LOCK, JTS_OID_SALDOS, FECHA, SEGMENTO, TIPO_CARGO_IMPOSITIVO, PADRON_BENEF_FISCAL, PADRON_MiPyme)
						  VALUES (0, SOURCE.JTS_OID_SALDOS, SOURCE.FECHA, SOURCE.SEGMENTO, SOURCE.TIPO_CARGO_IMPOSITIVO, SOURCE.PADRON_BENEF_FISCAL, SOURCE.PADRON);
