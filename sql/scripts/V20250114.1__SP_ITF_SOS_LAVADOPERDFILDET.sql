...
FROM CLI_PERSONASFISICAS A (NOLOCK)
  INNER JOIN CLI_DocumentosPFPJ B (NOLOCK) ON B.NUMEROPERSONAFJ = A.NUMEROPERSONAFISICA
    ...
    AND (A.FALLECIDO = ''N'' OR (A.FALLECIDO = ''S''
          AND NOT EXISTS(SELECT 1 FROM CLI_DOCUMENTOSPFPJ V WITH(NOLOCK)
                         WHERE V.NUMERODOCUMENTO = B.NUMERODOCUMENTO AND V.TIPOPERSONA = ''J'')))
UNION
...
FROM CRE_SOL_ACREDITACIONES_SUELDOS A (NOLOCK)
  ...
  INNER JOIN PersonasFJ pfj ON pfj.NUMERODOCUMENTO = c.NUMERODOC AND pfj.TIPODOCUMENTO = c.TIPODOC
...
FROM CRE_TAM_EMP_BALANCE A (NOLOCK)
	INNER JOIN PersonasFJ pfj ON pfj.NUMERODOCUMENTO = a.DOCUMENTO AND pfj.TIPODOCUMENTO = a.TIPO_DOCUMENTO
  AND a.F_INICIO = (SELECT MAX(F_INICIO) 
...
SELECT ''0311'' AS EMPRESA,
	...
	ISNULL((SELECT m.IngresoProyectado FROM ITF_MTO_X_MONOTRIBUTISTA m (NOLOCK) WHERE m.Categoria = afip.MONOTRIBUTO_CATEGORIA
	  AND m.FechaVigenciaDesde = (SELECT MAX(mn.FechaVigenciaDesde) FROM ITF_MTO_X_MONOTRIBUTISTA mn (NOLOCK)
	...
	@Periodo AS PERIODO
FROM PersonasFJ pfj
  INNER JOIN ITF_AFIP_PUCA afip ON afip.CUIT = pfj.NUMERODOCUMENTO
) Z
WHERE Z.MONTO > 0
