...
BEGIN
	...
	SELECT A.ORD_NRO_DOC, 
		   A.BEN_BANCO,
	   		...
	   		(CASE WHEN (A.ORD_TIPO_DOC NOT IN (SELECT TIPODOCUMENTO FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA='F') AND OP_TIPO<>11) or OP_TIPO=8  THEN SUBSTRING(CAST(A.ORD_NRO_DOC AS VARCHAR(11)),1,10) ELSE REPLICATE('0',10) END) AS CL_IDENTIFICACION_EMP_IND,
			(CASE WHEN (A.ORD_TIPO_DOC NOT IN (SELECT TIPODOCUMENTO FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA='F')AND OP_TIPO<>11) or OP_TIPO=8  THEN SUBSTRING(CAST(A.ORD_NRO_DOC AS VARCHAR(11)),11,1) ELSE '0' END) AS CL_DIGITO_VERIFICADOR,
			LEFT((CASE WHEN (A.ORD_TIPO_DOC NOT IN (SELECT TIPODOCUMENTO FROM CLI_TIPOSDOCUMENTOS WHERE TIPOPERSONA='F')AND OP_TIPO<>11) or OP_TIPO=8 THEN ORD_NOMBRE ELSE 'PARTICULARES' END) + REPLICATE(' ',16),16) AS CL_NOMBRE_EMPRESA_INDIVIDUO, 	   		
