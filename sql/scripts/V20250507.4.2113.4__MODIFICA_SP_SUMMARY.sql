...
EXEC('
CREATE OR ALTER PROCEDURE dbo.SP_TP_TLF_SUMMARY  
...
BEGIN
    DECLARE @FECHACAPTURA VARCHAR(12),
            @FECHACAPTURA2 VARCHAR(12),
            @RETORNO_CON_REGISTRO NUMERIC(12)
    ...
    SELECT 
        @FECHACAPTURA = MAX(SUBSTRING(ITF_TLF_POS.LKPOST, 3, 2) + SUBSTRING(ITF_TLF_POS.LKPOST, 5, 2)),
    ...
    INSERT INTO dbo.TJD_TLF_SUMMARY(
        TIPOMENSAJE, CODIGOPROCESAMIENTO, FECHACAPTURA, FECHATRANSACCION, HORATRANSACCION,
        CODIGORESPUESTA, NUMEROTARJETA, NUMEROTRANSACCION, NUMEROAUTORIZACION, IDENTIFICACIONCAJERO,
        IMPORTE, IMPORTEDISPENSADO, FROMACCOUNT, TOACCOUNT, MONEDA, CONCILIACION,
        IMPORTETLF, CONCEPTO_DEBIN, CONCEPTO_TRANSF_INM, ELEMENT43, ELEMENT62, ELEMENT120
    )
    SELECT DISTINCT 
        ...
        CASE 
            WHEN LKTCDE + LKTFRO + LKTTO IN (''711100'',''710100'') AND ITF_TLF_POS.LKORIG = 840 
            THEN REPLACE(CONVERT(VARCHAR(15), CAST(ROUND(ITF_TLF_POS.LKAMT1 * ITF_TLF_POS.LKTIPV, 2) AS DECIMAL(15,2)), 1),''.'','''')
            ELSE REPLACE(CONVERT(VARCHAR(15), ITF_TLF_POS.LKAMT1), ''.'', '''') 
        END AS IMPORTE,
        ...
        CASE 
            WHEN LKTCDE + LKTFRO + LKTTO IN (''711100'',''710100'') AND ITF_TLF_POS.LKORIG = 840 
            THEN (ITF_TLF_POS.LKTIPV * ITF_TLF_POS.LKAMT1) 
            ELSE ITF_TLF_POS.LKAMT1 
        END AS IMPORTETLF,
        SUBSTRING(ITF_TLF_POS.LKFI15, 36, 3) AS CONCEPTO_DEBIN,
        SUBSTRING(ITF_TLF_POS.LKFI15, 4, 3) AS CONCEPTO_TRANSF_INM,
        ITF_TLF_POS.LKFI10 + LKTERC AS ELEMENT43,
        ITF_TLF_POS.LKTERT AS ELEMENT62,
        ITF_TLF_POS.LKNOMB AS ELEMENT120
    FROM dbo.ITF_TLF_POS WITH (NOLOCK) 
    INNER JOIN TLF_COD_TRANSACCIONES WITH (NOLOCK) 
        ON COD_MSJ_TLF = LKTCDE + LKTFRO + LKTTO
    WHERE 
        ITF_TLF_POS.LKTYP IN (''210'', ''0220'', ''0420'') 
        AND ITF_TLF_POS.LKRECT = ''01''
        AND ITF_TLF_POS.LKPOST = @FECHACAPTURA2;
