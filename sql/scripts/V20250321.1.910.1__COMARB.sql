Execute('CREATE OR ALTER  PROCEDURE [dbo].[SP_COMARB_PADRON]  
	@usuario varchar(50),
	@ARCHIVO VARCHAR(50) 
AS
...
SELECT TOP 1 @fec_ini = DATEADD(MONTH, -1, CAST(SUBSTRING(PERIODO_APLICACION, 1, 4) + SUBSTRING(PERIODO_APLICACION, 5, 2) + ''01'' AS DATE)),
       @fec_fin = DATEADD(MONTH, -1, EOMONTH(CAST(SUBSTRING(PERIODO_APLICACION, 1, 4) + SUBSTRING(PERIODO_APLICACION, 5, 2) + ''01'' AS DATE)))
FROM ITF_COMARB_PADRON_IMPUESTOS_AUX;
	...
	FROM 
			(SELECT  DISTINCT f.NUMEROPERSONAFJ AS numeroPersona
								, c.cuit
								, C.letra_alicuota
								, CAST(SUBSTRING(PERIODO_APLICACION, 1, 4) + SUBSTRING(PERIODO_APLICACION, 5, 2) + ''01'' AS DATE) AS BOM
								, EOMONTH(CAST(SUBSTRING(PERIODO_APLICACION, 1, 4) + SUBSTRING(PERIODO_APLICACION, 5, 2) + ''01'' AS DATE)) AS EOM
