...
SELECT 
	...
	CASE WHEN PP.TZ_LOCK <> 0 THEN ISNULL(PPO.CAPITAL,0) -- Cuota adelantada
		 WHEN P.HP_JTS_OID IS NOT NULL AND P.FECHAVALOR < PP.C2302 AND (PP.C2309+PP.C2310)>0 THEN ISNULL(P.CAPITALPAGADO+P.CAPITALADELANTADO,0)
		 WHEN P.HP_JTS_OID IS NOT NULL AND PP.C2302 <= PP.C2302_ORIGINAL AND (PP.C2309+PP.C2310)=0 THEN ISNULL(P.CAPITALPAGADO+P.CAPITALADELANTADO,0)
		 ...
		 WHEN P.HP_JTS_OID IS NOT NULL AND ISNULL(P.SALDO_CAPITAL_ANTES,0) = 0 THEN 0 -- Cuota sin Capital
...
INNER JOIN (SELECT SALDOS_JTS_OID, HP_JTS_OID, CUOTA,max(FECHAVALOR) AS  FECHAVALOR, max(NROASIENTOMOV) AS NROASIENTOMOV , SUM(CAPITALPAGADO) AS CAPITALPAGADO , SUM(INTERESPAGADO) AS INTERESPAGADO ,
	    SUM(CAPITALADELANTADO) AS CAPITALADELANTADO , SUM(MORAPAGADA) AS MORAPAGADA , SUM(INTERES_COMP_MORA) AS INTERES_COMP_MORA,
