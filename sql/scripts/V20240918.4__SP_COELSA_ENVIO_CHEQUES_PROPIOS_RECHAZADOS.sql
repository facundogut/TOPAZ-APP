...
PRINT @SumaImportes
			...
			END
	   		...
	   		SET @FL_SUMA_TOTAL_DEBITO_LOTE =  RIGHT(concat(replicate('0', 16), (replace((SELECT sum(convert(bigint, substring(linea,61,16))) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '6%' AND LINEA NOT LIKE '622%' AND id>(SELECT max(id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '5%')), '.', ''))), 12); 
	   		SET @FL_SUMA_TOTAL_CREDITO_LOTE = RIGHT(concat(replicate('0', 16), (replace((SELECT sum(convert(bigint, substring(linea,61,16))) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '622%' AND id>(SELECT max(id) FROM ITF_ENVIO_CHEQUES_PROPIOS_RECHAZADOS_AUX WHERE LINEA LIKE '5%')), '.', ''))), 12); 
