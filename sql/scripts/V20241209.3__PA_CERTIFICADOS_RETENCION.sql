...
BEGIN
	...
	BEGIN
		...
		SELECT @FECHA_ANTERIOR = dbo.diaHabil(@FECHA-1, ''A'');
		...
		WITH DET_CONTABILIDAD_ORDENADO AS (
			...
			WHERE
			    GD.MOV_FECHA_PROCESO = @FECHA 
				AND ((GD.TZ_LOCK < 100000000000000 OR GD.TZ_LOCK >= 200000000000000)
    ...
    INSERT INTO CON_CERTIFICADOS_RETENCION
      	SELECT
		    ...
		    CASE WHEN D.MOV_MONEDA <>1 AND MON.C6403 <> ''I'' AND M.moneda NOT IN (SELECT C6399 FROM MONEDAS WITH(nolock)
																			     WHERE TZ_LOCK= 0 
																				   AND C6403 =''I'')
		    THEN
		    ...
		    WHEN D.MOV_MONEDA =1 AND MON.C6403 = ''N'' AND M.moneda IN (SELECT C6399 FROM MONEDAS WITH(nolock)
																			     WHERE TZ_LOCK= 0 
																				   AND C6403 =''I'')
		    THEN
		    round((	SELECT SUM(CAPITALREALIZADO)
		    	FROM MOVIMIENTOS_CONTABLES WITH(nolock)
		    	WHERE
		    		ASIENTO = D.MOV_ASIENTO
		    		AND SUCURSAL = D.MOV_SUCURSAL
		   ...
		   ELSE  
		    ...
		    CASE WHEN D.MOV_MONEDA <>1 AND MON.C6403 <> ''I'' THEN FL.MONTO_IMPONIBLE *CB.TC_VENTA 
		         ...
		         WHEN D.MOV_MONEDA =1 AND MON.C6403  = ''N'' AND M.moneda IN (SELECT C6399 FROM MONEDAS WITH(nolock) WHERE TZ_LOCK= 0 AND C6403 =''I'')
		              THEN round(FL.MONTO_IMPONIBLE,2)
		    ...
		    CASE WHEN D.MOV_MONEDA <>1 AND MON.C6403 <> ''I'' THEN FL.MONTO_FACTURABLE*CB.TC_VENTA 
		         WHEN (D.MOV_MONEDA <>1 AND MON.C6403  = ''I'') 
		          ...
		          WHEN D.MOV_MONEDA =1 AND MON.C6403  = ''N'' AND M.moneda IN (SELECT C6399 FROM MONEDAS WITH(nolock) WHERE TZ_LOCK= 0 AND C6403 =''I'')
		              THEN round(FL.MONTO_FACTURABLE,2)
		...
		INNER JOIN HISTORICOTIPOSCAMBIO	HTC WITH(nolock) ON HTC.Moneda = m.MONEDA
		                                     AND HTC.TZ_LOCK = 0 
		                                     AND HTC.Fecha_Cotizacion = CASE WHEN (SELECT Fecha_Cotizacion FROM HISTORICOTIPOSCAMBIO HTC2 WITH(nolock)
		                                                                        WHERE HTC2.TZ_LOCK = 0 AND HTC2.Moneda = m.moneda
		                                                                        AND HTC2.Fecha_Cotizacion = (@FECHA_ANTERIOR)) IS NULL
		                                                                 THEN (SELECT Fecha_Cotizacion FROM CON_COTIZACIONES_BNA HTC2 WITH(nolock)
		                                                                        WHERE HTC2.TZ_LOCK = 0 AND HTC2.Moneda = m.moneda
		                                                                        AND HTC2.Fecha_Cotizacion = (SELECT dbo.diaHabil(max(Fecha_Cotizacion), ''A'') 
		                                                                                                      FROM CON_COTIZACIONES_BNA WITH(nolock) WHERE TZ_LOCK = 0 AND Moneda = m.moneda
		                                                                                                      AND Fecha_Cotizacion<(@FECHA)))
		                                                                 ELSE
		                                                                      (@FECHA_ANTERIOR)
		                                                                 END
		...
		LEFT JOIN CON_COTIZACIONES_BNA	CB WITH(nolock) ON CB.Moneda = m.MONEDA
		                                     AND cb.TZ_LOCK = 0 
		                                     AND CB.Fecha_Cotizacion = CASE WHEN (SELECT Fecha_Cotizacion FROM CON_COTIZACIONES_BNA CB2 WITH(nolock)
		                                                                        WHERE CB2.TZ_LOCK = 0 AND CB2.Moneda = m.moneda
		                                                                        AND CB2.Fecha_Cotizacion = (@FECHA_ANTERIOR)) IS NULL
		                                                                 THEN (SELECT Fecha_Cotizacion FROM CON_COTIZACIONES_BNA CB2 WITH(nolock)
		                                                                        WHERE CB2.TZ_LOCK = 0 AND CB2.Moneda = m.moneda
		...
		LEFT JOIN CON_CERTIFICADOS_RETENCION AS C WITH(nolock) ON
			...
			AND C.NRO_CERTIFICADO = (	SELECT TOP 1 NRO_CERTIFICADO
										FROM CON_CERTIFICADOS_RETENCION WITH(nolock)
										WHERE
