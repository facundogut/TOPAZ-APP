...
BEGIN
    ...
    INSERT INTO CON_CERTIFICADOS_RETENCION
      	SELECT
		    ...
		    WHEN (D.MOV_MONEDA =1 AND D.MOV_MONEDA <> m.moneda AND M.moneda IN (SELECT C6399 FROM MONEDAS
																			     WHERE TZ_LOCK= 0 
																				   AND C6403 =''I'') )
		    ...
		    CASE WHEN D.MOV_MONEDA <>1 AND MON.C6403 <> ''I'' THEN FL.MONTO_IMPONIBLE *CB.TC_VENTA 
		         WHEN (D.MOV_MONEDA <>1 AND MON.C6403  = ''I'') OR (D.MOV_MONEDA =1 AND D.MOV_MONEDA <> m.moneda AND M.moneda IN (SELECT C6399 FROM MONEDAS
																															     WHERE TZ_LOCK= 0 
																																   AND C6403 =''I''))
