EXECUTE('
CREATE OR ALTER PROCEDURE  dbo.SP_ITF_TNG_AJUSTE_CONTABLE 
AS 
BEGIN 
...
FROM ITF_ASIENTOS_EXTERNOS (nolock) WHERE ID=@idCount;
	...
	SELECT
		(SELECT MAX(JTS_OID) FROM BANDEJA_ASIENTOS_IN WHERE ESTADO=''S'' AND ORIGEN=''TNA''),
		s.SUCURSAL,
		s.PRODUCTO,
		s.CUENTA,
		s.MONEDA,
		...
		CONCAT(''1.18.8 '',a.CONCEPTO_ASIENTO),
	...
	FROM ITF_ASIENTOS_EXTERNOS a INNER JOIN SALDOS s ON s.JTS_OID = (SELECT TOP 1 JTS_OID FROM SALDOS WHERE C1730=(CONCAT(a.CAPITULO_CONT,a.RUBRO_CONT,a.MONEDA_CONT,a.CUENTA_CONT,a.SUCUENTA_CONT)) AND MONEDA=a.MONEDA AND SUCURSAL=(SELECT NUMERICO_1 FROM ITF_MASTER_PARAMETROS WHERE CODIGO=530))
