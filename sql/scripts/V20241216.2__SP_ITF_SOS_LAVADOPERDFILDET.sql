EXECUTE('
CREATE PROCEDURE [dbo].[SP_ITF_SOS_LAVADOPERDFILDET]
AS
...
SELECT ''0311'' AS EMPRESA,
	...
	@UltimoDiaFechaDesde AS FECHAVIGENCIAHASTA,
	'''' AS OBSERVACIONES,
...
SELECT ''0311'' AS EMPRESA,
	...
	(SELECT m.IngresoProyectado FROM ITF_MTO_X_MONOTRIBUTISTA m (NOLOCK) WHERE m.Categoria = pfj.MONOTRIBUTO
	  AND m.FechaVigenciaDesde = (SELECT mn.FechaVigenciaDesde FROM ITF_MTO_X_MONOTRIBUTISTA mn (NOLOCK)
	  						      WHERE mn.Categoria = m.Categoria 
	  						        AND CONVERT(VARCHAR,mn.FechaVigenciaDesde,112) < @FechaHasta)) AS MONTO,
