...
BEGIN
	...
	DECLARE @CL_FECHA_VENCIMIENTO VARCHAR(6) =convert(VARCHAR,(SELECT fechaproximoproceso FROM parametros), 12);
	DECLARE @CL_FECHA_COMPENSACION VARCHAR(6)=convert(VARCHAR,(SELECT dbo.AgregarDiasHabiles(fechaproximoproceso,1) FROM parametros), 12);
...
--		END
		...
		IF(@Excedente=0 OR @Excedente<99999999)
   		BEGIN
			...
			SET @RA_INFO_ADICIONAL = LEFT(CONCAT((SELECT TOP 1 dbo.RemoveAccents(upper(descripcion)) FROM SNP_MOTIVOS_RECHAZO WHERE FUNCIONALIDAD='DD'AND CODIGO_NACHA=@CODIGO_RECHAZO),REPLICATE(' ',44)),44);
