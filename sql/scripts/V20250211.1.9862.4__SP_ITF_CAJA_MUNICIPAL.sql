...
BEGIN
 ...
 IF @formato = 'CM' AND @hayError = 0
	BEGIN
		...
		SET @TOT_IMPORTE = ( SELECT sum(NRO_02) FROM REC_EXT_RECAUDOS_CANAL_AUX WHERE FORMATO= @formato AND FECHA= @fch_proceso );
		SET @FechaCarga = (SELECT fechaproceso FROM PARAMETROS);
		...
		WHILE @@fetch_status = 0 
			BEGIN
				...
				(ID_CABEZAL, ID_LINEA, MONEDA, IMPORTE, CODIGO_BARRAS, CODIGO_BARRAS_RENDIDO, ESTADO, DETALLE_ESTADO, 
				TOTAL_CARGO_ESPECIFICO, TZ_LOCK, FECHA_COBRANZA, COMPROBANTE)
				VALUES (@ID_CABEZAL, @ID_LINEA, CAST(@CADENA_05 AS NUMERIC(4,0)), @NRO_02,  @CADENA_08, '', 'I', 'Ingresado', 
