	...
	**********************************/
	...
	FROM MOVIMIENTOS_CONTABLES MOV WITH (NOLOCK)
	  ...
	  INNER JOIN SALDOS SA WITH (NOLOCK) ON SA.C1785 IN (''5'',''8'') AND mov.SUCURSAL_CUENTA =SA.sucursal 
	    AND SA.OPERACION = (CASE WHEN CHARINDEX('':'', MOV.CONCEPTO) > 0 THEN CONVERT(NUMERIC(12),SUBSTRING(MOV.CONCEPTO, CHARINDEX('':'', MOV.CONCEPTO)+2, 12)) ELSE MOV.OPERACION_CUENTA END)
