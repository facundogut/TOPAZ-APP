...
-- 		    	    );
	...
	BEGIN TRY
        -- ¿Existe al menos un movimiento no conciliado?
        IF EXISTS ( SELECT 1 FROM TJD_TLF_SUMMARY s INNER JOIN TLF_CONCILIACION_CABECERA c
            ON  s.FECHATRANSACCION = c.LKTRAD 
            AND s.HORATRANSACCION = c.LKTRAT 
            AND s.NUMEROTRANSACCION = c.LKSEQN 
            AND s.IDENTIFICACIONCAJERO = c.LKTERM 
            AND RIGHT(s.FROMACCOUNT, 19) = c.LKFROM 
            AND RIGHT(s.TOACCOUNT, 19) = c.LKTOAC
            where 
                -- ARCHIVO=''E0321'' and ¿Es necesario filtrar por nombre de archivo?
                c.FECHA_PROCESO = (SELECT FECHAPROCESO FROM PARAMETROS)
                AND ESTADO_CONCILIACION not in (''OK'', ''CONCILIADO'')
        )
        BEGIN
            -- Marcar movimiento no conciliado en tabla summary
            UPDATE s
            SET s.CONCILIACION=''N''
            FROM TJD_TLF_SUMMARY s 
            INNER JOIN TLF_CONCILIACION_CABECERA c
                ON  s.FECHATRANSACCION = c.LKTRAD --@ELEMENT13
                AND s.HORATRANSACCION = c.LKTRAT --@ELEMENT12
                AND s.NUMEROTRANSACCION = c.LKSEQN --@ELEMENT37
                AND s.IDENTIFICACIONCAJERO = c.LKTERM --@ELEMENT41;
                AND RIGHT(s.FROMACCOUNT, 19) = c.LKFROM 
                AND RIGHT(s.TOACCOUNT, 19) = c.LKTOAC
                AND NOT (c.LKRVSL in (''20'') AND substring(cod_msj_tlf, 1, 2)=''10'')
                AND s.CODIGOPROCESAMIENTO = c.COD_MSJ_H2H
                AND c.FECHA_PROCESO = (SELECT FECHAPROCESO FROM PARAMETROS)
                AND ESTADO_CONCILIACION not in (''OK'', ''CONCILIADO'');
            
            SET @P_RET_PROCESO = 1 --OK
		    SET @P_MSG_PROCESO = ''El proceso que identifica movimientos no conciliados, ha finalizado correctamente. '';
        END
        ELSE 
        BEGIN
            SET @P_RET_PROCESO = 1 --OK
		    SET @P_MSG_PROCESO = ''No se encontraron movimientos no conciliados.'';
        END 	    
