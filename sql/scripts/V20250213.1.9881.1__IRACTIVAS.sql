...
IF (SELECT DB_NAME()) = @BD
  BEGIN
	...
	if exists (
			...
			select  ''Tabla CRE_GARANTIADOCUMENTOS no se ajusta a la integridad referencial con CRE_TIPOSGARANTIAS '',*
				from CRE_GARANTIADOCUMENTOS b with(nolock)
				where b.tpo_garantia not in ( 
											select a.tipo_garantia 
											from CRE_TIPOSGARANTIAS a with(nolock) )
	ELSE 
			print ''Comprobación OK. Tabla CRE_GARANTIADOCUMENTOS con CRE_TIPOSGARANTIAS ''
	...
	if exists (
				...
				where b.COD_ACTIVIDAD not in ( 
											select a.CODIGO_ACT_AFIP 
											from CLI_Cod_Act_AFIP a with(nolock) )
				)
			select ''Tabla CRE_PROD_ACTIVIDAD no se ajusta a la integridad referencial con CLI_Cod_Act_AFIP '',*
				from CRE_PROD_ACTIVIDAD b with(nolock)
				where b.COD_ACTIVIDAD not in ( 
											select a.CODIGO_ACT_AFIP 
											from CLI_Cod_Act_AFIP a with(nolock) )
	ELSE 
			print ''Comprobación OK. Tabla CRE_PROD_ACTIVIDAD con CLI_Cod_Act_AFIP ''
	if exists (
	...
	if exists (
				...
				where c1803 <> 0 and b.c1803 not in ( 
											select a.CODIGOCLIENTE 
											from CLI_CLIENTES a with(nolock) )
				)
	...
	if exists (
			...
			select ''Tabla CRE_PROD_CONVENIOS no se ajusta a la integridad referencial con CONV_CONVENIOS_PAG '',*
				from CRE_PROD_CONVENIOS b with(nolock)
				where b.TIPO_CONVENIO not in ( 
											select a.ID_ConvPago
											from CONV_CONVENIOS_PAG a with(nolock) )
	ELSE 
			print ''Comprobación OK. Tabla CRE_PROD_CONVENIOS con CONV_CONVENIOS_PAG ''
	if exists (
				select b.Id_TpoConv 
	...
	if exists (
				select b.CODIGO_ENTE 
				from CRE_SOL_DEUDA_EXTERNA b with(nolock)
				where b.CODIGO_ENTE not in ( 
											select a.codigo
											from CRE_ENTIDADEXTERNA a with(nolock) )
				)
				select ''Tabla CRE_SOL_DEUDA_EXTERNA no se ajusta a la integridad referencial con CRE_ENTIDADEXTERNA '',*
				from CRE_SOL_DEUDA_EXTERNA b with(nolock)
				where b.CODIGO_ENTE not in ( 
											select a.codigo
											from CRE_ENTIDADEXTERNA a with(nolock) )
	ELSE 
			print ''Comprobación OK. CRE_SOL_DEUDA_EXTERNA  con CRE_ENTIDADEXTERNA ''
	if exists (
				select b.saldos_jts_oid 
				from GASTOS_POR_CUOTA b with(nolock)
				where b.saldos_jts_oid not in ( 
											select a.jts_oid
											from SALDOS a with(nolock) )
				)
				select ''Tabla GASTOS_POR_CUOTA no se ajusta a la integridad referencial con SALDOS '',*
				from GASTOS_POR_CUOTA b with(nolock)
				where b.saldos_jts_oid not in ( 
											select a.jts_oid
											from SALDOS a with(nolock) )
	ELSE 
			print ''Comprobación OK. GASTOS_POR_CUOTA con SALDOS ''
	if exists (
				select b.PRODUCTO 
				from CRE_LIMITE_GARANTIA b with(nolock)
				where b.PRODUCTO not in ( 
				...
				select ''Tabla CRE_LIMITE_GARANTIA no se ajusta a la integridad referencial con PRODUCTOS '',*
				from CRE_LIMITE_GARANTIA b with(nolock)
				where b.PRODUCTO not in ( 
											select a.C6250
											from PRODUCTOS a with(nolock) )
	ELSE 
			print ''Comprobación OK. CRE_LIMITE_GARANTIA con PRODUCTOS ''
	if exists (
				select b.banco_girado 
				from CRE_REL_CAB_DOCUMENTOS b with(nolock)
				where b.banco_girado not in ( 
											select a.CODIGOBC
											from CLI_BANCOSPLAZA a with(nolock) )
				)
				select ''Tabla CRE_REL_CAB_DOCUMENTOS no se ajusta a la integridad referencial con CLI_BANCOSPLAZA '',*
				from CRE_REL_CAB_DOCUMENTOS b with(nolock)
				where b.banco_girado not in ( 
											select a.CODIGOBC
											from CLI_BANCOSPLAZA a with(nolock) )
	ELSE 
			print ''Comprobación OK. CRE_REL_CAB_DOCUMENTOS con CLI_BANCOSPLAZA ''
	if exists (
				select b.PRODUCTO 
				from CRE_PRODUCTOLIC b with(nolock)
				where b.PRODUCTO not in ( 
											select a.C6250
											from PRODUCTOS a with(nolock) )
				)
				select ''Tabla CRE_PRODUCTOLIC no se ajusta a la integridad referencial con PRODUCTOS '',*
				from CRE_PRODUCTOLIC b with(nolock)
				where b.PRODUCTO not in ( 
											select a.C6250
											from PRODUCTOS a with(nolock) )
	ELSE 
			print ''Comprobación OK. CRE_PRODUCTOLIC con PRODUCTOS ''
	if exists (
				select b.FAMILIAPROD 
				from CRE_FAMILIAS b with(nolock)
				where b.FAMILIAPROD not in ( 
											select a.C6283
											from PRODUCTOS a with(nolock) )
				)
				select ''Tabla CRE_FAMILIAS no se ajusta a la integridad referencial con PRODUCTOS '',*
				from CRE_FAMILIAS b with(nolock)
				where b.FAMILIAPROD not in ( 
											select a.C6283
											from PRODUCTOS a with(nolock) )
	ELSE 
			print ''Comprobación OK. CRE_FAMILIAS con PRODUCTOS ''
	if exists (
				select b.SUCURSAL 
				...
				select ''Tabla ASIENTOS no se ajusta a la integridad referencial con SUCURSALES '',*
				from ASIENTOS b with(nolock)
				where b.SUCURSAL not in ( 
											select a.SUCURSAL
											from SUCURSALES a with(nolock) )
	ELSE 
			print ''Comprobación OK. ASIENTOS con SUCURSALES ''
	if exists (
				select b.ASIENTO 
				...
				where b.ASIENTO not in ( 
											...
											from ASIENTOS a with(nolock) )
	ELSE 
			print ''Comprobación OK. MOVIMIENTOS_CONTABLES con ASIENTOS ''
	if exists (
				select b.NUMERO_DOCUMENTO 
				from CRE_LIBRADORES_DOCS b with(nolock)
				left join CHE_CUENTAS_CHEQUES a with(nolock) on b.NUMERO_DOCUMENTO = a.NUMERO_DOCUMENTO
												and b.TIPO_DOCUMENTO = a.TIPO_DOCUMENTO
				WHERE	a.NUMERO_DOCUMENTO IS NULL
				AND		a.TIPO_DOCUMENTO IS NULL
				)
				select ''Tabla CRE_LIBRADORES_DOCS no se ajusta a la integridad referencial con CHE_CUENTAS_CHEQUES '',*
				from CRE_LIBRADORES_DOCS b with(nolock)
				left join CHE_CUENTAS_CHEQUES a with(nolock) on b.NUMERO_DOCUMENTO = a.NUMERO_DOCUMENTO
												and b.TIPO_DOCUMENTO = a.TIPO_DOCUMENTO
				WHERE	a.NUMERO_DOCUMENTO IS NULL
				AND		a.TIPO_DOCUMENTO IS NULL
	ELSE 
			print ''Comprobación OK. CRE_LIBRADORES_DOCS con CHE_CUENTAS_CHEQUES ''
	if exists (	select cuenta 
				...
				where a.cuenta not in (select b.C6301 
										from PLANCTAS b with(nolock))
	ELSE 
			print ''Comprobación OK. SALDOS con PLANCTAS ''
	END
ELSE
