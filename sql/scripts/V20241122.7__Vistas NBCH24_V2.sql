...
IF EXISTS (SELECT name 
           FROM sys.indexes 
           WHERE name = ''IX_SALDOS_NBCH2401'' 
             AND object_id = OBJECT_ID(''dbo.SALDOS''))
BEGIN
    DROP INDEX IX_SALDOS_NBCH2401 ON dbo.SALDOS;
END;

IF EXISTS (SELECT name 
           FROM sys.indexes 
           WHERE name = ''IX_GRL_ESTADOS_DE_CUENTA_NBCH2401'' 
             AND object_id = OBJECT_ID(''dbo.GRL_ESTADOS_DE_CUENTA''))
BEGIN
    DROP INDEX IX_GRL_ESTADOS_DE_CUENTA_NBCH2401 ON dbo.GRL_ESTADOS_DE_CUENTA;
END;
');

execute('
CREATE NONCLUSTERED INDEX IX_PYF_APODERADOS_NBCH2401 ON [dbo].[PYF_APODERADOS] ([TZ_LOCK],[TIPO_ENTIDAD],[TIPO_PODER]) INCLUDE ([FECHA_VENCIMIENTO], [FECHA_INI_VIGENCIA]);
CREATE NONCLUSTERED INDEX IX_SALDOS_NBCH2401 ON [dbo].[SALDOS] ([TZ_LOCK], [C1665], [C1785], [C1604]) INCLUDE ([C1601], [C1612], [C1621], [C1627], [C1628], [C1632], [C1642], [C1644], [C1645], [JTS_OID]);
CREATE NONCLUSTERED INDEX IX_GRL_ESTADOS_DE_CUENTA_NBCH2401 ON [dbo].[GRL_ESTADOS_DE_CUENTA] ([TZ_LOCK], [JTSOID], [CORREO_TRADICIONAL]) INCLUDE ([FORMATO_MAIL], [TIPO_MAIL], [ORDINAL_MAIL]);
');

execute('
CREATE OR ALTER VIEW dbo.WV_NBCH24_ACCESOSCV
AS
...
inner join dbo.PYF_APODERADOS pa WITH (NOLOCK) on pa.id_entidad = s.jts_oid and pa.tipo_Entidad = 2 
...
inner join WV_NBCH24_ACCESOSCV acv on s.C1665 = acv.jts_oid
...
AS
	...
	em.EMAIL email 
	from  saldos s WITH (NOLOCK)
	inner join WV_NBCH24_ACCESOSCV acv on s.jts_oid = acv.jts_oid
...
inner join dbo.WV_NBCH24_ACCESOSCV acv  WITH (NOLOCK)  on acv.jts_oid = (case when cvt.JTS_OID_CTA_DESTINO  is not null then cvt.JTS_OID_CTA_DESTINO   else  s.c1665 end) 
WHERE s.C1785=4  --dpf
...
execute('
CREATE OR ALTER VIEW dbo.VW_NBCH24_CLI_PERSONA (idpersona, tipo, doc, apellido, nombre, razonSocial, fecha, nacionalidad, estadocivil, sexo, fechaAlta, estado, estadoDesc, motivoCancelacion, motivoCancelacionDesc)
AS
	select 
...
execute('
CREATE OR ALTER VIEW dbo.VW_NBCH24_CRE_PLAZO_GRACIA
AS
SELECT
...
execute('
CREATE OR ALTER VIEW dbo.VW_NBCH24_CTA_TJD(tipo, tarjeta, doc, jts_oid, descripcion, producto, clase, titularidad, primaria, ambito, comprasExterior)
AS
...
execute('
CREATE OR ALTER VIEW dbo.VW_NBCH24_CTA_BLOQUEOS 
AS
...
execute('
CREATE OR ALTER VIEW dbo.VW_NBCH24_CLI_CLIENTE
AS
