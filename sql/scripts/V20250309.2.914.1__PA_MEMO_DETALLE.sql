      ...
      SET @COTIZACION = (SELECT TOP 1 TIPO_CAMBIO_OFICIAL FROM HISTORICOTIPOSCAMBIO WITH(NOLOCK) WHERE MONEDA=2 AND FECHA_COTIZACION <= @FECHA_PROCESO ORDER BY FECHA_COTIZACION DESC)
			...
			UNION ALL
				...
				SELECT DISTINCT
				 ...
				 LEFT JOIN TJC_ADINTAR_DEUDASPLAS TAD WITH(NOLOCK) ON TAD.NRO_CUENTA = TMU.USUARIO AND TAD.ADMINISTRADORA = TMU.ADMINISTRADORA AND TAD.TZ_LOCK = 0 AND TAD.ESTADO = ''A'' AND TAD.CONDICION=''T''
