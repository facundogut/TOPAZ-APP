Execute('CREATE OR ALTER PROCEDURE [dbo].[SP_DD_ENVIO_PRESENTADOS_RECIVIDOS_V2]
   @TICKET NUMERIC(16)
AS 
BEGIN
...
--s	DECLARE @FA_SUMA_TOTAL_DEBITOS VARCHAR(12);
...
--s		IF (@SumaImportes > 9999999999.99 OR @SumaEntidad > 999999) -- 99 millones
		IF (@SumaImportes > 999999999999.99 OR @SumaEntidad > 99999999) -- 99 millones
		BEGIN
...
--s		   	SET @FL_SUMA_TOTAL_DEBITO_LOTE = RIGHT(concat(replicate(''0'', 12), (replace((SELECT sum(convert(bigint, substring(linea,30,10))) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''6%'' AND id>(SELECT max(id) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''5%'')), ''.'', ''''))), 12); 
		   	SET @FL_SUMA_TOTAL_DEBITO_LOTE = RIGHT(concat(replicate(''0'', 20), (replace((SELECT sum(convert(bigint, substring(linea,26,14))) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''6%'' AND id>(SELECT max(id) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''5%'')), ''.'', ''''))), 20); 
...
--s			SET @FA_SUMA_TOTAL_DEBITOS = RIGHT(concat(replicate(''0'', 12), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,30,10)))/100) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''6%'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''1%'') ), ''.'', ''''))), 12); 
			SET @FA_SUMA_TOTAL_DEBITOS = RIGHT(concat(replicate(''0'', 20), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,26,14)))/100) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''6%'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''1%'') ), ''.'', ''''))), 20); 
		...
		BEGIN
			...
			BEGIN
				--SET @FL_TOTALES_DE_CONTROL = concat((RIGHT(concat(replicate(''0'', 6), @SumaEntidad), 6)), (RIGHT(concat(replicate(''0'', 4), @SumaSucursal), 4)));
--			   	SET @FL_SUMA_TOTAL_DEBITO_LOTE = RIGHT(concat(replicate(''0'', 12), (replace(@SumaImportes, ''.'', ''''))), 12); 
--s			   	SET @FL_SUMA_TOTAL_DEBITO_LOTE = RIGHT(concat(replicate(''0'', 12), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,30,10)))/100) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''6%'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''5%'') ), ''.'', ''''))), 12); 
			   	SET @FL_SUMA_TOTAL_DEBITO_LOTE = RIGHT(concat(replicate(''0'', 20), (replace((SELECT convert(NUMERIC(15,2),sum(convert(NUMERIC,substring(linea,26,14)))/100) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''6%'' AND ID>(SELECT max(id) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE LINEA LIKE ''5%'') ), ''.'', ''''))), 20); 

--s			   	SET @FL_SUMA_TOTAL_CREDITO_LOTE = RIGHT(replicate(''0'', 12), 12);
			   	SET @FL_SUMA_TOTAL_CREDITO_LOTE = RIGHT(replicate(''0'', 20), 20);
			   	SET @TotalesDebitos += @SumaImportes;
			   	SET @FL_CANT_REG_INDIVIDUAL_ADICIONAL = RIGHT(concat(replicate(''0'', 6), (SELECT COUNT(*) FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX WHERE (LINEA LIKE ''6%'' or linea like ''7%'') and id>(select max(id) from ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX where linea like ''5%''))), 6); 
...
--s		SET @RI_CUENTA_DEBITAR = RIGHT(concat(replicate(''0'', 17),  right(@T_cbu,14)), 17);
		SET @RI_CUENTA_DEBITAR = RIGHT(concat(replicate(''0'', 14),  right(@T_cbu,14)), 14);
...
--s		IF(@Excedente=0 OR @Excedente<99999999)
		IF(@Excedente=0 OR @Excedente<999999999999)
   		BEGIN
