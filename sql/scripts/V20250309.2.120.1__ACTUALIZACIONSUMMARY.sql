...
BEGIN
   ...
   SELECT DISTINCT 
       ...
       (SELECT H2H 
		...
		CASE 
			WHEN LKTCDE + LKTFRO + LKTTO IN (''711100'',''710100'') AND ITF_TLF_POS.LKORIG = 840 THEN REPLACE(CONVERT(VARCHAR(15), (ITF_TLF_POS.LKAMT1 * ITF_TLF_POS.LKTIPV)), ''.'', '''') 
			ELSE REPLACE(CONVERT(VARCHAR(15), ITF_TLF_POS.LKAMT1), ''.'', '''') 
			END AS IMPORTE,
		NULL AS IMPORTEDISPENSADO,
		CASE 
			WHEN LKTCDE NOT IN (''29'', ''F4'') THEN LKTFRO + LKFROM 
			ELSE LKFROM END AS FROMACCOUNT,
       CASE 
       		WHEN LKTCDE IN (''29'', ''F4'', ''40'') THEN LKTTO + LKTOAC 
       		ELSE LKTOAC END AS TOACCOUNT,
       CASE ITF_TLF_POS.LKORIG 
       		WHEN 32 THEN ''N'' 
       		WHEN 840 THEN ''D'' 
       		ELSE ''N'' END AS MONEDA,
       ''S'' AS CONCILIACION,
       CASE 
			WHEN LKTCDE + LKTFRO + LKTTO IN (''711100'',''710100'') AND ITF_TLF_POS.LKORIG = 840 THEN (ITF_TLF_POS.LKTIPV * ITF_TLF_POS.LKAMT1) 
