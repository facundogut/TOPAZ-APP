...
BEGIN
	...
	DECLARE @CL_FECHA_VENCIMIENTO VARCHAR(6) 
	DECLARE @CL_FECHA_COMPENSACION VARCHAR(6)
...
--		END
		...
		SET @RI_REFERENCIA_UNIVOCA = RIGHT(concat(replicate(' ', 15), @T_REFERENCIA), 15);
		SET @RI_CLIENTE_PAGADOR = RIGHT(concat(replicate(' ', 22), @T_ID_CLIENTE), 22);
