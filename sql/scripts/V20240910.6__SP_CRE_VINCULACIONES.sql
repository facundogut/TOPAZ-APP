...
BEGIN
	...
	SET @P_RESULTADO = NULL
		...
		WHERE 
		    ...
		    AND (
		        ((CVR.PARAMETRO_613 != ''S'' OR CVR.PARAMETRO_613 is NULL) AND CV.FECHA_FIN >= (SELECT fechaproceso FROM PARAMETROS))
		        OR 
		        (CVR.PARAMETRO_613 = ''S'' AND CV.FECHA_FIN >= DATEADD(year, (SELECT importe FROM CRE_PARAMETROS WHERE CODIGO = 613), (SELECT fechaproceso FROM PARAMETROS)))
		    );
