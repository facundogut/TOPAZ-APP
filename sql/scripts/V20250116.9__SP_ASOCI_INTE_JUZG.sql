...
AS 
   ...
   BEGIN
	  ...
	  INNER JOIN PYF_APODERAMIENTO a ON tt.TIPO_PODER = a.CODPODER 
	 ...
	 INNER JOIN @Tabla_ASOCI_INTE_JUZG TA 
      ...
      BEGIN TRANSACTION	
       ...
       BEGIN TRY
				...
				AND TA.JTS_OID_CUENTA NOT IN (SELECT x.JTS_OID_CUENTA FROM DJ_CAUSAS z INNER JOIN DJ_CAUSA_CUENTA x ON z.NRO_CAUSA = x.NRO_CAUSA WHERE ESTADO = ''I'')
				...
				SELECT 
					...
					NULL,
					NULL,
					TRY_CAST(TA.JTS_OID_CUENTA AS VARCHAR(50)),
					TA.JTS_OID_CUENTA
				FROM @Tabla_ASOCI_INTE_JUZG  AS TA
				...
				AND TA.TIPO_ENTIDAD NOT IN (SELECT PA.TIPO_ENTIDAD FROM @Tabla_PYF_APODERADOS AS PA)
				AND TA.ID_PERSONA NOT IN (SELECT PA.ID_PERSONA FROM @Tabla_PYF_APODERADOS AS PA)
				AND TA.JTS_OID_CUENTA IN (SELECT x.JTS_OID_CUENTA FROM DJ_CAUSAS z INNER JOIN DJ_CAUSA_CUENTA x ON z.NRO_CAUSA = x.NRO_CAUSA WHERE ESTADO = ''I'')
				...
				INNER JOIN @Tabla_PYF_APODERADOS AS T ON
					...
					AND PA.ID_ENTIDAD IN (SELECT TRY_CAST(x.JTS_OID_CUENTA AS VARCHAR(50)) FROM DJ_CAUSAS z INNER JOIN DJ_CAUSA_CUENTA x ON z.NRO_CAUSA = x.NRO_CAUSA WHERE ESTADO = ''A'')
				
					
				/*SE ACTUALIZAN PODERES EXISTENTES VENCIDOS*/
				UPDATE PA
				SET 
					PA.FECHA_VENCIMIENTO = @FECHA_ANTERIOR,
					PA.FECHA_INI_SUSPENSION = @P_FECHA_VENCIMIENTO, --@P_FECHA_VENCIMIENTO,
					PA.FECHA_INI_VIGENCIA = @FECHA, --@FECHA,
					PA.ID_CLIENTE_SALDO = T.ID_ENTIDAD
				FROM
					PYF_APODERADOS AS PA WITH (NOLOCK)
				INNER JOIN @Tabla_PYF_APODERADOS AS T ON
					PA.ID_PERSONA = T.ID_PERSONA
					...
					AND PA.ID_ENTIDAD = T.ID_ENTIDAD
					AND PA.TIPO_PODER = T.TIPO_PODER
					AND PA.ID_ENTIDAD IN (SELECT TRY_CAST(x.JTS_OID_CUENTA AS VARCHAR(50)) FROM DJ_CAUSAS z INNER JOIN DJ_CAUSA_CUENTA x ON z.NRO_CAUSA = x.NRO_CAUSA WHERE ESTADO = ''I'')
			...
			SET @P_RET_PROCESO = 1
		    SET @P_MSG_PROCESO = ''Se asoció nuevo integrante correctamente.''
