     ...
     ---DECLARE @FECHA_PROCESO DATETIME 
      ...
      DELETE FROM MEMO_CABECERA WHERE Fecha_Info = @FECHA_PROCESO
               ...
               FROM MEMO_DETALLE WITH(NOLOCK) WHERE Fecha_Info = @FECHA_PROCESO GROUP BY Num_Doc) M ON M.Num_Doc = IC.NUMERODOC
				...
				SUM(CASE WHEN MD.Tipo_Gtia_BCRA = 1 THEN (MD.Saldo_Capital) ELSE 0 END) AS DEUDA_CON_GARANTIA_B,
				SUM(CASE WHEN MD.Tipo_Gtia_BCRA = 2 THEN (MD.Saldo_Capital + (MD.Int_Deven_Cobrar_Cont-MD.Int_Adelantados)) ELSE 0 END) AS DEUDA_CON_GARANTIA_A,
				IC.NUMERODOC
				FROM @INFO_CLIENTES IC
