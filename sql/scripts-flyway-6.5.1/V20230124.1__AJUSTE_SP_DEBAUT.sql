EXECUTE('
CREATE OR ALTER  PROCEDURE [dbo].[PA_REC_VAL_DEBITOSAUTOMATICOS]
...
AS
    ...
    BEGIN TRY
        ...
        IF @cantidad_validas + @cantidad_sin_cuenta + @cantidad_cuentas_cerradas + @cantidad_stoped + @cantidad_bloqueadas + @cantidad_inexistentes> 0
        BEGIN

            IF @cantidad_cuentas_cerradas = 0 AND @cantidad_sin_cuenta = 0 AND @cantidad_stoped = 0 AND @cantidad_bloqueadas = 0 AND @cantidad_inexistentes = 0
            BEGIN
            ...
            BEGIN
                ...
                END
            END
            SET @P_MSG_PROCESO = CONCAT(''Proceso finalizado correctamente, se procesaron ('',@cantidad_validas + @cantidad_sin_cuenta + @cantidad_cuentas_cerradas + @cantidad_stoped,'') registros, válidos ('',@cantidad_validas,''), con error ('',@cantidad_sin_cuenta + @cantidad_cuentas_cerradas + @cantidad_stoped + @cantidad_inexistentes,'')'')
        END
