      ...
      BEGIN TRANSACTION	
       ...
       BEGIN TRY
				...
				INNER JOIN @Tabla_PYF_APODERADOS AS T ON
					...
					AND TRY_CAST(PA.ID_ENTIDAD AS DECIMAL(10,0)) IN (SELECT x.JTS_OID_CUENTA FROM DJ_CAUSAS z INNER JOIN DJ_CAUSA_CUENTA x ON z.NRO_CAUSA = x.NRO_CAUSA WHERE ESTADO = ''A'')
				...
				INNER JOIN @Tabla_PYF_APODERADOS AS T ON
					...
					AND TRY_CAST(PA.ID_ENTIDAD AS DECIMAL(10,0)) = TRY_CAST(T.ID_ENTIDAD AS DECIMAL(10,0))
					AND PA.TIPO_PODER = T.TIPO_PODER
					AND TRY_CAST(PA.ID_ENTIDAD AS DECIMAL(10,0)) IN (SELECT x.JTS_OID_CUENTA FROM DJ_CAUSAS z INNER JOIN DJ_CAUSA_CUENTA x ON z.NRO_CAUSA = x.NRO_CAUSA WHERE ESTADO = ''I'')
