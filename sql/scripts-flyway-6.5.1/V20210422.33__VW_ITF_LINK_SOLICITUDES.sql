...
SELECT 
	...
	LEFT(tj.TIPO_SOLICITUD, 6) AS cod_transaccion,
	...
	LEFT(concat((SELECT DISTINCT NUMERO_CUENTA_PBF FROM TJD_SOLICITUD_CUENTAS_LINK WHERE ID_SOLICITUD = ID_SOLICITUD AND NRO_CUENTA = NRO_CTA_PRIMARIA) , replicate('' '', 19)) , 19) AS nro_cuenta,	
	...
	LEFT(CASE WHEN (SELECT NUMERO FROM CLI_TELEFONOS WHERE TIPO = ''PE'' AND ID = tj.COD_CLIENTE) IS NOT NULL THEN concat((SELECT NUMERO FROM CLI_TELEFONOS WHERE TIPO = ''PE'' AND ID = tj.COD_CLIENTE), replicate('' '', 15)) ELSE replicate(''.'', 15) END ,15) AS telefono,
	...
	dbo.itf_cuentas_LINK(ID_SOLICITUD, 16) AS cuentas,
...
FROM
	...
	TJD_SOLICITUD_LINK tj LEFT JOIN CLI_DIRECCIONES cl ON
	
	tj.COD_CLIENTE=cl.ID AND cl.FORMATO=''CL'' AND cl.TIPODIRECCION=tj.TIPO_DOMICILIO AND cl.ORDINAL_DIR=tj.ORDINAL
	
	left JOIN CLI_LOCALIDADES cloc ON cloc.[CODIGOPAIS]=32 AND cloc.DIM1=cl.PROVINCIA AND cloc.DIM2=cl.DEPARTAMENTO
