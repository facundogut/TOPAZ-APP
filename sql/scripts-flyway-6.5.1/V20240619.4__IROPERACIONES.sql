...
-------------------------------Tarjeta de Debito
	...
	if exists (	select b.canal 
				...
				left join TJD_CANAL_ORIGEN a with(nolock) on a.canal = b.canal
													and a.origen = b.origen							
				where a.canal is null
						and a.origen is null
			)	
				SELECT   ''Tabla TJD_TARJETAS no se ajusta a la integridad referencial con TJD_CANAL_ORIGEN '',*
				from TJD_TARJETAS b with(nolock) 
				left join TJD_CANAL_ORIGEN a with(nolock) on a.canal = b.canal
													and a.origen = b.origen							
				where a.canal is null
	...
	print ''Comprobación OK.Tabla TJD_TARJETAS con TJD_CANAL_ORIGEN ''
	...
	if exists (select b.ID_TARJETA 
				...
				SELECT   ''Tabla BITACORA_TARJETA_DEBITO no se ajusta a la integridad referencial con Tabla TJD_TARJETAS'',*
				from BITACORA_TARJETA_DEBITO b with(nolock) 
				where b.ID_TARJETA not in(select a.ID_TARJETA 
										from TJD_TARJETAS  A with(nolock))
	else
	print ''Comprobación OK.Tabla BITACORA_TARJETA_DEBITO con Tabla TJD_TARJETAS''
...
-------------------------------- CLEARING PROPIOS
	if exists (select b.CODIGO_CAUSAL_DEVOLUCION 
				...
				where b.CODIGO_CAUSAL_DEVOLUCION not in(select a.CODIGO_DE_CAUSAL
...
--------------------------------------OPERACIONES/CONVENIOS DE PAGOS
  	if exists (	SELECT B.Cliente 
				FROM CONV_CONVENIOS_PAG b with (nolock)
				WHERE B.Cliente  NOT IN (SELECT A.CODIGOCLIENTE 
										FROM CLI_CLIENTES a with (nolock)) 
				)	
				SELECT ''Tabla CONV_CONVENIOS_PAG no se ajusta a la integridad referencial con CLI_CLIENTES '',*
				FROM CONV_CONVENIOS_PAG b with (nolock)
				WHERE B.Cliente  NOT IN (SELECT A.CODIGOCLIENTE 
										FROM CLI_CLIENTES a with (nolock)) 
	else
		print ''Comprobación OK.Tabla CONV_CONVENIOS_PAG con CLI_CLIENTES ''
 ...
 if exists (	SELECT B.id_convenio 
				...
				SELECT ''Tabla CONV_CB_ESTRUCTURA no se ajusta a la integridad referencial con CONV_CONVENIOS_PAG '',*
				FROM CONV_CB_ESTRUCTURA b with (nolock)
				WHERE B.id_convenio  NOT IN (SELECT A.ID_CONVPAGO 
										FROM CONV_CONVENIOS_PAG a with (nolock)) 
	else
		print ''Comprobación OK.Tabla CONV_CB_ESTRUCTURA con CONV_CONVENIOS_PAG ''
	...
	if exists (	SELECT B.ID_CONVPAGO 
				...
				WHERE B.ID_CONVPAGO  NOT IN (SELECT A.ID_CONVPAGO
											FROM CONV_CONVENIOS_PAG a with (nolock)) 
				)	
...
----------------------------OPERACIONES/CONVENIOS DE RECAUDACIONES
	if exists (	SELECT b.ID_CODIGO_BARRAS
				FROM CONV_CB_ESTRUCTURA b with (nolock)
				where b.ID_CODIGO_BARRAS not in (	select a.ID_CODIGO_BARRAS 
													from  CONV_CB_CAMPOS a with (nolock)) 
		)	
				SELECT ''Tabla CONV_CB_ESTRUCTURA no se ajusta a la integridad referencial con CONV_CB_CAMPOS '',*
				FROM CONV_CB_ESTRUCTURA b with (nolock)
				where b.ID_CODIGO_BARRAS not in (	select a.ID_CODIGO_BARRAS 
													from  CONV_CB_CAMPOS a with (nolock)) 
	else
		print ''Comprobación OK.Tabla CONV_CB_ESTRUCTURA con CONV_CB_CAMPOS ''
	...
	if exists (	SELECT B.convenio 
				FROM REC_CAB_RECAUDOS_CAJA b with (nolock)
				WHERE B.convenio  NOT IN (SELECT A.ID_CONVrec 
										FROM CONV_CONVENIOS_REC a with (nolock)) 
				)	
				SELECT ''Tabla REC_CAB_RECAUDOS_CAJA no se ajusta a la integridad referencial con CONV_CONVENIOS_REC '',*
				FROM REC_CAB_RECAUDOS_CAJA b with (nolock)
				WHERE B.convenio  NOT IN (SELECT A.ID_CONVrec 
										FROM CONV_CONVENIOS_REC a with (nolock)) 

	else
		print ''Comprobación OK.Tabla REC_CAB_RECAUDOS_CAJA con CONV_CONVENIOS_REC ''
...
--------------------------------------OPERACIONES/TRANSFERENCIAS
	...
	if exists (	SELECT b.op_motivo
				fROM VTA_TRANSFERENCIAS b 
				where b.op_motivo not in ( select a.Id_motivo
									from SNP_MOTIVOS_RECHAZO a with (nolock))
				)	
				SELECT ''Tabla VTA_TRANSFERENCIAS no se ajusta a la integridad referencial con SNP_MOTIVOS_RECHAZO '',*
				fROM VTA_TRANSFERENCIAS b 
				where b.op_motivo not in ( select a.Id_motivo
									from SNP_MOTIVOS_RECHAZO a with (nolock))
	else
		print ''Comprobación OK.Tabla VTA_TRANSFERENCIAS con SNP_MOTIVOS_RECHAZO ''
