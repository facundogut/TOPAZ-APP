EXECUTE('
IF OBJECT_ID (''VW_TABLA_ATM'') IS NOT NULL
	DROP VIEW dbo.VW_TABLA_ATM
')

EXECUTE('
CREATE   VIEW [VW_TABLA_ATM]
AS
...
FROM TABLA_ATM A WITH (nolock)
        ...
        INNER JOIN CAJ_ATM_CATEGORIAS C WITH(nolock) ON C.CATEGORIA = a.CATEGORIA AND C.TZ_LOCK=0
