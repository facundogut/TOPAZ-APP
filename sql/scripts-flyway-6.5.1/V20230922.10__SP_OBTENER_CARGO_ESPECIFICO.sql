...
BEGIN
	...
	BEGIN	
		...
		BEGIN
			...
			0, 	/* TIPO COMPROBANTE */
			CASE WHEN SUBSTRING(CODIGO_BARRAS,4,1) = 0 THEN ''B'' 
				 WHEN SUBSTRING(CODIGO_BARRAS,4,1) = 1 THEN ''A''
				ELSE ''Y'' END,	 /* LETRA */
			...
			SELECT @TOTAL_CARGO_ESPECIFICO = SUM(TOTAL_CARGO_ESPECIFICO) FROM CONV_PADRONES P JOIN @TablaAuxNroFactura A
																		...
																		--AND P.TIPO_COMPROBANTE = A.TIPO_COMPROBANTE     NBCHSEG-4104
		...
		BEGIN
			...
			SELECT @TOTAL_CARGO_ESPECIFICO = SUM(TOTAL_CARGO_ESPECIFICO) FROM CONV_PADRONES P JOIN @TablaAuxNroFactura A
																		...
																		WHERE P.TZ_LOCK = 0
																		AND P.CONVENIO = @COD_ENTE 
																		AND P.TIPO_COMPROBANTE = A.TIPO_COMPROBANTE
																		AND P.LETRA = A.LETRA AND P.PUNTO_VENTA = A.PUNTO_VENTA
			RETURN @TOTAL_CARGO_ESPECIFICO;
		END
	END		
