...
ALTER VIEW VW_PAS_CV_INMOVILIZAR AS
WITH AUX1 AS(
...
--	WHEN PCI.ACTIVO = ''I'' THEN ''I''
	...
	, S.*
FROM
	SALDOS S (nolock) INNER JOIN TOPESPRODUCTO TP (nolock) ON S.PRODUCTO = TP.CODPRODUCTO AND S.MONEDA = TP.MONEDA 
	LEFT JOIN VTA_PARAM_CTAS_INMOV PCI (nolock) ON PCI.TIPO = ''N'' AND PCI.VALOR = S.MONEDA 
	AND PCI.TZ_LOCK = 0
WHERE
	S.C1785 IN (2, 3)
...
SELECT
	A.*
FROM
	AUX1 A (nolock), VTA_PARAM_CTAS_INMOV PCI_MIN (nolock), VTA_PARAM_CTAS_INMOV PCI_MAX (nolock)
WHERE
	PCI_MIN.TIPO = ''M''
...
FROM
	AUX2 A (nolock) 
	LEFT JOIN MOVIMIENTOS_CONTABLES MC (nolock) ON MC.SALDO_JTS_OID = A.JTS_OID
	LEFT JOIN VTA_PARAM_CTAS_INMOV VP_TRANS (nolock) ON VP_TRANS.ACTIVO = ''A'' AND VP_TRANS.TIPO = ''V''
...
FROM
	AUX3 A (nolock),
	PARAMETROS P (nolock),
	VTA_PARAM_CTAS_INMOV VP_DIAS (nolock)
WHERE
...
END EXCLUYE_BLOQUEO,
A.*
FROM AUX4 A (nolock) LEFT JOIN GRL_BLOQUEOS B (nolock) ON A.JTS_OID = B.SALDO_JTS_OID 
LEFT JOIN VTA_PARAM_CTAS_INMOV VP (nolock) ON VP.TIPO = ''B'' AND VP.ACTIVO = ''A'' AND VP.VALOR = B.COD_BLOQUEO
...
A.C1803 AS NRO_CLIENTE,
	...
	A.JTS_OID 
FROM AUX5 A (nolock) 
INNER JOIN CLI_CLIENTES C (nolock) ON A.C1803 = C.CODIGOCLIENTE,
	VTA_PARAM_CTAS_INMOV VP (nolock),
	PARAMETROS P (nolock)

WHERE A.JTS_OID NOT IN (SELECT JTS_OID FROM AUX5 WHERE EXCLUYE_BLOQUEO = ''E'')
	...
	AND A.JTS_OID NOT IN ( SELECT JTS_OID_DEBITO FROM COF_COFRES_CONTRATOS (nolock))
	AND A.JTS_OID NOT IN (SELECT SALDO_JTS_OID FROM TJD_REL_TARJETA_CUENTA (nolock))
	
--excluir o incluir contracuenta
	AND ((VP.ACTIVO = ''A''
	AND 
	((VP.INCLUIR_EXCLUIR = ''E'' AND NOT (A.C1665 = 1 AND A.C1785 IN (VP.VALOR))) OR (VP.INCLUIR_EXCLUIR = ''I'')))
	OR (VP.ACTIVO = ''I''));
