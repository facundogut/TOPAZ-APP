...
VALUES (45040, 1, 1, ''D'', NULL, 1, 1, 0, NULL, ''T'')
...
IF EXISTS 
	...
	BEGIN
		...
		SET VALOR = ISNULL((SELECT max(Id_CesionRec)+1 FROM CONV_CESIONES), 1)
		WHERE DIA = 0 AND MES = 0 AND ANIO = 0 AND SUCURSAL = 0 AND NUMERO = 45040
	END
ELSE
	BEGIN
		INSERT INTO dbo.NUMERATORVALUES(DIA,MES,ANIO,SUCURSAL,NUMERO,VALOR) 
		VALUES (0,0,0,0,45040,ISNULL((SELECT max(Id_CesionRec)+1 FROM CONV_CESIONES), 1))
	END 
')
