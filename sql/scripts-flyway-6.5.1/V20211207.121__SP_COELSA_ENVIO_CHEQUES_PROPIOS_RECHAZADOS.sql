EXECUTE('DROP PROCEDURE IF EXISTS dbo.SP_COELSA_ENVIO_CHEQUES_PROPIOS_RECHAZADOS;')
EXECUTE('
...
DECLARE @FechaPresentacion VARCHAR(6) = FORMAT((SELECT FECHAPROCESO FROM PARAMETROS) , ''yyMMdd'');;
DECLARE @HoraPresentacion VARCHAR(4) = FORMAT((SELECT FECHAPROCESO FROM PARAMETROS), ''hhmm'');;
...
DECLARE @FechaPresentacionCL VARCHAR(6) = FORMAT((SELECT FECHAPROCESO FROM PARAMETROS) , ''yyMMdd'');
DECLARE @FechaVencimientoCL VARCHAR(6) = FORMAT((SELECT FECHAPROXIMOPROCESO FROM PARAMETROS) , ''yyMMdd''); 
...
DECLARE @PrimerMotivoRechazoRA VARCHAR(3) = (concat(''R'', (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 62)));
...
SELECT ID, FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE, FECHA_PRESENTADO, CODIGO_POSTAL, FECHA_VENCIMIENTO, NRO_CHEQUE, PUNTO_INTERCAMBIO, TRACE_NUMBER, ESTADO, TIPO, COD_RECHAZO FROM ITF_COELSA_CHEQUES_RECHAZO WHERE ESTADO = ''P'' AND FECHAPROCESO = (SELECT FECHAPROCESO FROM PARAMETROS);
