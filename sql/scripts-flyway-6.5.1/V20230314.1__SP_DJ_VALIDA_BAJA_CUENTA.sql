EXECUTE('
CREATE OR ALTER PROCEDURE SP_DJ_VALIDA_BAJA_CUENTA
...
BEGIN
	...
	SELECT @vTipoCausa = C.TIPO_CAUSA 
	FROM DJ_CAUSAS C WITH (NOLOCK)
	WHERE C.NRO_CAUSA = @pNroCausa;
	SELECT @vProductoCta = S.PRODUCTO 
	FROM SALDOS S WITH (NOLOCK)
	WHERE S.JTS_OID = @pJtsOidCta;
	SELECT @vProductoCausa = TC.PRODUCTO 
	FROM DJ_TIPOS_CAUSAS TC WITH (NOLOCK)
	WHERE TC.[CODIGO] = @vTipoCausa;
