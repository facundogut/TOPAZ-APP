Execute('
CREATE OR ALTER PROCEDURE [dbo].[SP_COELSA_CHEQUES_PRESENTADOS_RECIBIDOS]
@TICKET NUMERIC(16)
AS
BEGIN
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
BEGIN  
   ...
   BEGIN
      ...
      IF(substring(@LINEA, 77, 2)=''01'' OR substring(@LINEA, 77, 2)=''11'' OR substring(@LINEA, 77, 2)=''03'')
      BEGIN 
      SET @TIPO = ''A'';
      END
      IF(@TICKET<>0)
      BEGIN
      INSERT INTO dbo.ITF_COELSA_CHEQUES_PROPIOS_AUX (ID_TICKET, ID_PROCESO, FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE, CODIGO_POSTAL, FECHA_PRESENTADO, FECHA_VENCIMIENTO, NRO_CHEQUE, PUNTO_INTERCAMBIO, TRACE_NUMBER, ESTADO, TIPO, INFO_ADICIONAL)
