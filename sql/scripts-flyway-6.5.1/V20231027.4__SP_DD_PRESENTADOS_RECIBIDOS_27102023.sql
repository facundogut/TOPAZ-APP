...
CREATE OR ALTER PROCEDURE [dbo].[SP_DD_PRESENTADOS_RECIBIDOS]
	@TICKET NUMERIC(16),
...
BEGIN 
	...
	--#validacion5
	--OPH26102023 Se comentan validaciones porque se incluye el kettle COELSA_ESTANDAR_NACHA_VALIDACION.ktr
	...
	BEGIN
	      ...
	      BEGIN
				...
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 32),@InfoAdicional);
	      	GOTO Final
	      END
	      ...
	      BEGIN
	      		INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 76),@InfoAdicional);
	      	GOTO Final
	      END
	      ...
	      BEGIN
	      		INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 4),@InfoAdicional);
	      ...
	      DECLARE @V15 NUMERIC(8);--OPH24102023
	      ...
	      BEGIN
	      		INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 92),@InfoAdicional);
	      	GOTO Final
	      END
	      ...
	      BEGIN
	      		INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 93),@InfoAdicional);
	      	GOTO Final
	      END
	      ...
	      BEGIN
	        ...
	        INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 94),@InfoAdicional);
	      	GOTO Final
	      END
	      ...
	      BEGIN
	        INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 95),@InfoAdicional);
	      	GOTO Final
	      END
	      ...
	      BEGIN
	      		...
	      		INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 58),@InfoAdicional);
	      	GOTO Final
	      END
	      ...
	      --DECLARE @NIdClientePegador NUMERIC(12); OPH24102023
	      
	      --SET @NIdClientePegador = convert(NUMERIC(12),RTRIM(@IdClientePegador)); OPH24102023
	      
	      
	      SET @V24 = (SELECT COUNT(1) FROM dbo.SNP_DEBITOS 
	      			  WHERE REFERENCIA=RTRIM(LTRIM(@ReferenciaUnivoca))
	      			    AND FECHA_VTO=@FechaVencimiento
	      			    AND FECHA_COMP=@FechaCompensacion 
	      			    AND MONEDA=@Moneda
						AND SALDO_JTS_OID = @saldo_jts_oid);--OPH24102023
	      			    --AND CODIGO_CLIENTE= @NIdClientePegador); OPH24102023
		  
		  
		  IF (@V24 > 0)
	      BEGIN
	      		INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 36),@InfoAdicional);
	      	GOTO Final
	      END
	      ...
	      BEGIN
	      		INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 14),@InfoAdicional);
	      	GOTO Final
	      END
	      
	      --------------
	      ----NACHA RX1 POR REVISAR VALIDACIONES PENDIENTES --OPH27102023
	      --------------
	      --La cuenta que intenta debitar no existe
		  DECLARE @RX1 NUMERIC(2);
	      SET @RX1 = (SELECT COUNT(*) FROM SALDOS S WHERE S.C1803 = @Cod_Cliente AND S.CUENTA = @NumCuenta );
			
		  IF (@RX1 = 0)
	      BEGIN
	      		INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 3),@InfoAdicional);
	      	GOTO Final
	      END
	      
	      --------------
	      ----NACHA RX2 POR REVISAR VALIDACIONES PENDIENTES --OPH27102023
	      --------------
	      --La cuenta a la que se intenta hacer el débito se encuentra cancelada
		  DECLARE @RX2 NUMERIC(2);
	      SET @RX2 = (SELECT COUNT(*) FROM SALDOS S WHERE S.C1803 = @Cod_Cliente AND S.CUENTA = @NumCuenta 
	      				AND S.JTS_OID = @saldo_jts_oid AND S.C1651=''1'');
	      ...
	      --------------
	      ----NACHA RX3 POR REVISAR VALIDACIONES PENDIENTES --OPH27102023
	      --------------
	      --Verificar que el Nro. Cta no contenga carácteres no válidos
		  DECLARE @RX3 NUMERIC(2);
	      SET @RX3 = (SELECT COUNT(*) FROM SALDOS S WHERE S.CUENTA = @NumCuenta );
	      ...
	      BEGIN
	      		INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  ESTADO,  CODIGO_ERROR, INFO_ADICIONAL)
				VALUES (@TICKET, @FECHADATE, @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @VImporte,  @ReferenciaUnivoca, @IdClientePegador,  ''I'',  (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 74),@InfoAdicional);
	      	GOTO Final
	      END
		...
		WHERE NUMEROPERSONAFJ = (SELECT TOP 1 NUMEROPERSONA FROM CLI_ClientePersona 
								 --WHERE CODIGOCLIENTE= @NIdClientePegador--@Cod_Cliente OPH24102023
								 WHERE CODIGOCLIENTE= @Cod_Cliente --OPH24102023
		...
		INSERT INTO SNP_DEBITOS(ID_DEBITO, ORIGEN, CUIT_EO, PRESTACION, FECHA_VTO, CODIGO_CLIENTE
		  ,REFERENCIA, PRIORIDAD, FECHA_COMP, MONEDA, IMPORTE, CBU, SALDO_JTS_OID, CODIGO_ORIGEN
		  ,INFO_ADICIONAL, MOTIVO_RECHAZO, ESTADO, JTS_OID_FINAL, FECHA_ASIENTO, NUMERO_ASIENTO, SUCURSAL_ASIENTO
		  , REVERSA
		  )
		--VALUES(@NroArchivo, ''C'', @CUIT_EO , @PRESTACION, @FechaVencimiento, @NIdClientePegador --@Cod_Cliente OPH24102023
		VALUES(@NroArchivo, ''C'', @CUIT_EO , @PRESTACION, @FechaVencimiento, @Cod_Cliente --OPH24102023
		  ,RTRIM(LTRIM(@ReferenciaUnivoca)), @CodigoPrioridad, @FechaCompensacion, @Moneda, @Importe, @CTA_CBU, @saldo_jts_oid, @CodigoOrigen
