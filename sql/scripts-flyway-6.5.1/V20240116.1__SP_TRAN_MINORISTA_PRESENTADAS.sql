execute('
ALTER   PROCEDURE [dbo].[SP_TRAN_MINORISTA_PRESENTADAS]
   @TICKET NUMERIC(16)
AS 
BEGIN
...
SELECT t.OP_NUMERO, v.JTS_OID_SALDO, t.OP_MONEDA, t.OP_IMPORTE, @T_FECHA_VTO, @T_FECHA_COMP, t.ORD_CBU, t.OP_NUMERO, s.C1803, t.ORD_NRO_DOC, t.ORD_NOMBRE, RIGHT(''00000000000'' + CAST(t.BEN_NRO_DOC AS VARCHAR(11)), 11) AS BEN_NRO_DOC, t.BEN_NOMBRE, t.BEN_TIPO_DOC,t.ORD_TIPO_DOC, t.OP_REFERENCIA,TT.ADICIONAL_PRESENT
...
OPEN CursorTM
FETCH NEXT FROM CursorTM INTO @T_ID_CREDITO, @T_JTS_OID, @T_MONEDA, @T_IMPORTE, @T_FECHA_VTO,@T_FECHA_COMP,@T_CBU, @T_REFERENCIA, @T_CODIGO_CLIENTE, @T_CUIT_ORD, @T_NOMBRE_ORDENANTE, @T_CUIT_BEN, @T_NOMBRE_BENEFICIARIO, @T_TIPO_DOC,@t_ord_tipo_doc, @t_op_referencia, @t_adicional_present 
...
BEGIN
	...
	SELECT @T_SUCURSAL=SUCURSAL FROM SALDOS WHERE JTS_OID=@T_JTS_OID;
	SELECT @T_CUENTA=RIGHT(cta_cbu, 14) from VTA_SALDOS where JTS_OID_SALDO=@T_JTS_OID;

	IF(@T_MONEDA= (SELECT C6399 FROM MONEDAS WHERE C6403=''N''))
	BEGIN
	SET @T_COD_BANCO=311;
	SET @RI_INFO_ADICIONAL=@t_adicional_present;
	END
	ELSE
	BEGIN
	SET @T_COD_BANCO=811;
