...
BEGIN	
	...
	DECLARE @fch DATE;
	DECLARE @fch_inicio DATETIME; -- 1er dia del mes de ejecucion							
	...
	---------------- Borro Tabla RRII_ESTADO a la fecha de ejecucion --------------------
	DELETE FROM RRII_ESTADO WHERE INFORME = ''INFORME_LELIQ'' AND FECHA_REPORTE = @fch ; --incluir borrado a fecha d ejecucion	
	-------------------------------------------------------------------------------------
	
	-------------------- Grabo Tabla RRII_ESTADO a la fecha de ejecucion ----------------
	WHILE @fch_aux <= @fch
