execute('
create or ALTER     PROCEDURE [dbo].[SP_ITF_COELSA_TRANS_MPD]
@TICKET NUMERIC(16)
AS
...
SELECT A.ORD_NRO_DOC, 
	...
	(CASE WHEN SUBSTRING(CAST(A.ORD_NRO_DOC AS VARCHAR(11)),1,1)=''3'' THEN
	 ...
	 LEFT(A.ORD_NOMBRE + REPLICATE('' '',16),16) AS CL_NOMBRE_EMPRESA_INDIVIDUO
	FROM 
	(
		SELECT DISTINCT VT1.ORD_NRO_DOC, VT1.BEN_BANCO,VT1.OP_CLASE_TRANS,VT1.OP_TIPO,VT1.ORD_NOMBRE
...
FETCH NEXT FROM CursorLote INTO @CL_ORD_NRO_DOC, @CL_BEN_BANCO, @CL_OP_TIPO,@CL_OP_CLASE_TRANS, @CL_NUM_LOTE_CAB, @CL_IDENTIFICACION_EMP_IND, @CL_DIGITO_VERIFICADOR, @CL_NOMBRE_EMPRESA_INDIVIDUO
...
SELECT
    ...
    TR.OP_IMPORTE AS IMPORTE,--6 importe
     CONCAT(
     ''VAR'', 
     RIGHT(REPLICATE(''0'',12) + CAST(TR.OP_NUMERO AS VARCHAR(12)) ,12)--Código de referencia de la transferencia.
	 ) AS RI_REFERENCIA_UNIVOCA,--Referencia univoca de la transferencia
    --8 Identificación del cliente beneficiario
     ...
     REPLICATE('' '',7),--5 espacios en blanco
     ''074'')
     --código de operatoria del BCRA:73,74,75
       AS VARCHAR(22)) + REPLICATE('' '',22),22) AS RI_IDENTIFICACION_CLIENTE_BEN, --código de operatoria del BCRA:73,74,75 ...POR REVISAR Y COMPLETAR
...
BEGIN
	...
	SET @RI_MONEDA=(SELECT TOP 1 adicional_present FROM VTA_TRANSFERENCIAS_TIPOS WHERE id_tipo=@cl_op_tipo)
	...
	SET @RA_CODIGO_CONCEPTO = CONCAT(@CL_ORD_NRO_DOC, LEFT(@CL_NOMBRE_EMPRESA_INDIVIDUO + REPLICATE('' '' ,69), 69));
