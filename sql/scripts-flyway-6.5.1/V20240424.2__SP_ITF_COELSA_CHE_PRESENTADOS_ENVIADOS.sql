...
--14/12/2022
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--- Modified : 12/12/2022 10:00 a.m.
--- Autor: Juan Pedrozo 
--- Se àgregan nuevos controles respecto al reproceso y se corrige contador de lotes debido al nuevo diseño de 1 lote por archivo
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
BEGIN
   	...
   	SET @FL_TOTALES_DE_CONTROL = (RIGHT(concat(replicate(''0'', 10), (SELECT sum(convert(numeric,substring(linea,4,8)))
		...
		--SET @FL_SUMA_TOTAL_DEBITO_LOTE = RIGHT(concat(replicate(''0'', 12), (replace(''0'', ''.'', ''''))), 12); 
...
BEGIN
	...
	BEGIN
		...
		IF (@SumaImportes > 9999999999 OR @SumaEntidad > 999000) -- 9999 millones
		BEGIN
	...
	IF(SELECT count(*)
	   ...
	   AND ESTADO = 1 
	   AND MONEDA = 1 
	   AND FECHA_ALTA = (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)))>0
	BEGIN 
		SET @CA_ID_REG = ''1''; -- fijo
		SET @CA_CODIGO_PRIORIDAD = ''01''; -- fijo
		SET @CA_DESTINO_INMEDIATO = '' 000000010''; --fijo
		SET @CA_ORIGEN_INMEDIATO  = (SELECT '' 031100''+CAST((SELECT NUMERICO FROM PARAMETROSGENERALES WHERE CODIGO=40) AS VARCHAR)+''0''); 
		SET @CA_FECHA_PRESENTACION = convert(VARCHAR,(SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)), 12); -- fijo
		SET @CA_HORA_PRESENTACION = concat (SUBSTRING((SELECT CONVERT (VARCHAR(8), SYSDATETIME(),24)),1,2), SUBSTRING((SELECT CONVERT (VARCHAR(8), SYSDATETIME(),24)),4,5)); -- fijo
		SET @CA_IDENTIFICADOR_ARCHIVO  = ''1''; --
		SET @CA_TAMANNO_REGISTRO= ''094''; -- fijo
		SET @CA_FACTOR_BLOQUE = ''10''; -- fijo
		SET @CA_CODIGO_FORMATO = ''1''; -- fijo
		SET @CA_NOMBRE_DEST_INMEDIATO = ''COELSA                 ''; -- fijo
		SET @CA_NOMBRE_ORIG_INMEDIATO =''NUEVO BANCO DEL CHACO S''; -- fijo
		SET @CA_CODIGO_REFERENCIA  = replicate('' '', 8); --Se conforma con espacios vacíos.
		...
		--- Variables cabecera lote (CL)
		SET @CL_ID_REG= ''5''; -- fijo
		SET @CL_CODIGO_CLASE_TRANSAC  = ''200''; -- fijo 
		SET @CL_RESERVADO  = replicate('' '', 46); -- 3 campos reservados
		SET @CL_TIPO_REGISTRO  = ''TRC''; -- fijo 
		SET @CL_DESCRIP_TRANSAC  = ''CHEQUESPRE''; -- fijo
		SET @CL_FECHA_PRESENTACION = convert(VARCHAR,(SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)), 12);
		SET @CL_FECHA_VENCIMIENTO = convert(VARCHAR,(SELECT FECHAPROXIMOPROCESO FROM PARAMETROS WITH(NOLOCK)), 12); 
		SET @CL_RESERVADO_CL  = ''000''; -- fijo
		SET @CL_CODIGO_ORIGEN  = ''1''; -- fijo
		SET @CL_ID_ENTIDAD_ORIGEN  = (SELECT ''031100''+CAST((SELECT NUMERICO FROM PARAMETROSGENERALES WHERE CODIGO=40) AS VARCHAR)); 
		SET @CL_NUMERO_LOTE = RIGHT(concat(replicate(''0'', 7), 1), 7); -- numero del lote
...
PRINT @RI_CODIGO_POSTAL
		...
		FETCH NEXT FROM CursorDPF INTO @D_TIPO_DOCUMENTO, @D_BANCO, @D_SUCURSAL, @D_CUENTA, @D_IMPORTE, @D_NUMERO_DPF, @D_CODIGO_POSTAL, @D_MONEDA, @D_FECHA, @D_SUCURSAL_DE_INGRESO
