...
--- Inserta en tabla auxiliar
   ...
   FROM @TBL_AUX T
	...
	SET @DEUDA_TOTAL = ISNULL((SELECT SUM(DEUDA_TOTAL) FROM @TBL_AUX WHERE PERSONA = @PERSONA),0) + @IMPORTE_SOLICITUD;
...
--CUENTA SI LA CANTIDAD DE CLIENTES QUE EXCEDEN GRADUACIÓN/FRACCIONAMIENTO ES >=1
	...
	ELSE IF (@RPC_CLIENTE = 0)
	    ...
	    END
	ELSE IF (SELECT COUNT(1) FROM @TBL_AUX WHERE EXCEDIDO_GRAD = ''S'' AND PERSONA = @PERSONA)>= 1
	    BEGIN
