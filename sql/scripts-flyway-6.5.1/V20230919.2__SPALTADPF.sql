...
SET @LINEA6 = REPLACE(FORMAT(@UVAS, ''C'', ''es-ar''),''$'','''')
...
SET @LINEA7 = @LINEA7 + REPLICATE('' '', 36-(DATALENGTH(@LINEA7)+DATALENGTH(''ALTA '')+DATALENGTH(CONVERT(VARCHAR,@ALTA,103)))) + ''ALTA '' + CONVERT(VARCHAR,@ALTA,103)
...
SELECT @TEM=CASE
		WHEN PR.C6253 = ''E'' --TASA EFECTIVA
			THEN CONVERT(NUMERIC(11,7),@TASA_INTERES/12)
		-- TASA
		WHEN PR.C6253 = ''M'' --TASA EFECTIVA MENSUAL
			THEN @TASA_INTERES			
		--CONVENCIÓN DEL BCRA
		WHEN PR.C6253 = ''N'' --TASA NOMINAL
			THEN CONVERT(NUMERIC(11,7),(@TASA_INTERES * 30.41666 / 365)/12)
		END
...
SET @CABECERA125 = ''3821P''+ SUBSTRING(CONVERT(VARCHAR,(SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)),112),3,6) + ''1036'';
