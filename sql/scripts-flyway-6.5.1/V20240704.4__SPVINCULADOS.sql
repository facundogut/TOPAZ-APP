...
IF(@PORCENTAJE_SING = 0 AND @PORCENTAJE_CONG = 0)
 BEGIN
  SET @DEUDA_12M = (SELECT MAYOR_DEUDA_12M_PREV FROM CRE_VINCULACIONES WITH(NOLOCK) WHERE ID_PERSONA_VINCULADA = @PERSONA AND TIPO_VINCULO = @TIPO_VINCULACION AND TZ_LOCK = 0)
  ...
  ELSE
   ...
   SET @DEUDA_TOTAL = (SELECT SUM(IMPORTE_DEUDA) FROM @TABLA_ASISTENCIAS WHERE PERSONA = @PERSONA)
