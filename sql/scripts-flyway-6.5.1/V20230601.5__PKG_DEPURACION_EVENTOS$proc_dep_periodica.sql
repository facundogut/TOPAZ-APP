execute('
CREATE or alter PROCEDURE [dbo].[PKG_DEPURACION_EVENTOS$proc_dep_periodica]
...
BEGIN TRY
	...
	SELECT @v_dias = PERIODO FROM PARAMETROS_DEPURACION WHERE MODULO = ''EVENTOS'' AND ID = ''EVENTOS_A_PROCESAR'' AND TZ_LOCK = 0;

	------------------------------------------------
	SET @v_tabla = ''EVENTOS_A_PROCESAR''
	...
	SET @v_where = ''EVENTOS_A_PROCESAR.FECHA_MODIFICACION < @fecha''	
	...
	EXECUTE PKG_DEPURACION$proc_dep_periodica @p_id_proceso,
													@p_dt_proceso,
													@v_tabla,
													''PKG_DEPURACION_EVENTOS$proc_dep_periodica'',
													'''',
													@v_where,
													''FECHA_MODIFICACION'',
													@v_fecha,
...
BEGIN CATCH
    ...
    SET @p_msg_proceso = ''El proceso PKG_DEPURACIONEVENTOS$proc_dep_periodica produjo un error. Error original: '' + ERROR_MESSAGE() + '' Linea: '' + CAST(ERROR_LINE() AS VARCHAR(50));	
