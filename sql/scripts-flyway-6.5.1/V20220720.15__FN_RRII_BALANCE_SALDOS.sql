EXECUTE ('
CREATE function FN_RRII_BALANCE_SALDOS (@fch_proceso DATETIME) 
RETURNS TABLE 
AS 
RETURN
SELECT DISTINCT
	...
	CASE 
		...
		ELSE ROUND(ROUND(SUM(CBR.SALDOFINMESMO),0) * (SELECT H.TIPO_CAMBIO_OFICIAL FROM HISTORICOTIPOSCAMBIO H WHERE H.FECHA_COTIZACION<=CBR.FECHA AND H.MONEDA=CBR.MONEDA),0)*-1 
	END IMPORTE,	
