EXECUTE('DROP VIEW IF EXISTS VW_ITF_AOJ_CUENTAS;')

EXECUTE('
CREATE VIEW VW_ITF_AOJ_CUENTAS AS
SELECT DISTINCT

RIGHT(REPLICATE(''0'', 50)+ CAST(CP.NUMEROPERSONA AS VARCHAR(50)), 50) AS ID_CLIENTE, 

concat(
		RIGHT(REPLICATE(''0'', 5)+ CAST(S.SUCURSAL AS VARCHAR(5)), 5), 
		''-'',
		RIGHT(REPLICATE(''0'', 5)+ CAST(S.PRODUCTO AS VARCHAR(5)), 5), 
		''-'',
		RIGHT(REPLICATE(''0'', 12)+ CAST(S.CUENTA AS VARCHAR(12)), 12), 
		''-'',
		RIGHT(REPLICATE(''0'', 4)+ CAST(S.MONEDA AS VARCHAR(4)), 4) ,  
		''-'',
		RIGHT(REPLICATE(''0'', 12)+ CAST(S.OPERACION AS VARCHAR(12)), 12), 
		''-'',
...
RIGHT(concat(''00000'',  (SELECT CAST(IMPORTE_1 AS  NUMERIC(15)) FROM ITF_MASTER_PARAMETROS WHERE [CODIGO_INTERFACE] = 7 AND NUMERICO_1 = S.PRODUCTO AND NUMERICO_2 = S.MONEDA)), 5) AS ''Tipo de Cuenta'',
...
concat(''${Top.Output}AOJ/ofjudiciales_cuentas_'',CONVERT(CHAR(8),FECHAPROCESO,112)) AS NOM_ARCHIVO

FROM PARAMETROS, CLI_CLIENTES c
JOIN CLI_ClientePersona cp ON cp.[CODIGOCLIENTE]=c.[CODIGOCLIENTE]
JOIN SALDOS s ON c.[CODIGOCLIENTE]=s.C1803
...
SELECT 
	...
	concat(''${Top.Output}AOJ/ofjudiciales_cuentas_'',CONVERT(CHAR(8),(SELECT FECHAPROCESO FROM PARAMETROS),112)) AS NOM_ARCHIVO
	
FROM COF_COFRES_CONTRATOS cc 
INNER JOIN CLI_CLIENTES c ON cc.CLIENTE=c.[CODIGOCLIENTE]
INNER JOIN CLI_ClientePersona cp ON cp.[CODIGOCLIENTE]=c.[CODIGOCLIENTE] AND cp.TITULARIDAD=''T''
WHERE cc.TZ_LOCK=0 AND cc.ESTADO=''C'';')
