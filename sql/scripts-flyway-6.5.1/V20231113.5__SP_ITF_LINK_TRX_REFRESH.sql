...
BEGIN
    SET @Error = '' '';
  	...
  	BEGIN TRY
		...
		WHEN MATCHED THEN
			UPDATE SET A.TIPO_TARJETA = (SELECT TIPO_TARJETA FROM TJD_TIPO_TARJETA (nolock) where CODIGO_PRODUCTO = substring(Linea, 178, 4)), 
				A.LIMITE_CREDITO = substring(Linea,769,2),
				A.LIMITE_DEBITO = substring(Linea,767,2), 
				A.NOMBRE_TARJETA = Concat(trim(substring(Linea, 548, 15)), '' '', substring(Linea, 563, 15)),
				A.DIGITO_VERIFICADOR = substring(Linea,764,1);	

		
		--Alta
		EXEC SP_ITF_LINK_TRX_FULL @IndRefreshFull, @Error output, @erroresCta output;
