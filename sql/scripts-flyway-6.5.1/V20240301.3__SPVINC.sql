...
SET @PORCENTAJE_SING = (SELECT PORC_GRANDES_EXP_SING FROM CRE_VINCULOS_RELACIONES WITH(NOLOCK) WHERE ID_VINCULO = @TIPO_VINCULACION AND TZ_LOCK = 0)

SET @PORCENTAJE_CONG = (SELECT PORC_GRANDES_EXP_CNG FROM CRE_VINCULOS_RELACIONES WITH(NOLOCK) WHERE ID_VINCULO = @TIPO_VINCULACION AND TZ_LOCK = 0)
...
IF(@PORCENTAJE_SING = 0 AND @PORCENTAJE_CONG = 0)
 ...
 BEGIN
 SET @EXCEDE_GRAD = ''S'';
 SET @MENSAJE_INDIVIDUAL = ''Exceso en Graduación: ''+CONVERT(VARCHAR(17),FORMAT(ROUND((@DEUDA_TOTAL+@IMPORTE_ANALISIS)-@DEUDA_TOPE,0), ''C'', ''es-ar''))+''.'';
 END
 ELSE 
 BEGIN
