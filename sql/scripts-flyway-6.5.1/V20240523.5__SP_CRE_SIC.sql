...
-- Clientes con cuentas
	   ...
	   P.PONDERACION
	   FROM (
			SELECT C.CLIENTE,C.NUEVO, MIN(TIPO_SOBREGIRO) AS TIPO_SOBREGIRO
				   FROM @CLIENTES C
				   INNER JOIN SALDOS S WITH(NOLOCK)  ON S.C1803=C.CLIENTE
				   INNER JOIN (SELECT SALDOS_JTS_OID, FECHA,
							   CASE WHEN SUM(SALDO_AJUSTADO)<0 AND SUM(CUPO_SOBREGIRO)=0 THEN 0 -- Sobregiro sin Acuerdo
						  	 		WHEN SUM(SALDO_AJUSTADO)<0 AND SUM(CUPO_SOBREGIRO)>0 AND SUM(SALDO_AJUSTADO)+SUM(CUPO_SOBREGIRO)<0 THEN 1 -- Sobregiro con Acuerdo
						  	 		WHEN SUM(SALDO_AJUSTADO)+SUM(CUPO_SOBREGIRO)>=0 THEN 2 -- Sin Sobregiro
								END AS TIPO_SOBREGIRO
				   			  FROM GRL_SALDOS_DIARIOS WITH(NOLOCK) 
				   			  WHERE FECHA BETWEEN DATEADD(MONTH, -12,@FECHAPROCESO) AND @FECHAPROCESO AND TZ_LOCK=0
				   			  GROUP BY SALDOS_JTS_OID, FECHA
				   			  ) GRL
				   			  ON GRL.SALDOS_JTS_OID=S.JTS_OID 				   			  
				   WHERE S.C1785 IN (2) AND S.TZ_LOCK=0
...
-- Clientes sin cuentas corrientes
	INSERT INTO dbo.CRE_SIC_BITACORA_DET (CLIENTE, FECHA, TIPO, PUNTOS_SIN_POND, PUNTOS_CON_POND, PONDERACION)
