EXECUTE('
/****** Object:  StoredProcedure [dbo].[SP_CONTROL_BAJAS]    Script Date: 02/06/2021 10:41:18 ******/
SET ANSI_NULLS ON
...
ALTER   PROCEDURE [dbo].[SP_CONTROL_BAJAS] 
							...
							@valor AS VARCHAR(20), 
							@resultado NUMERIC(5,0) OUTPUT, 
							@tablaOUT VARCHAR(30) OUTPUT
AS
BEGIN
	--------------- DECLARO VRIABLES ---------------
	...
	BEGIN
		...
		DECLARE cursor_diccionario cursor FOR
			...
			WHERE NUMERODECAMPO IN (SELECT NUMERODECAMPO 
									FROM DICCIONARIO WITH (NOLOCK) 
									WHERE (TABLADEAYUDA=@nro_ayuda OR TABLADEVALIDACION=@tabla) 
											AND TABLA<>0 
											AND TABLA NOT IN (SELECT TABLA 
																FROM DICCIONARIO WITH (NOLOCK) 
																WHERE NUMERODECAMPO=@campoIN
															)
									);
