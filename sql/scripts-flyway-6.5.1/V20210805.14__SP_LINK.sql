/****** Object:  StoredProcedure [dbo].[SP_LINK]    Script Date: 01/06/2021 14:57:21 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[SP_LINK] 
		...
		BEGIN
		    ...
		    BEGIN
			    ...
			    SELECT TOP (@cant_movimientos) hv.ASIENTO, hv.FECHA_PROCESADO, hv.SUCURSAL, hv.NUMERO_MOVIMIENTO, hv.ORDINAL, hv.TRTIPO, 
				    ...
				    WHERE hv.SALDO_JTS_OID=@oid 
							AND hv.FECHA_PROCESADO <= @fch_proceso 
					ORDER BY hv.FECHA_PROCESADO desc;
			END
			ELSE
			BEGIN
			    INSERT INTO ITF_MOVCONFORMADOS (asiento, fch_procesado, sucursal, nro_mov, ordinal, TRTIPO, 
