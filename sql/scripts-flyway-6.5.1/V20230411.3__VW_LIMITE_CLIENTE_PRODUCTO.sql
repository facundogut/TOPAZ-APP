...
SELECT	l.CLIENTE, 
			...
			CASE WHEN isnull(sum(scl.IMPORTE),0) = 0 THEN 0
				 WHEN p.MONTO - (isnull(sum(scl.IMPORTE),0)+isnull(sum(j.montoSolicitado),0)) < 0 THEN p.MONTO 
			     ELSE (isnull(sum(scl.IMPORTE),0)+isnull(sum(j.montoSolicitado),0)) END AS UTILIZADO
