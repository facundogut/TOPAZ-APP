ALTER   PROCEDURE [dbo].[SP_TRAN_SUELDOS_PRESENTADAS]
   @TICKET NUMERIC(16)
AS 
BEGIN
	...
	IF((SELECT COUNT(1) 
		...
		INNER JOIN VTA_SALDOS v ON t.ORD_CBU=v.CTA_CBU AND ((t.OP_CLASE_TRANS='R' AND t.ESTADO='RC') OR (t.OP_CLASE_TRANS='E' AND t.ESTADO='PP')) AND t.OP_MONEDA=1 AND t.OP_TIPO=2 INNER JOIN SALDOS s ON v.JTS_OID_SALDO=s.JTS_OID
...
PRINT @T_TIPO_DOC    
    	IF(@T_TIPO_DOC ='CUIT')
PRINT @T_TIPO_DOC 
    		SET @RI_CLIENTE_PAGADOR = CONCAT('1',@T_CUIT_BEN,'       ',RIGHT(concat('000',@operatoria_bcra),3));
    	IF(@T_TIPO_DOC ='CUIL')
    		SET @RI_CLIENTE_PAGADOR = CONCAT('2',@T_CUIT_BEN,'       ',RIGHT(concat('000',@operatoria_bcra),3));
    	IF(@T_TIPO_DOC ='CDI')
    		SET @RI_CLIENTE_PAGADOR = CONCAT('3',@T_CUIT_BEN,'       ',RIGHT(concat('000',@operatoria_bcra),3));
...
PRINT @T_TIPO_DOC 
    	...
    	IF(@T_CLASE='R')
		BEGIN
    		UPDATE dbo.VTA_TRANSFERENCIAS SET ESTADO='RP', TRACENUMBER=@RI_CONTADOR_REGISTRO WHERE OP_NUMERO=@T_REFERENCIA;
    	END
    	IF(@T_CLASE='E')
		BEGIN
    		UPDATE dbo.VTA_TRANSFERENCIAS SET ESTADO='PR', TRACENUMBER=@RI_CONTADOR_REGISTRO WHERE OP_NUMERO=@T_REFERENCIA;
    	END
		-----------
		...
		BEGIN		     	       				
			...
			FETCH NEXT FROM CursorTM INTO @T_ID_CREDITO, @T_JTS_OID, @T_MONEDA, @T_IMPORTE, @T_FECHA_VTO,@T_FECHA_COMP,@T_CBU, @T_REFERENCIA, @T_CODIGO_CLIENTE, @T_CUIT_ORD, @T_NOMBRE_ORDENANTE, @T_CUIT_BEN, @T_NOMBRE_BENEFICIARIO, @T_TIPO_DOC, @T_CLASE							    , @t_ord_tipo_doc
									    , @t_op_referencia
									    , @t_ben_cbu
									    , @t_ben_mismo_titular
							    		, @t_ben_banco
