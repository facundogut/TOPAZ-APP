Execute('CREATE OR ALTER  PROCEDURE dbo.SP_ITF_LOTIPAGOS
AS
...
DECLARE	cur_Convenios CURSOR LOCAL FORWARD_ONLY FOR
	SELECT b.ID_CONVREC,SUM(Importeabonado)/100 AS ImporteTotal,nomarchivo,fechapago,count(*) AS TotalRegXConvenio,nomconvrec,nroconvenio
	FROM itf_intermedia_lotipagos a,CONV_REL_ENTECONV b WHERE cod_ente = nroconvenio AND a.TZ_LOCK=0 AND tiporegistro = 2
...
WHILE @@fetch_status = 0
	BEGIN
		...
		FROM itf_intermedia_lotipagos WHERE TZ_LOCK = 0 AND NroConvenio = @CursorEnte AND TipoRegistro = 2 AND ConError = ''N'';
