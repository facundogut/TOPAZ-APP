...
SELECT 
	...
	CASE 
		 ...
		 ELSE '''' 
	END AS PAGOPARCIAL ,
	ISNULL((BPC.CAPITAL_CONTAB+	BPC.INTERES_CONTAB+	BPC.MORA_CONTAB+BPC.ICMORA_CONTAB+BPC.GASTOS_CONTAB),0) AS CONDONADO,
	ISNULL((SELECT SUM(CAPITALREALIZADO) FROM MOVIMIENTOS_CONTABLES WITH (NOLOCK) WHERE FECHAPROCESO=BH.FECHAPROCESOMOV AND ASIENTO=BH.NROASIENTOMOV 
		AND SUCURSAL=BH.SUCURSALMOV AND CONCEPTO=''Impuestos''),0) AS CONDONADOIMP	
...
SALDO_INTERES,DIASATRASO,FECHAVALOR,TASAINTERES,PAGOPARCIAL,CONDONADO
')