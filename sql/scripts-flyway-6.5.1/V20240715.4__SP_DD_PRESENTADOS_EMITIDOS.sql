...
BEGIN
	...
	--SET @CL_CABECERA = concat(@CL_ID_REG, @CL_CODIGO_CLASE_TRANSAC, @CL_RESERVADO, @CL_TIPO_REGISTRO, @CL_DESCRIP_TRANSAC, @CL_FECHA_COMPENSACION, @CL_FECHA_VENCIMIENTO, @CL_RESERVADO_CL, @CL_CODIGO_ORIGEN, @CL_ID_ENTIDAD_ORIGEN, @CL_NUMERO_LOTE);
	...
	SELECT o.CORRELATIVO AS CORRELATIVO,
			...
			dbo.diaHabil( dateadd(dd,1, o.FECHA_VENCIMIENTO),''A'') AS FECHA_COMPENSACION, 
			o.CBU AS CBU, 
...
PRINT @T_FECHA_COMP
		...
		IF @T_TIPO=''O''
		BEGIN	
			SET @CL_FECHA_VENCIMIENTO = CONVERT(VARCHAR, @T_PRES_1ER_VTO, 12);--revisar
			SET @CL_FECHA_COMPENSACION = convert(VARCHAR,dbo.diaHabil( dateadd(dd,1, @T_PRES_1ER_VTO),''A''), 12); 
		END
		ELSE 
		BEGIN
	...
	--SE HACE MAS ARRIBA ESTE SET
		...
		SET @T_SUCURSAL=try_convert(NUMERIC,substring(@t_cbu,5,4))
...
--			SET @RI_REFERENCIA_UNIVOCA=concat(RIGHT(concat(replicate(''0'',8),substring(@t_Cbu,1,8)),8),replicate('' '',7))
		...
		BEGIN
			...
			IF @RI_INFO_ADICIONAL=''06''
				SET @RI_REGISTRO_INDIVIDUAL = concat(''705''
						, LEFT(concat(@RI_ENTIDAD_DEBITAR,SUBSTRING(@t_cbu, 9, 14),replicate('' '',80)),80)
						, @RO_NUM_SECUENCIA_ADICIONAL
						, RIGHT(@T_TRACENUMBER, 7)
						);
