...
IF OBJECT_ID (''dbo.ITF_COELSA_CHEQUES_PROPIOS'') IS NOT NULL
	DROP TABLE dbo.ITF_COELSA_CHEQUES_PROPIOS


CREATE TABLE dbo.ITF_COELSA_CHEQUES_PROPIOS
	(
	...
	CONSTRAINT PK__ITF_COELSA_CHEQUES_PROPIOS PRIMARY KEY (ID)
	)
...
ALTER PROCEDURE [dbo].[SP_COELSA_CHEQUES_PRESENTADOS_RECIBIDOS]
@TICKET NUMERIC(16)
AS
BEGIN
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
BEGIN  
      ...
      BEGIN
      	...
      	BEGIN
      		...
      		VALUES((SELECT ISNULL(MAX(ORDINAL),0) + 1 FROM CLE_CHEQUES_AJUSTE), @NumeroCheque, @Entidad, @CuentaDebitar, @Sucursal, ''I'', @CodigoPostal, (SELECT FECHAPROCESO FROM PARAMETROS), @Sucursal, @Importe, @PuntoIntercambio, ''R'', @ContadorRegistros);
      	END
      END
