...
DECLARE @codigoEmpresa VARCHAR(10) = ''311''; -- aun no esta definido el codigo de la empresa
...
--cab	
	...
	INSERT INTO ITF_GIRELEC_AUX (ID_CABEZAL, CAMPO, TIPO) 		
		...
		FROM CONV_CONVENIOS_REC CONV JOIN 
	   		REC_RENDICION REN ON CONV.Id_ConvRec = ren.CONVENIO OR CONV.Id_ConvRec = REN.CONVENIO_PADRE JOIN --le agregue filtro convenio padre
	   		REC_LIQUIDACION LIQ ON liq.ID_RENDICION = ren.ID_RENDICION JOIN 
	   		REC_CAB_RECAUDOS_CANAL CAB ON cab.ID_LIQUIDACION = liq.ID_LIQUIDACION JOIN 
			SUC_FICTICIA_CONV_CANALES SUCFIC ON SUCFIC.CANAL = CONV.Canal		
		
		WHERE 
			LIQ.ESTADO = ''R'' AND 
		  	CONV.Id_ConvRec = @convenio AND 
			REN.FECHA = CAST(@fecha_rendicion AS DATE) AND
						
			REN.TZ_LOCK = 0 AND 
			LIQ.TZ_LOCK = 0 AND
			CONV.TZ_LOCK = 0 AND
			CAB.TZ_LOCK = 0 AND 
			CONV.FORMATO_RENDICION = 			
			(
	...
	INSERT INTO ITF_GIRELEC_AUX (ID_CABEZAL, CAMPO, TIPO) 	
		...
		FROM CONV_CONVENIOS_REC CONV JOIN 
			REC_RENDICION REN ON CONV.Id_ConvRec = ren.CONVENIO OR CONV.Id_ConvRec = REN.CONVENIO_PADRE JOIN
		 	REC_LIQUIDACION LIQ ON liq.ID_RENDICION = ren.ID_RENDICION JOIN 
			REC_CAB_RECAUDOS_CAJA CAB ON CAB.ID_LIQUIDACION = LIQ.ID_LIQUIDACION JOIN 
			REC_DET_RECAUDOS_CAJA DET ON CAB.ID = DET.ID_CABEZAL
		WHERE 
			LIQ.ESTADO = ''R'' AND 
			CONV.Id_ConvRec = @convenio AND 
			REN.FECHA = CAST(@fecha_rendicion AS DATE) AND
	...
	INSERT INTO ITF_GIRELEC_AUX (ID_CABEZAL, CAMPO, TIPO) 
		...
		FROM CONV_CONVENIOS_REC CONV JOIN 
			REC_RENDICION REN ON CONV.Id_ConvRec = REN.CONVENIO OR CONV.Id_ConvRec = REN.CONVENIO_PADRE JOIN 
		 	REC_LIQUIDACION LIQ ON liq.ID_RENDICION = ren.ID_RENDICION JOIN 
			REC_CAB_RECAUDOS_CANAL CAB ON  CAB.ID_LIQUIDACION = LIQ.ID_LIQUIDACION JOIN 
	   		REC_DET_RECAUDOS_CANAL DET ON CAB.ID = DET.ID_CABEZAL
		WHERE 
			LIQ.ESTADO = ''R'' AND 
			CONV.Id_ConvRec = @convenio AND 
			REN.FECHA = CAST(@fecha_rendicion AS DATE) AND
...
--pie
	INSERT INTO ITF_GIRELEC_AUX (ID_CABEZAL, CAMPO, TIPO) 
		SELECT 
			...
			Concat(''P'',
				RIGHT(CONCAT(REPLICATE(''0'',5),@codigoEmpresa),5), -- aun no esta definido el codigo de la empresa
				RIGHT(CONCAT(REPLICATE(''0'',5),(SELECT numerico FROM PARAMETROSGENERALES WHERE CODIGO = 2)),5),
