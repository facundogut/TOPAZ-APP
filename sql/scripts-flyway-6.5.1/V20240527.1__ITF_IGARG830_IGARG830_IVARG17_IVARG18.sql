EXECUTE('
IF OBJECT_ID (''dbo.ITF_IGARG830_PERCEPCION_ULT_PADRON_AUX'') IS NOT NULL
	DROP TABLE dbo.ITF_IGARG830_PERCEPCION_ULT_PADRON_AUX


CREATE TABLE dbo.ITF_IGARG830_PERCEPCION_ULT_PADRON_AUX
	(
	...
	, CONSTRAINT PK_ITF_IGARG830_PERCEPCION_ULT_PADRON_AUX PRIMARY KEY (ID)
	)
');
...
--inserto en bitacora las bajas de los registgros que no vinieron en el nuevo padron
	...
	INSERT INTO dbo.CON_BITACORA_IMPUESTOS (JTS_NOVEDAD, 
											...
											SELECT  ISNULL((SELECT MAX(JTS_NOVEDAD) FROM CON_BITACORA_IMPUESTOS),0)+ (ROW_NUMBER() OVER (ORDER BY VV.TIPOCARGO)), 
													...
													''B'',
													''AFIP_RG830.txt'',
													a.porcentaje-vv.TASA*A.PORCENTAJE,
													''ITF'',
													a.certificado,
													VV.SEGMENTO
