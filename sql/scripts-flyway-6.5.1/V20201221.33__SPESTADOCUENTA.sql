   ...
   BEGIN
     ...
     @vSIRCREB NUMERIC(15,2),
      ...
      BEGIN TRY
         ...
         IF @p_FechaIni = ''1800-01-01''
         SET @p_FechaIni = NULL
         
         
   		 IF @p_FechaFin = ''1800-01-01''
   		 SET @p_FechaFin = NULL
...
SET @vFechaFin = @p_FechaFin
       ...
       IF @vDiaProx <= 15
         ...
         WHERE GRL_DET_ENVIO_ESTCTA.LEGAL = @vLegal AND GRL_DET_ENVIO_ESTCTA.PERIODO = @vPeriodo
         ...
         BEGIN
         DELETE dbo.GRL_DET_ENVIO_ESTCTA
         WHERE GRL_DET_ENVIO_ESTCTA.LEGAL = @vLegal AND GRL_DET_ENVIO_ESTCTA.PERIODO = @vPeriodo AND
         GRL_DET_ENVIO_ESTCTA.JTSOID = @p_JTSOID

         DELETE dbo.GRL_CAB_ENVIO_ESTCTA
         WHERE GRL_CAB_ENVIO_ESTCTA.LEGAL = @vLegal AND GRL_CAB_ENVIO_ESTCTA.PERIODO = @vPeriodo AND
         GRL_CAB_ENVIO_ESTCTA.JTSOID = @p_JTSOID
         END
         ...
         DECLARE
          ...
          s.MONEDA, 
               ...
               FROM 
                  dbo.SALDOS  AS s, 
                  dbo.PLANCTAS  AS p, 
                dbo.GRL_ESTADOS_DE_CUENTA  AS E, 
                  dbo.PRODUCTOS  AS Pr
               WHERE 
                  s.TZ_LOCK = 0 AND 
                  p.TZ_LOCK = 0 AND 
                  ...
                  E.TZ_LOCK = 0 AND 
                  s.C1730 = p.C6326 AND 
                  s.C1785 BETWEEN 2 AND 5 AND 
                  s.CUENTA = E.CUENTA AND 
                  s.PRODUCTO = E.PRODUCTO AND 
                  s.MONEDA = E.MONEDA AND 
                  s.SUCURSAL = E.SUCURSAL AND 
                  s.OPERACION = E.OPERACION AND 
                  s.ORDINAL = E.ORDINAL AND 
                  s.PRODUCTO = Pr.C6250 AND 
                  Pr.C6250 = E.PRODUCTO AND 
                  (s.JTS_OID= @vJTSOID or isnull(@vJTSOID, -1) = -1) AND
                  (/* Obtener periodicidad dependiendo de si es Legal o no*/((E.PERIODICIDAD = @vPeriodo OR @vPeriodo = ''P'') AND @vLegal <> ''L'') OR /*OR (pr.PERIODOLEGALESTCTA=vPeriodo AND vLegal=''L'' AND E.PERIODICIDAD=vPeriodo)*/(
                  E.PERIODICIDAD = @vPeriodo AND 
                  E.TIPO_EMISION = ''N'' AND 
                  @vlegal = ''L'')) 
               ORDER BY 
                  s.C1803, 
                  s.SUCURSAL, 
                        ...
                        BEGIN
                           ...
                           BEGIN
                              ...
                              BEGIN TRY
                                 ...
                                 FROM dbo.CLE_CHEQUES_SALIENTE  AS C, dbo.CLE_DEPOSITOS  AS D
                                 WHERE 
                                    D.SALDO_JTS_OID = @OBTENER_IMPORTES_PENDIENTES$pJTSOID AND 
                                    D.NUMERO_DEPOSITO = C.NUMERO_DEPOSITO AND 
                                    C.ACREDITADO = 0 AND 
           							C.DESTINO_CHEQUE IN ( 1, 5 )
           ...
           /*Obtengo nombre sucursal*/
					...
					BEGIN
                        ...
                        BEGIN TRY
                          ...
                          FROM dbo.SUCURSALES
         WHERE SUCURSALES.SUCURSAL = @vSucursal AND SUCURSALES.TZ_LOCK = 0
					...
					BEGIN
                        ...
                        BEGIN TRY
                           ...
                           FROM dbo.PRODUCTOS
                           WHERE PRODUCTOS.C6250 = @vProd AND PRODUCTOS.TZ_LOCK = 0
             ...
             BEGIN TRY
  						   ...
  						   BEGIN
  						   SELECT @vMonedaCAB = C6400 FROM dbo.MONEDAS
                           WHERE C6399 = (SELECT MONNAC FROM PARAMETROS) AND TZ_LOCK = 0
  						   END
  						   ...
  						   BEGIN
                           SELECT @vMonedaCAB = C6400 FROM dbo.MONEDAS
                           WHERE C6399 = @vMoneda AND TZ_LOCK = 0
                           END
                           
                           SELECT @vResumenEn = C6400 FROM dbo.MONEDAS
                           WHERE C6399 = @vMoneda AND TZ_LOCK = 0
					...
					BEGIN
                        ...
                        BEGIN TRY
                           ...
                           FROM dbo.VTA_SALDOS
                           WHERE VTA_SALDOS.JTS_OID_SALDO = @vJTSOID AND VTA_SALDOS.TZ_LOCK = 0
        ...
        BEGIN
					...
					BEGIN
                       ...
                       -- BEGIN TRY
                           ...
                           FROM dbo.GRL_SALDOS_DIARIOS
                           WHERE GRL_SALDOS_DIARIOS.SALDOS_JTS_OID = @vJTSOID AND GRL_SALDOS_DIARIOS.TZ_LOCK = 0
...
--                        END CATCH
                     ...
                     BEGIN
                        ...
                        BEGIN TRY
						   ...
						   WHERE CODIGOCLIENTE = @vCliente AND TITULARIDAD = ''T''

                           SELECT @vnomcli = CLI_CLIENTES.NOMBRECLIENTE, @vTipoCliente = CLI_CLIENTES.TIPO
                           FROM dbo.CLI_CLIENTES
                           WHERE CLI_CLIENTES.CODIGOCLIENTE = @vCliente AND CLI_CLIENTES.TZ_LOCK = 0
    ...
    @vPiso = CLI_DIRECCIONES.PISO, 
                              @vCiudadLocalidad = (SELECT DESCRIPCION_DIM3 FROM CLI_LOCALIDADES WHERE CODIGOPAIS = CLI_DIRECCIONES.PAIS AND
                              DIM1 = CLI_DIRECCIONES.PROVINCIA AND DIM2 = CLI_DIRECCIONES.DEPARTAMENTO AND DIM3 = CLI_DIRECCIONES.LOCALIDAD),
                              @vProvincia = (SELECT DESCRIPCION FROM CLI_PROVINCIAS WHERE DIM1 = CLI_DIRECCIONES.PROVINCIA),
                              @vCodigoPais = CLI_DIRECCIONES.PAIS, 
                              @vCodigoPostal = CLI_DIRECCIONES.CPA_NUEVO,
                              @vBarrio = CLI_DIRECCIONES.BARRIO
                           FROM dbo.CLI_DIRECCIONES
                           WHERE 
                              CLI_DIRECCIONES.ID = @vNroPersona AND 
                              CLI_DIRECCIONES.TIPODIRECCION = @vTipoDireccion AND 
                              CLI_DIRECCIONES.FORMATO = @vFormato AND
                              CLI_DIRECCIONES.ORDINAL_DIR = (SELECT MIN(DIR.ORDINAL_DIR) FROM CLI_DIRECCIONES DIR WHERE 
                              DIR.ID = @vNroPersona AND DIR.TIPODIRECCION = @vTipoDireccion AND DIR.FORMATO = @vFormato)
							  AND CLI_DIRECCIONES.TZ_LOCK = 0
    ...
    DECLARE
                     ...
                     BEGIN
						...
						SELECT @vCantTitulares = count(*) FROM VW_CLIENTES_PERSONAS WHERE CODIGOCLIENTE = @vCliente
						   
						SELECT @vDocTitular = NUMERODOC, @vNomTitular = NOMBRE FROM VW_CLIENTES_PERSONAS 
						WHERE CODIGOCLIENTE = @vCliente AND TITULARIDAD = ''T''
						
						IF(@vCantTitulares = 2)
						SELECT TOP 1 @vDocTitular2 = NUMERODOC,@vNomTitular2 = NOMBRE FROM VW_CLIENTES_PERSONAS 
						WHERE CODIGOCLIENTE = @vCliente AND TITULARIDAD != ''T'' 
						...
						(
						 ...
						 FROM VW_CLIENTES_PERSONAS WHERE CODIGOCLIENTE = @vCliente AND TITULARIDAD != ''T'' 
               ...
               DECLARE
                   cMovsContables CURSOR LOCAL FOR 
                     SELECT 
                        ...
                        M.COD_TRANSACCION
                     FROM dbo.MOVIMIENTOS_CONTABLES  AS M, dbo.ASIENTOS  AS A
                     WHERE 
                        M.FECHAPROCESO BETWEEN @vFechaIni AND @vFechaFin AND 
                        M.SUCURSAL_CUENTA = @vSucursal AND 
                        M.RUBROCONTABLE = @vRubro AND 
                        M.MONEDA = @vMoneda AND 
                        M.CLIENTE = @vCliente AND 
                        M.CUENTA = @vCuenta AND 
                        M.OPERACION_CUENTA = @vOperacion AND 
                        M.ORDINAL_CUENTA = @vOrdinal AND 
 ...
 @vMonto, 
      ...
      @vOperacionTopaz,
					...
					BEGIN
                        ...
                        BEGIN
                           ...
                           FROM TTR_CODIGO_TRANSACCION_DEF
                           WHERE TZ_LOCK = 0 AND CODIGO_TRANSACCION = @vCodTransaccion
                           END 
                             ELSE 
						BEGIN
						PRINT ''Es NULL''
                     ...
                     /* OBTENGO CREDEB Y SIRCREB */
                        ...
                        /* CREDEB */
						IF @vCodTransaccion IN (SELECT CODIGO_TRANSACCION FROM CI_CARGOS WHERE TIPO_CARGO_IMPOSITIVO = 6) OR @vCodTransaccion IN (SELECT CODIGO_TRANSACCION FROM CI_IMPUESTOS WHERE TIPO_IMPUESTO = 6)
						BEGIN
						SET @vCREDEBAUX = NULL
						declare @vFechaCredeb date
						SET @vFechaCredeb = (Select CONVERT(varchar,@vFechaProcesado,23))
						PRINT @vFechaCredeb
                        SELECT @vCREDEBAUX = MONTO_FACTURABLE FROM CI_FACTURA_LINEA WHERE ASIENTO = @vAsiento 
                        AND OPERACION = @vOperacionTopaz AND SUCURSAL_ASIENTO = @vSucursalOrigen
                        --AND FECHA_PROCESO_ASIENTO = @vFechaProcesado AND 
                        AND FECHA_PROCESO_ASIENTO = @vFechaCredeb 
                        AND (ID_CARGO IN (SELECT ID_CARGO FROM CI_CARGOS WHERE TIPO_CARGO_IMPOSITIVO = 6) OR ID_IMPUESTO IN (SELECT ID_IMPUESTO FROM CI_IMPUESTOS WHERE TIPO_IMPUESTO = 6))
                        PRINT ''CREDEB''
                        PRINT @vCREDEBAUX
                        PRINT @vAsiento
						...
						IF @vCodTransaccion IN (SELECT CODIGO_TRANSACCION FROM CI_CARGOS WHERE TIPO_CARGO_IMPOSITIVO = 5) OR @vCodTransaccion IN (SELECT CODIGO_TRANSACCION FROM CI_IMPUESTOS WHERE TIPO_IMPUESTO = 5)
						BEGIN
						SET @vSIRCREBAUX = NULL
						declare @vFechasircreb date
						SET @vFechasircreb = (Select CONVERT(varchar,@vFechaProcesado,23))
						PRINT @vFechaCredeb
                        SELECT @vSIRCREBAUX = MONTO_FACTURABLE FROM CI_FACTURA_LINEA WHERE ASIENTO = @vAsiento 
                        AND OPERACION = @vOperacionTopaz AND SUCURSAL_ASIENTO = @vSucursalOrigen
                        AND FECHA_PROCESO_ASIENTO = @vFechasircreb AND 
                        (ID_CARGO IN (SELECT ID_CARGO FROM CI_CARGOS WHERE TIPO_CARGO_IMPOSITIVO = 5) OR ID_IMPUESTO IN (SELECT ID_IMPUESTO FROM CI_IMPUESTOS WHERE TIPO_IMPUESTO = 5))
  ...
  END
                     ...
                     IF @vCodTransaccion IN (48,49,50)
                        ...
                        SELECT @vSucursalPF = SUCURSALMOV, @vCapitalPF = CAPITALORIGINAL, 
                        @vInteresesPF = INTALVTO
                        FROM BS_HISTORIA_PLAZO WHERE NROASIENTOMOV = @vAsiento
         ...
         WHERE D.LEGAL = ''L'' AND NOT EXISTS 
            (
              ...
              C.CODIGOCLIENTE, 
                  ...
                  C.FECHAEMISION, 
                  C.PROCESADO
               FROM dbo.GRL_CAB_ENVIO_ESTCTA  AS C
               WHERE 
                  C.LEGAL = D.LEGAL AND 
                  C.PERIODO = D.PERIODO AND 
                  C.IDCAB = D.IDCAB AND 
                  C.CODIGOPAIS IN ( 0, (SELECT NUMERICO FROM PARAMETROSGENERALES WHERE CODIGO = 1) ) --C.CODIGOPAIS IN ( 0, 858 )
            )
