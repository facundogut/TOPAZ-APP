...
BEGIN  
      ...
      BEGIN
      	...
      	DECLARE @CodRechazo NUMERIC(2)=  CAST(@PuntoIntercambio AS NUMERIC);
      	...
      	BEGIN
      		...
      		UPDATE dbo.CLE_CHEQUES_AJUSTE SET ESTADO_AJUSTE = ''R'' WHERE ESTADO_AJUSTE IS NULL AND TRACKNUMBER = @ContadorRegistros AND FECHA_ACREDITACION = (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)) AND TZ_LOCK = 0;
      	END
      	
      	ELSE IF (CAST(@CuentaDebitar AS NUMERIC) = 77777777777)
      	BEGIN
   			
      		SELECT @EXISTE = 1, @TIPO_DOCUMENTO = TIPO_DOCUMENTO, @NRO_DPF_CHEQUE = NUMERO_DPF, @BANCO_GIRADO = BANCO_GIRADO, @SUCURSAL_BANCO = SUCURSAL_BANCO_GIRADO, @FECHA_ALTA = FECHA_ALTA, @IMPORTE_TOTAL = IMPORTE, @CODIGO_POSTAL = COD_POSTAL  
      		FROM CLE_DPF_SALIENTE WITH(NOLOCK) WHERE TRACKNUMBER = @ContadorRegistros AND TZ_LOCK = 0; 
   			...
   			BEGIN
   				-- Insertamos en el historial
   			   	...
   			   	VALUES(@TICKET, @FECHA_ALTA, @BANCO_GIRADO, @SUCURSAL_BANCO, CAST(@CuentaDebitar AS NUMERIC), @IMPORTE_TOTAL, @CODIGO_POSTAL, @NRO_DPF_CHEQUE, @PuntoIntercambio, @ContadorRegistros, ''D'', @MONEDA, @TIPO_DOCUMENTO, @CodRechazo);
      	...
      	BEGIN
      		...
      		SELECT @EXISTE = 1, @NRO_DPF_CHEQUE= NRO_CHEQUE, @SERIE_DEL_CHEQUE = SERIE_DEL_CHEQUE, 
      		@BANCO_GIRADO = BANCO, @FECHA_ALTA = FECHA_PRESENTADO, @SUCURSAL_BANCO = SUCURSAL, @NRO_CUENTA = CUENTA, 
      		@TIPO_DOCUMENTO = TIPO_DOCUMENTO, @IMPORTE_TOTAL = IMPORTE, @CODIGO_POSTAL = CODIGO_POSTAL, @MONEDA = MONEDA
      		FROM ITF_COELSA_CHEQUES_OTROS WITH(NOLOCK) WHERE TRACE_NUMBER = @ContadorRegistros;
      		...
      		IF @EXISTE > 0
   			BEGIN
   				-- Insertamos en el historial
   			   	...
   			   	VALUES(@TICKET, @FECHA_ALTA, @BANCO_GIRADO, @SUCURSAL_BANCO, CAST(@CuentaDebitar AS NUMERIC), @IMPORTE_TOTAL, @CODIGO_POSTAL, @NRO_DPF_CHEQUE, @PuntoIntercambio, @ContadorRegistros, ''C'', @MONEDA, @TIPO_DOCUMENTO, @CodRechazo, @SERIE_DEL_CHEQUE);
...
UPDATE dbo.CLE_CHEQUES_AJUSTE SET ESTADO_AJUSTE = ''A'' WHERE (ESTADO_AJUSTE IS NULL OR ESTADO_AJUSTE <> ''R'') AND ESTADO = ''P'' AND FECHA_ACREDITACION = (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)); 
------------------------------------------------------------------------------------------------------------------------------
