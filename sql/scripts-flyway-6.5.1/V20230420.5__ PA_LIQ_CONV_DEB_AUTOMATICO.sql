   ...
   BEGIN
      ...
      BEGIN TRY
			...
			BEGIN
				...
				SELECT @ACRED = Acreditacion FROM CONV_CONVENIOS_REC C 
					JOIN @Tabla_CAB_DEBITOSAUTOMATICOS CAB
		 			ON C.Id_ConvRec = CAB.CONVENIO 
		 			AND CAB.ID=@IdTemp  
				...
				--COMPLETO TABLA AUXILIAR DE DETALLES (ACRED DIA)--
					...
					SELECT DISTINCT 
						det.ID_CABEZAL,
				      	det.ID_LINEA,
				      	det.ID_LIQUIDACION 
				    FROM REC_DET_DEBITOSAUTOMATICOS det 
					JOIN @Tabla_CAB_DEBITOSAUTOMATICOS cab ON det.ID_CABEZAL = cab.ID AND cab.ID = @IdTemp
					WHERE det.ESTADO = ''P'' AND det.FECHA_COBRANZA = (SELECT FECHAPROCESO FROM PARAMETROS)
				END
				...
				BEGIN
					...
					AND RDT.FECHA_COBRANZA = (SELECT FECHAPROCESO FROM PARAMETROS)
				END
				ELSE
				BEGIN
					SELECT @SUMA_IMPORTE = sum(RDT.IMPORTE) , @CANTIDAD_LIQ = count(*) 
			   ...
			   BEGIN	  
				...
				BEGIN
					...
					FROM REC_DET_DEBITOSAUTOMATICOS rdt
					JOIN @Tabla_DET_DEBITOSAUTOMATICOS det
						ON rdt.ID_CABEZAL = det.ID_CABEZAL
						AND rdt.ID_LINEA = det.ID_LINEA
					JOIN @Tabla_CAB_DEBITOSAUTOMATICOS cab
						ON cab.ID = det.ID_CABEZAL
					WHERE rdt.FECHA_COBRANZA = (SELECT FECHAPROCESO FROM PARAMETROS)
				END
			  END 
			  
			  /* Actualizo ID liquidacion si la acreditacion es DIA
			  	y todavia quedan detalles por procesar  */
			  IF(@ACRED = ''DIA''AND @CANT_CAB != 0)
			  BEGIN
				...
				-- actualizo detalles con el id liquidacion
				UPDATE REC_DET_DEBITOSAUTOMATICOS
				SET ID_LIQUIDACION = @v_numerador
				FROM REC_DET_DEBITOSAUTOMATICOS rdt
				JOIN @Tabla_DET_DEBITOSAUTOMATICOS det
