...
IF (SELECT DB_NAME()) = @BD
  BEGIN
	...
	if exists (	select a.CODIGO_SOCIEDAD 
				from CLI_TIPO_SOCIEDAD a with(nolock) 
				where a.CODIGO_SOCIEDAD not in (	select b.TIPO_SOCIEDAD
												from CLI_ROLES_OBL_CARGO b with(nolock))
				)
				select  ''Tabla CLI_TIPO_SOCIEDAD no cumple la integridad referencial con CLI_ROLES_OBL_CARGO  '',* 
				from CLI_TIPO_SOCIEDAD a with(nolock) 
				where a.CODIGO_SOCIEDAD not in (
													select b.TIPO_SOCIEDAD
													from CLI_ROLES_OBL_CARGO b with(nolock))
	else
	print ''Comprobación OK. Tabla CLI_TIPO_SOCIEDAD con CLI_ROLES_OBL_CARGO ''
