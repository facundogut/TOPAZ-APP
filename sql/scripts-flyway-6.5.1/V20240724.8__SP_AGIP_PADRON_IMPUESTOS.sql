...
BEGIN	
	...
	WHERE (IIBB_CABA_PERCEPCION_COND IN (''NA'', NULL) OR IIBB_CABA_RETENCION_COND IN (''NA'', NULL))
	AND ESTADO=0
	...
	SET IIBB_CABA_PERCEPCION_COND = CASE WHEN convert(NUMERIC(11,7),replace(t.IIBB_CABA_PERCEPCION_ALI,'','',''''))/100=0 THEN ''EX'' ELSE ''AC'' END
		...
		, IIBB_CABA_RETENCION_COND = CASE WHEN convert(NUMERIC(11,7),replace(t.IIBB_CABA_retencion_ALI,'','',''''))/100=0 THEN ''EX'' ELSE ''AC'' END
...
--								, t.letra_alicuota 
	...
	DELETE FROM ITF_AGIP_PADRON WHERE Fecha_Publicacion=(SELECT TOP 1 fecha_public FROM ITF_AGIP_PADRON_IMPUESTOS_AUX)
   	...
   	BEGIN 
   		...
   		DELETE FROM ITF_AGIP_PADRON WHERE try_CONVERT(DATE, SUBSTRING(fecha_publicacion,5,4)+''-'' +SUBSTRING(fecha_publicacion,3,2)+''-''+SUBSTRING(fecha_publicacion,1,2))<@fechaLimite
   	END 
   	-----------------------------------------------------------------------------------------------------------
