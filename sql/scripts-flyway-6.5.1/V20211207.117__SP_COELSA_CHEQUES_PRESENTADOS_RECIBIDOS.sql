EXECUTE('DROP PROCEDURE dbo.SP_COELSA_CHEQUES_PRESENTADOS_RECIBIDOS;')
EXECUTE('
CREATE PROCEDURE dbo.SP_COELSA_CHEQUES_PRESENTADOS_RECIBIDOS @TICKET NUMERIC(16)
AS
BEGIN
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
BEGIN  
      ...
      BEGIN
      	...
      	VALUES (@TICKET, @TICKET, (SELECT FECHAPROCESO FROM PARAMETROS), @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @Importe, @CodigoPostal, @FechaPresentacion, @FechaVencimiento, @NumeroCheque, @PuntoIntercambio, @ContadorRegistros, ''P'', (CASE WHEN CAST(@CuentaDebitar AS NUMERIC) = 88888888888 THEN ''A'' WHEN CAST(@CuentaDebitar AS NUMERIC) = 77777777777 THEN ''D'' ELSE ''C'' END));
      	...
      	BEGIN
      		...
      		VALUES((SELECT MAX(ORDINAL) + 1 FROM CLE_CHEQUES_AJUSTE), @NumeroCheque, @Entidad, @CuentaDebitar, @Sucursal, ''I'', @CodigoPostal, (SELECT FECHAPROCESO FROM PARAMETROS), @Sucursal, @Importe, @PuntoIntercambio, ''R'', @ContadorRegistros);
      	END
      END
