Execute('CREATE OR ALTER PROCEDURE [dbo].[SP_BCRA_PADFYJ_FULL]  
   @P_TICKET  NUMERIC(12),
...
AS 
   BEGIN
	...
	/* Se modifican los registros en la tabla de fallecidos cuando corresponden */
	...
	(b.MARCA_FALLECIDO=''1'' OR b.MARCA_FALLECIDO=''2'') AND b.CUIL<30000000000 AND b.CUIL>19999999999 AND EXISTS (SELECT 1 FROM CLI_MAESTRO_FALLECIDOS (nolock) 
	WHERE CUIL=b.CUIL AND (ESTADO<>''P'' OR (ESTADO<>''I'' AND TIPO_MOV<>''A''))) AND ISNUMERIC(b.CUIL)=1



	/* Se hace insert de todos los registros nuevos fallecidos informados con la MARCA_FALLECIDO = 1 */
	
	INSERT INTO dbo.CLI_MAESTRO_FALLECIDOS (CUIL, NOMBRE, TIPO_MOV, CANAL, TZ_LOCK, ENTIDAD_DENUN, ESTADO, FECHA_ALTA, TICKET )
	SELECT b.CUIL, ISNULL(MAX(b.NOMBRE),'' ''), ''A'', ''B'', 0, ''BCRA'', ''I'', @V_FECHAPROCESO, @P_TICKET
	...
	WHERE (b.MARCA_FALLECIDO=''1'' OR b.MARCA_FALLECIDO=''2'') AND b.CUIL<30000000000 AND b.CUIL>19999999999 AND NOT EXISTS (SELECT 1 FROM CLI_MAESTRO_FALLECIDOS (nolock) WHERE CUIL=b.CUIL ) AND ISNUMERIC(b.CUIL)=1
	GROUP BY b.CUIL
	
	INSERT INTO dbo.CLI_MAESTRO_FALLECIDOS (CUIL, NOMBRE, TIPO_MOV, CANAL, TZ_LOCK, ENTIDAD_DENUN, ESTADO, FECHA_ALTA, TICKET )
	SELECT b.CUIL, ISNULL(MAX(b.NOMBRE),'' ''), ''A'', ''B'', 0, ''BCRA'', ''P'', @V_FECHAPROCESO, @P_TICKET
	...
	/* Se hace insert de todos los registros sin problemas en la tabla ITF_BCRA_PADFYJ */
	...
	SELECT @V_FECHAPROCESO, @V_HORA, @P_USUARIO, ''BCRA'', ''CUIL'', b.CUIL, b.NOMBRE, ''DNI'', (SELECT SUBSTRING(CONVERT(VARCHAR(20),b.CUIL),3,8)), ''Alta'', ''Persona existente en el sistema. Ya fue dada de alta como fallecida.'', @P_TICKET 
	...
	AND b.CUIL<30000000000 AND b.CUIL>19999999999 AND ISNUMERIC(b.CUIL)=1 AND (b.MARCA_FALLECIDO=''1'' OR b.MARCA_FALLECIDO=''2'')
