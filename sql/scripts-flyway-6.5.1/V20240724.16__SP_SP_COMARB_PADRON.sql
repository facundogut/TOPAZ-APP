...
BEGIN
	...
	INNER JOIN VW_CLIENTES_PERSONAS V WITH (NOLOCK) ON C.CODIGOCLIENTE=V.CODIGOCLIENTE
	...
	FROM CLI_PersonasFisicas PF WITH (NOLOCK)
	INNER JOIN VW_CLI_PERSONAS p WITH (NOLOCK) ON pf.NUMEROPERSONAFISICA = p.[Numero de Persona]
	...
	SET SIRCREB=a.LETRA_ALICUOTA	
	FROM CLI_Personasjuridicas PF WITH (NOLOCK)
	INNER JOIN VW_CLI_PERSONAS p WITH (NOLOCK) ON pf.NUMEROPERSONAjuridica = p.[Numero de Persona]
	INNER JOIN ITF_COMARB_PADRON_IMPUESTOS_AUX A WITH (NOLOCK) ON p.[Numero de Documento]=a.CUIT
	...
	FROM 
			(SELECT  DISTINCT p.[Numero de Persona] AS numeroPersona
				...
				inner JOIN VW_CLI_PERSONAS p WITH (NOLOCK) ON f.NUMEROPERSONAFJ = p.[Numero de Persona]
			) t
