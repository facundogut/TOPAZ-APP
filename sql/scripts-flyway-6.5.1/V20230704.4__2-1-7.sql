EXECUTE('
ALTER     PROCEDURE dbo.[SP_AFIP_IVA_RG18]
...
OPEN cursorOk
    ...
    BEGIN 
		...
		BEGIN  	
			...
			SET @hayUpdate = (SELECT COUNT(*) FROM CI_CARGOS_TARIFAS A JOIN VW_CARGOS_IMPUESTOS B ON A.ID_CARGO = B.ID_CARGO  WHERE a.id_cliente = @codCli AND a.TASA = 0 AND TZ_LOCK = 0 AND (ISNULL(fecha_hasta, ''19000101'') <>  ISNULL(@fechaHasta, ''19000101'') or ISNULL(fecha_desde, ''19000101'') <> ISNULL(@fechaDesde, ''19000101'')) And a.ID_CARGO IN (SELECT ID_CARGO FROM VW_CARGOS_IMPUESTOS));				
			...
			BEGIN													
				...
				WHERE id_cliente = @codCli AND TASA = 0 AND TZ_LOCK = 0 AND (ISNULL(fecha_hasta, ''19000101'') <>  ISNULL(@fechaHasta, ''19000101'') or ISNULL(fecha_desde, ''19000101'') <> ISNULL(@fechaDesde, ''19000101''))	And ID_CARGO IN (SELECT ID_CARGO FROM VW_CARGOS_IMPUESTOS);
		   	END
