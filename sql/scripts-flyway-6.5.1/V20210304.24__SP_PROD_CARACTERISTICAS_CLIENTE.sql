...
/****** Object:  StoredProcedure [dbo].[SP_PROD_CARACTERISTICAS_CLIENTE]    Script Date: 23/02/2021 14:04:13 ******/
DROP PROCEDURE [dbo].[SP_PROD_CARACTERISTICAS_CLIENTE]
GO
...
CREATE PROCEDURE [dbo].[SP_PROD_CARACTERISTICAS_CLIENTE]
/*
JMB 23/02/2021
Se comentan los casos para personas jurídicas en los campos EDAD y ESTADO_CIVIL.
No se estaba entrando a los case ya que los integrantes de Personas Jurídicas ya no son titulares.
Se determina que no se necesitan determinar esos campos para los casos de PJ.
---https://topsystems.atlassian.net/browse/NBCHSEG-737
*/
  ...
  OPEN EdadesIntegrantesCur;
			...
			WHILE @@FETCH_STATUS = 0 
				...
				IF @EDADREC>=	(	SELECT numerico 
					...
					AND @TITULARIDAD=@TITULARIDADREG
			    ...
			    IF (@ENCONTROEMANCIPADO_ > 0 OR @ESTADOCIVILREG<>'S')	AND @EDADREC<	(
																						SELECT numerico 
																						FROM PARAMETROSGENERALES with (nolock) 
																						WHERE CODIGO=13 
																						)
	    		...
	    		FETCH NEXT FROM EdadesIntegrantesCur 
							INTO @NUMEROPERSONAREC,@TIPOPERSONAREG,@TITULARIDADREG,@EDADREC,@ESTADOCIVILREG;
