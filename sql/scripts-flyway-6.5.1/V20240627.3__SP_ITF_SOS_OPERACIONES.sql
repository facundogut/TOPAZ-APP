...
(CASE WHEN mov.MONEDA = 1 THEN 
        (CASE WHEN sa.C1785 = 4 AND mov.OPERACION = 5001 THEN 
				(SELECT CAPITALORIGINAL FROM BS_HISTORIA_PLAZO BS WHERE BS.FECHAPROCESOMOV = mov.FECHAPROCESO AND BS.SUCURSALMOV = mov.SUCURSAL AND BS.NROASIENTOMOV = mov.ASIENTO)
			  WHEN sa.C1785 = 4 AND mov.OPERACION = 8620 THEN
			    (SELECT SUM(CAPITALORIGINAL) - (SUM(CAPITALPAGADO) + SUM(INTERESPAGADO)) FROM BS_HISTORIA_PLAZO BS WHERE BS.FECHAPROCESOMOV = mov.FECHAPROCESO AND BS.SUCURSALMOV = mov.SUCURSAL AND BS.NROASIENTOMOV = mov.ASIENTO)
...
(CASE WHEN sos.Subsistema IN (''PF'',''CE'',''ME'') THEN '''' 
      ...
      WHEN sos.Subsistema IN (''CC'',''AC'') AND sos.TIPOPERACION IN (501, 503) 
         THEN (CASE WHEN (SELECT COUNT(DISTINCT CLIENTE) FROM MOVIMIENTOS_CONTABLES mc 
...
--18
sos.CodTransaccion AS CODIGOSTOPAZ,
--19
(CASE WHEN mov.MONEDA = 1 THEN 0 ELSE 
  (CASE WHEN sos.SubSistema IN (''AC'',''CC'',''TC'',''ME'',''TT'') THEN (SELECT TipC.TIPO_CAMBIO_OFICIAL FROM HISTORICOTIPOSCAMBIO TipC WHERE TipC.FECHA_COTIZACION = mov.FECHAPROCESO AND Tipc.MONEDA = sa.MONEDA AND Tipc.TZ_LOCK = 0) 
...
LEFT((
CASE WHEN dbo.FN_RTE_SOS_OPERACIONES(mov.COD_TRANSACCION, mov.CAPITALREALIZADO) > 0 
          THEN (SELECT (CASE WHEN vmov.TIPO_PERSONA_I = ''F'' THEN ''PF'' WHEN vmov.TIPO_PERSONA_I = ''J'' THEN ''PJ'' END) FROM VTA_CUMPLIMIENTO vmov WHERE vmov.SUCURSAL = mov.SUCURSAL
		       AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO)
	ELSE '''' END
...
LEFT((
CASE WHEN dbo.FN_RTE_SOS_OPERACIONES(mov.COD_TRANSACCION, mov.CAPITALREALIZADO) > 0 
           THEN (SELECT vmov.APELLIDO_I FROM VTA_CUMPLIMIENTO vmov WHERE vmov.SUCURSAL = mov.SUCURSAL
		       AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO)
	ELSE '''' END
...
LEFT((
CASE WHEN dbo.FN_RTE_SOS_OPERACIONES(mov.COD_TRANSACCION, mov.CAPITALREALIZADO) > 0 
           THEN (SELECT vmov.NOMBRE_I FROM VTA_CUMPLIMIENTO vmov WHERE vmov.SUCURSAL = mov.SUCURSAL
		       AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO)
	ELSE '''' END
...
LEFT((
CASE WHEN dbo.FN_RTE_SOS_OPERACIONES(mov.COD_TRANSACCION, mov.CAPITALREALIZADO) > 0 
          THEN (SELECT (CASE WHEN vmov.TIPO_PERSONA_M = ''F'' THEN ''PF'' WHEN vmov.TIPO_PERSONA_I = ''J'' THEN ''PJ'' END) FROM VTA_CUMPLIMIENTO vmov WHERE vmov.SUCURSAL = mov.SUCURSAL
		       AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO)
	ELSE '''' END
...
LEFT((
CASE WHEN dbo.FN_RTE_SOS_OPERACIONES(mov.COD_TRANSACCION, mov.CAPITALREALIZADO) > 0 
           THEN (SELECT concat(vmov.APELLIDO_M,'' '', vmov.NOMBRE_M) FROM VTA_CUMPLIMIENTO vmov WHERE vmov.SUCURSAL = mov.SUCURSAL
		       AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO)
	ELSE '''' END
...
LEFT((
CASE WHEN dbo.FN_RTE_SOS_OPERACIONES(mov.COD_TRANSACCION, mov.CAPITALREALIZADO) > 0 
        THEN (SELECT vmov.APELLIDO_M FROM VTA_CUMPLIMIENTO vmov WHERE vmov.SUCURSAL = mov.SUCURSAL
		       AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO)
	ELSE '''' END
...
LEFT((
CASE WHEN dbo.FN_RTE_SOS_OPERACIONES(mov.COD_TRANSACCION, mov.CAPITALREALIZADO) > 0 
        THEN (SELECT vmov.NOMBRE_M FROM VTA_CUMPLIMIENTO vmov WHERE vmov.SUCURSAL = mov.SUCURSAL
		       AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO)
	ELSE '''' END
...
CASE WHEN dbo.FN_RTE_SOS_OPERACIONES(mov.COD_TRANSACCION, mov.CAPITALREALIZADO) > 0  
           THEN (SELECT (CASE WHEN vmov.TIPO_PERSONA_M = ''F'' THEN ''96''
						      WHEN vmov.TIPO_PERSONA_M = ''J'' THEN ''80'' END) FROM VTA_CUMPLIMIENTO vmov WHERE vmov.SUCURSAL = mov.SUCURSAL
		       AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO)
	ELSE '''' END
...
CASE WHEN dbo.FN_RTE_SOS_OPERACIONES(mov.COD_TRANSACCION, mov.CAPITALREALIZADO) > 0  
           THEN (SELECT (CASE WHEN vmov.TIPO_PERSONA_M = ''F'' THEN SUBSTRING(CONVERT(VARCHAR,vmov.NRO_DOC_M),3,8)
						      WHEN vmov.TIPO_PERSONA_M = ''J''  THEN SUBSTRING(CONVERT(VARCHAR,vmov.NRO_DOC_M),1,10) END) FROM VTA_CUMPLIMIENTO vmov WHERE vmov.SUCURSAL = mov.SUCURSAL
		       AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO)
	ELSE '''' END
...
(select sucx.SucursalBCRA from SOS_SUCURSALES sucx where sucx.sucursal = sa.SUCURSAL and sucx.TZ_LOCK = 0) AS SUCURSALSISCENCTA,
--38
LEFT((CASE WHEN sos.IND_RTE = ''N'' THEN '''' ELSE ''Titular'' END), 30) AS REL_FON_TIT,
--39
LEFT((CASE WHEN sos.IND_RTE = ''N'' THEN '''' ELSE (CASE WHEN (SELECT COUNT(1) FROM VW_CLIENTE_PERSONAS V 
  														inner join VTA_CUMPLIMIENTO vmov on V.TIPODOCUMENTO = vmov.TIPO_DOC_I AND V.NUMERODOCUMENTO= vmov.NRO_DOC_I
														   WHERE vmov.SUCURSAL = mov.SUCURSAL AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO
														     AND  V.CODIGOCLIENTE = sa.C1803) > 0 THEN ''S'' ELSE ''N'' END) END), 30) AS REL_PRO_TIT,
--40
LEFT((CASE WHEN sos.IND_RTE = ''N'' THEN '''' ELSE ''Operador'' END), 30) AS REL_FON_OPER,
--41
LEFT((CASE WHEN sos.IND_RTE = ''N'' THEN '''' ELSE (CASE WHEN (SELECT COUNT(1) FROM VW_CLIENTE_PERSONAS V 
  															inner join VTA_CUMPLIMIENTO vmov on V.TIPODOCUMENTO = vmov.TIPO_DOC_M AND V.NUMERODOCUMENTO= vmov.NRO_DOC_M
														   WHERE vmov.SUCURSAL = mov.SUCURSAL AND vmov.ASIENTO = mov.ASIENTO AND vmov.FECHA_PROCESO = mov.FECHAPROCESO
														     AND  V.CODIGOCLIENTE = sa.C1803) > 0 THEN ''S'' ELSE ''N'' END) END), 30) AS REL_PRO_OPER
--SELECT mov.*
...
--12
LEFT((SELECT m.C6402 FROM MONEDAS m WHERE m.C6399 = (CASE WHEN sa.MONEDA IN (988,999) THEN 1 ELSE sa.MONEDA END)),10) AS ESPECIETRANSADATIPO,
--13
'''' AS BANCOCORRESPONSALSWIFT,
--14
'''' AS BENEFICIARIOORDENANTEDELEXTE,
...
--18
tra.CODTRANSACCION AS CODIGOSTOPAZ,
--19
(CASE WHEN mov.PAGO_MN > 0 AND mov.PAGO_ME = 0 THEN 0
  ELSE (SELECT TipC.TIPO_CAMBIO_OFICIAL FROM HISTORICOTIPOSCAMBIO TipC 
...
--SUCURSAL BCRA
(SELECT sucx.SucursalBCRA FROM SOS_SUCURSALES sucx where sucx.sucursal = B.SUC_CUENTA_COBRO and sucx.TZ_LOCK = 0) AS SUCURSALSISCENCTA,
--38
'''' AS REL_FON_TIT,
--39
''NO'' AS REL_PRO_TIT,
...
FROM TJC_HISTORICO_PAGOS mov WITH (NOLOCK)
  INNER JOIN MOVIMIENTOS_CONTABLES movc WITH (NOLOCK) ON movc.FECHAPROCESO = mov.FECHA AND movc.ASIENTO = mov.ASIENTO
  	/*AND movc.OPERACION = 2854*/ AND movc.SUCURSAL_CUENTA = (SELECT NUMERICO FROM PARAMETROSGENERALES WHERE CODIGO = 400)
  INNER JOIN SOS_TRANSACCIONES tra ON tra.CodTransaccion = movc.COD_TRANSACCION 
...
--12
LEFT((SELECT m.C6402 FROM MONEDAS m WHERE m.C6399 = (CASE WHEN mov.CODMONEDA IN (988,999) THEN 1 ELSE mov.CODMONEDA END)),10) AS ESPECIETRANSADATIPO,
--13
mov.BANCOCORRESP AS BANCOCORRESPONSALSWIFT,
--14
mov.BENEFORDENEXT AS BENEFICIARIOORDENANTEDELEXTE,
--15
(SELECT pais.CODIGOSWIFT FROM CLI_PAISES pais WHERE pais.CODIGOPAIS = mov.CODPAISBENEF AND pais.TZ_LOCK = 0) AS PAISDELBENEFICIARIOORDENANTE,
--16
'''' AS DATOSADICIONALES,
--17
...
(CASE WHEN mov.CODMONEDA = 1 THEN 0
  ELSE 
   (SELECT TipC.TIPO_CAMBIO_OFICIAL FROM HISTORICOTIPOSCAMBIO TipC 
    WHERE TipC.FECHA_COTIZACION = mov.FECINGRESO AND Tipc.MONEDA = mov.CODMONEDA AND Tipc.TZ_LOCK = 0)
END)
  AS COTIZACION,
--20
...
--SUCURSAL BCRA
(select bx.SucursalBCRA from SOS_SUCURSALES bx WHERE bx.SUCURSAL = b.CODSUCURSAL and bx.TZ_LOCK = 0) AS SUCURSALSISCENCTA,
--38
'''' AS REL_FON_TIT,
--39
''NO'' AS REL_PRO_TIT,
...
FROM ITF_I2000_MOVS_HIST mov WITH (NOLOCK)
  ...
  INNER JOIN SOS_TRANSACCIONES trx WITH (NOLOCK) ON trx.Subsistema = ''ME''
WHERE mov.FECINGRESO = @Fecha
  ...
  AND NOT EXISTS(SELECT 1 FROM CLI_CLIENTES cli WITH(NOLOCK)
   					INNER JOIN CLI_ClientePersona p WITH(NOLOCK) ON p.CODIGOCLIENTE = cli.CODIGOCLIENTE
  					INNER JOIN CLI_PERSONASFISICAS nat WITH(NOLOCK) ON p.NUMEROPERSONA = nat.NUMEROPERSONAFISICA
  				 WHERE cli.CODIGOCLIENTE = C.CODCLIENTE
  				   AND nat.SECTOR IN (6,7))
  AND NOT EXISTS(SELECT 1 FROM CLI_CLIENTES cli WITH(NOLOCK)
   					INNER JOIN CLI_ClientePersona p WITH(NOLOCK) ON p.CODIGOCLIENTE = cli.CODIGOCLIENTE
  					INNER JOIN CLI_PERSONASJURIDICAS jur WITH(NOLOCK) ON p.NUMEROPERSONA = jur.NUMEROPERSONAJURIDICA
