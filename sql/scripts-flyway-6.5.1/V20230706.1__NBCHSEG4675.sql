...
--Se realizaron modificaciones - NBCHSEG-3999 - AE (Se corrige control 002 y 003)
--Se realizaron modificaciones - NBCHSEG-4023 - AE (Se corrige control 005)
--Se realizó modificación por NBCHSEG-4675 - AE (Casos que no determinaba el JTS_OID, la culpa era del kettle, validación de cta incorrecta en step js, se paso la val para acá)
...
BEGIN
    ...
    BEGIN
		...
		IF ISNUMERIC(@NRO_CUENTA) = 1
		BEGIN
			SET @cant_cuenta = (
				SELECT COUNT(*)
				FROM SALDOS WITH (NOLOCK)
				WHERE CUENTA = Cast(@NRO_CUENTA as numeric)
					AND MONEDA = 1
        ...
        (
            SELECT count(*)
            FROM SALDOS AS s,
                 GRL_BLOQUEOS AS g WITH (NOLOCK)
            WHERE C1679 = ''1''
                  AND s.JTS_OID = g.SALDO_JTS_OID
                  AND s.CUENTA = @NRO_CUENTA
                  AND s.TZ_LOCK = 0
                  AND g.TZ_LOCK = 0
                  AND C1785 = @TIPO_CUENTA
    ...
    END
    IF @FECHA = 1
       Or IsDate(@FECHA_SORTEO) = 0 -- Fecha invalida
    BEGIN
    ...
    (
        ...
        WHERE FECHA_SORTEO = Case
                                 When isdate(@FECHA_SORTEO) = 1 Then
                                     CAST(@FECHA_SORTEO AS DATETIME)
                                 Else
                                     null
                             End
              AND NRO_SORTEO_NOCTURNO = @NRO_SORTEO
              AND AGENCIA = @AGENCIA
              AND TZ_LOCK = 0
    )
