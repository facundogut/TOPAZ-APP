EXECUTE('DROP FUNCTION IF EXISTS dbo.itf_cuentas_REDLINK;')
EXECUTE('
CREATE FUNCTION dbo.itf_cuentas_REDLINK (@id_solicitud NUMERIC(15,0), @cantidad NUMERIC(3,0)) 
RETURNS VARCHAR(500)
...
BEGIN		
	...
	SELECT tipo_cuenta,numero_cuenta_pbf,estado_cuenta FROM TJD_SOLICITUD_CUENTAS_LINK WHERE ID_SOLICITUD=@id_solicitud AND TZ_LOCK=0; 
	
	DECLARE @tipo_cuenta NUMERIC(2,0);
	DECLARE @nro_cuenta VARCHAR(19);
	...
	BEGIN							
		...
		SET @resultado = concat(@resultado,LEFT(concat(@nro_cuenta,replicate('' '',19)),19));
