...
AS
	...
	LEFT JOIN CRE_AUX_SOL_CALCULO_ADELANTO ca ON p.C6250 = ca.PRODUCTO AND c.ID_ConvPago = ca.CONVENIO AND s.C1803 = ca.CLIENTE AND vc.ID_JURISDICCION = ca.ID_JURIDICCION AND can.CANAL = ca.CANAL
	WHERE vc.TZ_LOCK = 0
	UNION
	SELECT
	pc.PRODUCTO AS "Producto", 
