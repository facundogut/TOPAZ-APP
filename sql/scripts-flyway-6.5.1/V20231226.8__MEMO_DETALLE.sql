   ...
   BEGIN
      ...
      DELETE FROM MEMO_DETALLE WHERE Fecha_Info = @FECHA_PROCESO
                ...
                SELECT DISTINCT
                 ...
                 INNER JOIN SALDOS S WITH(NOLOCK) ON S.C1803 = MC.Cliente AND S.TZ_LOCK = 0 AND
                 ((C1604<>0 AND C1785 IN (5,6)) OR (C1785 = 2 AND C1651<>''1'')
                 OR (C1785 = 1 AND PRODUCTO IN (SELECT C6250 FROM PRODUCTOS WITH(NOLOCK) WHERE TZ_LOCK = 0 AND C6800=''T'')))
                 ...
                 WHERE MC.Fecha_Info = @FECHA_PROCESO 
