/****** Object:  StoredProcedure [dbo].[SP_ITF_COELSA_CHE_PRESENTADOS_ENVIADOS]    Script Date: 02/06/2021 16:05:44 ******/
SET ANSI_NULLS ON
GO
...
SELECT COD_BANCO 
FROM dbo.CLE_CHEQUES_ENVIADOS WITH (NOLOCK) 
WHERE TZ_LOCK=0 
GROUP BY COD_BANCO 
    					...
    					BEGIN
    					   ...
    					   --Varibles para conformar @RI_ENTIDAD_DEBITAR
    						DECLARE @ENTIDAD VARCHAR(4) = (SELECT RIGHT(REPLICATE('0', 4)+ CAST(COD_BANCO AS VARCHAR(4)), 4) AS Entidad_destino
    											 FROM CLE_CHEQUES_ENVIADOS WITH (NOLOCK)
    											 WHERE NUMERO_DEL_CHEQUE = @T_NUMERO_DEL_CHEQUE 
														AND COD_BANCO = @BANCO);
    						...
    						SET @RI_ENTIDAD_DEBITAR = @ENTIDAD + @SUCURSAL;
				 			...
				 			--CUENTA DEBITAR  
				 			SET @CMC7 = (SELECT s.CMC7 FROM CLE_CHEQUES_SALIENTE s  WITH (NOLOCK)
						 								JOIN CLE_CHEQUES_ENVIADOS e WITH (NOLOCK) 
																ON e.COD_BANCO=s.BANCO_GIRADO 
																	AND e.NUMERO_DEL_CHEQUE=s.NUMERO_CHEQUE
						 											AND e.COD_BANCO = @BANCO  
																	AND e.NUMERO_DEL_CHEQUE = @T_NUMERO_DEL_CHEQUE);					   				 
