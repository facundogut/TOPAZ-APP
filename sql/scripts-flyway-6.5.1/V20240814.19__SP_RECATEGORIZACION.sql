...
BEGIN
	...
	VS.JTS_OID_SALDO IN (SELECT JTS_OID FROM SALDOS WITH(NOLOCK) WHERE C1803 = CLC.CLIENTE AND C1785 = 2 AND TZ_LOCK=0) AND VS.TZ_LOCK = 0
	...
	SELECT CLIENTE,TIPO,''S'', --- Tipo sobregiro
	NRO_AUTORIZACION,
	CATEGORIA_COMERCIAL,
	CATEGORIA,
	@f_proceso
	FROM (
		SELECT T.CLIENTE,
		CASE WHEN T.EXTRAORDINARIO_AUTORIZADO = ''S'' THEN ''ET'' ELSE ''AE'' END AS TIPO,
		VS.NRO_AUTORIZACION,
		...
		AND VS.FECHA_VENCIMIENTO >= @f_proceso) Z
	GROUP BY CLIENTE,TIPO,NRO_AUTORIZACION,CATEGORIA_COMERCIAL,CATEGORIA
	...
	INSERT INTO CRE_RECATEGORIZACION_COMERCIAL_BITACORA 
	SELECT CLIENTE,TIPO,''A'', --- Tipo Asistencia
	SALDOS_JTS_OID,
	CATEGORIA_COMERCIAL,
	CATEGORIA,
	@f_proceso
	FROM (
		...
		WHERE C1803 = T.CLIENTE AND PRODUCTO NOT IN (SELECT C6250 FROM PRODUCTOS WITH(NOLOCK) WHERE C6800=''R'')) 
		INNER JOIN VW_ASISTENCIAS_DEUDA D WITH(NOLOCK) ON CR.SALDOS_JTS_OID = D.JTS_OID
		WHERE D.SALDO_DEUDA <> 0) Z
	GROUP BY CLIENTE,TIPO,SALDOS_JTS_OID,CATEGORIA_COMERCIAL,CATEGORIA
	...
	WHERE D.SALDO_DEUDA <> 0 AND cs.SALDOS_JTS_OID NOT IN 
	(SELECT JTS_OID FROM SALDOS WITH(NOLOCK) WHERE PRODUCTO IN (SELECT C6250 FROM PRODUCTOS WITH(NOLOCK) WHERE C6800=''R''))
	
	SET @p_msg_proceso = ''El Proceso de recategorización ha finalizado correctamente. Registros procesados: ''+ CONVERT(VARCHAR(10), @contador)
	SET @p_ret_proceso = 1 		
