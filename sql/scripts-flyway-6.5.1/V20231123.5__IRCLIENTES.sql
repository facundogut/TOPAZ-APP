  ...
  BEGIN
	...
	if exists (	select a.PERSONA_VINCULANTE
				...
				where a.PERSONA_VINCULANTE not in (	select b.NUMEROPERSONAJURIDICA  
												...
												from CLI_INSTITUCIONFINANCIERA c with(nolock))
				)	
			select   ''Tabla cli_vinculaciones no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS y CLI_PERSONASFISICAS Y CLI_INSTITUCIONFINANCIERA'',*
				from cli_vinculaciones a with(nolock) 
				where a.PERSONA_VINCULANTE not in (	select b.NUMEROPERSONAJURIDICA  
												from CLI_PERSONASJURIDICAS b with(nolock)
												union all 
