EXECUTE('
IF OBJECT_ID (''dbo.RRII_OperacionesPasivas'') IS NOT NULL
	DROP VIEW dbo.RRII_OperacionesPasivas
')
EXECUTE('
CREATE OR ALTER  view [dbo].[RRII_OperacionesPasivas] as
SELECT
...
/*2*/ (	SELECT ID3 
		...
		WHERE CODIGO = 5 
			AND ID1 = s.PRODUCTO 
			AND ID2= s.MONEDA) as TipoOperacion, 
...
/*4*/	CASE WHEN s.C1785<>4 
		THEN s.CUENTA 
		ELSE s.OPERACION 
		END as NroCuenta,
/*5*/ CONCAT(YEAR(s.C1620), RIGHT(''0'' + RTRIM(MONTH(s.C1620)), 2), RIGHT(''0'' + RTRIM(DAY(s.C1620)), 2)) as FechaAperturaCuenta,
/*6*/ CASE	WHEN s.C1785 IN(2,3) 
			THEN ''000000000000000'' 
			ELSE s.OPERACION 
			END as NroCertificadoPlazoFijo, --Para CA se completa con ceros.
/*7*/ CASE 
		WHEN s.MONEDA NOT IN (998, 999)
			THEN (SELECT s2.C1785 FROM SALDOS s2 WITH (NOLOCK) WHERE s2.JTS_OID = s.C1665 AND s.c1688 = 0)
			ELSE (SELECT s3.C1785 FROM SALDOS s3 WITH (NOLOCK)  WHERE s3.JTS_OID = 
					(SELECT cv.JTS_OID_CTA_FUENTE FROM CV_TRANSFERENCIA cv WITH (NOLOCK)  WHERE cv.JTS_OID_CTA_DESTINO = s.JTS_OID AND s.c1688 = 0))
...
/*20*/ --CASE 
		--WHEN s.C1632 = 0 then 0 
		--ELSE s.C1632 --ELSE try_convert(numeric(11, 7), FORMAT(s.C1632, ''0000.00''))--cast(FORMAT(TasaInteresPactada, ''0000.00'') as numeric(11,7))    
		--END TasaInteresPactada, 
...
/*24*/ (SELECT SUM(mc.CAPITALREALIZADO) from MOVIMIENTOS_CONTABLES mc WITH (NOLOCK)  /*join s on s.JTS_OID = mc.SALDO_JTS_OID*/ where mc.DEBITOCREDITO = ''D'' and MONTH(mc.FECHAPROCESO)= MONTH(gsd.FECHA)  ) as DebitosTotales, --Debitos totales
/*25*/ (SELECT SUM(mc.CAPITALREALIZADO) from MOVIMIENTOS_CONTABLES mc WITH (NOLOCK)  /*join s on s.JTS_OID = mc.SALDO_JTS_OID*/ where mc.DEBITOCREDITO = ''C'' and MONTH(mc.FECHAPROCESO)= MONTH(gsd.FECHA)  ) as CreditosTotales, --Creditos totales (preguntar si es suma de capital realizado)
...
FROM SALDOS s WITH (NOLOCK)  --, PRODUCTOS p, 
join GRL_SALDOS_DIARIOS gsd WITH (NOLOCK)  on s.JTS_OID = gsd.SALDOS_JTS_OID 
join VW_CLIENTES_PERSONAS vcp WITH (NOLOCK)  on s.C1803 = vcp.CODIGOCLIENTE
join MONEDAS m WITH (NOLOCK)  on s.MONEDA = m.C6399
join PRODUCTOS prd WITH (NOLOCK)  on s.PRODUCTO = prd.C6250
where s.TZ_LOCK = 0
')