EXECUTE('
DROP PROCEDURE IF EXISTS dbo.SP_CALCULO_DEUDA_PONDERADA
')

EXECUTE('
CREATE PROCEDURE dbo.SP_CALCULO_DEUDA_PONDERADA
...
AS 
   ...
   WITH(NOLOCK) WHERE C1803 = @P_CLIENTE AND C1604 <> 0 AND C1785 IN (1,5,6) AND CATEGORIA_COMERCIAL = ''C'')
   
   SELECT @COMERCIAL=COUNT(1) FROM CRE_SALDOS WITH(NOLOCK) WHERE SALDOS_JTS_OID IN (SELECT JTS_OID FROM SALDOS
   WITH(NOLOCK) WHERE C1803 = @P_CLIENTE AND C1604 <> 0 AND C1785 IN (1,5,6) 
