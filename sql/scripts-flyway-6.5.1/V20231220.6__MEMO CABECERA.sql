EXECUTE('
CREATE PROCEDURE PA_MEMOS_CABECERA
     ...
     ---DECLARE @FECHA_PROCESO DATETIME 
      ...
      DELETE FROM MEMO_CABECERA WHERE Fecha_Info = @FECHA_PROCESO
                ...
                CASE WHEN C.CLIENTE_DEVENGA_SUSPENSO = 1 THEN ''S'' ELSE ''N'' END AS SIT_JUD_EXT
                 ...
                 /*LEFT JOIN CRE_GRAD_FRACC_BITACORA BGF WITH(NOLOCK) ON BGF.CODIGOCLIENTE = C.CODIGOCLIENTE AND BGF.TIPO = ''B'' AND BGF.FECHAPROCESO = (SELECT
                 TOP 1 FECHAPROCESO FROM CRE_GRAD_FRACC_BITACORA WITH(NOLOCK) WHERE CODIGOCLIENTE = C.CODIGOCLIENTE AND FECHAPROCESO <= @FECHA_PROCESO ORDER BY
                 FECHAPROCESO DESC)*/
