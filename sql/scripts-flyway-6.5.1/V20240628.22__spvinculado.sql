...
IF(@PORCENTAJE_SING = 0 AND @PORCENTAJE_CONG = 0)
 ...
 BEGIN
 SET @EXCEDE_GRAD = ''S'';
 SET @MENSAJE_INDIVIDUAL = ''Exceso en Grandes Exposiciones: ''+CONVERT(VARCHAR(17),FORMAT(ROUND((@DEUDA_TOTAL+@IMPORTE_ANALISIS)-@DEUDA_TOPE,2), ''C'', ''es-ar''))+''.'';
 END
 ELSE 
 BEGIN
 ...
 BEGIN
 SET @MENSAJE_GRUPO = ''Exceso en Graduación: ''+CONVERT(VARCHAR(17),FORMAT(ROUND(@DEUDA_GRUPO-@DEUDA_TOPE,2), ''C'', ''es-ar''))+''.''; ---''Tope deuda = '' + CONVERT(VARCHAR, @DEUDA_TOPE) + '' Deuda grupo = ''+@DEUDA_GRUPO;
 END
 ELSE 
