EXECUTE('
IF OBJECT_ID (''dbo.VW_RRII_ACTUALIZACIONES_PADRON'') IS NOT NULL
	DROP VIEW dbo.VW_RRII_ACTUALIZACIONES_PADRON
')

EXECUTE('
CREATE   VIEW VW_RRII_ACTUALIZACIONES_PADRON
AS
WITH AUX AS(
...
--''0'' AS SCOD_POSTAL_CAMBIO, 
		DIR.CPA_NUEVO AS SCOD_POSTAL_CAMBIO,
		ROW_NUMBER() OVER(PARTITION BY D.NUMERODOCUMENTO ORDER BY D.NUMERODOCUMENTO ASC) AS NUMERADOR_REPETIDO
FROM PARAMETROS P WITH(nolock), CLI_INTEGRANTESPJ IPJ WITH(nolock), CLI_DIRECCIONES DIR WITH(nolock), CLI_DocumentosPFPJ D WITH(nolock)
LEFT JOIN CLI_PERSONASJURIDICAS PJ WITH(nolock) ON D.NUMEROPERSONAFJ = PJ.NUMEROPERSONAJURIDICA 
LEFT JOIN RRII_PARAMETROS_PADRON PP (nolock) ON D.TIPOPERSONA = PP.TIPO_PERSONA
...
SELECT	MAX(BD.FECHA) AS MAX_FECHA, 
		...
		PJ.RAZONSOCIAL AS DENOMINACION, 
		''0'' AS PEP_CAMBIO,
		BD_AUX.CPA_NUEVO AS SCOD_POSTAL_CAMBIO, ROW_NUMBER() OVER(PARTITION BY DOC.NUMERODOCUMENTO ORDER BY DOC.NUMERODOCUMENTO ASC) AS NUMERADOR_REPETIDO
FROM PARAMETROS P WITH(nolock), 
