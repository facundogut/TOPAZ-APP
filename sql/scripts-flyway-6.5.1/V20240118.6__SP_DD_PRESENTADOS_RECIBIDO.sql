execute('
ALTER   PROCEDURE [dbo].[SP_DD_PRESENTADOS_RECIBIDOS]
	@TICKET NUMERIC(16),
...
BEGIN 
	...
	BEGIN
      ...
      BEGIN
		...
		BEGIN
			...
			begin
				SET @FechaVencimientoRA = convert(DATE, substring(@linea,4,6));
      ...
      BEGIN
			...
			--SELECT @CodigoOrigen AS CodigoOrigen
			--
	...
	BEGIN
	      ...
	      */
	      ...
	      --------------
	      --SELECT @Cod_Cliente AS ''Cod_Cliente'',@NumCuenta AS ''NumCuenta'' HOYYYY
	      --cambio JI
	      --La cuenta que intenta debitar no existe
		  DECLARE @RX1 NUMERIC;	      
	      SET @RX1 = (SELECT convert(NUMERIC,COUNT(*)) FROM SALDOS S WHERE S.C1803 = @Cod_Cliente AND S.CUENTA = @NumCuenta );
		...
		--oph 05012024
		--SELECT @IdEntidadOrigen AS IdEntidadOrigen
		--
		-- OPH 05012024
		SET @CUIT_EO = CONCAT(@IdentificacionEI,@DigitoV)
		SET @PRESTACION = @VPrestacion
		--SELECT @CUIT_EO AS CUIT_EO,@PRESTACION AS PRESTACION, @InfoAdicional AS InfoAdicional
		--
