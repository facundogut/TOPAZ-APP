...
--			PRINT @LINEA
	...
	IF @MonedaCta IS NULL
  	...
  	END
	      ...
	      ----NACHA R15
	      --------------
	      IF @ReferenciaUnivoca=''CUOTA BCO CHACO''
--	      PRINT @cuit_eo
	      
	      IF @CUIT_EO=33693450239
	      BEGIN 
	      PRINT ''Mi registro objetivo''
	      END
	      ...
	      BEGIN
	      			      		INSERT INTO dbo.ITF_DD_PRESENT_RECIB_RECHAZOS (ID_TICKET,  
													   ...
													   VALUES (@TICKET, 
													   		   ...
													   		   @IdClientePegador,  
													   		   ''I'',  
													   		   (SELECT CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 36),
													   		   @InfoAdicional,
...
--  PRINT @ReferenciaUnivoca 
--  PRINT  @FechaVencimientoRA 
--  PRINT @TraceNumberRA  
--PRINT @RegistrosAdicionales
 IF convert(INT,@RegistrosAdicionales)=1
 BEGIN
 IF @ReferenciaUnivoca=''AUTONO09/23    ''
 BEGIN 
 PRINT ''modificar ref, fechavencimientoRA, tracenumberRA''
 PRINT @ReferenciaUnivoca
 PRINT @FechaVencimientoRA
 PRINT @TraceNumberRA 
 
 SELECT ESTADO FROM SNP_MSG_ORDENES WHERE REFERENCIA= @ReferenciaUnivoca AND FECHA_VENCIMIENTO=@FechaVencimientoRA AND (TRACE_N_1PRE=@TraceNumberRA OR TRACE_N_2PRE=@TraceNumberRA)
 END 
 
 UPDATE SNP_MSG_ORDENES
 SET ESTADO=''RC''
 WHERE REFERENCIA= @ReferenciaUnivoca 
 AND FECHA_VENCIMIENTO=CONVERT(varchar, @FechaVencimientoRA, 23) 
