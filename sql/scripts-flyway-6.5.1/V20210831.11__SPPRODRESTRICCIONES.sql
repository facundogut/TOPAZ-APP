...
--Guardamos la primera iteración de para comparara en las siguientes ejecuciones
	...
	SET @v_TIPO_BLOQUEO = (SELECT TIPO_BLOQUEO FROM @TablaAuxiliarDiaria)
	SET @v_MENSAJE = (SELECT MENSAJE FROM @TablaAuxiliarDiaria)
	SET @v_SUMA = (SELECT Suma_Total FROM @TablaAuxiliarDiaria)
...
IF (ISNULL(@v_TIPO_BLOQUEO,'' '')like ''A''  or ISNULL(@v_TIPO_BLOQUEO,'' '')like '' '' )
	...
	SET @v_TIPO_BLOQUEO = (SELECT TIPO_BLOQUEO FROM @TablaAuxiliarMensual)
	SET @v_MENSAJE = (SELECT MENSAJE FROM @TablaAuxiliarMensual)
	SET @v_SUMA = (SELECT Suma_Total FROM @TablaAuxiliarMensual)
...
BEGIN
	...
	SET @v_TIPO_BLOQUEO_ANT = @v_TIPO_BLOQUEO
	SET @v_TIPO_BLOQUEO = (SELECT TIPO_BLOQUEO FROM @TablaAuxiliarOper)
	SET @v_MENSAJE = (SELECT MENSAJE FROM @TablaAuxiliarOper)
	SET @v_SUMA = (SELECT Suma_Total FROM @TablaAuxiliarOper)
