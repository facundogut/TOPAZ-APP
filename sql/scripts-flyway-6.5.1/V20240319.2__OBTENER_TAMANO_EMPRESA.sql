...
AS 
   ...
   --BEGIN
     ...
     --@CODIGOCLIENTE NUMERIC (12)= NULL,
      ...
      @PROM_PERSONAL_OCUPADO NUMERIC (7,2)= NULL,
      ...
      @NUMEROPERSONA_CERT NUMERIC (12) = 0,      
      @INICIO_ACTIVIDAD VARCHAR(10),
      @INICIO_ACTIVIDAD_CONVERT VARCHAR(20),
      @FECHA_INICIO_ACTIVADAD DATE,
      @TIPO_PERSONA VARCHAR(1),
      @TIENE_CERTIFICADO_MIPYME NUMERIC (2) = 0,
      @GRUPO_ECONOMICO_CANTIDAD NUMERIC	(3) = 0,
      @GRUPO_ECONOMICO_CANT_CERTIFICADOS NUMERIC (3) = 0
	...
	--	promedio personal ocupado
			...
			SELECT TOP 1 @NUMEROPERSONA = NUMEROPERSONA FROM CLI_ClientePersona WITH(NOLOCK) WHERE TITULARIDAD = ''T'' AND TZ_LOCK = 0 AND CODIGOCLIENTE = @P_CODIGO_CLIENTE       
       		...
       		IF @TIPO_PERSONA = ''F''
       			BEGIN
			 		...
			 		WHERE CODIGO_PERSONA_CLIENTE = @NUMEROPERSONA AND TZ_LOCK = 0 
			 	   -- NBCHSEG-3459 - Determinar antiguedad sin tener en cuenta orden AFIP
			 	   -- CODIGO_ACTIVIDAD = @CODIGO_ACT_AFIP AND 
		...
		IF (@TIENE_GRUPO_ECONOMICO <> 0)   
			BEGIN
				...
				-- Si algun integrante es gran empresa, el grupo hereda
				SELECT @ES_GRAN_EMPRESA=CASE WHEN count(*)=0 THEN 0 ELSE 1 END 
				FROM dbo.CLI_GRUPOSECONOMICOSCLIENTE A WITH(NOLOCK)
				INNER JOIN  dbo.CLI_CLIENTES B WITH(NOLOCK) ON A.CODIGOCLIENTE = B.CODIGOCLIENTE 
				AND B.TZ_LOCK = 0				      
				INNER JOIN CLI_CLIENTEPERSONA cp WITH(NOLOCK) ON cp.CODIGOCLIENTE=a.CODIGOCLIENTE AND cp.TITULARIDAD = ''T'' AND cp.TZ_LOCK = 0
				INNER JOIN CLI_ACTIVIDAD_ECONOMICA cae WITH(NOLOCK) ON cae.CODIGO_PERSONA_CLIENTE=cp.NUMEROPERSONA AND cae.TZ_LOCK=0
				INNER JOIN CLI_Cod_Act_AFIP af WITH(NOLOCK) ON af.CODIGO_ACT_AFIP=cae.CODIGO_ACTIVIDAD AND af.Cat_Por_GEM=''S'' AND af.TZ_LOCK=0 
				...
				SELECT @GRUPO_ECONOMICO_CANTIDAD = count(a.CODIGOGRUPOECONOMICO)
				  FROM dbo.CLI_GRUPOSECONOMICOSCLIENTE A WITH(NOLOCK)
				  INNER JOIN  dbo.CLI_CLIENTES B WITH(NOLOCK) ON A.CODIGOCLIENTE = B.CODIGOCLIENTE 
				  			AND A.CODIGOGRUPOECONOMICO = @COD_GRUPO_ECONOMICO AND B.TZ_LOCK = 0				   
				 WHERE A.TZ_LOCK = 0								
			    -- Cantidad de certificados del grupo economico
			    SELECT  @GRUPO_ECONOMICO_CANT_CERTIFICADOS = count(T.CUIT)			    
				  FROM  CLI_DocumentosPFPJ DOC WITH(NOLOCK) 				  		
				  INNER JOIN CLI_CLIENTEPERSONA cp WITH(NOLOCK) ON DOC.NUMEROPERSONAFJ = cp.NUMEROPERSONA 
				  		AND cp.TITULARIDAD = ''T'' AND cp.TZ_LOCK = 0																																							 
				  INNER JOIN CLI_GRUPOSECONOMICOSCLIENTE g WITH(NOLOCK) ON cp.CODIGOCLIENTE = g.CODIGOCLIENTE 
				  		AND g.TZ_LOCK = 0 AND g.CODIGOGRUPOECONOMICO = @COD_GRUPO_ECONOMICO
				  INNER JOIN (SELECT A.CUIT FROM CRE_TAM_EMP_CER_MIPYMES A WHERE A.F_INICIO + a.PLAZO_VIGENCIA > @FECHA_PROCESO
				   		AND A.ESTADO = ''V'' AND A.TZ_LOCK =0 GROUP BY A.CUIT) T
				   ON T.CUIT=DOC.NUMERODOCUMENTO
				  WHERE DOC.TZ_LOCK=0				  		
				
				IF (@GRUPO_ECONOMICO_CANTIDAD=@GRUPO_ECONOMICO_CANT_CERTIFICADOS)
	 				BEGIN 
	 					-- Determinar Certificado con mayor tamanio, este sera el que determine el mismo.
					    SELECT TOP 1 @NUMEROPERSONA_CERT=NUMEROPERSONAFJ FROM (
							SELECT  doc.NUMEROPERSONAFJ,T.CUIT, MAX(T.CATEGORIA) AS CATEGORIA		    
											  FROM  CLI_DocumentosPFPJ doc WITH(NOLOCK) 				  		
											  INNER JOIN CLI_CLIENTEPERSONA cp WITH(NOLOCK) ON doc.NUMEROPERSONAFJ = cp.NUMEROPERSONA 
											  		AND cp.TITULARIDAD = ''T'' AND cp.TZ_LOCK = 0																																							 
											  INNER JOIN CLI_GRUPOSECONOMICOSCLIENTE g WITH(NOLOCK) ON cp.CODIGOCLIENTE = g.CODIGOCLIENTE 
											  		AND g.TZ_LOCK = 0 AND g.CODIGOGRUPOECONOMICO = @COD_GRUPO_ECONOMICO
											  INNER JOIN (SELECT A.CUIT, 
															CASE WHEN a.CATEGORIA=''M'' THEN 1
																 WHEN a.CATEGORIA=''P'' THEN 2
																 WHEN a.CATEGORIA=''1'' THEN 3
																 WHEN a.CATEGORIA=''2'' THEN 4
															END AS CATEGORIA
															FROM CRE_TAM_EMP_CER_MIPYMES A WITH(NOLOCK)
															WHERE A.F_INICIO + a.PLAZO_VIGENCIA > @FECHA_PROCESO
																  AND A.ESTADO = ''V'' AND A.TZ_LOCK =0 
															GROUP BY A.CUIT, A.CATEGORIA) T
											   ON T.CUIT=DOC.NUMERODOCUMENTO
											  WHERE DOC.TZ_LOCK=0
							GROUP BY  doc.NUMEROPERSONAFJ,T.CUIT				
							) Z 
						GROUP BY NUMEROPERSONAFJ,CUIT, CATEGORIA 
				...
				-- Determinar tamanio	 					 				
		 				SELECT @TAMANIO_MIPYME = b.OPCIONINTERNA,
	                   		   ...
	                   		   @P_NRO_REGISTRO = A.NRO_REGISTRO	                   		   
					    FROM CRE_TAM_EMP_CER_MIPYMES A WITH(NOLOCK)
					    	 INNER JOIN OPCIONES b WITH(NOLOCK) ON a.CATEGORIA = b.OPCIONINTERNA
					    	 INNER JOIN CLI_DocumentosPFPJ DOC WITH(NOLOCK) ON DOC.NUMERODOCUMENTO=A.CUIT AND DOC.TZ_LOCK=0
					    	 INNER JOIN CLI_CLIENTEPERSONA cp WITH(NOLOCK) ON DOC.NUMEROPERSONAFJ = CP.NUMEROPERSONA 
					    	 	AND CP.TITULARIDAD = ''T'' AND CP.TZ_LOCK = 0	     				   
						WHERE  b.NUMERODECAMPO = 43106
						   AND b.IDIOMA = ''E''
						   AND A.TZ_LOCK = 0
						   AND cp.NUMEROPERSONA= @NUMEROPERSONA_CERT
						   AND A.F_INICIO + A.PLAZO_VIGENCIA > @FECHA_PROCESO
						   AND A.ESTADO = ''V''
						   
						   -- Si existe certificado seteamos variable
						   IF (@P_NRO_REGISTRO IS NOT NULL)
						   	BEGIN
						   		SET @TIENE_CERTIFICADO_MIPYME=1
						   	END
       ...
       SELECT @AnalizaVentas = CASE WHEN EXISTS (
        ...
        IF (@TIENE_CERTIFICADO_MIPYME > 0 AND @TIENE_GRUPO_ECONOMICO = 0)
          BEGIN
       ...
       */ -- por el jira NBCHSEG-2203 se quita de la condicion que no sea grupo economico, dado que en esos casos igual el cliente tiene que tener cargado los balances
       IF (@TIENE_BALANCE = 0) AND (@ANTIGUEDAD > 540) AND (@VALIDACION IS NULL)  AND (@TIENE_CERTIFICADO_MIPYME = 0)
        ...
        BEGIN 
         ...
         SET @P_STATUS = 1
         SET @P_Descripcion = ''Tamaño Empresa no Determinado'' -- ''Verificar Vigencia del Balance''
         SET @P_PROCESO = ''FINALIZO OK'' 
         SET @P_Tamaño_Calculado = ''N'' 
   ...
   BEGIN
     ...
     END
    -- END mz
    -- Si actividad economica esta excluida/gran empresa es NO MIPYME. 
    -- Esto aplica para clientes individuales y para grupo, en donde si uno esta marcado asi, el resto hereda
    -- asi no sea el de mayores ventas
	BEGIN 
       IF @ES_GRAN_EMPRESA = 1
          BEGIN 
             SET @P_STATUS = 0
     ...
     END
       ...
       BEGIN CATCH
           SET @P_STATUS = 1
           SET @P_Descripcion = ''Tamaño Empresa no Determinado''
           SET @P_Tamaño_Calculado = ''N''
