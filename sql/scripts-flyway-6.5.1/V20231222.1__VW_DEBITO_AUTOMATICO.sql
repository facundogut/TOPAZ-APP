...
SELECT 
    ...
    ISNULL(L.ID_LIQUIDACION, RC.ID) AS ''ID'',
...
LEFT JOIN REC_LIQUIDACION L WITH (NOLOCK) ON RC.CONVENIO = L.CONVENIO AND L.ID_LIQUIDACION = CASE WHEN RC.ESTADO IN (''V'',''Z'') AND RC.FECHACORTE = (SELECT FECHAPROCESO FROM PARAMETROS) THEN 0 ELSE RC.ID_LIQUIDACION END
JOIN CONV_CONVENIOS_REC C WITH (NOLOCK) ON (L.CONVENIO = C.Id_ConvRec OR c.id_convPadre = L.CONVENIO OR C.Id_ConvRec = RC.CONVENIO OR c.id_convPadre = RC.CONVENIO)
    ...
    OR EXISTS (
        SELECT FECHAPROCESO FROM PARAMETROS
        WHERE FECHAPROCESO =  RC.FECHACORTE
    )
)
...
WHERE 
    ...
    AND ((L.tz_lock < 300000000000000 OR L.tz_lock >= 400000000000000)
     OR (RC.tz_lock < 300000000000000 OR RC.tz_lock >= 400000000000000))    
    AND ((L.tz_lock < 100000000000000 OR L.tz_lock >= 200000000000000)
     OR (RC.tz_lock < 100000000000000 OR RC.tz_lock >= 200000000000000))
 ')