...
'' '' AS ESTADO
FROM ITF_AFIP_ACT_ECONOMICA i LEFT JOIN CLI_DocumentosPFPJ d ON i.CUIL=d.NUMERODOCUMENTO
...
AND e.CODIGO_ACTIVIDAD=i.COD_ACTIVIDAD AND e.ORDEN_AFIP=i.ORDEN AND ((e.TZ_LOCK < 300000000000000 OR e.TZ_LOCK >= 400000000000000) AND (e.TZ_LOCK < 100000000000000 OR e.TZ_LOCK >= 200000000000000))
WHERE ((i.TZ_LOCK < 300000000000000 OR i.TZ_LOCK >= 400000000000000) AND (i.TZ_LOCK < 100000000000000 OR i.TZ_LOCK >= 200000000000000))
...
CLI_Cod_Act_AFIP a ON c.CODIGO_ACTIVIDAD=a.CODIGO_ACT_AFIP 
...
(CASE 
WHEN c.ORDEN_AFIP=1 THEN ''Primaria''
WHEN (SELECT COUNT(*) FROM ITF_AFIP_ACT_ECONOMICA WHERE CUIL=c.CUIL)=0 AND ((SELECT MIN(u.ORDINAL_ACTIVIDAD) FROM CLI_ACTIVIDAD_ECONOMICA u WHERE u.TZ_LOCK=0 AND u.CODIGO_PERSONA_CLIENTE=c.CODIGO_PERSONA_CLIENTE AND ((u.TZ_LOCK < 300000000000000 OR u.TZ_LOCK >= 400000000000000) AND (u.TZ_LOCK < 100000000000000 OR u.TZ_LOCK >= 200000000000000)))=c.ORDINAL_ACTIVIDAD 
OR ( (SELECT MIN(u.ORDINAL_ACTIVIDAD) FROM CLI_ACTIVIDAD_ECONOMICA u WHERE u.TZ_LOCK=0 AND u.CODIGO_PERSONA_CLIENTE=c.CODIGO_PERSONA_CLIENTE AND ((u.TZ_LOCK < 300000000000000 OR u.TZ_LOCK >= 400000000000000) AND (u.TZ_LOCK < 100000000000000 OR u.TZ_LOCK >= 200000000000000)))IS NULL AND 
(SELECT MIN(t.ORDINAL_ACTIVIDAD) FROM TEMP_CLI_ACTIVIDAD_ECONOMICA t WHERE t.TZ_LOCK=0 AND t.CODIGO_PERSONA_CLIENTE=c.CODIGO_PERSONA_CLIENTE AND ((t.TZ_LOCK < 300000000000000 OR t.TZ_LOCK >= 400000000000000) AND (t.TZ_LOCK < 100000000000000 OR t.TZ_LOCK >= 200000000000000)))=c.ORDINAL_ACTIVIDAD)) THEN ''Primaria''
ELSE ''Secundaria''
END) AS TIPO_ACTIVIDAD,
c.VERIFICADO_AFIP AS VERIFICADO_AFIP,
c.FECHA_VERIFICADO,
...
CLI_Cod_Act_AFIP a ON c.CODIGO_ACTIVIDAD=a.CODIGO_ACT_AFIP --c.ESTADO_REGISTRO<>''B'' AND 
LEFT JOIN CLI_DocumentosPFPJ d ON d.NUMEROPERSONAFJ=c.CODIGO_PERSONA_CLIENTE
WHERE ((c.TZ_LOCK < 300000000000000 OR c.TZ_LOCK >= 400000000000000) AND (c.TZ_LOCK < 100000000000000 OR c.TZ_LOCK >= 200000000000000))
...
ORDER BY ORDEN_AFIP, ORDINAL ASC
