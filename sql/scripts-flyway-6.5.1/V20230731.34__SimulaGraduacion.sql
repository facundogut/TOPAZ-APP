EXECUTE('
DROP PROCEDURE IF EXISTS dbo.SP_PA_GRADUACION_FRACCIONAMIENTO
')

EXECUTE('
CREATE PROCEDURE dbo.SP_PA_GRADUACION_FRACCIONAMIENTO
...
DECLARE @TOTAL_DEUDA NUMERIC (15, 2)
DECLARE @TRAMO_SIN_G NUMERIC (15, 2)
DECLARE @TRAMO_CON_G NUMERIC (15, 2)
DECLARE @TRAMO_SIN_G_B NUMERIC (15, 2)
DECLARE @TRAMO_CON_G_B NUMERIC (15, 2)
DECLARE @RPC_BANCO NUMERIC (15, 2)
DECLARE @EXCLUSIONES NUMERIC (15, 2)
DECLARE @TOT_CONTROL_GRAD NUMERIC (15, 2)
DECLARE @RPC_CLIENTE NUMERIC (15, 2)
DECLARE @RPC_CLIENTE_MAX NUMERIC (15, 2)
DECLARE @RPC_NETO_BANCO NUMERIC (15, 2)
DECLARE @MARGEN_RESGUAR NUMERIC (15, 2)
DECLARE @LIMITE_GRAD NUMERIC (15, 2)
DECLARE @RESULTADO_GRADUACION VARCHAR (200)
DECLARE @RESULTADO_FRACCIONAMIENTO VARCHAR (200)
DECLARE @IMPORTE_SING_GRUPO NUMERIC (15, 2)
DECLARE @IMPORTE_CONG_GRUPO NUMERIC (15, 2)
DECLARE @PERIODO VARCHAR (10)
DECLARE @CN1_BANCO NUMERIC (15, 2)
...
DECLARE @EXCEDE_FRACCIONAMIENTO_O VARCHAR(1)
	...
	SELECT VF.CLIENTE, VCD.NUMEROPERSONA,
	ROW_NUMBER() OVER (ORDER BY VF.CLIENTE, VCD.NUMEROPERSONA)
	FROM VW_FAMILIAS_LIMITE VF WITH(NOLOCK)
	INNER JOIN VW_CLI_X_DOC VCD WITH(NOLOCK) ON VF.CLIENTE = VCD.CODIGOCLIENTE
	GROUP BY VF.CLIENTE, VCD.NUMEROPERSONA
