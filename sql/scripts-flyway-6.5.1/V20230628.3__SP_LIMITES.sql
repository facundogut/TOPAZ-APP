EXECUTE('
CREATE PROCEDURE dbo.SP_LIMITES
@CLIENTE NUMERIC(12)
AS
BEGIN
	DECLARE @TMP_LIMITE_CLIENTE_PRODUCTO TABLE
	(
		RIESGO NUMERIC(2),
	 	FAMILIA NUMERIC(12),
	 	PRODUCTO NUMERIC(5),
	...
	DELETE FROM CRE_LIMITES_CLIENTE WHERE CLIENTE = @CLIENTE;
	
	INSERT INTO @TMP_LIMITE_CLIENTE_PRODUCTO
	SELECT RIESGO, FAMILIA, PRODUCTO, IDLIMITE, UTILIZADO, CLIENTE 
	FROM VW_LIMITE_CLIENTE_PRODUCTO WITH(NOLOCK) WHERE CLIENTE=@CLIENTE;
	
	INSERT INTO CRE_LIMITES_CLIENTE
	SELECT CLIENTE, 
		NIVEL, 
		TIPO,
		IDLIMITE, 
...
FROM(			
	...
	INNER JOIN MONEDAS m WITH(NOLOCK) ON l.MONEDA_LIMITE = m.C6399 
									AND m.TZ_LOCK = 0
	LEFT JOIN @TMP_LIMITE_CLIENTE_PRODUCTO lcf ON lcf.IDLIMITE=l.IDLIMITE AND lcf.CLIENTE = l.CLIENTE
	WHERE l.CLIENTE = @CLIENTE AND l.TZ_LOCK = 0
	GROUP BY l.CLIENTE,  
			l.IDLIMITE, 
			m.C6399, 
			l.MONTO, 
	...
	INNER JOIN CRE_TIPOS_RIESGOS t WITH(NOLOCK) ON r.CODIGO_RIESGO = t.CODIGO_RIESGO 
											AND t.TZ_LOCK = 0  
	LEFT JOIN @TMP_LIMITE_CLIENTE_PRODUCTO lcf ON lcf.IDLIMITE=l.IDLIMITE AND lcf.RIESGO=r.CODIGO_RIESGO AND
	lcf.CLIENTE = l.CLIENTE
	WHERE l.CLIENTE = @CLIENTE AND l.TZ_LOCK = 0
	GROUP BY l.CLIENTE,  
			r.IDLIMITE, 
			r.CODIGO_RIESGO, 
	...
	INNER JOIN CRE_FAMILIALIC f  WITH(NOLOCK) ON r.IDLIMITE = f.IDLIMITE 
											...
											AND f.TZ_LOCK = 0 
	LEFT JOIN @TMP_LIMITE_CLIENTE_PRODUCTO lcf ON lcf.IDLIMITE=l.IDLIMITE AND lcf.FAMILIA=F.FAMILIA 
	AND lcf.RIESGO=r.CODIGO_RIESGO AND lcf.CLIENTE = l.CLIENTE
	WHERE l.CLIENTE = @CLIENTE AND l.TZ_LOCK = 0
	GROUP BY l.CLIENTE, 
			f.IDLIMITE , 
			r.CODIGO_RIESGO, 
	...
	INNER JOIN CRE_PRODUCTOLIC p  WITH(NOLOCK) ON f.IDLIMITE = p.IDLIMITE 
											...
											AND p.TZ_LOCK = 0											
	LEFT JOIN @TMP_LIMITE_CLIENTE_PRODUCTO lcf ON lcf.IDLIMITE=l.IDLIMITE AND lcf.FAMILIA=F.FAMILIA AND lcf.PRODUCTO=P.PRODUCTO AND
	lcf.RIESGO=r.CODIGO_RIESGO AND lcf.CLIENTE = l.CLIENTE
	WHERE l.CLIENTE = @CLIENTE AND l.TZ_LOCK = 0									
											) o
ORDER BY	CLIENTE, 
			IDLIMITE, 
