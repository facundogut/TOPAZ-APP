...
(CASE WHEN sos.Subsistema = ''PR''
  THEN CONCAT(RIGHT(''0''+CONVERT(VARCHAR,sa.SUCURSAL),2), sos.Subsistema, CONVERT(VARCHAR,mov.ASIENTO), CONVERT(VARCHAR,mov.ORDINAL))
  ELSE CONCAT(RIGHT(''0''+CONVERT(VARCHAR,sa.SUCURSAL),2), sos.Subsistema, CONVERT(VARCHAR,mov.CUENTA)) END)AS NUMERODECUENTA,
--4
(CASE WHEN sos.Subsistema IN (''AC'', ''CC'') THEN LEFT(concat(mov.ASIENTO, CONVERT(VARCHAR,mov.FECHAPROCESO,112), mov.SUCURSAL, mov.ORDINAL),22) ELSE '''' END) AS NUMERODEOPERACION,
--5
...
(CASE WHEN mov.MONEDA = 1 THEN 
  (CASE WHEN mov.OPERACION = 8620 THEN (select dpf.IMPORTE_MN from SOLICAPERTURADPF dpf where dpf.CUENTA = mov.CUENTA AND dpf.OPERACION = MOV.OPERACION_CUENTA) ELSE mov.CAPITALREALIZADO END) ELSE 0 END) AS MONTOPESOS, --PENDIENTE
--11
(CASE WHEN mov.MONEDA <> 1 THEN 
  (CASE WHEN mov.OPERACION = 8620 THEN (select dpf.IMPORTE_MN from SOLICAPERTURADPF dpf where dpf.CUENTA = mov.CUENTA AND dpf.OPERACION = MOV.OPERACION_CUENTA) ELSE mov.CAPITALREALIZADO END) ELSE 0 END) AS ESPECIETRANSADACANTIDAD, --PENDIENTE
--12
...
--18
(CASE WHEN sos.Subsistema IN (''CC'',''AC'')
  THEN CONVERT(VARCHAR,mov.COD_TRANSACCION) ELSE '''' END) AS CODIGOSTOPAZ,
--19
(CASE WHEN mov.MONEDA = 1 THEN 0 ELSE (CASE WHEN sos.SubSistema IN (''AC'',''CC'',''TC'',''ME'',''TT'') THEN (SELECT TipC.TIPO_CAMBIO_OFICIAL FROM HISTORICOTIPOSCAMBIO TipC WHERE TipC.FECHA_COTIZACION = mov.FECHAPROCESO AND Tipc.MONEDA = sa.MONEDA AND Tipc.TZ_LOCK = 0) 
...
--3
CONCAT(RIGHT(''0''+CONVERT(VARCHAR, B.SUC_CUENTA_COBRO),2), ''TC'', CONVERT(VARCHAR, B.TIPO_TJC), CONVERT(VARCHAR,mov.USUARIO)) AS NUMERODECUENTA,
--4
'''' AS NUMERODEOPERACION,
--5
''TC'' AS CLASEDECUENTA,
--6
''400'' AS TIPODECUENTAUOPERACION,
--7
...
--15
CONVERT(VARCHAR,mov.CODPAISBENEF) AS PAISDELBENEFICIARIOORDENANTE,
--16
'''' AS DATOSADICIONALES,
--17
