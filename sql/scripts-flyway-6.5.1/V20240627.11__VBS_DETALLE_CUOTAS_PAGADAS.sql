...
SELECT 
	...
	ROW_NUMBER() OVER(PARTITION BY PP.SALDO_JTS_OID, PP.C2300 ORDER BY pp.C2300, P.HP_JTS_OID ASC) AS NUMERADOR_HP, 
...
INNER JOIN 
(SELECT SALDOS_JTS_OID, HP_JTS_OID, CUOTA,max(FECHAVALOR) AS  FECHAVALOR, max(NROASIENTOMOV) AS NROASIENTOMOV , SUM(CAPITALPAGADO) AS CAPITALPAGADO , SUM(INTERESPAGADO) AS INTERESPAGADO ,
