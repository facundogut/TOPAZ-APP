...
@NOMBRE VARCHAR(15), @APELLIDO VARCHAR (15), @LIMITE_CREDITO VARCHAR(2), @NRO_TARJETA_TITULAR VARCHAR(19), @SALIDA NUMERIC(1) OUTPUT
...
BEGIN
	...
	IF (@flag = 0 AND @NRO_PERSONA <> 0 AND @NRO_CLIENTE <> 0)
	  BEGIN
	  	INSERT INTO TJD_LINK_MAESTRO (ID_TARJETA, COD_TRAN, TIPO_TARJETA, ESTADO_TARJETA, FECHA_ENTREGA_TARJ, VENCIMIENTO_TARJETA, LIMITE_MONTO_TARJETA, NUM_VERSION_TARJ, DIGITO_VERIFICADOR_TARJ, NRO_PERSONA, NRO_CLIENTE, PRODUCTO, NOMBRE_TARJETA, LIMITE_CREDITO, TARJETA_TITULAR) 
	  	VALUES (@NRO_TARJETA, @COD_TRAN, @TIPO, @ESTADO, @FECHA_ENTREGA, @VENCIMIENTO, @MONTO_LIMITE, @NUM_VERSION, @DIGITO_VERIF, @NRO_PERSONA, @NRO_CLIENTE, @PRODUCTO, @NOMBRE_COMPLETO, @LIMITE_CREDITO, @NRO_TARJETA_TITULAR);  
	  	SET @SALIDA = 0;
      END 
    ELSE
      BEGIN
