...
BEGIN	
	...
	WHERE PF.IIBB_CABA_PERCEPCION_COND NOT IN ('NA', NULL) 
	AND PF.ESTADO=0
	...
	FROM CLI_PersonasFisicas PF
	LEFT JOIN CLI_DocumentosPFPJ D ON PF.NUMEROPERSONAFISICA=D.NUMEROPERSONAFJ	
	WHERE PF.IIBB_CABA_RETENCION_COND NOT IN ('NA', NULL) 
	AND PF.ESTADO=0
	...
	WHERE c.IIBB_CABA_PERCEPCION_COND NOT IN ('NA', NULL) 
	...
	FROM cLI_CLIENTES C
	INNER JOIN VW_CLIENTES_PERSONAS V ON C.CODIGOCLIENTE=V.CODIGOCLIENTE
	WHERE C.IIBB_CABA_RETENCION_COND NOT IN ('NA', NULL) 
	AND V.ESTADOCLIENTE=0
	AND V.ESTADOPERSONA=0
	...
	WHERE (IIBB_CABA_PERCEPCION_COND NOT IN ('NA', NULL) OR IIBB_CABA_RETENCION_COND NOT IN ('NA', NULL))
	AND ESTADO=0
	...
	UPDATE CLI_PersonasFisicas
	SET IIBB_CABA_PERCEPCION_COND = CASE WHEN convert(NUMERIC(11,7),replace(A.Alicuota_Percepcion,',',''))/100=0 THEN 'EX' ELSE 'AC' END
		, IIBB_CABA_PERCEPCION_ALI = convert(NUMERIC(11,7),replace(A.Alicuota_Percepcion,',',''))/100  
		, IIBB_CABA_RETENCION_COND = CASE WHEN convert(NUMERIC(11,7),replace(A.Alicuota_RETENCION,',',''))/100=0 THEN 'EX' ELSE 'AC' END
...
--	UPDATE clientes percepcion
	UPDATE CLI_CLIENTES
	SET IIBB_CABA_PERCEPCION_COND = CASE WHEN convert(NUMERIC(11,7),replace(t.IIBB_CABA_PERCEPCION_ALI,',',''))/100=0 THEN 'EX' ELSE 'AC' END
...
--	UPDATE clientes retencion
	UPDATE CLI_CLIENTES
	SET IIBB_CABA_RETENCION_COND = CASE WHEN convert(NUMERIC(11,7),replace(t.IIBB_CABA_retencion_ALI,',',''))/100=0 THEN 'EX' ELSE 'AC' END
...
--								, t.letra_alicuota 
	...
	--- ACTUALIZO PADRON-----
	DELETE FROM ITF_AGIP_PADRON WHERE Fecha_VIGENCIA_DESDE=(SELECT TOP 1 FECHA_VIG_DESDE FROM ITF_AGIP_PADRON_IMPUESTOS_AUX)
   	...
   	SELECT fecha_vig_desde
   			...
   			, Tipo_Contr_Insc
   			, Marca_alta_sujeto
   			, convert(NUMERIC(11,7),replace(Alicuota_Percepcion,',',''))/100 
   			, convert(NUMERIC(11,7),replace(Alicuota_Retencion,',',''))/100    
   	...
   	BEGIN 
   		...
   		DELETE FROM ITF_AGIP_PADRON WHERE datediff(MONTH,try_CONVERT(DATE, SUBSTRING(fecha_publicacion,5,4)+'-' +SUBSTRING(fecha_publicacion,3,2)+'-'+SUBSTRING(fecha_publicacion,1,2)),@fechaLimite)>0
   	END 
   	-----------------------------------------------------------------------------------------------------------
