...
----------------------------OPERACIONES/CONVENIOS DE RECAUDACIONES
	...
	if exists (	SELECT * 
				FROM CONV_TIPOS b with (nolock)
				left join CONV_CONVENIOS_REC a with (nolock) on a.Id_TpoConv = b.Id_TpoConv 
				where a.Id_TpoConv is null
			)	
				SELECT ''Tabla CONV_TIPOS no se ajusta a la integridad referencial con CONV_CONVENIOS_REC '',*
				FROM CONV_TIPOS b with (nolock)
				left join CONV_CONVENIOS_REC a with (nolock) on a.Id_TpoConv = b.Id_TpoConv  
				where a.Id_TpoConv is null
	else
		print ''Comprobación OK.Tabla CONV_TIPOS con CONV_CONVENIOS_REC''
