...
(CASE WHEN sos.Subsistema IN (''PF'',''CE'',''ME'') THEN '''' 
	  WHEN sos.Subsistema IN (''CC'',''CA'') AND sos.TIPOPERACION NOT IN (501, 503) THEN '''' 
      WHEN sos.Subsistema IN (''CC'',''CA'') AND sos.TIPOPERACION IN (501, 503) 
...
'''' AS CODIGOSTOPAZ,
--19
(SELECT TipC.TIPO_CAMBIO_OFICIAL FROM HISTORICOTIPOSCAMBIO TipC WHERE TipC.FECHA_COTIZACION = mov.FECHA AND Tipc.MONEDA = sa.MONEDA AND Tipc.TZ_LOCK = 0) AS COTIZACION,
--20
ISNULL((SELECT (CASE WHEN asi.ESTADO = 42 THEN ''SI'' ELSE ''NO'' END) FROM ASIENTOS asi 
