...
SELECT ID_DEBITO, SALDO_JTS_OID, MONEDA, IMPORTE, FECHA_VTO, FECHA_COMP, CBU, REFERENCIA, CODIGO_CLIENTE, CUIT_EO, MOTIVO_RECHAZO,PRESTACION
FROM SNP_DEBITOS  
WHERE ESTADO = ''RC''
...
--    SET @CantRegistros = 0;
	...
	--SET @FA_NUMERO_BLOQUES = RIGHT(concat(replicate(''0'', 6), @CantRegistrosPrev+1), 6);
	    SET @FA_NUMERO_BLOQUES = RIGHT(concat(replicate(''0'', 6), FLOOR((SELECT COUNT(*) 
																			 FROM ITF_DD_PRESENTADOS_RECIBIDOS_ENVIAR_AUX 
																			 WHERE LINEA LIKE ''7%'' 
																			 OR LINEA LIKE ''6%'')/10)+1), 6);
