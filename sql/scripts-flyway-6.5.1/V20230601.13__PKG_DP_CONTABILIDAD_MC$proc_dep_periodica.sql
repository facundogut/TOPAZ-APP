execute('
CREATE or alter PROCEDURE [dbo].[PKG_DP_CONTABILIDAD_MC$proc_dep_periodica]
...
BEGIN TRY
	...
	----------------------------------------------------------------------------------
	SET @v_tabla = ''MOVIMIENTOS_CONTABLES''
	...
	SET @v_where = ''MOVIMIENTOS_CONTABLES.FECHAPROCESO < @fecha''				
	EXECUTE PKG_DEPURACION$proc_dep_periodica @p_id_proceso,
													@p_dt_proceso,
													@v_tabla,
													''PKG_DP_CONTABILIDAD_MC$proc_dep_periodica'',
													'''',
													@v_where,
													'' FECHAPROCESO '',
													@v_fecha,
	...
	----------------------------------------------------------------------------------
	SET @v_tabla = ''GRL_DETALLE_CONTABILIDAD''
	SET @v_old_name = SUBSTRING(@v_tabla, 0, 18) + ''_OLD_'' + replace(convert(VARCHAR,getdate(), 104), ''.'' , '''');
	SET @v_stmt = ''SELECT * INTO '' + @v_old_name + '' FROM '' + @v_tabla + '' with (nolock) '' + '' WHERE 1 = 0'';
	print ''Creando tabla de BACKUP '' + @v_old_name;
	EXECUTE(@v_stmt);	
	...
	SET @v_where = ''GRL_DETALLE_CONTABILIDAD.MOV_FECHA_PROCESO < @fecha''
	EXECUTE PKG_DEPURACION$proc_dep_periodica @p_id_proceso,
													@p_dt_proceso,
													@v_tabla,
													''PKG_DP_CONTABILIDAD_MC$proc_dep_periodica'',
...
BEGIN CATCH
    ...
    SET @p_msg_proceso = ''El proceso PKG_DP_CONTABILIDAD_MC$proc_dep_periodica produjo un error. Error original: '' + ERROR_MESSAGE() + '' Linea: '' + CAST(ERROR_LINE() AS VARCHAR(50));	
