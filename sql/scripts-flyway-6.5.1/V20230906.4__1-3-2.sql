...
ALTER  PROC  SP_ITF_AD_ASIENTOS_CONTABLES
...
FROM ITF_ASIENTOS_EXTERNOS (nolock) WHERE ID=@idCount;
	...
	SELECT  (SELECT MAX(JTS_OID) FROM BANDEJA_ASIENTOS_IN WHERE ESTADO=''S'' AND ORIGEN=''AAC''), s.SUCURSAL, s.PRODUCTO, s.CUENTA, s.MONEDA, s.OPERACION, s.ORDINAL, 0, a.COD_MOVIMIENTO, (a.IMPORTE_DEBE + a.IMPORTE_HABER), CONCAT(''1.3.2 ADINTAR CONTABILIDAD - '',a.CONCEPTO_ASIENTO), CONCAT(a.CAPITULO_CONT,a.RUBRO_CONT,a.MONEDA_CONT,a.CUENTA_CONT,a.SUCUENTA_CONT), (SELECT FECHAPROCESO FROM PARAMETROS), a.MONEDA
