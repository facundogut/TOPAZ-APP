...
SELECT CLIENTE AS ''Cliente'',''Total'' AS ''Total'',SUM(DEUDA_EXIGIBLE) AS ''Deuda Exigible'',SUM(DEUDA_NO_EXIGIBLE) AS ''Deuda no Exigible'',SUM(DEUDA_TOTAL) ''Deuda Total'',SUM(DEUDA_CONTINGENTE) ''Deuda Contingente'',
CASE WHEN SUM(DEUDA_TOTAL) < SUM(DEUDA_CONTINGENTE) THEN SUM(DEUDA_CONTINGENTE) 
	 ELSE SUM(DEUDA_TOTAL) END AS ''Riesgo Total''
FROM ( 
SELECT CLIENTE, CASE WHEN CLASIFICACION=''L'' THEN ''Leasing''
...
WHERE vw.SALDO>0 --AND vw.CLIENTE=90026
GROUP BY CLIENTE,CLASIFICACION
) T
GROUP BY CLIENTE
