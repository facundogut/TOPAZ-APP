...
CREATE OR ALTER PROCEDURE [SP_ITF_CAJA_MUNICIPAL] @formato AS VARCHAR(20), @fch_proceso AS VARCHAR(8)
AS
BEGIN
	--------------- DECLARO VRIABLES --------------- 						
	...
	BEGIN
		DECLARE cursor_ayuda cursor FOR
			SELECT DISTINCT cadena_03 FROM REC_EXT_RECAUDOS_CANAL WHERE FORMATO = @formato AND FECHA = @fch_proceso;
		
		OPEN cursor_ayuda;
		FETCH NEXT FROM cursor_ayuda INTO @convenio	
		...
		BEGIN		
			...
			DECLARE cursor_convenios cursor FOR
				...
				FROM REC_EXT_RECAUDOS_CANAL WHERE FORMATO= @formato AND FECHA= @fch_proceso AND CADENA_03= @convenio;	
			...
			BEGIN
				...
				VALUES (@ID_CABEZAL, @ID_LINEA, @NRO_06, @NRO_02, @CADENA_12, @CADENA_12, ''I'', '' '', 0, @TZ_LOCK);
			...
			VALUES (@ID_CABEZAL, ''I'', ''REC_CANAL_065'', 65, @fch_proceso, 111, 1, @ID_LINEA, @TOT_IMPORTE, @TZ_LOCK, 3333, ''Conv Sacheep - RAPIPAGOS - Canal'');
	...
	BEGIN
		...
		INSERT INTO dbo.REC_CAB_RECAUDOS_CANAL (ID, ESTADO, ARCHIVO, CONVENIO, FECHACARGA, ID_LIQUIDACION, MONEDA, TOTALREGISTROS, TOTALIMPORTE, TZ_LOCK, TOTAL_CARGO_ESPECIFICO, NOMBRE)
			VALUES (@ID_CABEZAL, ''I'', ''REC_CANAL_099'', 99, @fch_proceso, 111, 1, @CANT_REG, @TOT_IMPORTE, @TZ_LOCK, 9999, ''Conv Sacheep - RAPIPAGOS - Canal'');
