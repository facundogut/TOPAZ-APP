...
SELECT DISTINCT L.ID_LIQUIDACION AS ''ID_LIQUIDACION'', 
		L.ESTADO AS ''ESTADO'', 
		...
		CASE WHEN l.CONVENIO_PADRE = 0 THEN 
		CONVERT(DATETIME,(CASE WHEN C.Acreditacion = OE.OPCIONINTERNA AND OE.OPCIONINTERNA = ''24H''
			...
			THEN dbo.diaHabil(DATEADD(day,2,L.FECHA),''A'') 
			WHEN C.Acreditacion = OE.OPCIONINTERNA AND 	OE.OPCIONINTERNA = ''DIA''
			THEN dbo.diaHabil(DATEADD(day,0,L.FECHA),''A'')
			WHEN C.Acreditacion = OE.OPCIONINTERNA AND 	OE.OPCIONINTERNA = ''MES''
			THEN dbo.ULTIMODIAHABIL(L.FECHA)
			END)) 
		 ELSE L.FECHA	
		...
		CONVERT(DATETIME,(CASE WHEN C.Acreditacion = OE.OPCIONINTERNA AND OE.OPCIONINTERNA = ''24H''
			...
			WHEN C.Acreditacion = OE.OPCIONINTERNA AND 	OE.OPCIONINTERNA = ''48H''
			THEN dbo.diaHabil(DATEADD(day,2,RC.Fecha_Cobranza),''A'') 
			WHEN C.Acreditacion = OE.OPCIONINTERNA AND 	OE.OPCIONINTERNA = ''DIA''
			THEN dbo.diaHabil(DATEADD(day,0,RC.Fecha_Cobranza),''A'')
			WHEN C.Acreditacion = OE.OPCIONINTERNA AND 	OE.OPCIONINTERNA = ''MES''
			THEN dbo.ULTIMODIAHABIL(RC.Fecha_Cobranza)
			END)) AS ''FECHA_REND_CANAL'',
			
		CONVERT(DATETIME,(CASE WHEN C.Acreditacion = OE.OPCIONINTERNA AND OE.OPCIONINTERNA = ''24H''
			THEN dbo.diaHabil(DATEADD(day,1,RJ.FECHACARGA),''A'')
			WHEN C.Acreditacion = OE.OPCIONINTERNA AND 	OE.OPCIONINTERNA = ''48H''
			THEN dbo.diaHabil(DATEADD(day,2,RJ.FECHACARGA),''A'') 
			WHEN C.Acreditacion = OE.OPCIONINTERNA AND 	OE.OPCIONINTERNA = ''DIA''
			THEN dbo.diaHabil(DATEADD(day,0,RJ.FECHACARGA),''A'')
			WHEN C.Acreditacion = OE.OPCIONINTERNA AND 	OE.OPCIONINTERNA = ''MES''
			THEN dbo.ULTIMODIAHABIL(RJ.FECHACARGA)
			END)) AS ''FECHA_REND_CAJA'',
		RC.ID AS ''ID_CABEZAL''
