    					  ...
    					  -- (COD_BANCO, NUMERO_DEL_CHEQUE, NUMERO_CUENTA_GIRADORA, FECHA_DEL_CHEQUE, SUCURSAL, SERIE_DEL_CHEQUE, TIPO_DOCUMENTO)
    					   ...
    					   --Varibles para conformar @RI_ENTIDAD_DEBITAR
    						...
    						SET @RI_ENTIDAD_DEBITAR = @ENTIDAD + @SUCURSAL;
				 			...
				 			--CUENTA DEBITAR
				 			-- PRIMARY KEY (TIPO_DOCUMENTO, SERIE_DEL_CHEQUE, NUMERO_CHEQUE, BANCO_GIRADO, SUCURSAL_BANCO_GIRADO, NUMERICO_CUENTA_GIRADORA)
				 			SET @CMC7 = (SELECT s.CMC7 FROM CLE_CHEQUES_SALIENTE s WHERE s.SERIE_DEL_CHEQUE = @T_SERIE_DEL_CHEQUE AND s.NUMERO_CHEQUE = @T_NUMERO_DEL_CHEQUE AND s.BANCO_GIRADO = @BANCO AND s.SUCURSAL_BANCO_GIRADO = @T_SUCURSAL AND s.NUMERICO_CUENTA_GIRADORA = @T_NUMERO_CUENTA_GIRADORA);
				 			
				 			/*(SELECT s.CMC7 FROM CLE_CHEQUES_SALIENTE s 
    						...
    						/*(SELECT RIGHT(REPLICATE(''0'', 11)+ CAST(IMPORTE AS VARCHAR(11)), 11) AS IMPORTE
    											 FROM CLE_CHEQUES_ENVIADOS
    				 		...
    				 		/*(SELECT RIGHT(REPLICATE(''0'', 13)+ CAST(NUMERO_DEL_CHEQUE AS VARCHAR(13)), 13) AS NUMERO_DEL_CHEQUE 
    						            		 FROM CLE_CHEQUES_ENVIADOS 
    											 WHERE NUMERO_DEL_CHEQUE = @T_NUMERO_DEL_CHEQUE AND COD_BANCO = @BANCO);*/
