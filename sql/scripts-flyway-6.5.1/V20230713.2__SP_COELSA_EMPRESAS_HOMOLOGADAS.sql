...
BEGIN
	...
	DECLARE @DD_NombreEmpresa VARCHAR(30);
	...
	DECLARE @DD_NomEntidad VARCHAR(30);
	...
	BEGIN
		...
		(CUIT_EO,NOMBRE_EMPRESA,	PRESTACION,			ENTIDAD,	ORIGINANTE,		LARGO_ID,			SEGUNDO_VTO,			
		PERIODICIDAD_PAGO,ESTADO,FECHA_ALTA,ID_CONVENIO, NOMENTIDAD) VALUES
		(@IdEmpresa,@NombreEmpresa ,@DescTransaccion ,@EntidadOrigen ,'''' ,@LongClaveCliente ,@SegundoVencimiento ,
		@PeriocidadPago ,@ComisionUsoCuenta, (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)), (SELECT MAX(X.Id_ConvRec) 
																																																		   FROM CONV_CONVENIOS_REC X
																																																		   WHERE X.CUIT = @IdEmpresa
																																																		     AND X.Estado = ''A''
	...
	SELECT CUIT_EO,NOMBRE_EMPRESA,PRESTACION,ENTIDAD,ORIGINANTE,LARGO_ID,SEGUNDO_VTO,PERIODICIDAD_PAGO,ESTADO,ID_CONVENIO,NOMENTIDAD FROM dbo.SNP_PRESTACIONES_EMPRESAS_AUX

	OPEN alta_cursor 

	FETCH NEXT FROM alta_cursor INTO @DD_IdEmpresa,@DD_NombreEmpresa ,@DD_DescTransaccion ,@DD_EntidadOrigen ,@DD_NombreEntidad ,@DD_LongClaveCliente ,@DD_SegundoVencimiento ,@DD_PeriocidadPago ,@DD_ComisionUsoCuenta, @DD_IdConvenio, @DD_NomEntidad
	...
	BEGIN
		...
		ELSE --MODIFICACION
			IF((SELECT COUNT(*) FROM SNP_PRESTACIONES_EMPRESAS 
			    WHERE (CUIT_EO = @DD_IdEmpresa AND PRESTACION = @DD_DescTransaccion) 
			    --AND (NOMBRE_EMPRESA <> @DD_NombreEmpresa OR ENTIDAD <> @DD_EntidadOrigen OR ORIGINANTE <> @DD_NombreEntidad OR LARGO_ID <> @DD_LongClaveCliente OR SEGUNDO_VTO <> @DD_SegundoVencimiento OR PERIODICIDAD_PAGO <> @DD_PeriocidadPago OR ESTADO <> @DD_ComisionUsoCuenta OR ID_CONVENIO <> @DD_IdConvenio OR NOMENTIDAD <> @DD_NomEntidad)
			    ) > 0)
			BEGIN 	
