...
IF (SELECT DB_NAME()) = @BD
  BEGIN
	...
	if exists (
			...
			select top 1 ''Tabla CLI_INSTITUCIONFINANCIERA no se ajusta a la integridad referencial con CLI_CLIENTES '',*
			from CLI_INSTITUCIONFINANCIERA b with(nolock)
			where b.CODIGOCLIENTE not in ( 
										select a.CODIGOCLIENTE 
										from CLI_CLIENTES a with(nolock) )
	...
	if exists (
				...
				select top 1  ''Tabla CLI_CLIENTES no se ajusta a la integridad referencial con CLI_ENTIDAD_CALIFICADORA '',*
				from CLI_CLIENTES b with(nolock)
				where b.ENTIDADCALIFICADORA  not in ( 
											select a.ENTIDAD 
											from CLI_ENTIDAD_CALIFICADORA a with(nolock) )
	...
	if exists (	select a.NROCLIENTE   
			...
			select  top 1 ''Tabla CLI_BLOQUEOS no se ajusta a la integridad referencial con CLI_CLIENTES '',*
			from CLI_BLOQUEOS a with(nolock)
			where a.NROCLIENTE not in (	select b.CODIGOCLIENTE 
										from CLI_CLIENTES b with(nolock))
	else
	...
	if exists (	select a.SEGMENTOCLIENTE  
				...
				select  top 1 ''Tabla CLI_CLIENTES no se ajusta a la integridad referencial con CLI_SEGMENTOS '',*
				from CLI_CLIENTES a with(nolock)
				where a.SEGMENTOCLIENTE not in (select b.COD_SEGMENTO 
											from CLI_SEGMENTOS b with(nolock))
	else
	...
	if exists (select a.COD_SUBSEGMENTO  
			...
			select  top 1 ''Tabla CLI_SUBSEGMENTOS no se ajusta a la integridad referencial con CLI_SEGMENTOS '',*
			from CLI_SUBSEGMENTOS a with(nolock)
			where a.COD_SEGMENTO not in (select b.COD_SEGMENTO  
										from CLI_SEGMENTOS b with(nolock))
	else
	...
	if exists (select * 
			...
			select  top 1 ''Tabla BITACORA_PERSONAS_JURIDICAS no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS '',*
			from BITACORA_PERSONAS_JURIDICAS a with(nolock)
			where a.NUMEROPERSONAJURIDICA not in (	select b.NUMEROPERSONAJURIDICA 
													from CLI_PERSONASJURIDICAS b with(nolock))
	else
	...
	if exists (select b.TIPOSOCIEDAD 
			...
			select  top 1 ''Tabla CLI_PERSONASJURIDICAS no se ajusta a la integridad referencial con CLI_TIPO_SOCIEDAD '',*
			from CLI_PERSONASJURIDICAS b  with(nolock)
			where b.TIPOSOCIEDAD not in (	select a.CODIGO_SOCIEDAD 
											from CLI_TIPO_SOCIEDAD a with(nolock)) 
	else
	...
	if exists ( select a.CODIGO_ACTIVIDAD 
			...
			select top 1  ''Tabla cli_actividad_economica no se ajusta a la integridad referencial con CLI_Cod_Act_AFIP '',*
			from cli_actividad_economica a with(nolock)
			where a.CODIGO_ACTIVIDAD not in (select b.CODIGO_ACT_AFIP 
											 from CLI_Cod_Act_AFIP b with(nolock))
	else
	...
	if exists (select a.CODIGO_SECCION 
			...
			select  top 1 ''Tabla CLI_Cod_Act_AFIP no se ajusta a la integridad referencial con CLI_SECCION '',*
			from CLI_Cod_Act_AFIP a with(nolock)
			where a.CODIGO_SECCION not in (select b.CODIGO_SECCION 
											from CLI_SECCION b with(nolock))
	else
	...
	if exists (select a.CODIGO_BCRA 
			...
			select  top 1 ''Tabla CLI_Cod_Act_AFIP no se ajusta a la integridad referencial con CLI_Cod_Act_BCRA '',*
			from CLI_Cod_Act_AFIP a with(nolock)
			where a.CODIGO_BCRA not in (select b.CODIGO_BCRA 
										from CLI_Cod_Act_BCRA b with(nolock))
	else
	...
	if exists (select a.CODIGO_SECTOR 
			...
			select  top 1 ''Tabla CLI_Cod_Act_AFIP no se ajusta a la integridad referencial con CLI_ACTIVIDAD_SECTOR '',*
			from CLI_Cod_Act_AFIP a with(nolock) 
			where a.CODIGO_SECTOR not in (	select b.CODIGO_SECTOR 
											from CLI_ACTIVIDAD_SECTOR b with(nolock))
	else
	...
	if exists ( select a.CODIGOPAIS 
				...
				left join CLI_DIRECCIONES b on b.PROVINCIA = a.DIM1
												and b.DEPARTAMENTO = a.DIM2
												and b.LOCALIDAD = a.DIM3

				where	b.provincia is null
					and b.departamento is null
					and b.localidad is null				
				)	
			select  top 1 ''Tabla cli_localidades no se ajusta a la integridad referencial con CLI_DIRECCIONES '',*
			from cli_localidades a with(nolock)
			left join CLI_DIRECCIONES b on  b.PROVINCIA = a.DIM1
										and b.DEPARTAMENTO = a.DIM2
										and b.LOCALIDAD = a.DIM3
	...
	select * from cli_departamentos
	select * from cli_localidades
	if exists ( select	a.DIM2 
				from	cli_localidades a with(nolock)
				where	A.DIM2 NOT IN (SELECT b.departamento FROM cli_departamentos b with(nolock))			
				)	
			select  top 1 ''Tabla cli_localidades no se ajusta a la integridad referencial con cli_departamentos '',*
			from	cli_localidades a with(nolock)
			where	A.DIM2 NOT IN (SELECT b.departamento FROM cli_departamentos b with(nolock))
	else
	...
	if exists ( select	a.PAIS 
			...
			select  top 1 ''Tabla CLI_DIRECCIONES no se ajusta a la integridad referencial con cli_Paises '',*
			from	cli_DIRECCIONES a with(nolock)
			where	A.PAIS NOT IN (SELECT b.codigoPAIS FROM CLI_PAISES b with(nolock))	
	else
	...
	if exists ( select	a.provincia 
			...
			select  top 1 ''Tabla CLI_DIRECCIONES no se ajusta a la integridad referencial con CLI_PROVINCIAS '',*
			from	cli_DIRECCIONES a with(nolock)
			where	A.PAIS NOT IN (SELECT b.dim1 FROM CLI_PROVINCIAS b with(nolock))	
	else
	...
	if exists ( select	a.departamento 
			...
			select  top 1 ''Tabla CLI_DIRECCIONES no se ajusta a la integridad referencial con CLI_DEPARTAMENTOS '',*
			from	cli_DIRECCIONES a with(nolock)
	...
	if exists (select a.CODIGOPAIS 
			...
			select  top 1 ''Tabla CLI_PROVINCIAS no se ajusta a la integridad referencial con CLI_PAISES '',*
			from CLI_PROVINCIAS a with(nolock)
			where a.CODIGOPAIS not in (select b.CODIGOPAIS 
										from CLI_PAISES b with(nolock))
	else
	...
	if exists ( select a.dim1 
				from CLI_PROVINCIAs a with(nolock)
				where a.dim1 not in (	select b.provincia 
										from cli_departamentos b with(nolock))
				)	
			select  top 1 ''Tabla CLI_PROVINCIAS no se ajusta a la integridad referencial con cli_departamentos '',*
			from CLI_PROVINCIAS a with(nolock)
			where a.dim1 not in (select b.provincia 
								 from cli_departamentos b with(nolock))
	else
	print ''Comprobación OK. Tabla CLI_PROVINCIAS con cli_departamentos ''
	...
	if exists (select a.TIPO_ROL 
			...
			select  top 1 ''Tabla cli_vinculos no se ajusta a la integridad referencial con CLI_ROLES '',*
			from cli_vinculos a with(nolock)
			where a.TIPO_ROL not in (select b.TIPO_ROL 
									from CLI_ROLES b with(nolock))
	else
	...
	if exists (select a.VINCULO 
			...
			select  top 1 ''Tabla CLI_VINCULACIONES no se ajusta a la integridad referencial con CLI_VINCULOS '',*
			from CLI_VINCULACIONES a with(nolock)
			where a.VINCULO not in (select b.ID 
									from CLI_VINCULOS b with(nolock))
	else
	...
	if exists (select a.ID 
			...
			select  top 1 ''Tabla CLI_VINCULOS no se ajusta a la integridad referencial con CLI_VINCULOS_PRIMARIOS '',*
			from CLI_VINCULOS a with(nolock)
			where a.VINCULO_PRIMARIO not in (select b.ID 
								from CLI_VINCULOS_PRIMARIOS b with(nolock))
	else
	...
	if exists (select a.ID 
			...
			select top 1  ''Tabla CLI_VINCULOS no se ajusta a la integridad referencial con CLI_VINCULOS_SECUNDARIOS '',*
			from CLI_VINCULOS a with(nolock)
			where a.VINCULO_SECUNDARIO not in (select b.ID 
								from CLI_VINCULOS_SECUNDARIOS b with(nolock))
	else
	...
	if exists (	select a.NUMEROPERSONAFISICA 
			...
			select  top 1 ''Tabla CLI_PFCONYUGE no se ajusta a la integridad referencial con CLI_VINCULACIONES '',*
			from CLI_PFCONYUGE a with(nolock)
			where a.NUMEROPERSONAFISICA not in (select b.PERSONA_VINCULADA 
												from CLI_VINCULACIONES b with(nolock))
	else
	...
	if exists (	select * 
			...
			select  top 1 ''Tabla CLI_INTEGRANTESPJ no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS ''
			FROM CLI_INTEGRANTESPJ a with(nolock)
			WHERE a.NUMEROPERSONAJURIDICA NOT IN (	SELECT b.NUMEROPERSONAJURIDICA 
													FROM CLI_PERSONASJURIDICAS b with(nolock))
	else
	...
	if exists (	SELECT * 
			...
			select  top 1 ''Tabla CLI_PERSONASJURIDICAS no se ajusta a la integridad referencial con CLI_SECTORES '',*
			FROM CLI_PERSONASJURIDICAS b with(nolock)
			WHERE b.SECTOR NOT IN (	select a.SECTOR 
									FROM CLI_SECTORES a with(nolock))
	else
	...
	if exists (	SELECT b.SECTOR 
			...
			select top 1  ''Tabla CLI_PERSONASFISICAS no se ajusta a la integridad referencial con CLI_SECTORES '',*
			FROM CLI_PERSONASFISICAS b with(nolock)
			WHERE b.SECTOR NOT IN (	select a.SECTOR 
									FROM CLI_SECTORES a with(nolock))
	else
	...
	if exists (	select a.ID_PERSONA 
				...
				select  top 1 ''Tabla CLI_INGRESOS_ANUALES no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS '',*
				from CLI_INGRESOS_ANUALES a with(nolock)
				where a.ID_PERSONA not in (	select b.NUMEROPERSONAJURIDICA 
											from CLI_PERSONASJURIDICAS b with(nolock))
	else
	...
	if exists (	select a.NUMERO_PERSONA 
			...
			select  top 1 ''Tabla ITF_MATRIZ_IGR no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS y CLI_PERSONASFISICAS '',*
			from ITF_MATRIZ_IGR a with(nolock) 
			where a.NUMERO_PERSONA in (select b.NUMEROPERSONAJURIDICA  
										from CLI_PERSONASJURIDICAS b with(nolock)
										union all 
	...
	if exists (	select a.ID_PERSONA
		...
		select  top 1 ''Tabla CLI_PERFILDOCUMENTAL no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS o CLI_PERSONASFISICAS '',*
		from CLI_PERFILDOCUMENTAL  a with(nolock)
		where a.ID_PERSONA not in (	select b.NUMEROPERSONAJURIDICA
									from CLI_PERSONASJURIDICAS b with(nolock)
									union all 
	...
	if exists (	select a.TIPODOCUMENTO 
			...
			select  top 1 ''Tabla CLI_DocumentosPFPJ no se ajusta a la integridad referencial con CLI_TIPOSDOCUMENTOS '',*
			from CLI_DocumentosPFPJ a with(nolock)
			where a.TIPODOCUMENTO not in (	select b.TIPODOCUMENTO 
											from CLI_TIPOSDOCUMENTOS b with(nolock))
	else
	...
	if exists (	select a.TIPODOC_IDENT 
			...
			select  top 1 ''Tabla CLI_TIPOS_DOC_FISICOS no se ajusta a la integridad referencial con CLI_TIPOSDOCUMENTOS '',*
			from CLI_TIPOS_DOC_FISICOS  a with(nolock)
			where a.TIPODOC_IDENT not in (	select b.TIPODOCUMENTO
											from CLI_TIPOSDOCUMENTOS b with(nolock))
	else
	...
	if exists (	select a.CODIGO_DOCUMENTO
				...
				select  top 1  ''Tabla CLI_DOCUMENTOS_PERSONAS no se ajusta a la integridad referencial con CLI_DOCUMENTOS '',*
				from CLI_DOCUMENTOS_PERSONAS a with(nolock)
				where a.CODIGO_DOCUMENTO not in (	select b.CODIGO_DOCUMENTO
													from CLI_DOCUMENTOS  b with(nolock))
	else
	...
	if exists (	select a.NUMEROPERSONAFJ 
			...
			select  top 1 ''Tabla CLI_DocumentosPFPJ no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS O CLI_PERSONASFISICAS '',*
			from CLI_DocumentosPFPJ  a  with(nolock)
			where a.NUMEROPERSONAFJ not in (select b.NUMEROPERSONAJURIDICA
											from CLI_PERSONASJURIDICAS b with(nolock)
											union all
	...
	if exists (	select A.COD_PROFESION 
			...
			select  top 1 ''Tabla CLI_PERSONASFISICAS no se ajusta a la integridad referencial con CLI_PROFESIONES '',*
			from CLI_PERSONASFISICAS A with(nolock)
			WHERE A.COD_PROFESION NOT IN (	select B.PROFESION 
											from CLI_PROFESIONES B with(nolock))
	else
	...
	if exists (	select a.SUCURSALVINCULADA
		...
		select   top 1 ''Tabla CLI_CLIENTES no se ajusta a la integridad referencial con SUCURSALES '',*
		from CLI_CLIENTES  a with(nolock)
		where a.SUCURSALVINCULADA not in (	select b.SUCURSAL
											from SUCURSALES b with(nolock))
	else
	...
	if exists (	select a.GRUPOAFINIDAD
		...
		select   top 1 ''Tabla CLI_CLIENTES no se ajusta a la integridad referencial con CLI_GRUPOSAFINIDAD '',*
		from CLI_CLIENTES  a with(nolock)
		where a.GRUPOAFINIDAD not in (	select b.CODIGOGRUPOAFINIDAD
										from CLI_GRUPOSAFINIDAD b with(nolock))
	else
	...
	if exists (	select a.CODIGOCLIENTE
		...
		select  top 1 ''Tabla CLI_SUCESIONES no se ajusta a la integridad referencial con CLI_CLIENTES '',*
		from CLI_SUCESIONES  a  with(nolock)
		where a.CODIGOCLIENTE not in (	select b.CODIGOCLIENTE
										from CLI_CLIENTES b with(nolock))
	else
	...
	if exists (	select a.NUMEROPERSONAFJ 
			...
			select  top 1 ''Tabla CLI_CONCURSO_ACREEDORES no se ajusta a la integridad referencial con CLI_PERSONASFISICAS O CLI_PERSONASJURIDICAS '',*
			from CLI_DocumentosPFPJ  a with(nolock)
			inner join CLI_CONCURSO_ACREEDORES b with(nolock)
				on b.CUIT_CUIL = a.NUMERODOCUMENTO
			where  a.NUMEROPERSONAFJ not in (	select c.NUMEROPERSONAFISICA 
...
/*	if exists (	select a.NUMEROPERSONAFJ 
		...
		select ''Tabla CLI_CONCURSO_ACREEDORES no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS '',*
		from CLI_DocumentosPFPJ  a with(nolock)
		inner join CLI_CONCURSO_ACREEDORES b with(nolock)
			on b.CUIT_CUIL = a.NUMERODOCUMENTO
		where  a.NUMEROPERSONAFJ not in (select c.NUMEROPERSONAJURIDICA 
...
*/
	if exists (	select a.CUIT 
			...
			select  top 1 ''Tabla ITF_BCRA_PADFYJ no se ajusta a la integridad referencial con CLI_PERSONASFISICAS o CLI_PERSONASJURIDICAS '',*
			from ITF_BCRA_PADFYJ a with(nolock)
			inner join CLI_DocumentosPFPJ b with(nolock) on a.cuit = b.NUMERODOCUMENTO
			where b.NUMEROPERSONAFJ not in (	select b.NUMEROPERSONAFISICA 
												from CLI_PERSONASFISICAS b with(nolock)
	...
	if exists (	SELECT A.CUIT 
			...
			select  top 1 ''Tabla CLI_INHABILITADOS_INAES no se ajusta a la integridad referencial con CLI_PERSONASFISICAS o CLI_PERSONASJURIDICAS'',*
			FROM CLI_INHABILITADOS_INAES A with(nolock)
			INNER JOIN CLI_DocumentosPFPJ B with(nolock) ON A.CUIT = B.NUMERODOCUMENTO
			WHERE B.NUMERODOCUMENTO NOT IN (	select b.NUMEROPERSONAJURIDICA
													from CLI_PERSONASJURIDICAS b with(nolock)
	...
	if exists (	SELECT A.CUIT_CUIL
			...
			select   top 1 ''Tabla CLI_INHABILITADOS_UIF no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS o CLI_PERSONASFISICAS '',*
			FROM CLI_INHABILITADOS_UIF A with(nolock)
			INNER JOIN CLI_DocumentosPFPJ B with(nolock) ON A.CUIT_CUIL = B.NUMERODOCUMENTO 
															AND A.TIPODOCUMENTO= B.TIPODOCUMENTO 
															AND  A.TIPOPERSONA = B.TIPOPERSONA
	...
	if exists (	select a.DOCUMENTO 
		...
		select  top 1 ''Tabla CLI_LISTA_BLANCA no se ajusta a la integridad referencial con CLI_PERSONASFISICAS o CLI_PERSONASJURIDICAS '',*
		from CLI_LISTA_BLANCA a with(nolock)
		inner join CLI_DocumentosPFPJ b on a.TIPO_DOCUMENTO = b.tipodocumento 
										and a.DOCUMENTO = b.NUMERODOCUMENTO
		where b.NUMEROPERSONAFJ not in (	select b.NUMEROPERSONAJURIDICA
...
/*
	if exists (	select a.CUIL 
		...
		select  top 1 ''Tabla CLI_MAESTRO_FALLECIDOS no se ajusta a la integridad referencial con CLI_PERSONASFISICAS '',*
		from CLI_MAESTRO_FALLECIDOS a with(nolock)
					inner join CLI_DocumentosPFPJ b with(nolock)on a.CUIL = b.NUMERODOCUMENTO
					where b.NUMEROPERSONAFJ not in (select c.NUMEROPERSONAFISICA
													from CLI_PERSONASFISICAS c with(nolock))
...
*/ 
	...
	if exists (select a.CODIGOCLIENTE 
		...
		select  top 1 ''Tabla CLI_ClientePersona no se ajusta a la integridad referencial con CLI_clientes '',*
		from CLI_ClientePersona a with(nolock) 
		where a.CODIGOCLIENTE not in (select b.CODIGOCLIENTE 
										from CLI_clientes b with(nolock))
	else
	...
	if exists (select b.CODIGOCLIENTE 
		...
		select  top 1 ''Tabla CLI_clientes no se ajusta a la integridad referencial con CLI_ClientePersona '',* 
		from CLI_clientes b with(nolock)
		where b.CODIGOCLIENTE not in (select a.CODIGOCLIENTE 
										from CLI_ClientePersona a with(nolock))
	else
	...
	if exists (select a.numeropersona 
				...
				select  top 1 ''Tabla CLI_ClientePersona no se ajusta a la integridad referencial con CLI_PERSONASFISICAS O CLI_PERSONASJURIDICAS '',*
				from CLI_ClientePersona a with(nolock) 
				where a.numeropersona not in (	select b.NUMEROPERSONAFISICA 
												from CLI_PERSONASFISICAS b with(nolock)
											UNION ALL 
	...
	if exists (	select a.COD_CLIENTE
		...
		select   top 1 ''Tabla CLI_CORRESPONSALES no cumple la integridad referencial con CLI_INSTITUCIONFINANCIERA  '',* 
		from CLI_CORRESPONSALES a
		where a.COD_CLIENTE not in (select b.CODIGOCLIENTE 
									from CLI_INSTITUCIONFINANCIERA b)
	else
	...
	if exists (	select a.CODOFICIAL
				from cli_oficuenta a
				where a.CODOFICIAL not in (select b.EJECUTIVOCLIENTE 
											from cli_clientes b)
				)
		select   top 1 ''Tabla cli_oficuenta no cumple la integridad referencial con cli_clientes  '',* 
		from cli_oficuenta a
		where a.CODOFICIAL not in (select b.EJECUTIVOCLIENTE 
									from cli_clientes b)
	else
	print ''Comprobación OK.Tabla cli_oficuenta con cli_clientes ''
