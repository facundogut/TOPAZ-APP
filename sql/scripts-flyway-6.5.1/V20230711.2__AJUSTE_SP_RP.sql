...
BEGIN
	...
	SET @existeConv = (SELECT CASE WHEN isnumeric(@convenio) = 1 AND (SELECT COUNT(*) FROM CONV_CONVENIOS_REC (nolock) WHERE id_convrec = @convenio) > 0  THEN 1 ELSE 0 END);
	...
	BEGIN
		...
		FROM REC_CAB_RECAUDOS_CAJA cab JOIN
			...
			REC_RENDICION ren ON ren.ID_RENDICION = liq.ID_RENDICION 
				...
				WHERE ren.CONVENIO = @convenio AND ren.FECHA = CAST(@fecha AS DATE)
					GROUP BY ren.fecha;
		...
		FROM 
			...
			REC_LIQUIDACION liq ON liq.ID_LIQUIDACION = cab.ID_LIQUIDACION JOIN 
			REC_RENDICION ren ON ren.ID_RENDICION = liq.ID_RENDICION 
				WHERE cab.CONVENIO = @convenio AND ren.FECHA = CAST(@fecha AS DATE);
	END
	ELSE 	
