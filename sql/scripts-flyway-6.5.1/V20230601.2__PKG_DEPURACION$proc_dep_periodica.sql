execute('
CREATE or alter PROCEDURE [dbo].[PKG_DEPURACION$proc_dep_periodica]
...
BEGIN TRY
	...
	EXECUTE sp_executesql @v_stmt, N''@v_cantidad float(38) output, @fecha datetime'', @v_cantidad OUTPUT, @v_fecha;
		...
		begin		
			...
			begin				
				...
				WHILE @v_procesadosOk < @v_cantidad 
					...
					BEGIN
						BEGIN TRY
							EXECUTE sp_executesql @v_stmt, N''@fecha datetime'', @v_fecha;
			...
			EXEC USP_DisableEnableNonClusteredIndexes  @DB_NAME,2, @p_tabla, '' ''-- REBUILD NON-CLUSTERED INDEXES cambiando el Recovery Model a Simple para el rebuild.
	...
	''Se depuraron '' + ISNULL(str(@v_cantidad_dep), ''0'') + 
