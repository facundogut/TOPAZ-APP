...
ALTER   PROCEDURE SP_CALCULA_GRADUACION
    ...
    @EXCLUIDO VARCHAR(1),
    @CON_GTIA VARCHAR(1),
    @TOTAL_DEUDA    NUMERIC(15,2) OUTPUT,
    @TRAMO_SIN_G   NUMERIC(15,2) OUTPUT, 
    @TRAMO_CON_G  NUMERIC(15,2) OUTPUT,
    @TRAMO_SIN_G_B   NUMERIC(15,2) OUTPUT, 
    @TRAMO_CON_G_B  NUMERIC(15,2) OUTPUT,
    @RPC_BANCO NUMERIC(15,2)OUTPUT,
    @EXCLUSIONES NUMERIC(15,2)OUTPUT,
    @TOT_CONTROL_GRAD NUMERIC(15,2)OUTPUT,
    @RPC_CLIENTE NUMERIC(15,2)OUTPUT,
    @RPC_CLIENTE_MAX NUMERIC(15,2)OUTPUT,
    @RPC_NETO_BANCO NUMERIC(15,2)OUTPUT,
    @MARGEN_RESGUAR NUMERIC(15,2)OUTPUT,
    @LIMITE_GRAD NUMERIC(15,2)OUTPUT,
    ...
    --- Parametros
    @CN1_BANCO NUMERIC(15,2) OUTPUT, ---600
    @MARGEN_BASICO NUMERIC(15,2) OUTPUT, ---601
    @RPC_CLIENTE_O NUMERIC(15,2) OUTPUT, ---602
    @RPC_BANCO_O NUMERIC(15,2) OUTPUT, ---603
    @RPC_CLIENTE_SGR_FGP NUMERIC(15,2) OUTPUT, ---604
    @RPC_BANCO_SGR_FGP NUMERIC(15,2) OUTPUT, ---605
    @PORC_TRAMO_SING NUMERIC(15,2) OUTPUT, ---606
    @PORC_TRAMO_CONG NUMERIC(15,2) OUTPUT, ---607
    @MARGEN_RESGUARDO NUMERIC(15,2) OUTPUT, ---608
...
BEGIN
	...
	DECLARE @MENOR_MARGEN NUMERIC(15,2);
	DECLARE @TIENE_GRUPO NUMERIC(5);
	DECLARE @GRUPO NUMERIC(12);
	DECLARE @DEUDA_CLIENTE NUMERIC(15,2);
	DECLARE @EXCLUSION_CLIENTE_G NUMERIC(15,2);
	DECLARE @EXCLUSION_ASISTENCIA_G NUMERIC(15,2);
	DECLARE @EXCLUSION_PRODUCTO_G NUMERIC(15,2);
	DECLARE @EXCLUSION_GARANTIA_G NUMERIC(15,2);
	DECLARE @EXCLUSION_DESCUENTO_G NUMERIC(15,2);
	/*DECLARE @EXCLUSION_CLIENTE_F_CONG NUMERIC(15,2);
	DECLARE @EXCLUSION_CLIENTE_F_SING NUMERIC(15,2);
	DECLARE @EXCLUSION_ASISTENCIA_F_CONG NUMERIC(15,2);
	DECLARE @EXCLUSION_ASISTENCIA_F_SING NUMERIC(15,2);
	DECLARE @EXCLUSION_PRODUCTO_F_CONG NUMERIC(15,2);
	DECLARE @EXCLUSION_PRODUCTO_F_SING NUMERIC(15,2);
	DECLARE @EXCLUSION_GARANTIA_F_SING NUMERIC(15,2);
	DECLARE @EXCLUSION_GARANTIA_F_CONG NUMERIC(15,2);*/
	...
	DECLARE @TOTAL_DEUDA_GRUPO NUMERIC (15,2);
	DECLARE @EXCLUSIONES_GRUPALES_GRADUACION NUMERIC (15,2);
    ...
    ELSE
	    BEGIN
	    	...
	    	SET @TIPO_VINCULO = (SELECT TOP 1 TIPO_VINCULO FROM CRE_VINCULACIONES CV WITH(NOLOCK)  
								...
								TZ_LOCK = 0 AND CODIGOCLIENTE = @CLIENTE) AND (CASE WHEN CVR.PARAMETRO_613 = ''N'' THEN CV.FECHA_FIN
								ELSE DATEADD(yy,(SELECT IMPORTE FROM CRE_PARAMETROS WITH(NOLOCK) WHERE CODIGO = 613),CV.FECHA_FIN) END) >=
								(SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)) ORDER BY CV.TIPO_VINCULO ASC, CV.TIPO_RELACION DESC)
...
---------------ASISTENCIAS DEL CLIENTE------------------------------------------------------	
	...
	INNER JOIN CRE_FAMILIAS f WITH(NOLOCK) ON p.C6283 = f.FAMILIAPROD 
   ...
   --- Exclusiones cliente	
	...
	SET @EXCLUSION_DESCUENTO_G = ISNULL((SELECT SUM(IMPORTE_DEUDA) FROM @TABLA_ASISTENCIAS WHERE CLIENTE = @CLIENTE
	...
	SET @EXCLUSIONES = @EXCLUSION_CLIENTE_G+@EXCLUSION_ASISTENCIA_G+@EXCLUSION_PRODUCTO_G+@EXCLUSION_GARANTIA_G+@EXCLUSION_DESCUENTO_G;
	...
	IF(@CON_GTIA = ''S'')
		BEGIN
			SET @TRAMO_CON_G = ISNULL((ISNULL((SELECT SUM(IMPORTE_DEUDA) FROM @TABLA_ASISTENCIAS  
	   							WHERE CLIENTE=@CLIENTE AND TIENE_GARANTIA = ''S''),0)+@IMPORTE_SOLICITUD),0);
			SET @TRAMO_SIN_G = ISNULL((SELECT SUM(IMPORTE_DEUDA) FROM @TABLA_ASISTENCIAS 
								WHERE CLIENTE=@CLIENTE AND TIENE_GARANTIA = ''N''),0);
		END
	ELSE
		BEGIN
			SET @TRAMO_CON_G = ISNULL(ISNULL((SELECT SUM(IMPORTE_DEUDA) FROM @TABLA_ASISTENCIAS
	   							WHERE CLIENTE=@CLIENTE AND TIENE_GARANTIA = ''S''),0),0);
			SET @TRAMO_SIN_G = (ISNULL((SELECT SUM(IMPORTE_DEUDA) FROM @TABLA_ASISTENCIAS 
								WHERE CLIENTE=@CLIENTE AND TIENE_GARANTIA = ''N''),0)+@IMPORTE_SOLICITUD);
		END
...
--RESULTADO ANALISIS
	IF(@TIENE_GRUPO=0 OR @TIENE_GRUPO IS NULL)
		--GRADUACION
		BEGIN 			
    		IF((@LIMITE_GRAD>=@TOT_CONTROL_GRAD OR @RPC_CLIENTE=0))
    			BEGIN
			...
			ELSE IF(@CLIENTE_EXCLUIDO_GRAD>0)
				BEGIN
				 	...
				 	SET @EXCEDE_GRADUACION_O = ''N'';
				END
		 	ELSE IF (@TOPE_ANALISIS<@TOT_CONTROL_GRAD)
				BEGIN
			...
			ELSE
				BEGIN
					SET @RESULTADO_GRADUACION = ''Exceso en Graduación: ''+ CONVERT(VARCHAR(17),FORMAT(ROUND(@TOT_CONTROL_GRAD-@LIMITE_GRAD,0), ''C'', ''es-ar'')) + ''.'';;
		...
		--GRANDES EXPOSICIONES
			...
			ELSE IF (@TRAMO_SIN_G_B>=@TRAMO_SIN_G AND @TRAMO_CON_G_B<(@TRAMO_CON_G+@TRAMO_SIN_G) )
				BEGIN
					SET @RESULTADO_FRACCIONAMIENTO = ''Encuadrado en Grandes Exposiciones sin garantía.'' +char(10) + char(13) + ''Exceso en Grandes Exposiciones con y sin garantía: '' + CONVERT(VARCHAR(17),FORMAT(ROUND((@TRAMO_CON_G+@TRAMO_SIN_G)-@TRAMO_CON_G_B,0), ''C'', ''es-ar'')) + ''.'';
					SET @EXCEDE_FRACCIONAMIENTO_O = ''S'';
				END	
			ELSE IF (@TRAMO_SIN_G_B<@TRAMO_SIN_G AND @TRAMO_CON_G_B>=(@TRAMO_CON_G+@TRAMO_SIN_G) )
				BEGIN
					SET @RESULTADO_FRACCIONAMIENTO = ''Exceso en Grandes Exposiciones sin garantía: '' + CONVERT(VARCHAR(17),FORMAT(ROUND(@TRAMO_SIN_G-@TRAMO_SIN_G_B,0), ''C'', ''es-ar'')) + ''.'' +char(10) + char(13) + ''Encuadrado en Grandes Exposiciones con y sin garantía. '';
					SET @EXCEDE_FRACCIONAMIENTO_O = ''S'';
				END
			ELSE IF (@TRAMO_SIN_G_B<@TRAMO_SIN_G AND @TRAMO_CON_G_B<(@TRAMO_CON_G+@TRAMO_SIN_G) )
				BEGIN
					SET @RESULTADO_FRACCIONAMIENTO = ''Exceso en Grandes Exposiciones sin garantía: '' + CONVERT(VARCHAR(17),FORMAT(ROUND(@TRAMO_SIN_G-@TRAMO_SIN_G_B,0), ''C'', ''es-ar'')) + ''.'' +char(10) + char(13) + ''Exceso en Grandes Exposiciones con y sin garantía: '' + CONVERT(VARCHAR(17),FORMAT(ROUND((@TRAMO_CON_G+@TRAMO_SIN_G)-@TRAMO_CON_G_B,0), ''C'', ''es-ar'')) + ''.'';
					SET @EXCEDE_FRACCIONAMIENTO_O = ''S'';
				END										
 		END
...
IF(@ES_VINCULADO > 0 AND @TIPO_VINCULO <> 1)
	--CONTROL DE VINCULADOS
		BEGIN
			EXECUTE dbo.SP_CONTROL_GYF_VINCULADOS @CLIENTE, @PERSONA, @TIPO_VINCULO, @IMPORTE_SOLICITUD, @EXCEDE_GRAD_VINC OUT, @RESULTADO_IND_VINC OUT, @RESULTADO_GRUP_VINC OUT
