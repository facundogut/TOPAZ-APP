/****** Object:  StoredProcedure [dbo].[SP_COELSA_CHEQUES_PRESENTADOS_RECIBIDOS]    Script Date: 02/06/2021 15:40:39 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER PROCEDURE [dbo].[SP_COELSA_CHEQUES_PRESENTADOS_RECIBIDOS] @TICKET NUMERIC(16)
AS
BEGIN
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
SELECT LINEA 
FROM dbo.ITF_CHEQUES_RECIBIDOS_AUX WITH (NOLOCK)
...
BEGIN  
      ...
      BEGIN
      	...
      	INSERT INTO dbo.ITF_COELSA_CHEQUES_PROPIOS (ID_TICKET, ID_PROCESO, FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE, CODIGO_POSTAL, FECHA_PRESENTADO, FECHA_VENCIMIENTO, NRO_CHEQUE, PUNTO_INTERCAMBIO, TRACE_NUMBER, ESTADO, TIPO)
      	VALUES (@TICKET, @TICKET, (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK)), @CodTransaccion, @EntidadDebitar, @CuentaDebitar, @Importe, @CodigoPostal, @FechaPresentacion, @FechaVencimiento, @NumeroCheque, @PuntoIntercambio, @ContadorRegistros, 'P', (CASE WHEN CAST(@CuentaDebitar AS NUMERIC) = 88888888888 THEN 'A' WHEN CAST(@CuentaDebitar AS NUMERIC) = 77777777777 THEN 'D' ELSE 'C' END));
      	...
      	BEGIN
      		...
      		INSERT INTO dbo.CLE_CHEQUES_AJUSTE(ORDINAL, NUMERO_CHEQUE, BANCO, NUMERO_CUENTA, SUCURSAL_BANCO_GIRADO, ESTADO, CODIGO_POSTAL, FECHA_ALTA, SUCURSAL_CLIENTE, IMPORTE, P_INTERCAMBIO, ENVIADO_RECIBIDO, TRACKNUMBER)
      		VALUES((SELECT MAX(ORDINAL) + 1 
					FROM CLE_CHEQUES_AJUSTE WITH (NOLOCK)), @NumeroCheque, @Entidad, @CuentaDebitar, @Sucursal, 'I', @CodigoPostal, (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK) ), @Sucursal, @Importe, @PuntoIntercambio, 'R', @ContadorRegistros);
      	END
      END
