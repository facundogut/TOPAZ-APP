EXECUTE('
CREATE PROCEDURE [dbo].[SP_TASAS_DPF_POS]
(
  @PCliente NUMERIC(12),
  @PPlazo	NUMERIC(10),
...
BEGIN
	...
	SELECT TOP 1 @TASA = Tasa FROM TasasBase WITH(NOLOCK) WHERE (TipoTasaBase=''N'' AND TZ_LOCK = 0) 
	...
	SET @CABECERA = ''382''+''1P''+CONVERT(VARCHAR,(SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)),12)+''10''+''36'';
	SET @CABECERA = @CABECERA + REPLICATE('' '', 22 - LEN(@CABECERA));
	SET @LINEA1 = (SELECT C6251 FROM PRODUCTOS WITH(NOLOCK) WHERE C6250 = @PProducto);
	...
	SET @TASA_FINAL = REPLACE(SUBSTRING(CONVERT(VARCHAR(20),@TASA_CALCULADA),1,7),''.'','','')+''%''; 
...
BEGIN
	...
	SET @LINEA2 = @LINEA2 + REPLICATE('' '', 36 - LEN(@LINEA2));
	...
	BEGIN
		...
		SET @PRECANCELA = CONVERT(VARCHAR, @TASA_PRECANCELA) + ''%'';
		
		SET @LINEA3 = ''TASA FIJA DE PRECANCELACION: '';
		SET @LINEA3 = @LINEA3 + REPLICATE('' '',36-(LEN(@LINEA3)+LEN(@PRECANCELA)))+@PRECANCELA;
		
		SET @LINEA4 = ''........IMPORTE DESDE'';
		SET @LINEA4 = @LINEA4 + REPLICATE('' '', 33-LEN(@LINEA4)) + ''TNA'';
...
IF(@PPlazo = 0 AND @PImporte > 0)
	BEGIN
		...
		SET @LINEA2 = ''IMPORTE: '';
		SET @LINEA2 = @LINEA2 + REPLICATE('' '', 36 - (LEN(@LINEA2)+LEN(CONVERT(VARCHAR(17),FORMAT(@PImporte, ''C'', ''es-ar''))))) + CONVERT(VARCHAR(17),FORMAT(@PImporte, ''C'', ''es-ar''));
