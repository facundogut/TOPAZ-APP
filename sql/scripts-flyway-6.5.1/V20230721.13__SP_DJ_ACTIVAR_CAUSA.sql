...
AS 
   ...
   BEGIN
	 ...
	 SELECT  
	  ...
	  INNER JOIN VW_DJ_PODERES po WITH (NOLOCK) ON tt.TIPO_PODER = po.TIPO_PODER
	...
	--COMPLETO TABLA BENEFICIARIOS--
	  ...
	  INNER JOIN VW_DJ_PODERES_BENEFICIARIO po WITH (NOLOCK) ON tt.TIPO_PODER = po.TIPO_PODER
	  WHERE  s.NRO_JUZGADO  =  @P_NUM_JUZG
	  	AND s.NRO_SOLICITUD = @P_NUM_SOLIC
	  	AND s.NRO_CAUSA = @P_NUM_CAUSA
	  	AND s.TZ_LOCK = 0
	...
	--COMPLETO TABLA AUXILIAR DE REGISTROS EXISTENTES EN PYF_APODERADOS--
	 ...
	 FROM VW_DJ_PODERES PA WITH (NOLOCK) 
	 INNER JOIN @Tabla_ACTIVAR_CAUSA TA 
      ...
      BEGIN TRANSACTION	
       ...
       BEGIN TRY
				...
				AND TRY_CAST(TA.JTS_OID_CUENTA AS DECIMAL(10,0)) NOT IN (SELECT PA.ID_ENTIDAD FROM @Tabla_PYF_APODERADOS AS PA)
