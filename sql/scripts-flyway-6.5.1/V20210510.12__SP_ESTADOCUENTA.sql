					...
					BEGIN
                        BEGIN TRY
							...
							WHERE SUCURSALES.SUCURSAL = @vSucursal 
									AND ((SUCURSALES.TZ_LOCK < 300000000000000 OR SUCURSALES.TZ_LOCK >= 400000000000000) AND (SUCURSALES.TZ_LOCK < 100000000000000 OR SUCURSALES.TZ_LOCK >= 200000000000000))
                           --EXECUTE sysdb.ssma_oracle.db_error_exact_one_row_check @@ROWCOUNT
                        END TRY
                        BEGIN CATCH
                           DECLARE
					...
					BEGIN
                        BEGIN TRY
                           ...
                           WHERE PRODUCTOS.C6250 = @vProd 
								AND ((PRODUCTOS.TZ_LOCK < 300000000000000 OR PRODUCTOS.TZ_LOCK >= 400000000000000) AND (PRODUCTOS.TZ_LOCK < 100000000000000 OR PRODUCTOS.TZ_LOCK >= 200000000000000))
                           --EXECUTE sysdb.ssma_oracle.db_error_exact_one_row_check @@ROWCOUNT
                        END TRY
                        BEGIN CATCH
                           DECLARE
