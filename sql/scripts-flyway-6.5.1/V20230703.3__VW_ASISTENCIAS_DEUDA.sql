EXECUTE('
IF OBJECT_ID (''dbo.VW_ASISTENCIAS_DEUDA'') IS NOT NULL
	DROP VIEW dbo.VW_ASISTENCIAS_DEUDA
')

EXECUTE('
CREATE VIEW VW_ASISTENCIAS_DEUDA
AS
SELECT 
...
INNER JOIN ( 
	...
	WHERE ((PP.C2302<= (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK))) OR ((SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK)) BETWEEN PP.C2301 AND PP.C2302 AND PP.C2301 < (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK)))) AND ( (PP.TZ_LOCK < 300000000000000 OR PP.TZ_LOCK >= 400000000000000) AND (PP.TZ_LOCK < 100000000000000 OR PP.TZ_LOCK >= 200000000000000)  )
	) T
...
GROUP BY S.JTS_OID,s.C1803,s.c1604,S.C1614,S.ICMORA_CONTABILIZADO,IVA_FINANCIADO,IVA_PERCEPCION_FINANCIADO,C1609,C1610,s.ICMORA_CONTABILIZADO
')