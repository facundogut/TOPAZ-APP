EXECUTE('
CREATE OR ALTER        PROCEDURE [dbo].[SP_ITF_LINK_TRX_REFRESH] 
...
BEGIN
    SET @Error = '' '';
  	...
  	BEGIN TRY
  		...
  		USING (
  		       SELECT DISTINCT ID_TARJETA = SUBSTRING(Linea, 731, 19),
		...
		WHERE EXISTS(SELECT 1 FROM TJD_TIPOS_NOVEDADES_LINK (nolock) WHERE CODIGO_LINK = SUBSTRING(Linea, 1, 6)  AND ACTUALIZA_BD = ''S'' AND TZ_LOCK = 0)
		  ...
		  AND (substring(Linea,470,6) IS NOT NULL AND substring(Linea,470,6) <> ''      '')
		  AND (substring(Linea,476,16) IS NOT NULL AND substring(Linea,476,16) <> ''                '')
		  AND (substring(Linea,492,6) IS NOT NULL AND substring(Linea,492,6) <> ''      '')
		  AND (substring(Linea,498,16) IS NOT NULL AND substring(Linea,498,16) <> ''                '')
		) AS B
		...
		USING (SELECT DISTINCT Tipo_Doc = substring(Linea,536,3),
		...
		WHERE EXISTS(SELECT 1 FROM ITF_LINK_TRK A 
					 WHERE SUBSTRING(A.Linea, 1, 6) IN (190000,190002,190007)
					   AND Nro_Raiz=substring(Linea,132,19) AND Nro_Version=CONVERT(NUMERIC,substring(Linea,763,1))-1
		...
		SET Estado =''X''
		FROM ITF_LINK_TRK 
		WHERE SUBSTRING(Linea, 1, 6) IN (190000,190002,190007) 
			AND ID_TARJETA_BASE=substring(Linea,132,19) AND NUM_VERSION=CONVERT(NUMERIC,substring(Linea,763,1))-1;
		...
		SET Estado =''3''
		FROM ITF_LINK_TRK 
		WHERE SUBSTRING(Linea, 1, 6) IN (190010) 
			AND ID_TARJETA_BASE=substring(Linea,132,19) AND NUM_VERSION=CONVERT(NUMERIC,substring(Linea,763,1))-1;

		--
		MERGE INTO TJD_ITF_TARJETAS_COMPLETAS AS A
		USING (SELECT DISTINCT Nro_Tarjeta = substring(Linea,731,19),
		...
		ON A.NroTarjeta = B.NroTarjeta AND A.CUENTA = B.CUENTA 
		...
		USING (
				SELECT ID_TARJETA, JTS_OID, Tipo_Cuenta, PRODUCTO, CUENTA, MONEDA, SUCURSAL, ORDINAL_PREFERENCIA, Estado_Cuenta, CUENTA_PBF, MAX(PRIORITARIA) PRIORITARIA
				FROM (
						SELECT ID_TARJETA = substring(LINK.Linea,731,19),
		    		  ...
		    		  PRIORITARIA = (CASE WHEN (ROW_NUMBER() OVER(PARTITION BY NroTarjeta  ORDER by NroTarjeta)) = 1 THEN 1 ELSE 0 END),
		    		  LINK.LINEA
            ...
            JOIN SALDOS S ON S.JTS_OID = VS.JTS_OID_SALDO 
              ...
              AND (substring(Linea,731,19) <> ''                   '' and  substring(Linea,731,19) <> ''0000000000000000000'') 
				...
				GROUP BY ID_TARJETA, JTS_OID, Tipo_Cuenta, PRODUCTO, CUENTA, MONEDA, SUCURSAL, ORDINAL_PREFERENCIA, Estado_Cuenta, CUENTA_PBF		
		) as B
		 ON A.ID_TARJETA = B.ID_TARJETA AND A.SALDO_JTS_OID = B.JTS_OID
		WHEN MATCHED
		  THEN
			UPDATE SET A.ESTADO = B.Estado_Cuenta,
					   A.PRIORITARIA = B.PRIORITARIA
		WHEN NOT MATCHED THEN
		...
		USING (SELECT DISTINCT ID_TARJETA = substring(Linea,731,19),  
			   	ESTADO = substring(Linea, 780, 1),
			   	FECHA_ENTREGA = CASE WHEN substring(Linea, 825, 8) <> ''00000000'' THEN CAST(substring(Linea, 825, 8) AS DATETIME) ELSE NULL End,
	        	VENCIMIENTO = TRY_CAST(substring(Linea, 776, 4)+''01'' as DATETIME),
	        	...
	        	NUM_DOC_FISICO = REPLACE(substring(Linea,539,9),''.'' ,''''),
			   ...
			   FROM ITF_LINK_TRK TRX (nolock)
				JOIN VTA_SALDOS VS (nolock) ON VS.CTA_REDLINK = substring(TRX.Linea, 183, 21)
				...
				JOIN CLI_DocumentosPFPJ DPFJ (nolock) ON FORMAT(TRY_CAST(DPFJ.NUM_DOC_FISICO as NUMERIC), ''000000000'') = REPLACE(substring(Linea,539,9),''.'' ,'''') AND DPFJ.TIPO_DOC_FISICO = substring(Linea,536,3)
		 ...
		 WHEN MATCHED
			...
			UPDATE SET A.TIPO_TARJETA = (SELECT TIPO_TARJETA FROM TJD_TIPO_TARJETA (nolock) where CODIGO_PRODUCTO = B.CODIGO_PRODUCTO), 
				...
				A.ESTADO = B.ESTADO,
				A.NRO_PERSONA = B.NRO_PERSONA,
				A.NRO_CLIENTE = B.NRO_CLIENTE
		 WHEN NOT MATCHED AND B.JTS_OID_GTOS IS NOT NULL THEN
		...
		--Elimina inconsistencias que queden en el módulo entre TJD_TARJETAS y TJD_REL_TARJETAS
		DELETE from TJD_REL_TARJETA_CUENTA
		WHERE ID_TARJETA not in (select ID_TARJETA from TJD_TARJETAS)
