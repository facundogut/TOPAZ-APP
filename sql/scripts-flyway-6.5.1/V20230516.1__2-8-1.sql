...
SELECT concat(RIGHT(concat(replicate(''0'', 4), CC.NUMERO_BANCO), 4), RIGHT(concat(replicate(''0'',4), CC.NUMERO_DEPENDENCIA), 4)) AS EntidadDebitar,CC.NUMERO_BANCO AS CODBANCO, CC.NUMERO_DEPENDENCIA AS SUCURSAL,  RIGHT(concat(replicate(''0'', 12), CC.CUENTA), 12) AS CuentaDebitar,

 CC.IMPORTE AS Importe,  CC.NUMERO_CHEQUE AS NroCheque,

concat(''00'', RIGHT(concat(replicate(''0'', 4), CC.NUMERO_BANCO), 4)) AS CodigoPostal, concat(''0000'', RIGHT(concat(''0000'', CR.CODIGO_CAUSAL_DEVOLUCION), 4), replicate('' '', 8)) AS PtoIntercambio,
...
WHERE CC.NUMERO_CHEQUE = CR.NUMERO_CHEQUE AND (CR.CANJE_INTERNO=''N'' OR CR.CANJE_INTERNO IS NULL) AND CC.FECHA_VALOR = (SELECT FECHAPROCESO FROM PARAMETROS) AND CR.ESTADO_DEVOLUCION<>0 AND CC.TZ_LOCK = 0 AND CR.TZ_LOCK = 0;
...
SELECT NUMERO_CHEQUE, IMPORTE, BANCO, SUCURSAL_BANCO_GIRADO, CODIGO_POSTAL, TRACKNUMBER FROM CLE_CHEQUES_AJUSTE WHERE ESTADO = ''F'' AND ENVIADO_RECIBIDO = ''R'' AND ESTADO_AJUSTE = ''R'';
