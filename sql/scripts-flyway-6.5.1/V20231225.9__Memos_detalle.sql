...
AS 
   BEGIN
      ...
      ---Cabecera: Que es prevision? Historico prevision cliente - prevision final (Confirmar vives), Seccion?, Suc cliente? Sit atraso? CATEGORIAOBJETIVA,  Sector persona-Sector activ? Deudor encuadrado? Cliente refi? Sit juridica? Sit juridica ext? Max atraso?
      
      ---Detalle: Cuenta orden? Nro acuerdo? Int deven cobrar? - Historico devengamiento o saldo, Int adelantados? Iva financiado? RG Financiado? - BS_IVA_FINANCIADO_X_CUOTA o de SALDOS IVA_FINANCIADO, IVA_PERCEPCION, Seguro financ? - No existe, Otros accesorios? - Son gastos?, Prevision? Capital vencido? - Capital cuotas vencidas planpagos, Int deven vencido? 2310
      ---Capital original? Fecha 1ervto impago? Fecha vto? Dias atraso? Tasa nom anual? TNA original? Prom mens sdos diar? Acuerdo no utilizado? Fecha alta ac cc, numeric? Dias consec sdo deudor?
      ---Sit revista? ,Imp valor tasacion? Importe de cre_garantiasrecibidas Productos tipo GO, Aforo? CFT TNA? - Cre saldos, CFT TEA? - Cre_saldos, AJUSTE NIIF? Vives, NUMERAL? Número cualquiera
      ---Cuales cuentas? Solo asistencia?
      
      /*
      Importe valor tasacion
      aforo
      tipo instrumento NBCH
      */
      ...
      DECLARE @CANT_CABECERA NUMERIC(10)
      ...
      SET @CANT_CABECERA = (SELECT COUNT(1) FROM MEMO_CABECERA WITH(NOLOCK) WHERE FECHA_INFO = @FECHA_PROCESO)
      ...
      DELETE FROM MEMO_DETALLE WHERE Fecha_Info = @FECHA_PROCESO
                ...
                SELECT DISTINCT
                 DOC.TIPODOCUMENTO,
                 DOC.NUMERODOCUMENTO,
                 CASE WHEN C.TIPO = ''F'' THEN PF.PRIMERNOMBRE + '' '' + PF.APELLIDOPATERNO ELSE PJ.RAZONSOCIAL END AS NOMBRE,
                 C.SUCURSALVINCULADA,
                 S.CUENTA,
                 ...
                 C.CODIGOCLIENTE,
                 CASE WHEN CLD.TIPO = ''CHEQUE'' THEN CLD.numero ELSE 0 END AS NUMERO_CHEQUE,
                 CASE WHEN CLD.TIPO = ''FACTURA'' THEN CLD.numero ELSE 0 END AS NUMERO_BOLETA,
                 TMU.NUM_TJC,
                 TMA.DESCRIPCION AS ADMINISTRADORA,
                 ISNULL(DC.SALDO,0), --- AS Capital adeudado
                 ...
                 CASE WHEN S.C1785 = 5 THEN (SELECT COUNT(1) FROM PLANPAGOS WITH(NOLOCK) WHERE SALDO_JTS_OID = S.JTS_OID) ELSE 0 END AS CANT_CUOTAS,
                 S.C1601,
                 HCS.CALIFICACION_OBJETIVA,
                 CS.ID_CONVENIO,
                 CCP.NomConvPago,
                 CD.ID_DOMINIO,
                 CD.DESCRIPCION,
                 SRS.SITUACION_REVISTA,
                 HPD.PREVISION_FINAL,
                 S.IVA_FINANCIADO,
                 ...
                 S.C1632 AS TNA,
                 BHP.TASAINTERES,
                 (SELECT AVG(SALDO_AJUSTADO) FROM GRL_SALDOS_DIARIOS WITH(NOLOCK) WHERE SALDOS_JTS_OID = VA.JTS_OID_SALDO AND TZ_LOCK = 0 AND MONTH(FECHA) = MONTH(@FECHA_PROCESO) AND YEAR(FECHA) = YEAR(@FECHA_PROCESO)),
                 ...
                 (SELECT SUM(DEVENGADO) FROM BS_COMISIONDESEMBOLSO WITH(NOLOCK) WHERE SALDO_JTS_OID=S.JTS_OID)
                 FROM MEMO_CABECERA MC WITH(NOLOCK)
                 INNER JOIN SALDOS S WITH(NOLOCK) ON S.C1803 = MC.Cliente AND S.TZ_LOCK = 0
                 ...
                 LEFT JOIN VW_DEUDA_CLIENTES DC WITH(NOLOCK) ON DC.JTS_OID = S.JTS_OID
                 LEFT JOIN BS_HISTORIA_PLAZO BHP WITH(NOLOCK) ON BHP.SALDOS_JTS_OID = S.JTS_OID AND BHP.TZ_LOCK = 0 AND BHP.TIPOMOV IN (''A'',''I'')
				 ...
				 LEFT JOIN VTA_SOBREGIROS VA WITH(NOLOCK) ON VA.JTS_OID_SALDO = S.JTS_OID AND VA.ESTADO = 1 AND VA.TZ_LOCK = 0
                 ...
                 LEFT JOIN HISTORICO_PREVISIONES_DETALLE HPD WITH(NOLOCK) ON HPD.SALDO_JTS_OID = S.JTS_OID AND
                 HPD.FECHA_DE_PROCESO = (SELECT TOP 1 FECHA_DE_PROCESO FROM HISTORICO_PREVISIONES_DETALLE WITH(NOLOCK) WHERE SALDO_JTS_OID = S.JTS_OID
                 AND FECHA_DE_PROCESO <= @FECHA_PROCESO ORDER BY FECHA_DE_PROCESO DESC)
                 LEFT JOIN VW_ASISTENCIAS_DEUDA VAD WITH(NOLOCK) ON DC.JTS_OID = VAD.JTS_OID
                 WHERE S.TZ_LOCK = 0 AND (DC.SALDO_DEUDA <> 0 OR DC.SALDO_DEUDA IS NULL) AND MC.Fecha_Info = @FECHA_PROCESO 
                 AND C.CODIGOCLIENTE IN (SELECT C1803 FROM SALDOS WITH(NOLOCK) WHERE (C1604<>0 AND C1785 IN (5,6)) OR (C1785 = 2 AND C1651<>''1'')
              ...
              ELSE
                ...
                BEGIN	
				  ...
				  SET @P_MSG_PROCESO = ''No se han encontrado registros en MEMO_CABECERA para la fecha''
					         
				  DECLARE
					@PKG_CONSTANTES$C_LOG_TIPO_INFORMACION$9 varchar(8000)
					
					SET @PKG_CONSTANTES$C_LOG_TIPO_INFORMACION$9 = ''E''
				  ...
				  EXECUTE dbo.PKG_LOG_PROCESO$PROC_INS_LOG_PROCESO 
					...
					@P_COD_ERROR = @P_RET_PROCESO, 
					@P_MSG_ERROR = @P_MSG_PROCESO, 
					@P_TIPO_ERROR = @PKG_CONSTANTES$C_LOG_TIPO_INFORMACION$9
