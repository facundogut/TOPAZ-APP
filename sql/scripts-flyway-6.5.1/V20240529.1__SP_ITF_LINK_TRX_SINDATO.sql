EXECUTE('
CREATE OR ALTER PROCEDURE    [dbo].[SP_ITF_LINK_TRX_SINDATO]
...
BEGIN
	...
	BEGIN TRY
	   ...
	   WHERE NOT EXISTS(SELECT 1 FROM VTA_SALDOS VS (nolock)
		JOIN SALDOS S (nolock) ON VS.CTA_REDLINK = substring(TRX.Linea, 183, 21) AND VS.JTS_OID_SALDO = S.JTS_OID AND S.C1785 = CASE WHEN substring(TRX.Linea, 183, 2) = ''01'' THEN 2 ELSE 3 END
		...
		--JOIN CLI_DocumentosPFPJ DPFJ (nolock) ON DPFJ.NUM_DOC_FISICO = REPLACE(substring(Linea,539,9),''.'' ,'''') AND DPFJ.TIPO_DOC_FISICO = substring(Linea,536,3)
		...
		FROM(
			...
			FROM TJD_ITF_TARJETAS_CUENTAS TC (nolock)
			  ...
			  JOIN VTA_SALDOS VS (nolock) ON CONCAT(TC.TIPO_CUENTA, TC.NUMERO_CUENTA) = VS.CTA_REDLINK 
		...
		--Merge para TJD_TARJETAS (Criterio 1)

		MERGE INTO TJD_TARJETAS as Target
		USING (
				...
				WHERE Orden=1 --AND ID_TARJETA NOT IN (SELECT ID_TARJETA FROM TJD_TARJETAS)
		) as Source
		ON Target.ID_TARJETA = Source.ID_TARJETA
		WHEN NOT MATCHED THEN
		    INSERT (JTS_OID_GTOS, TIPO_TARJETA, ID_TARJETA, NRO_CLIENTE, TITULARIDAD, ESTADO, FECHA_ENTREGA, VENCIMIENTO,
            NOMBRE_TARJETA, SUCURSAL, ID_TARJETA_TITULAR, NRO_PERSONA, PERMISO, LIMITE_CREDITO, LIMITE_DEBITO, NUM_VERSION,
            DIGITO_VERIFICADOR, ID_TARJETA_BASE)
			VALUES (Source.JTS_OID_GTOS, Source.TIPO_TARJETA, Source.ID_TARJETA, Source.NRO_CLIENTE, Source.TITULARIDAD, Source.ESTADO,
            Source.FECHA_ENTREGA, Source.VENCIMIENTO, Source.NOMBRE_TARJETA, Source.SUCURSAL, Source.ID_TARJETA_TITULAR,
            Source.NRO_PERSONA, Source.PERMISO, Source.LIMITE_CREDITO, Source.LIMITE_DEBITO, Source.NUM_VERSION,
            Source.DIGITO_VERIFICADOR, Source.ID_TARJETA_BASE)
		WHEN MATCHED THEN
			UPDATE SET Target.LIMITE_CREDITO = Source.LIMITE_CREDITO,
				Target.LIMITE_DEBITO = Source.LIMITE_DEBITO, 
				Target.NOMBRE_TARJETA = Source.NOMBRE_TARJETA,
				Target.DIGITO_VERIFICADOR = Source.DIGITO_VERIFICADOR,
				Target.ESTADO = Source.ESTADO;
		
		--Merge para TJD_TARJETAS (Criterio 2)
		...
		--Merge para TJD_TARJETAS (Criterio 3)
...
--Merge para TJD_TARJETAS (Criterio 4)
