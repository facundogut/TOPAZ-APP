...
DECLARE @periodoproceso datetime = dateadd(month,-1, CAST(CONCAT(SUBSTRING(@perParam,3,4),SUBSTRING(@perParam,1,2),''01'') AS DATETIME));
DECLARE @fecha datetime = CONVERT(DATETIME, (SELECT FECHAPROCESO FROM PARAMETROS), 103);

DECLARE @periodoActual DATETIME = @periodoproceso --CAST(CONCAT(SUBSTRING(@perParam,3,4),SUBSTRING(@perParam,1,2),''01'') AS DATETIME);
DECLARE @periodoAnterior DATETIME = dateadd(month,-1, @periodoproceso);
DECLARE @anio INT 
DECLARE @mes INT 
...
--var auxiliares
DECLARE @alicuota_perc_Aux NUMERIC(11,7), @alicuotaPercMax NUMERIC(11,7), @alicuotaPercVIEJA NUMERIC(11,7), @recaud_perc_Aux VARCHAR (2),
 @vinoEnPadron INT;
...
DECLARE @alicuota_rete_Aux NUMERIC(11,7), @alicuotaRetenMax NUMERIC(11,7), @alicuotaRetenVIEJA NUMERIC(11,7),
...
-- Eliminar los registros anteriores a la fecha limite
DELETE FROM ITF_AGIP_PADRON
