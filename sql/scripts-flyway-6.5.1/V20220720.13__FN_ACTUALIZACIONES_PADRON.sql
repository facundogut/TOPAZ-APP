EXECUTE('
CREATE OR ALTER function FN_ACTUALIZACIONES_PADRON (@fch_proceso DATETIME) 
RETURNS TABLE 
AS 
RETURN
WITH AUX AS(
SELECT	DISTINCT 
		...
		CASE 
			...
			THEN RIGHT(Ltrim(Rtrim(CP.CODIGOSWIFT)) + ''00'',4)
			ELSE DIR.CPA_NUEVO	
...
INNER JOIN CLI_DIRECCIONES DIR WITH(nolock) ON DIR.ID = PF.NUMEROPERSONAFISICA
											AND DIR.TIPODIRECCION = ''PR''

-- voy con PAIS de CLI_DIRECCIONES a la CLI_PAISES por su campo CODIGOPAIS   
LEFT JOIN CLI_PAISES CP WITH(nolock) ON CP.CODIGOPAIS = DIR.PAIS 
...
WHERE PF.FECHAALTA BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0) AND dateadd(d,-(day(dateadd(m,1,@fch_proceso))),dateadd(m,1,@fch_proceso))
	AND PF.NUMEROPERSONAFISICA IN (	SELECT NUMEROPERSONA 
									FROM CLI_CLIENTEPERSONA CP WITH(nolock) 
 									LEFT JOIN CLI_CLIENTES C WITH(nolock) ON CP.CODIGOCLIENTE = C.CODIGOCLIENTE 
										AND C.FECHAAPERTURA BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0) AND dateadd(d,-(day(dateadd(m,1,@fch_proceso))),dateadd(m,1,@fch_proceso))
  									WHERE CP.FECHAALTARELACION BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0) AND dateadd(d,-(day(dateadd(m,1,@fch_proceso))),dateadd(m,1,@fch_proceso)))
...
INNER JOIN CLI_DIRECCIONES DIR WITH(nolock) ON DIR.ID = PJ.NUMEROPERSONAJURIDICA 
											AND DIR.TIPODIRECCION = ''L''

-- voy con PAIS de CLI_DIRECCIONES a la CLI_PAISES por su campo CODIGOPAIS   
INNER JOIN CLI_PAISES CP WITH(nolock) ON CP.CODIGOPAIS = DIR. PAIS

LEFT JOIN RRII_PARAMETROS_PADRON PP WITH(nolock) ON D.TIPOPERSONA = PP.TIPO_PERSONA
WHERE PJ.FECHAALTA BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0) 
	AND dateadd(d,-(day(dateadd(m,1,@fch_proceso))),dateadd(m,1,@fch_proceso))
	AND PJ.NUMEROPERSONAJURIDICA IN (SELECT NUMEROPERSONA 
									FROM CLI_CLIENTEPERSONA CP WITH(nolock) 
 									LEFT JOIN CLI_CLIENTES C WITH(nolock) ON CP.CODIGOCLIENTE = C.CODIGOCLIENTE 
									AND C.FECHAAPERTURA BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0) 
									AND dateadd(d,-(day(dateadd(m,1,@fch_proceso))),dateadd(m,1,@fch_proceso))
  									WHERE CP.FECHAALTARELACION BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0) 
										AND dateadd(d,-(day(dateadd(m,1,@fch_proceso))),dateadd(m,1,@fch_proceso)))
...
SELECT	DISTINCT 
		...
		CASE 
			WHEN (PF.CODIGODERESIDENCIA = 3)
			THEN RIGHT(Ltrim(Rtrim(CPais.CODIGOSWIFT)) + ''00'',4)
			ELSE DIR.CPA_NUEVO	
		END SCOD_POSTAL_CAMBIO--,
		--ROW_NUMBER() OVER(PARTITION BY D.NUMERODOCUMENTO ORDER BY D.NUMERODOCUMENTO ASC) AS NUMERADOR_REPETIDO
FROM PARAMETROS P WITH(nolock),
...
-- voy con PAIS de CLI_DIRECCIONES a la CLI_PAISES por su campo CODIGOPAIS   
INNER JOIN CLI_PAISES CPais WITH(nolock) ON CPais.CODIGOPAIS = DIR.PAIS 
LEFT JOIN RRII_PARAMETROS_PADRON PP WITH(nolock) ON D.TIPO_DOC_FISICO = PP.NOMBRE_TIPO_DOC 
												AND D.TIPOPERSONA = PP.TIPO_PERSONA
												AND PP.NACIONALIDAD = CASE WHEN PF.PAISNACIMIENTO = 32 THEN '' ''
																			WHEN PF.PAISNACIMIENTO <> 32 THEN ''E''
...
AND PF.NUMEROPERSONAFISICA IN (SELECT NUMEROPERSONA 
								FROM CLI_CLIENTEPERSONA CP WITH(nolock))
AND B.FECHABLOQUEO BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0) 
AND dateadd(d,-(day(dateadd(m,1,@fch_proceso))),dateadd(m,1,@fch_proceso))
...
WHERE BD.FECHA < DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0)
		AND BD_AUX.FECHA BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0) AND @fch_proceso
...
aux3 AS (
		...
		LEFT JOIN RRII_PARAMETROS_PADRON PP  WITH(nolock) ON D.TIPO_DOC_FISICO = PP.NOMBRE_TIPO_DOC 
													AND D.TIPOPERSONA = PP.TIPO_PERSONA
		WHERE   BPF.FECHA < DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0)
				AND BPF_AUX.FECHA BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, @fch_proceso), 0)AND @fch_proceso
