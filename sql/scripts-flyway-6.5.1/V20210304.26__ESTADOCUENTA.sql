/****** Object:  StoredProcedure [dbo].[ESTADOCUENTA]    Script Date: 23/02/2021 15:30:45 ******/
DROP PROCEDURE [dbo].[ESTADOCUENTA]
GO
         ...
         DECLARE
             cSaldos CURSOR LOCAL FOR 
               ...
               FROM 
					...
					dbo.PLANCTAS  AS p with (nolock), 
					dbo.GRL_ESTADOS_DE_CUENTA  AS E with (nolock), 
					dbo.PRODUCTOS  AS Pr with (nolock)
               WHERE 
                  s.TZ_LOCK = 0 AND 
                  p.TZ_LOCK = 0 AND 
                  Pr.TZ_LOCK = 0 AND 
                  ...
                  (/* Obtener periodicidad dependiendo de si es Legal o no*/((	E.PERIODICIDAD = @vPeriodo 
																				OR @vPeriodo = 'P'
																			  ) AND @vLegal <> 'L'
																			 ) 
                           ...
                           BEGIN
                              BEGIN TRY
                                 ...
                                 FROM dbo.CLE_CHEQUES_SALIENTE  AS C  with (nolock)
										, dbo.CLE_DEPOSITOS  AS D with (nolock)
                                 WHERE 
                  ...
                  BEGIN
					...
					BEGIN
                     ...
                     /* OBTENGO CREDEB Y SIRCREB */
						...
						PRINT @vFechaCredeb
                        SELECT @vSIRCREBAUX = MONTO_FACTURABLE 
						FROM CI_FACTURA_LINEA with (nolock)
						WHERE ASIENTO = @vAsiento 
							AND OPERACION = @vOperacionTopaz 
							AND SUCURSAL_ASIENTO = @vSucursalOrigen
							AND FECHA_PROCESO_ASIENTO = @vFechasircreb AND 
							(ID_CARGO IN (SELECT ID_CARGO 
											FROM CI_CARGOS with (nolock) 
											WHERE TIPO_CARGO_IMPOSITIVO = 5
										) 
							OR ID_IMPUESTO IN (SELECT ID_IMPUESTO 
