EXECUTE('
IF OBJECT_ID(''dbo.ITF_DD_PRESENTADOS_EMITIDOS_AUX'') IS NOT NULL
DROP TABLE ITF_DD_PRESENTADOS_EMITIDOS_AUX;

IF OBJECT_ID(''dbo.SP_DD_PRESENTADOS_EMITIDOS'') IS NOT NULL
DROP PROCEDURE [dbo].[SP_DD_PRESENTADOS_EMITIDOS];
...
VALUES (0, 128, ''Débitos Directos prsentados emitidos 2.8.8'', ''ITF_COELSA_DEB_DIRECTOS_PRESENTADOS_EMITIDOS.kjb'', '''', '''', '''', '' '', '''', '''', '''', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', '' '', ''J'', '' '', 0, ''M'')
...
VALUES (167, 0, ''Cont Arch. COELSADPP'', '' '', '' '', '' '', 1, 0, ''20330707'', 0, 0, 0)

INSERT INTO dbo.ITF_MASTER_PARAMETROS (CODIGO, CODIGO_INTERFACE, FUNCIONALIDAD, ALFA_1, ALFA_2, ALFA_3, NUMERICO_1, NUMERICO_2, FECHA, IMPORTE_1, IMPORTE_2, TZ_LOCK)
VALUES (148, 11, ''Sec COELSA 2.8.8'', '' '', '' '', '' '', 5, 0, ''20330707'', 0, 0, 0)

ALTER TABLE SNP_MSG_ORDENES ADD PRESENTACION_PRIMER_VTO  DATETIME;
ALTER TABLE SNP_MSG_ORDENES ADD PRESENTACION_SEGUNDO_VTO  DATETIME;
ALTER TABLE SNP_MSG_ORDENES ADD ID_ARCHIVO_REVERSADO NUMERIC(9,0);
')
EXECUTE('
CREATE TABLE dbo.ITF_DD_PRESENTADOS_EMITIDOS_AUX
	(
...
EXECUTE('
CREATE PROCEDURE [dbo].[SP_DD_PRESENTADOS_EMITIDOS]
   @TICKET NUMERIC(16)
AS 
BEGIN
...
DECLARE @T_REVERSADO INT=0;
		...
		IF((SELECT COUNT(1) FROM SNP_MSG_ORDENES WHERE ESTADO=''PP'')=0)
		BEGIN
		---------------- Grabar Cabecera Lote ---------------------------
...
SELECT o.CORRELATIVO,r.SALDO_JTS_OID, o.MONEDA,o.IMPORTE, o.FECHA_VENCIMIENTO, o.FECHA_COMPENSACION, o.CBU, o.REFERENCIA, o.COD_CLIENTE_PAGADOR, o.CUIT_EO, o.ID_ARCHIVO_REVERSADO
FROM SNP_MSG_ORDENES o LEFT JOIN SNP_MSG_CABEZAL c ON o.CUIT_EO=c.CUIT_EO AND o.ID_ARCHIVO=c.ID_ARCHIVO AND o.NRO_ARCHIVO=c.NRO_ARCHIVO
 ...
 WHERE ((o.ESTADO=''PP'' AND o.PRESENTACION_PRIMER_VTO=(SELECT FECHAPROCESO FROM PARAMETROS)) OR (o.ESTADO=''R'' AND o.PRESENTACION_SEGUNDO_VTO=(SELECT FECHAPROCESO FROM PARAMETROS)))
ORDER BY o.CUIT_EO, o.MONEDA, o.ID_ARCHIVO_REVERSADO
				        		
OPEN CursorDD
FETCH NEXT FROM CursorDD INTO @T_ID_DEBITO, @T_JTS_OID, @T_MONEDA, @T_IMPORTE, @T_FECHA_VTO,@T_FECHA_COMP,@T_CBU, @T_REFERENCIA, @T_CODIGO_CLIENTE, @T_CUIT, @T_ID_ARCHIVO_REVERSADO
