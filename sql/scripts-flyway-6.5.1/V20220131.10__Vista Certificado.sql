...
SELECT 
		...
		FROM CLI_DIRECCIONES DD with (nolock)
		INNER JOIN  CLI_PROVINCIAS PPr with (nolock) ON S.SUCURSAL = DD.ID 
													AND DD.FORMATO = ''S'' 
													AND DD.TIPODIRECCION = ''R'' 
		...
		CASE (C6399) 
			WHEN (SELECT C6399 FROM MONEDAS with (nolock) 
				  WHERE C6403=''N'') THEN ''EN PESOS ARGENTINOS''
			WHEN (SELECT NUMERICO FROM PARAMETROSGENERALES with (nolock) 
				  WHERE CODIGO =18 ) THEN ''EN UNIDADES DE VALOR ADQUISITIVO ACTUALIZABLES POR CER (UVA)''
			WHEN (SELECT NUMERICO FROM PARAMETROSGENERALES with (nolock) 
				  WHERE CODIGO =709 ) THEN ''EN UNIDADES DE VIVIENDA ACTUALIZABLES POR CER (UVI)''
		END AS MONEDALETRA,
