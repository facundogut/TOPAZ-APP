...
SELECT r.NomConvRec, 
       ...
       (SELECT count(p2.ID) FROM CONV_PADRONES P2 
        WHERE P2.ID_TICKET = p2.ID_TICKET 
          AND P2.CONVENIO = (SELECT COD_ENTE FROM CONV_REL_ENTECONV WHERE ID_CONVREC =r.Id_ConvPadre)
  ...
  INNER JOIN CONV_CONVENIOS_REC R  WITH (nolock)
                                  ON P.CONVENIO = (SELECT COD_ENTE FROM CONV_REL_ENTECONV WHERE ID_CONVREC =r.Id_ConvPadre)
                                  AND R.Id_TpoConv = 18
   								  AND R.TZ_LOCK = 0
   								  AND R.Estado = ''A''
 WHERE P.TZ_LOCK = 0
