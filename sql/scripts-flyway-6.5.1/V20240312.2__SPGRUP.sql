...
INSERT INTO @TBL_AUX (CLIENTE, DEUDA_TOTAL, DEUDA_CON_G, DEUDA_SIN_G)
   ...
   AND TZ_LOCK = 0)) AND TIPO_GARANTIA IN
   (SELECT TIPO_GARANTIA FROM CRE_EXCLUSIONES_TIPOSGARANTIAS WITH(NOLOCK) WHERE TZ_LOCK=0)),0) 
   FROM @TBL_AUX T
   
  
	UPDATE @TBL_AUX
    SET TOT_CONTROL_GRAD=T.DEUDA_TOTAL-(EXCLUSION_CLI_GRAD+EXCLUSION_ASIST_GRAD+EXCLUSION_PROD_GRAD+EXCLUSION_GAR_GRAD+EXCLUSION_DESC_GRAD)
    FROM @TBL_AUX T
	...
	SET @TOT_CONTROL_GRAD = ISNULL((SELECT SUM(TOT_CONTROL_GRAD) FROM @TBL_AUX WHERE CLIENTE = @CLIENTE),0) + @IMPORTE_SOLICITUD;
...
--CUENTA SI LA CANTIDAD DE CLIENTES QUE EXCEDEN GRADUACIÓN/FRACCIONAMIENTO ES >=1
	...
	IF (SELECT COUNT(1) FROM @TBL_AUX WHERE EXCEDIDO_GRAD = ''S'' AND CLIENTE = @CLIENTE)>= 1
	    BEGIN
