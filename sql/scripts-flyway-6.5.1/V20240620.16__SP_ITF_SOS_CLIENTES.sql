...
SELECT DISTINCT mov.CUENTACLIENTE
  INTO #TMP_CLIENTES
FROM SOS_MOVS_HISTORICOS mov WHERE MOV.FECHADEINFORMACION = @Fecha AND mov.TIPODOC = ''''

SELECT DISTINCT mov.TIPODOC, mov.NUMDOC
  INTO #TMP_CLIENTES_TIPNUMDOC
FROM SOS_MOVS_HISTORICOS mov WHERE MOV.FECHADEINFORMACION = @Fecha AND mov.TIPODOC <> '''' 
...
WHERE cli.TZ_LOCK = 0
  ...
  AND EXISTS(SELECT 1 FROM #TMP_CLIENTES x WHERE x.CUENTACLIENTE = cli.CODIGOCLIENTE)
