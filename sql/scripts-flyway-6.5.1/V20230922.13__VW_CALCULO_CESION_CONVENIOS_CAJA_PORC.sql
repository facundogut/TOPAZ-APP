...
EXECUTE('
IF OBJECT_ID (''dbo.VW_CALCULO_CESION_CONVENIOS_CAJA_PORC'') IS NOT NULL
	DROP VIEW dbo.VW_CALCULO_CESION_CONVENIOS_CAJA_PORC
')
EXECUTE('
CREATE VIEW VW_CALCULO_CESION_CONVENIOS_CAJA_PORC
				(ID_CABEZAL,
				ID_DETALLE,
				MONEDA,
				IMPORTE_DETALLE,
...
SELECT DISTINCT c.ID ID_CABEZAL,
     ...
     (SELECT SUM(cc.Porcentaje) FROM CONV_CESIONES CC WHERE cc.Id_ConvRec = c.CONVENIO AND cc.TpoCes = ''P'' AND TZ_LOCK=0) AS SUMA_PORCENTAJE,
...
-
  ...
  ( ( (d.IMPORTE-(
	...
	ELSE 
	  ...
	  ELSE
	    ROUND((d.IMPORTE-(d.IMPORTE*(SELECT (cc.Porcentaje) /100 FROM CONV_CESIONES CC WHERE cc.Id_ConvRec = 22 AND cc.TpoCes = ''P'' AND TZ_LOCK=0 AND CC.Prioridad=1)))
	     *(SELECT (cc.Porcentaje) /100 FROM CONV_CESIONES CC WHERE cc.Id_ConvRec = 22 AND cc.TpoCes = ''P'' AND TZ_LOCK=0 AND CC.Prioridad=2),2)
	  END 
	END AS IMPORTE_CESION_2,
...
isnull(CASE WHEN 
	    ...
	    (d.IMPORTE*(SELECT (cc.Porcentaje)/100 FROM CONV_CESIONES CC WHERE cc.Id_ConvRec = c.CONVENIO AND cc.TpoCes = ''P'' AND TZ_LOCK=0 AND CC.Prioridad=1))  
	  ELSE
	   ROUND( (d.IMPORTE*(SELECT (cc.Porcentaje)/100 FROM CONV_CESIONES CC WHERE cc.Id_ConvRec = c.CONVENIO AND cc.TpoCes = ''P'' AND TZ_LOCK=0 AND CC.Prioridad=1)),2)  
...
FROM REC_DET_RECAUDOS_CAJA d WHERE d.ID_CABEZAL = c.ID)AS RESTO_TOTAL
