	   ...
	   FROM (
			SELECT C.CLIENTE,C.NUEVO, MIN(TIPO_SOBREGIRO) AS TIPO_SOBREGIRO
				   ...
				   INNER JOIN (SELECT SALDOS_JTS_OID, FECHA,
							   CASE WHEN SUM(SALDO_AJUSTADO)<0 AND SUM(CUPO_SOBREGIRO)=0 THEN 0
						  	 		WHEN SUM(SALDO_AJUSTADO)<0 AND SUM(CUPO_SOBREGIRO)>0 THEN 1
						  	 		WHEN SUM(SALDO_AJUSTADO)>=0 THEN 2
