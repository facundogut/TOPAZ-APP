...
BEGIN
  ...
  BEGIN TRY
    ...
    END
      ...
      IN (SELECT CODIGO_LINK FROM TJD_TIPOS_NOVEDADES_LINK WHERE ACTUALIZA_BD = ''S'' AND ACCION LIKE @accion AND TZ_LOCK = 0)
      ...
      MERGE INTO TJD_ITF_PERSONAS AS TJD
		USING (
			...
			WHERE SUBSTRING(Linea, 1, 6) IN (
				...
				WHERE ACTUALIZA_BD = ''S'' AND ACCION LIKE @accion AND TZ_LOCK = 0
			)
	...
	GROUP BY Tipo_Doc, Num_Doc;
      ...
      EXEC SP_ITF_LINK_TRX_TARJETA_CUENTA @IndRefreshFull, @ErrorMaestroCuenta;
      ...
      BEGIN
	      ...
	      SELECT S.JTS_OID,TTT.TIPO_TARJETA,substring(Linea, 731, 19) NroTar,S.C1803, CCP.TITULARIDAD,
	        /*1 EMATOS*/substring(Linea, 780, 1) Estado,CASE WHEN substring(Linea, 825, 8) <> ''00000000'' THEN CAST(substring(Linea, 825, 8) AS DATETIME) ELSE NULL End FecEnt,
	        ...
	        S.SUCURSAL,substring(Linea, 731, 19),DPFJ.NUMEROPERSONAFJ,/*''P'' EMATOS*/ substring(Linea,771,2) Permiso,substring(Linea,769,2) LimCre, substring(Linea,767,2) LimDeb,
	        substring(Linea,763,1) Version,substring(Linea,764,1) Verificador, /*substring(Linea, 731, 19) EMATOS */substring(Linea, 132, 19) TarjetaBase
      ...
      BEGIN
		...
		JOIN CLI_DocumentosPFPJ DPFJ (nolock) ON DPFJ.NUM_DOC_FISICO = CAST(CAST(REPLACE(substring(Linea,539,9),''.'' ,'''') AS NUMERIC) AS VARCHAR(20)) AND DPFJ.TIPO_DOC_FISICO = substring(Linea,536,3)
		JOIN CLI_ClientePersona CCP (nolock) ON CCP.CODIGOCLIENTE = S.C1803 AND CCP.NUMEROPERSONA = DPFJ.NUMEROPERSONAFJ
		WHERE NOT EXISTS(SELECT 1 FROM TJD_TARJETAS x WHERE x.ID_TARJETA = substring(TRX.Linea, 731, 19))
		--WHERE isnumeric(substring(Linea,539,9)) = 1;
      END
