...
FROM ITF_ASIENTOS_EXTERNOS (nolock) WHERE ID=@idCount;
	...
	SELECT  (SELECT MAX(JTS_OID) FROM BANDEJA_ASIENTOS_IN WHERE ESTADO=''S'' AND ORIGEN=''AAC''), s.SUCURSAL, s.PRODUCTO, s.CUENTA, s.MONEDA, s.OPERACION, s.ORDINAL, 0, a.COD_MOVIMIENTO, (a.IMPORTE_DEBE + a.IMPORTE_HABER), CONCAT(''1.3.2 AD '',a.CONCEPTO_ASIENTO), CONCAT(a.CAPITULO_CONT,a.RUBRO_CONT,a.MONEDA_CONT,a.CUENTA_CONT,a.SUCUENTA_CONT), (SELECT FECHAPROCESO FROM PARAMETROS), a.MONEDA,(select C6340 from PLANCTAS WHERE C6326=CONCAT(a.CAPITULO_CONT,a.RUBRO_CONT,a.MONEDA_CONT,a.CUENTA_CONT,a.SUCUENTA_CONT))
