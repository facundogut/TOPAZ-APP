...
SELECT 
    ...
    FROM PARAMETROS p,
    REC_RENDICION ren JOIN 
    CONV_CONVENIOS_REC conv ON (conv.Id_ConvRec = ren.CONVENIO OR conv.Id_ConvRec = ren.CONVENIO_PADRE) AND conv.Estado = ''A'' JOIN
    REC_LIQUIDACION liq ON ren.ID_RENDICION = liq.ID_RENDICION JOIN 
    REC_CAB_RECAUDOS_CAJA cab ON cab.ID_LIQUIDACION = liq.ID_LIQUIDACION JOIN
    SALDOS s ON conv.CuentaRec = s.JTS_OID LEFT JOIN
    importeIVA iva ON ren.FECHA = iva.fecha LEFT JOIN 
    importeRetenciones reten ON ren.FECHA = reten.fecha
    
    WHERE (ren.CONVENIO = @convenio OR ren.CONVENIO_PADRE = @convenio) AND ren.FECHA = CAST(@fecha AS DATE) AND 
    cab.TZ_LOCK = 0
        AND liq.TZ_LOCK = 0
        AND ren.TZ_LOCK = 0
        AND conv.TZ_LOCK = 0
...
SELECT 
    ...
    CONV_CONVENIOS_REC conv ON (conv.Id_ConvRec = ren.CONVENIO OR conv.Id_ConvRec = ren.CONVENIO_PADRE) AND conv.Estado = ''A'' JOIN
    REC_LIQUIDACION liq ON ren.ID_RENDICION = liq.ID_RENDICION JOIN 
    REC_CAB_RECAUDOS_CANAL cab ON cab.ID_LIQUIDACION = liq.ID_LIQUIDACION JOIN
    SALDOS s ON conv.CuentaRec = s.JTS_OID LEFT JOIN
    importeIVA iva ON ren.FECHA = iva.fecha LEFT JOIN 
    importeRetenciones reten ON ren.FECHA = reten.fecha
...
SELECT TOP 1 @archivoRen = ARCHIVO_REN FROM ITF_RP_REPORTE_PARAMETROS
')