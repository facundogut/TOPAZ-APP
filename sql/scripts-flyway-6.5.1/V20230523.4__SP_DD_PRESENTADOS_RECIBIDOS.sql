EXECUTE('CREATE OR ALTER  PROCEDURE [dbo].[SP_DD_PRESENTADOS_RECIBIDOS]
	@TICKET NUMERIC(16),
...
BEGIN 
	...
	BEGIN
		SET @MSJ = ''No concuerda la suma de Debitos individuales con el Total Debitos'';
		RETURN
	END
	...
	BEGIN
		SET @MSJ = ''No concuerda la suma de Creditos individuales con el Total Creditos '';
		RETURN
	END
	...
	BEGIN
      ...
      BEGIN
			...
			BEGIN
				SET @MSJ =  ''Tamanio registro debe ser 094'';
				RETURN
			END
      ...
      BEGIN
			...
			IF (@CodigoRegistro <> ''CCD'') 
			BEGIN
				SET @MSJ =  ''Codigo de registro debe ser CCD'';
				RETURN;
			END
	...
	BEGIN
	      ...
	      SET @V93 = (SELECT COUNT(*) FROM FERIADOS WHERE (SUCURSAL=CONVERT(INT,RIGHT(@EntidadDebitar,4)) OR SUCURSAL=-1) AND DIA=FORMAT(@FECHADATE,''dd'') AND MES=FORMAT(@FECHADATE,''MM'') AND (ANIO=FORMAT(@FECHADATE,''yyyy'') OR ANIO=0)); 
	      ...
	      SET @NIdClientePegador = convert(NUMERIC(12),RTRIM(@IdClientePegador));
	      ...
	      SET @V24 = (SELECT COUNT(1) FROM dbo.SNP_DEBITOS 
	      			  WHERE REFERENCIA=@ReferenciaUnivoca 
	      			    ...
	      			    AND MONEDA=@Moneda 
	      			    AND CODIGO_CLIENTE= @NIdClientePegador);
		...
		VALUES(@NroArchivo, ''C'', @CUIT_EO , @PRESTACION, @FechaVencimiento, @Cod_Cliente
		  ,@ReferenciaUnivoca, @CodigoPrioridad, @FechaCompensacion, @Moneda, @Importe, @CTA_CBU, @saldo_jts_oid, @CodigoOrigen
