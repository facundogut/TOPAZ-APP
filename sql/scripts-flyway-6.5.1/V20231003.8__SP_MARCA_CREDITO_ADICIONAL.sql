...
BEGIN
	...
	BEGIN TRY
		...
		WHERE s.C1604 < 0 AND s.C1785 = 5 AND s.C8748=''N'' AND s.C1695 = '' '' AND s.TZ_LOCK = 0 AND s.C1621 = (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK)))				
		...
		BEGIN
			...
			INNER JOIN CLI_CLASUBJETIVA co WITH (NOLOCK) ON c.CATEGORIARESULTANTE = co.CATEGORIASUB AND co.TZ_LOCK = 0 AND 
			round(((s.C1604*-1)*100/NULLIF((sd.DEUDA - (s.C1604*-1)),0)),2) <= co.PORCENTAJE_SF  
			WHERE s.C1604 < 0 AND s.C1785 = 5 AND s.C8748=''N'' AND s.C1695 = '' '' AND s.TZ_LOCK = 0 AND s.C1621 = (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK))) a
