...
----------------------------------------13
	if exists (	select a.SUCURSAL 
				...
				SELECT    ''Tabla CAJ_HISTORIAL_BILLETAJE no se ajusta a la integridad referencial con SUCURSALES '',*
				from CAJ_HISTORIAL_BILLETAJE a with(nolock) 
				where a.SUCURSAL not in(	select b.SUCURSAL
												from SUCURSALES B with(nolock))
	else
	print ''Comprobación OK.Tabla CAJ_HISTORIAL_BILLETAJE con SUCURSALES ''
...
----------------------------------------27
	if exists (	select a.DENOMINACION 
				...
				SELECT    ''Tabla REM_DENOMINACIONREMESA no se ajusta a la integridad referencial con DENOMINACIONES '',*
				from REM_DENOMINACIONREMESA a with(nolock) 
				where a.DENOMINACION not in(	select b.DENOMINACION
												from DENOMINACIONES B with(nolock))
	else
	print ''Comprobación OK.Tabla REM_DENOMINACIONREMESA con DENOMINACIONES ''

select * from rem_DENOMINACIONremesa
