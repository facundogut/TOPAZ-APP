...
SELECT L.ID_LIQUIDACION AS ''ID_LIQUIDACION'', 
		...
		CONVERT(DATETIME,(CASE WHEN C.Acreditacion = OE.OPCIONINTERNA AND OE.OPCIONINTERNA = ''24H''
			...
			THEN dbo.diaHabil(DATEADD(day,2,RC.Fecha_Cobranza),''A'') END)) AS ''FECHA_REND''
...
JOIN REC_CAB_RECAUDOS_CANAL RC WITH (NOLOCK) ON RC.ID_LIQUIDACION = l.ID_LIQUIDACION AND RC.CONVENIO = c.Id_ConvRec
