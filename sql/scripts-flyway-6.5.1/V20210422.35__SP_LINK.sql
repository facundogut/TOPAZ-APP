EXECUTE('DROP PROCEDURE dbo.SP_LINK;')
		...
		BEGIN
		    ...
		    BEGIN
			    ...
			    SELECT TOP (@cant_movimientos) hv.ASIENTO, hv.FECHA_PROCESADO, hv.SUCURSAL, hv.NUMERO_MOVIMIENTO, hv.ORDINAL, hv.TRTIPO, 
				    ...
				    WHERE hv.SALDO_JTS_OID=@oid AND hv.FECHA_PROCESADO <= @fch_proceso ORDER BY hv.FECHA_PROCESADO desc;
			END
			ELSE
			BEGIN
			    INSERT INTO ITF_MOVCONFORMADOS (asiento, fch_procesado, sucursal, nro_mov, ordinal, TRTIPO, 
		...
		BEGIN
		    ...
		    BEGIN
			    ...
			    SELECT TOP (@cant_movimientos) hv.ASIENTO, hv.FECHA_PROCESADO, hv.SUCURSAL, hv.NUMERO_MOVIMIENTO, hv.ORDINAL, hv.TRTIPO, 
				    ...
				    hv.SALDO_JTS_OID				
				    FROM HISTORIA_VISTA (nolock) hv 
				    WHERE hv.SALDO_JTS_OID=@oid ORDER BY hv.FECHA_PROCESADO desc;
			END
			ELSE
			BEGIN
			    INSERT INTO ITF_MOVCONFORMADOS (asiento, fch_procesado, sucursal, nro_mov, ordinal, TRTIPO, 
