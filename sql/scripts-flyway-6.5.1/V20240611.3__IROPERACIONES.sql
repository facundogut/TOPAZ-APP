...
-------------------------------Debitos Directos
	...
	if exists (select b.cuit 
				from CONV_CONVENIOS_REC b with(nolock) 
				where b.cuit not in(select a.cuit_eo 
										from SNP_PRESTACIONES_EMPRESAS  A with(nolock))
				)	
				SELECT   ''Tabla CONV_CONVENIOS_REC no se ajusta a la integridad referencial con Tabla SNP_PRESTACIONES_EMPRESAS'',*
				from CONV_CONVENIOS_REC b with(nolock) 
				where b.cuit not in(select a.cuit_eo 
										from SNP_PRESTACIONES_EMPRESAS  A with(nolock))
	else
	print ''Comprobación OK.Tabla CONV_CONVENIOS_REC con Tabla SNP_PRESTACIONES_EMPRESAS''
