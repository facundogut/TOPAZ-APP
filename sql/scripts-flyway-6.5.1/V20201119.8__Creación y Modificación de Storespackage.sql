...
BEGIN
	...
	BEGIN TRY
		...
		WHILE @@FETCH_STATUS = 0 AND @CorreoDestino IS NOT NULL
			...
			BEGIN
				...
				SET @MonedaUVAUVI = (SELECT C6403 FROM MONEDAS WHERE C6399=@Moneda AND TZ_LOCK=0)
				
				IF(@MonedaUVAUVI = ''I'')
					BEGIN
						SET @JtsOidCtaDestino = (SELECT JTS_OID_CTA_DESTINO FROM CV_TRANSFERENCIA WHERE JTS_OID_CTA_FUENTE = @ContraCuenta)
				
						INSERT INTO DPF_SALDOS_INMOVILIZADOS (TZ_LOCK, JTS_OID_SALDO, CUENTA, FECHA_ALTA, FECHA_INMOVILIZACION, MOTIVO_INMOVILIZACION, JTS_OID_SALDO_DPF)
						VALUES (0, @JtsOidCtaDestino, @Cuenta, @FechaApertura, @FechaProceso, ''No tiene instrucción de Cancelar o Renovar al Vencimiento'', @JtsOid)
					END
				ELSE
					BEGIN
						INSERT INTO DPF_SALDOS_INMOVILIZADOS (TZ_LOCK, JTS_OID_SALDO, CUENTA, FECHA_ALTA, FECHA_INMOVILIZACION, MOTIVO_INMOVILIZACION, JTS_OID_SALDO_DPF)
						VALUES (0, @ContraCuenta, @Cuenta, @FechaApertura, @FechaProceso, ''No tiene instrucción de Cancelar o Renovar al Vencimiento'', @JtsOid)
					END
