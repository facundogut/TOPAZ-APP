...
BEGIN  
      ...
      BEGIN
      	...
      	BEGIN
      		INSERT INTO CLE_RECEPCION_DPF_DEV (NUMERO_DPF, BANCO_GIRADO, FECHA_ALTA, SUCURSAL_BANCO_GIRADO, TIPO_DOCUMENTO, IMPORTE_DPF, [CODIGO_CAMARA])
      		VALUES (@NumeroCheque, @Entidad, (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK)), @Sucursal, 1, @Importe, (SELECT [CODIGO_DE_CAMARA] FROM CLE_CAMARAS_COMPENSADORAS WITH (NOLOCK)));
      	END
      	
      	ELSE
      	BEGIN
      		INSERT INTO CLE_RECEPCION_CHEQUES_DEV (BANCO_GIRADO, SUCURSAL_BANCO_GIRADO, SERIE_DEL_CHEQUE, IMPORTE_CHEQUE, [CODIGO_RECHAZO], NUMERO_CHEQUE, ESTADO_DEVOLUCION, [CODIGO_CAMARA], TIPO_DOCUMENTO, FECHA_ALTA, NUMERO_CUENTA_GIRADORA)
      		VALUES (@Entidad, @Sucursal, '''', @Importe, @CodRechazo, @NumeroCheque, 0, 1, 0, (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK)), CAST(@CuentaDebitar AS NUMERIC));
