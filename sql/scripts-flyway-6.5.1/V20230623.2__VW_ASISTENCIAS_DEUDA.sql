...
INNER JOIN ( 
	...
	WHERE ((PP.C2302<= (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK))) OR (SELECT FECHAPROCESO FROM PARAMETROS WITH (NOLOCK)) BETWEEN PP.C2301 AND PP.C2302) AND ( (PP.TZ_LOCK < 300000000000000 OR PP.TZ_LOCK >= 400000000000000) AND (PP.TZ_LOCK < 100000000000000 OR PP.TZ_LOCK >= 200000000000000)  )
	) T
