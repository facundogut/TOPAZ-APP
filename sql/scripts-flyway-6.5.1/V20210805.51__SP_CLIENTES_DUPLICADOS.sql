/****** Object:  StoredProcedure [dbo].[SP_CLIENTES_DUPLICADOS]    Script Date: 02/06/2021 17:45:55 ******/
SET ANSI_NULLS ON
GO
...
begin
  ...
  set @query = 'declare clip cursor for ' + @sentencia + ' where NUMEROPERSONA in (' + @p_personas + ') and TZ_LOCK = 0;'
  ...
  while @@FETCH_STATUS = 0 AND @duplicado = 0
    begin
      ...
      SELECT @vCantIntCliB = COUNT(*) 
	  FROM CLI_CLIENTEPERSONA CP WITH (NOLOCK)
	  WHERE CP.CODIGOCLIENTE = @cliente AND CP.TZ_LOCK = 0;
      ...
      BEGIN
      	...
      	(
			...
			FROM CLI_CLIENTEPERSONA CP WITH (NOLOCK) 
			WHERE CP.CODIGOCLIENTE = @cliente 
				AND CP.TZ_LOCK = 0 
      	) X
      	IF @vCantIntDif = 0
      	BEGIN
      		SELECT @vNroIntTitularB = CP.NUMEROPERSONA 
			FROM CLI_CLIENTEPERSONA CP WITH (NOLOCK)
			WHERE CP.CODIGOCLIENTE = @cliente 
