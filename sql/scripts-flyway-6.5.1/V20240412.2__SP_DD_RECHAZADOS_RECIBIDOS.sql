...
--			  		 AND (PRESENTACION_PRIMER_VTO=@FechaVencimiento OR PRESENTACION_SEGUNDO_VTO=@FechaVencimiento)
			  ...
			  INSERT INTO dbo.ITF_DD_RECHAZOS_RECIBIDOS (ID_TICKET,  FECHAPROCESO, CODIGO_TRANSACCION, SUCURSAL, MONEDA, CUENTA, IMPORTE,  REFERENCIA_UNIVOCA, ID_CLIENTE_PAGADOR,  MOTIVO_RECHAZO, INFO_ADICIONAL)
       		  VALUES (@TICKET, (SELECT FECHAPROCESO FROM PARAMETROS), @CodTransaccion, CONVERT(INT,RIGHT(@EntidadDebitar,4)), @Moneda, @CuentaDebitar, @Importe,  @ReferenciaUnivoca, @IdClientePagador,  substring(@LINEA, 4, 3) ,@InfoAdicional);
...
--	SET @CL_NOM_EMPRESA =  LEFT((SELECT NOMBRE_EMPRESA FROM SNP_PRESTACIONES_EMPRESAS WHERE CUIT_EO=@T_CUIT AND PRESTACION=@T_PRESTACION),16);
	...
	FETCH NEXT FROM CursorDD INTO @T_ID_DEBITO, @T_JTS_OID, @T_MONEDA, @T_IMPORTE, @T_FECHA_VTO,@T_FECHA_COMP,@T_CBU, @T_REFERENCIA, @T_CODIGO_CLIENTE, @T_CUIT, @T_ID_ARCHIVO_REVERSADO,@T_CLIENTE_PAGADOR,@T_PRES_1ER_VTO,@T_PRES_2DO_VTO,@T_ID_ARCHIVO,@T_NRO_ARCHIVO,@T_PRESTACION
