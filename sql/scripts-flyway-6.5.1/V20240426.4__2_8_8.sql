execute('
CREATE or alter FUNCTION ProximoDiaHabilDespuesNDias(@d DATE, @n INT)
RETURNS DATE
AS
BEGIN
    ...
    -- Calcular la fecha después de ''a'' días desde ''d''
    SET @fechaSiguiente = DATEADD(DAY, @n, @d);

    -- Calcular los días hábiles entre ''d'' y @fechaSiguiente
    SET @diasHabiles = dbo.DiasHabilesEntreFechas(@d, @fechaSiguiente);

    -- Si @fechaSiguiente no es un día hábil, avanzar al próximo día hábil
    WHILE @diasHabiles <> @n
    BEGIN
        SET @fechaSiguiente = DATEADD(DAY, 1, @fechaSiguiente);
        SET @diasHabiles = dbo.DiasHabilesEntreFechas(@d, @fechaSiguiente);
    END;
...
DECLARE @T_NRO_ARCHIVO NUMERIC(15,0);
		...
		IF((SELECT COUNT(1) 
			...
			+ 
		   (SELECT COUNT(1) 
			FROM SNP_ADHESIONES o LEFT JOIN SNP_CUENTAS_RELACIONADAS r ON r.CUIT_EO=o.CUIT_EO AND r.PRESTACION=o.PRESTACION AND r.RELACION_CUENTA=1 and r.TZ_LOCK=0
			WHERE o.TZ_LOCK=0 )
			=0)
		BEGIN
		---------------- Grabar Cabecera Lote ---------------------------
...
--	SET @CL_NOM_EMPRESA =  LEFT((SELECT NOMBRE_EMPRESA FROM SNP_PRESTACIONES_EMPRESAS WHERE CUIT_EO=@T_CUIT AND PRESTACION=@T_PRESTACION),16);
    ...
    INSERT INTO dbo.ITF_DD_PRESENTADOS_EMITIDOS_AUX (LINEA, CORRELATIVO,ID_ARCHIVO,NRO_ARCHIVO) VALUES (replace(@RI_REGISTRO_INDIVIDUAL, ''.'', ''''),@T_ID_DEBITO,@T_ID_ARCHIVO,@T_NRO_ARCHIVO);
	-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	FETCH NEXT FROM CursorDD INTO @T_ID_DEBITO, @T_JTS_OID, @T_MONEDA, @T_IMPORTE, @T_FECHA_VTO,@T_SEGUNDO_VTO,@T_FECHA_COMP,@T_CBU, @T_REFERENCIA, @T_CODIGO_CLIENTE, @T_CUIT, @T_ID_ARCHIVO_REVERSADO,@T_CLIENTE_PAGADOR,@T_PRES_1ER_VTO,@T_PRES_2DO_VTO,@T_ID_ARCHIVO,@T_NRO_ARCHIVO,@T_PRESTACION,@T_TIPO
