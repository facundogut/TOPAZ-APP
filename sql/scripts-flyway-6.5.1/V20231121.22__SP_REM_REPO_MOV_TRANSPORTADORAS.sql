...
AS 
   ...
   BEGIN
	  ...
	  DECLARE @Tabla_SP_REM_REPO_MOV_TRANSPORTADORAS  TABLE(
		...
		INNER JOIN REM_TRANSPORTADORAS T ON CAST(REM.MEDIO_TRANSP AS INTEGER) = T.NRO_CLIENTE AND S2.TZ_LOCK = 0
		WHERE ((REM.TIPOORIGEN = 7 AND REM.TIPODESTINO = 8) OR (REM.TIPOORIGEN = 8 AND REM.TIPODESTINO = 7))
      ...
      BEGIN TRANSACTION	
        ...
        BEGIN TRY
        	...
        	DELETE CR 
        	FROM REM_REPO_MOV_TRANSPORTADORAS AS CR WITH (NOLOCK)
        	INNER JOIN @Tabla_SP_REM_REPO_MOV_TRANSPORTADORAS AS TC ON
		    ...
		    DELETE CR 
        	FROM REM_REPO_MOV_TRANSPORTADORAS AS CR WITH (NOLOCK)
        	INNER JOIN @Tabla_SP_REM_REPO_MOV_TRANSPORTADORAS_DOBLE AS TC ON
				TC.NRO_SOLICITUD = CR.NRO_SOLICITUD
				AND CR.ORDEN = 2
