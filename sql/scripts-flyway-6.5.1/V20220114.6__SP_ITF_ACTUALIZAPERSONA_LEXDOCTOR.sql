EXECUTE('DROP PROCEDURE IF EXISTS [dbo].[SP_ITF_ACTUALIZAPERSONA_LEXDOCTOR];')

execute('
CREATE PROCEDURE [dbo].[SP_ITF_ACTUALIZAPERSONA_LEXDOCTOR] 
   @P_CUIT VARCHAR(11),
...
AS 
   ...
   DECLARE @flag INT;
      ...
      IF @P_TIPO_PERSONA = ''F''
      	BEGIN
      		SET @flag = (SELECT COUNT(*) FROM CLI_PERSONASFISICAS WHERE NUMEROPERSONAFISICA = @P_NRO_PERSONA AND ESTADO = 0 AND ((TZ_LOCK < 300000000000000) AND (TZ_LOCK = 0 OR TZ_LOCK >= 200000000000000)));
      ...
      ELSE
      	BEGIN
      		SET @flag = (SELECT COUNT(*) FROM CLI_PERSONASJURIDICAS WHERE NUMEROPERSONAJURIDICA = @P_NRO_PERSONA AND ESTADO = 0 AND ((TZ_LOCK < 300000000000000) AND (TZ_LOCK = 0 OR TZ_LOCK >= 200000000000000)));
