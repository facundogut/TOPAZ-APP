...
--SELECT ''ENTRO'';
	...
	BEGIN
		...
		SET @TotalesCreditos += @SumaImportes
	   	...
	   	SET @FL_FIN_LOTE = concat(@FL_ID_REG, @FL_CODIGO_CLASE_TRANSAC,RIGHT(CONCAT(REPLICATE(''0'',6),@FL_CANT_REG_INDIVIDUAL_ADICIONAL),6), @FL_TOTALES_DE_CONTROL, @FL_SUMA_TOTAL_DEBITO_LOTE, @FL_SUMA_TOTAL_CREDITO_LOTE, @CL_IDENTIFICACION_EMP_IND, @FL_RESERVADO2, @FL_RESERVADO3, @FL_REG_ENTIDAD_ORIGEN, @CL_NUMERO_LOTE);	
		...
		SET @FA_SUMA_TOTAL_CREDITOS = RIGHT(concat(replicate(''0'', 12), @TotalesCreditos*100),12);

		SET @FA_FIN_ARCHIVO = concat(@FA_ID_REG, @FA_CANT_LOTES, @FA_NUMERO_BLOQUES, @FA_CANT_REG_INDIVIDUAL_ADICIONAL, @FL_TOTALES_DE_CONTROL, @FA_SUMA_TOTAL_DEBITOS, @FA_SUMA_TOTAL_CREDITOS, @FA_RESERVADO);
	...
	--
	--SELECT @RI_ENTIDAD_AACREDITAR, concat(@RI_ENTIDAD_AACREDITAR, RIGHT(concat(replicate(''0'', 7), (SELECT a.NUMERICO_1 FROM ITF_MASTER_PARAMETROS a WHERE CODIGO = 442)), 7)), @RI_CONTADOR_REGISTRO
	--
