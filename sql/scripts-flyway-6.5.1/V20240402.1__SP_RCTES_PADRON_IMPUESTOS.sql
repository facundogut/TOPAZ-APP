...
DECLARE @periodoAcomparar DATETIME = (SELECT CASE WHEN @reproceso = ''S'' OR @reproceso = ''s''  THEN @periodoActual ELSE dateadd(month,0,@periodoActual) END); --si hay reproceso se compara con el mes actual, sino, se compara con el mes anterior
...
OPEN cursorAux
    ...
    BEGIN
		...
		IF (@vinoEnPadron > 0 AND @alicuotaVieja = @alicuota)
     	...
     	ELSE --hubo cambios, van a la bitacora con cod_cliente = 0     		
     	   INSERT INTO CON_BITACORA_IMPUESTOS (JTS_NOVEDAD, TIPO_NOVEDAD, OPERACION_TOPAZ, FECHA_PROCESO, HORA, ASIENTO, COD_USUARIO, ARCHIVO_PADRON, NUM_CERTIFICADO, F_EMISION, SUCURSAL, ID_CLIENTE, ID_PERSONA, TIPO_ID, CUIT, TIPO_CARGO_IMPOSITIVO, TABLA_CARGOS_IMPUESTOS, ID_CARGO_IMPUESTO, CONDICION, VALOR_EXCLUSION, ALICUOTA, VALOR_ALICUOTA_IIBB, FECHA_INICIO, FECHA_FIN, COMENTARIOS, CBU)
...
IF upper(@reproceso)=''S''
DELETE FROM ITF_PADRON_IMPUESTOS WHERE PERIODO_ARCHIVO=@perParam AND CUIT NOT IN (SELECT convert(NUMERIC(11),substring(registro,1,11)) FROM ITF_PADRON_IMPUESTOS_AUX)
