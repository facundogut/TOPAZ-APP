    ...
    BEGIN TRY
    		...
    		INSERT INTO @TMPSaldos
		   			SELECT 
		                  ...
		                  ISNULL(( SELECT sum(C.IMPORTE)
								 ...
								 inner join dbo.CLE_DEPOSITOS  AS D with (nolock) on D.SALDO_JTS_OID = s.JTS_OID
																					AND D.NUMERO_DEPOSITO = C.NUMERO_DEPOSITO
																					AND C.ACREDITADO = 0 
																					AND C.DESTINO_CHEQUE IN (1,5)),0) AS ImportesPend,
					     0 AS Transferencia,
					     0 AS Disponible
  	               ...
  	               FROM 
							...
							inner join dbo.PLANCTAS  AS p with (nolock) on s.TZ_LOCK = 0
																			...
																			AND s.C1730 = p.C6326
																			AND s.C1785 in (2,3)
																			AND (s.JTS_OID = @p_JTSOID or isnull(@p_JTSOID, -1) = -1)---IN (43814,43184,43218,43113,43106,43081)
							...
							inner join dbo.GRL_ESTADOS_DE_CUENTA  AS E with (nolock) on s.CUENTA = E.CUENTA
																						...
																						AND 
																								(
																									((E.PERIODICIDAD = upper(@p_Periodo) OR upper(@p_Periodo) = ''P'') AND @p_Legal <> ''L''
																									) 
																										OR 
																									(E.PERIODICIDAD = upper(@p_Periodo) AND	E.TIPO_EMISION = ''N'' AND @p_Legal = ''L''
																									)
																								)
							...
							inner JOIN dbo.SUCURSALES  AS n WITH  (nolock) ON n.SUCURSAL=s.SUCURSAL
																			  AND ((n.TZ_LOCK < 300000000000000 
										   										OR n.TZ_LOCK >= 400000000000000 ) 
										   									  AND (n.TZ_LOCK < 100000000000000 
																				OR n.TZ_LOCK >= 200000000000000 )
																				)
							  			/*Obtengo nombre Moneda*/
							  			
							inner JOIN dbo.MONEDAS AS m WITH  (nolock) ON m.C6399 =s.MONEDA   
																	     AND ((m.TZ_LOCK < 300000000000000 OR m.TZ_LOCK >= 400000000000000) 
																		 AND (m.TZ_LOCK < 100000000000000 OR m.TZ_LOCK >= 200000000000000))	 
										/*Obtengo CBU*/	
																	 
							LEFT JOIN  VTA_SALDOS AS v WITH (nolock) ON v.JTS_OID_SALDO =s.JTS_OID AND v.TZ_LOCK=0										
		    ...
		    BEGIN TRY
		     ...
		     -- Cargo tabla temporal de datos del titular
                  ...
                  INSERT INTO @TMPDatosTitular 
                           ...
                           FROM @TMPSaldos s
                           INNER JOIN VW_CLIENTES_PERSONAS cp with (nolock) ON cp.CODIGOCLIENTE=s.Cliente
                           INNER JOIN CLI_CLIENTES c with (nolock) ON c.CODIGOCLIENTE=s.Cliente
                   ...
                   INSERT INTO @TMPDireccionTitular       				
                           SELECT
                              ...
                              (SELECT DESCRIPCION_DIM3 
							   FROM CLI_LOCALIDADES with (nolock) 
									WHERE CODIGOPAIS = d.PAIS AND
										  DIM1 = d.PROVINCIA AND 
										  DIM2 = d.DEPARTAMENTO AND 
										  DIM3 = d.LOCALIDAD) AS CiudadLocalidad,
                              (SELECT DESCRIPCION 
							   FROM CLI_PROVINCIAS with (nolock)
									WHERE DIM1 = d.PROVINCIA) AS Provincia,
                              d.PAIS AS CodigoPais, 
                              d.CPA_NUEVO AS CodigoPostal,
                              d.BARRIO AS Barrio,
                              t.JTSOID
                           FROM dbo.CLI_DIRECCIONES d with (nolock)
                           INNER JOIN @TMPDatosTitular t ON t.NroPersona=d.ID 
                           ...
                           WHERE 
                               d.ORDINAL_DIR = (SELECT MIN(DIR.ORDINAL_DIR) 
																...
																INNER JOIN @TMPDatosTitular dt ON dt.NroPersona=DIR.ID 
                  ...
                  BEGIN try
                      	...
                      	INSERT INTO @TMPIntegrantesCliente 
                      	        	SELECT
			                      	                  ...
			                      	                  (CASE WHEN (count(*) = 2) THEN
									        					  (SELECT TOP 1 NUMERODOC
																   FROM VW_CLIENTES_PERSONAS cp with (nolock)
													   			   INNER JOIN @TMPSaldos s ON s.Cliente=cp.CODIGOCLIENTE 
																   WHERE TITULARIDAD != ''T'') 
											        		  WHEN (count(*) > 2) THEN 
																   (SELECT TOP 1 NUMERODOC
																    FROM VW_CLIENTES_PERSONAS cp with (nolock)
											    	   ...
											    	   END) AS NomTitular2,
											    	  (CASE WHEN (count(*) > 2) THEN 
																  (SELECT TOP 1 NUMERODOC FROM VW_CLIENTES_PERSONAS with (nolock)
																		WHERE NUMEROPERSONA IN (SELECT TOP 2 NUMEROPERSONA
																								 FROM VW_CLIENTES_PERSONAS cp with (nolock)
																								 INNER JOIN @TMPSaldos s ON s.Cliente=cp.CODIGOCLIENTE 
																								 WHERE TITULARIDAD != ''T''
								   					   ...
								   					   END) AS DocTitular3,
			    				  					   (CASE WHEN (count(*) > 2) THEN 
																   (SELECT TOP 1 NOMBRE FROM VW_CLIENTES_PERSONAS with (nolock)
																		WHERE NUMEROPERSONA IN  (SELECT TOP 2 NUMEROPERSONA
																								 FROM VW_CLIENTES_PERSONAS cp with (nolock)
																								 INNER JOIN @TMPSaldos s ON s.Cliente=cp.CODIGOCLIENTE 
																								 WHERE TITULARIDAD != ''T''
		    ...
		    INSERT INTO @TMPSaldosDiarios 
		    		SELECT 
							 ...
							 0 AS TeapagoSD, 
				         	 0 AS TeacobroSD 
                                 ...
                                 inner JOIN @TMPSaldos s ON s.JTSOID=sd.SALDOS_JTS_OID 
                                 WHERE 
					...
					BEGIN try
                        ...
                        LEFT JOIN GRL_SALDOS_DIARIOS g WITH (nolock) ON g.SALDOS_JTS_OID = sd.JTSOID
                        			AND g.TZ_LOCK = 0
				    ...
				    BEGIN TRY
				 		 ...
				 		 INSERT INTO dbo.GRL_CAB_ENVIO_ESTCTA(
					                        ...
					                        (SELECT 
											   ...
											   d.Provincia
											FROM @TMPSaldos s
											LEFT JOIN @TMPDatosTitular t ON t.Cliente=s.Cliente AND t.JTSOID = s.JTSOID
											LEFT JOIN @TMPDireccionTitular d ON d.Cliente=s.Cliente AND d.JTSOID = s.JTSOID
											LEFT JOIN @TMPSaldosDiarios sd ON sd.JTSOID=s.JTSOID
											LEFT JOIN @TMPIntegrantesCliente i ON i.Cliente=s.Cliente AND i.JTSOID = s.JTSOID
											)
					     ...
					     BEGIN TRY
					            INSERT into dbo.GRL_DET_ENVIO_ESTCTA(
					                        ...
					                        (SELECT
					                           ...
					                           s.JTSOID
					                        FROM @TMPSaldos s
											inner JOIN @TMPDatosTitular t ON t.Cliente=s.Cliente AND t.JTSOID = s.JTSOID
										  	LEFT JOIN @TMPSaldosDiarios sd ON sd.JTSOID=s.JTSOID
											)
            ...
            INSERT INTO @TMPMovimientosContables (FechaProcesado, SignoMov, SignoSal, Monto, RegistroxJTS, JTSOID)
            SELECT @vFechaIniAnt, s.SignoSal, s.SignoSal, sd.SalDiaIni, 0, sd.JTSOID FROM @TMPSaldosDiarios sd
            INNER JOIN @TMPSaldos s ON s.JTSOID = sd.JTSOID
			...
			INSERT INTO @TMPMovimientosContables
    				...
    				/* Referencia (antes sacaba referencia de hist vista(tprod 2 y 3), ahora es el asiento del movtoctable en todos los casos)*/
                        (CASE WHEN (M.REFERENCIA IS NULL OR M.REFERENCIA=0) THEN
                        		M.ASIENTO 
                        ELSE 
                        		M.REFERENCIA
                        END) AS Referencia, 
                        ...
                        s.JTSOID AS JTSOID,
                        ISNULL((CASE when (M.CAPITALREALIZADO<>0 AND M.DEBITOCREDITO = ''D'') then 
                        				    	 M.CAPITALREALIZADO
                        		       
                        		END),0) AS ImporteD,
                        ISNULL((CASE when (M.CAPITALREALIZADO<>0 AND M.DEBITOCREDITO = ''C'') then 
                        				    	 M.CAPITALREALIZADO
                        		       
                        		END),0) AS ImporteC,
                        		
                      /*Obtengo detalle transacción y si corresponde info extendida*/
                        ISNULL((CASE when (M.COD_TRANSACCION IS NOT NULL OR M.COD_TRANSACCION <> 0) then 
	                          	(SELECT DESCRIPCION
                           	  ...
                           	  when (M.COD_TRANSACCION IS NOT NULL OR M.COD_TRANSACCION = 0) then
                         	  		M.CONCEPTO  
                         END), m.CONCEPTO) AS DetalleTr,
                         
                         /* INFO EXTENDIDA */
                         (CASE when (M.COD_TRANSACCION IS NOT NULL OR M.COD_TRANSACCION <> 0) then 
                         ...
                         isnull((CASE WHEN (m.Cod_Transaccion IN (SELECT [CODIGO_TRANSACCION] 
                        ...
                        DENSE_RANK() OVER (PARTITION BY S.JTSOID ORDER BY M.FECHAPROCESO, 
                        												   M.HORASISTEMA,
                      													   M.SUCURSAL_CUENTA, 
                        												   M.MONEDA, 
                        												   M.CUENTA,       				   													   
                        												   M.PRODUCTO, 
                     													   M.OPERACION_CUENTA, 
                       													   M.ORDINAL_CUENTA, 
                       													   M.ASIENTO,
                       													   M.JTS_OID ASC) AS RegistroxJTS
                      
                        
                     FROM	dbo.MOVIMIENTOS_CONTABLES  AS M with (nolock)
                     		INNER JOIN  @TMPSaldos AS S ON S.Sucursal=M.SUCURSAL_CUENTA
                  ...
                  BEGIN
		   		...
		   		INSERT INTO @TMPMovimientosContablesAux
		   				SELECT 	 m.JTSOID AS JTSOID,
  				         		 m.RegistroxJTS AS RegistroxJTS,
							     SUM(
							        a.Monto *
							        CASE 
							            WHEN a.SignoSal = a.SignoMov THEN 1 
							            ELSE -1
							        END
							       )  
                        ...
                        LEFT JOIN @TMPMovimientosContables a   ON a.JTSOID = m.JTSOID
                        										and	a.RegistroxJTS <= m.RegistroxJTS

                        GROUP BY m.RegistroxJTS,
                        		 m.JTSOID

						ORDER BY m.RegistroxJTS, m.JTSOID
                        ...
                        FROM @TMPMovimientosContables m
 						inner JOIN @TMPMovimientosContablesAux a ON a.JTSOID = m.JTSOID
  				         		 								and	a.RegistroxJTS = m.RegistroxJTS
		 		     ...
		 		     UPDATE s
                        SET    s.CantDeb =  ISNULL((SELECT count(*) FROM @TMPMovimientosContables m WHERE m.Monto<>0 AND m.SignoMov = ''D''
                        							AND m.JTSOID = s.JTSOID),0),
                        
                     		   s.CantCred = ISNULL((SELECT count(*) FROM @TMPMovimientosContables m WHERE m.Monto<>0 AND m.SignoMov = ''C''
                     		   					    AND m.JTSOID = s.JTSOID),0),
               
  					   		   s.SaldoCalcLinea=  ISNULL((SELECT TOP 1 SaldoCalcLineaAux FROM @TMPMovimientosContables m WHERE m.JTSOID = s.JTSOID
                           						  ORDER BY m.RegistroxJTS DESC),0),
    	                	   /*CREDEB*/
    	                	   s.CREDEB= ISNULL((SELECT sum(CREDEBAux) FROM @TMPMovimientosContables m WHERE m.JTSOID = s.JTSOID),0),
							   /*SIRCREB*/
							   s.SIRCREB = ISNULL((SELECT sum(SIRCREBAux) FROM @TMPMovimientosContables m WHERE m.JTSOID = s.JTSOID),0)
                        FROM @TMPSaldos s 
                        LEFT JOIN @TMPMovimientosContables m ON m.JTSOID = s.JTSOID 
                 ...
                 INSERT INTO @TMPInfoExt
		   				SELECT 	 m.JTSOID AS JTSOID,
								 ...
								 (CASE WHEN m.CodTransaccion IN (28,29,30,31,32,33,34) AND mc.DEBITOCREDITO=''D'' THEN
								 ...
								 /* Transferencias para débito */	
								 	   WHEN m.CodTransaccion IN (21,22,23,24,25,26,27) AND mc.DEBITOCREDITO=''C''	THEN
								 ...
								 /* Plazo fijo */	   
								   	   WHEN m.CodTransaccion IN (48,49,50) THEN
								   	   ''Suc.: '' + convert(VARCHAR(8), h.SUCURSALMOV) + '' Cert.: '' + convert(VARCHAR(12),m.Cuenta) 
                        				+ '' Cap.: '' + convert(VARCHAR(20), h.CAPITALORIGINAL) + '' Int.: '' + convert(VARCHAR(20), h.INTALVTO) 
								 /* Préstamos */
								 		WHEN m.CodTransaccion IN (3016) THEN
								   	   ''Préstamo: '' + convert(VARCHAR(8), (SELECT OPERACION FROM SALDOS WHERE JTS_OID IN 
								   	   (SELECT SALDO_JTS_OID FROM MOVIMIENTOS_CONTABLES WHERE ASIENTO = m.Asiento AND FECHAPROCESO = m.FechaProcesado) AND C1785 = 5)) + 
								   	   '' Línea: '' + convert(VARCHAR(12),(SELECT PRODUCTO FROM SALDOS WHERE JTS_OID IN 
								   	   (SELECT SALDO_JTS_OID FROM MOVIMIENTOS_CONTABLES WHERE ASIENTO = m.Asiento AND FECHAPROCESO = m.FechaProcesado) AND C1785 = 5)) 
                        				+ '' Cuota: '' + convert(VARCHAR(20), (SELECT TOP 1 CUOTA FROM BS_PAYS_DETAIL WHERE SALDOS_JTS_OID IN (SELECT JTS_OID FROM SALDOS WHERE C1785 = 5) AND NROASIENTOMOV = m.Asiento AND FECHAPROCESOMOV = m.FechaProcesado
										ORDER BY CUOTA DESC)) + 
                        ...
                        FROM @TMPMovimientosContables m
                        INNER JOIN MOVIMIENTOS_CONTABLES mc with (nolock) ON mc.Asiento = m.Asiento
                        INNER JOIN VW_CLIENTES_PERSONAS cp  with (nolock) ON cp.CODIGOCLIENTE=mc.CLIENTE AND TITULARIDAD=''T''
                        LEFT JOIN BS_HISTORIA_PLAZO h with (nolock) ON h.NROASIENTOMOV=m.Asiento
                        WHERE m.InfoExtendida=''S'' /*AND m.CodTransaccion IN (28,29,30,31,32,33,34, -- Crédito
                        ...
                        FROM @TMPMovimientosContables m
                        INNER JOIN @TMPInfoExt i ON i.ASIENTO=m.Asiento AND m.CodTransaccion = i.CodTransaccion
               ...
               END 
                           ...
                           INSERT dbo.GRL_DET_ENVIO_ESTCTA(
                              ...
                              (SELECT
                                 ...
                                 s.JTSOID
                                 FROM @TMPMovimientosContables m
                                 	LEFT JOIN @TMPSaldos s ON s.JTSOID=m.JTSOID
								 	LEFT JOIN @TMPDatosTitular t ON t.Cliente=s.Cliente AND t.JTSOID = s.JTSOID
								   	LEFT JOIN @TMPSaldosDiarios sd ON sd.JTSOID=s.JTSOID
								   	WHERE m.RegistroxJTS <> 0                                
                                 )
							END TRY
							BEGIN CATCH
							SET @p_ret_proceso = ERROR_NUMBER()
		         ...
		         BEGIN
					...
					BEGIN try
						...
						UPDATE sd
                        SET sd.SaldiaFin = g.SALDO_AL_CORTE,
                        	...
                        	/* TEAPAGO, TEACOBRO según tipo tasa y tipo producto*/
                          	...
                          	sd.[Teapago]= ISNULL((case when s.TipoProd IN (2,3) AND s.TipoTasa = ''E'' then 
                        								sd.TeapagoSD
                        				  	   else 
                        				  	    0
                        				  	   end),0),
                        				  	   
                        	sd.Teacobro= ISNULL((case when s.TipoProd IN (2,3) AND s.TipoTasa = ''E'' then 
                        								sd.TeacobroSD
                        				  	   else 
                        				  	    0
                        				  	   end),0)
		    		...
		    		END CATCH
                      ...
                      /* Disponible, Transferencia y Garantía, Según tipo de producto y moneda*/     
						...
						UPDATE s
                        SET s.Transferencia = ISNULL((case when s.TipoProd IN (2,3) AND m.C6403=''E''then 
                        								sd.SalDiaFin + sd.CuposobregiroDiario
                        				  	    
                        				  	   end),0),
                        	s.Disponible = ISNULL((case when s.TipoProd IN (2,3) AND m.C6403<>''E''then 
                        								sd.SalDiaFin + sd.CuposobregiroDiario
                 ...
                 END
                  ...
                  INSERT dbo.GRL_DET_ENVIO_ESTCTA(
                       ...
                       (SELECT
                           ...
                           @vPeriodo, 
                           s.IdCab, 
                           ISNULL(((SELECT TOP 1 RegistroxJTS FROM @TMPMovimientosContables m WHERE m.JTSOID = s.JTSOID
                           	ORDER BY RegistroxJTS DESC)+ 1),@vIdDet+1), 
					       s.Sucursal, 
					       s.TipoProd, 
					       s.Prod, 
					       s.Cuenta, 
