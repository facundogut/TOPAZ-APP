/****** Object:  StoredProcedure [dbo].[SP_LINK_MOV_CONFORMADOS]    Script Date: 02/06/2021 16:00:24 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SELECT
		...
		(SELECT NUMERICO 
			...
			WHERE [CODIGO] = 2) AS 'Banco',
		(CASE
			WHEN s.MONEDA = (SELECT MONNAC FROM PARAMETROS WITH (NOLOCK)) and p.c6252=2 and p.TIPO_CUENTA_VISTA =14 THEN '0'
			WHEN s.MONEDA = (SELECT MONNAC FROM PARAMETROS WITH (NOLOCK)) and p.c6252=3 and p.TIPO_CUENTA_VISTA =13 THEN '6'
			WHEN s.MONEDA = 2 and p.c6252=3 and p.TIPO_CUENTA_VISTA =13 THEN '7'
			WHEN s.MONEDA = (SELECT MONNAC FROM PARAMETROS WITH (NOLOCK)) and p.c6252 in (2,3) and p.TIPO_CUENTA_VISTA not in (13,14) then '1'
	...
	FROM
		ITF_MOVCONFORMADOS itf WITH (NOLOCK)
	INNER JOIN HISTORIA_VISTA hv WITH (NOLOCK) ON
	...
	INNER JOIN saldos s WITH (NOLOCK) ON
		s.JTS_OID = itf.jts_oid 
		AND s.C1679='0'
		AND s.TZ_LOCK=0
	INNER JOIN vta_saldos vt WITH (NOLOCK) ON
											vt.JTS_OID_SALDO = itf.jts_oid
											AND vt.TZ_LOCK=0
	INNER JOIN productos p WITH (NOLOCK) on p.c6250=itf.tpo_prod
											AND	p.TZ_LOCK=0
	INNER JOIN CLI_CLIENTES AS c  WITH (NOLOCK) ON c.[CODIGOCLIENTE]=s.C1803
													AND c.TZ_LOCK=0
	INNER JOIN VTA_CUMPLIMIENTO vc  WITH (NOLOCK) ON vc.ASIENTO=hv.ASIENTO 
										AND vc.SALDO_JTS_OID=hv.SALDO_JTS_OID
	INNER JOIN PROD_RELCANALES pr WITH (NOLOCK) ON pr.CANAL = 3 
												AND pr.HABILITADO = 'S'
	JOIN CLI_ClientePersona cp WITH (NOLOCK) ON cp.[CODIGOCLIENTE] = c.[CODIGOCLIENTE]
	JOIN VW_CLI_DOCUMENTOSPFPJ_MEJORADA  vpj WITH (NOLOCK) ON vpj.NUMEROPERSONAFJ = cp.NUMEROPERSONA
   	WHERE
		itf.fch_procesado <= @fch_proceso 
	  	AND c.SUBDIVISION1 <> '02'
	
  UPDATE ITF_LINK_MOVCONFORMADOS SET cant_registros= (SELECT max(secuencia_gral) 
														FROM ITF_LINK_MOVCONFORMADOS WITH (NOLOCK))
END;
