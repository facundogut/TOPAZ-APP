...
SELECT A.ORD_NRO_DOC, A.BEN_BANCO, A.OP_CLASE_TRANS,
	...
	(
		...
		WHERE VT1.OP_CLASE_TRANS = ''E'' and VT1.ESTADO=''PP'' and VT1.OP_MONEDA = 2 AND VT1.TZ_LOCK=0
	) A
...
SELECT
    ...
    WHERE 
    	TR.OP_CLASE_TRANS = ''E'' and TR.ESTADO=''PP'' 
    	AND TR.OP_MONEDA = 2 AND TR.TZ_LOCK=0
    	AND TR.TZ_LOCK=0 AND TR.ORD_NRO_DOC=@CL_ORD_NRO_DOC AND TR.BEN_BANCO=@CL_BEN_BANCO AND TR.OP_CLASE_TRANS=@CL_OP_CLASE_TRANS
...
BEGIN
	...
	SET @RI_CONTADOR_REGISTRO = concat(@RI_ENTIDAD_AACREDITAR, RIGHT(concat(replicate(''0'', 7), (SELECT a.NUMERICO_1 FROM ITF_MASTER_PARAMETROS a WHERE CODIGO = 442)), 7)); 
	--
