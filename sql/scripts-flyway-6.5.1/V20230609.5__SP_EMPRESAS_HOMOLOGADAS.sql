	...
	BEGIN
		...
		ELSE --MODIFICACION
			IF((SELECT COUNT(*) FROM SNP_PRESTACIONES_EMPRESAS 
			    WHERE (CUIT_EO = @DD_IdEmpresa AND PRESTACION = @DD_DescTransaccion) AND (NOMBRE_EMPRESA <> @DD_NombreEmpresa OR ENTIDAD <> @DD_EntidadOrigen OR ORIGINANTE <> @DD_NombreEntidad OR LARGO_ID <> @DD_LongClaveCliente OR SEGUNDO_VTO <> @DD_SegundoVencimiento OR PERIODICIDAD_PAGO <> @DD_PeriocidadPago OR ESTADO <> @DD_ComisionUsoCuenta OR ID_CONVENIO <> @DD_IdConvenio OR NOMENTIDAD <> @DD_NomEntidad)) > 0)
			BEGIN 	
