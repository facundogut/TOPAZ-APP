...
ALTER VIEW VW_ITF_LINK_SOLICITUDES AS 
SELECT 
	...
	RIGHT((CASE WHEN TIPO_DOCUMENTO_AD IS NULL OR TIPO_DOCUMENTO_AD = '' '' THEN ''N/D'' ELSE TIPO_DOCUMENTO_AD END), 3) AS tpo_doc_apoderado,
	...
	LEFT((CASE WHEN APELLIDO IS NOT NULL AND rtrim(ltrim(APELLIDO)) <> '''' THEN concat(APELLIDO, replicate('' '', 15)) ELSE replicate(''.'', 15) END), 15) AS persona_apllido,
	
	LEFT( (CASE WHEN NOMBRE IS NOT NULL AND rtrim(ltrim(NOMBRE)) <> '''' THEN concat(NOMBRE, replicate('' '', 15)) ELSE replicate(''.'', 15) END) ,15) AS persona_nombre,
	...
	LEFT(concat(''ARGENTINA'', replicate('' '', 15)) ,15) AS cuil_nacionalidad,
	...
	LEFT((CASE WHEN TIPO_SOLICITUD = ''120005'' THEN ''02'' ELSE CASE WHEN Miembro IS NULL OR Miembro = '' '' THEN ''00'' ELSE concat(''0'', Miembro) END END), 2) AS tt1_cat_comision,  
	...
	RIGHT((CASE WHEN COD_PRODUCTO_TARJETA IS NOT NULL AND rtrim(ltrim(COD_PRODUCTO_TARJETA)) <> '''' THEN concat(''00'',COD_PRODUCTO_TARJETA) ELSE ''00'' END) , 2) AS tt1_tpo_tarjeta,
	...
	LEFT(CASE WHEN TIPO_SOLICITUD IN (''120005'', ''120006'') THEN 0 ELSE CASE WHEN tj.ESTADO = ''I'' THEN 0 ELSE 1 END END, 1) AS tt1_estado,
	
	(SELECT RIGHT(''00'' + CAST(count(*) AS VARCHAR(2)),2) FROM TJD_SOLICITUD_CUENTAS_LINK WHERE ID_SOLICITUD = ID_SOLICITUD) AS tt1_cant_cuen_asoc,	 
	
	RIGHT(concat(replicate(''0'', 12), tj.COD_CLIENTE) ,12) AS tt1_ref_cliente,
	
	replicate(''0'', 2) AS tt1_cant_pin_impresos,
	
	(CASE WHEN TIPO_SOLICITUD IN (''120005'',''120006'') THEN ''N'' ELSE ''D'' END) AS marca_pin, 
	...
	(CASE WHEN TIPO_SOLICITUD IN (''120005'',''120006'') THEN ''N'' ELSE '' '' END) AS marca_plastico,
	
	(CASE WHEN TIPO_SOLICITUD IN (''120005'', ''120006'') THEN replicate(''0'', 8) ELSE convert(VARCHAR, FECHA_SOLICITUD, 112) END) AS fch_emision_plastico,	
	
	(CASE WHEN TIPO_SOLICITUD IN (''120005'', ''120006'') THEN replicate(''0'', 8) ELSE (SELECT convert(VARCHAR, FECHA_ENTREGA, 112) FROM TJD_TARJETAS WHERE ID_TARJETA =  tj.nro_tarjeta_completa)  END) AS fch_entrega_plastico, 	
	
	(CASE WHEN TIPO_SOLICITUD IN(''120005'',''120006'') THEN replicate(''0'', 16) ELSE replicate(''0'', 16) END) AS cod_denuncia,
	
	replicate(''0'', 4) AS grupo_afinidad,
	
	dbo.itf_cuentas_REDLINK(tj.ID_SOLICITUD, 16) AS cuentas,
