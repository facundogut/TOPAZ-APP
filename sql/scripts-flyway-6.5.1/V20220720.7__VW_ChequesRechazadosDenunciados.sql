EXECUTE('
CREATE  OR ALTER view [dbo].[VW_ChequesRechazadosDenunciados] as
SELECT 
--1
...
''00000000'' as SinUso,
--17
Cast(Year(FECHA_PAGO_MULTA) As Char(4)) as [FechaPagoMulta_Anio],
...
--18
Cast(Year(FECHA_CIERRE_CTA) As Char(4)) as FechaCierreCuenta_Anio, -- Se usa campo FECHA_CIERRE_CTA
replicate(''0'', 2 - len(Cast(MONTH(FECHA_CIERRE_CTA) AS varchar))) + CAST(MONTH([FECHA_PAGO_MULTA]) AS varchar) as FechaCierreCuenta_Mes, -- Se usa campo FECHA_CIERRE_CTA
replicate(''0'', 2 - len(Cast(DAY(FECHA_CIERRE_CTA) AS varchar))) + CAST(DAY([FECHA_PAGO_MULTA]) AS varchar) as FechaCierreCuenta_Dia, -- Se usa campo FECHA_CIERRE_CTA
--19
...
FECHA_RECHAZO_O_PRES_COBRO as FechaParametro
