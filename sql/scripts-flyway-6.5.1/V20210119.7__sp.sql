EXECUTE('
CREATE PROCEDURE PA_BITACORA_EXCEPCIONES 
...
AS 
   BEGIN
	  ...
	  DECLARE @MSG_MAIL_INICIADOS  varchar(2048)
	  DECLARE @MSG_MAIL_FINALIZADOS  varchar(2048)
	  DECLARE @MAIL_TO   varchar(128)
	  DECLARE @MAIL_FROM varchar(128)
	  DECLARE @SUBJECT   varchar(255)
	  DECLARE @MAIL_OID  numeric(10)
	  DECLARE @FECHA     date
	  DECLARE @FECHASTR  varchar(12)
      ...
      BEGIN
         ...
         BEGIN TRY
			...
			SET @TEXTO_FINALIZADOS = N''<table>''
			    + N''<tr><th>Grupo</th><th>Título</th><th>Operación</th><th>Descripción</th><th>Fecha Inicio</th><th>Fecha Fin</th></tr>''
			    + CAST((
					SELECT  CONCAT(p.GRUPO_CODIGO , '' - '', (SELECT DESCRIPCION FROM GRUPOS WHERE GRUPO = p.GRUPO_CODIGO), ''  '') AS td,
			        ...
			        FROM    BITACORA_RESTRICCIONES p
			        WHERE   p.TIPO_RESTRICCION = ''O'' AND CAST(p.FECHA_HABILITACION_RESTRICCION AS DATE) = @FECHA
			...
			SET @MAIL_TO  = ''chacochaco@hotmail.com''
			SET @MAIL_FROM = ''NBCH@localhost.com''
			SET @SUBJECT   = ''Finalizacion Excepciones''
			SET @MAIL_OID  = (SELECT TOP 1 MAIL_OID FROM CORREOS_A_ENVIAR ORDER BY MAIL_OID DESC) + 1
			...
			SET @TEXTO_INICIADOS = N''<table>''
			    ...
			    + CAST((
					SELECT  CONCAT(p.GRUPO_CODIGO , '' - '', (SELECT DESCRIPCION FROM GRUPOS WHERE GRUPO = p.GRUPO_CODIGO), ''  '') AS td,
						...
						convert(varchar, p.FECHA_HABILITACION_RESTRICCION, 3) AS td
			        FROM    BITACORA_RESTRICCIONES p
			        WHERE   p.TIPO_RESTRICCION = ''O'' AND CAST(p.FECHA_HORA_MODIFICADO AS DATE) = @FECHA
			        FOR XML RAW(''tr''), ELEMENTS
			    ) AS NVARCHAR(MAX))
			    + N''</table>''
