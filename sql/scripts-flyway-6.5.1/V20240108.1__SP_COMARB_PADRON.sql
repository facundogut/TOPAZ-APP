execute('
CREATE OR ALTER PROCEDURE [dbo].[SP_COMARB_PADRON] 
 	@perParam VARCHAR(6),
...
DROP TABLE IF EXISTS #Tmp_ITF_COMARB_PADRON
CREATE TABLE #Tmp_ITF_COMARB_PADRON
	(
...
INSERT INTO #Tmp_ITF_COMARB_PADRON
SELECT * FROM ITF_COMARB_PADRON
...
@cantPad NUMERIC(10,0) = (SELECT COUNT(*) FROM ITF_COMARB_PADRON), --caso base, si es 0 solo hay que grabar porque es la primer ejecucion y no se compara
...
--elmino registros ITF_COMARB_PADRON para el mismo periodo y la misma persona
IF @reproceso=''S''
BEGIN 
DELETE FROM ITF_COMARB_PADRON
WHERE PERIODO_APLICACION= @perParam 
--AND CUIT IN (SELECT cuit FROM ITF_COMARB_PADRON_IMPUESTOS_AUX)

DELETE FROM ITF_LOG_PADRON_IMPUESTOS
WHERE PERIODO_PADRON= @perParam
--AND  COD_CLIENTE IN  (SELECT cp.CODIGOCLIENTE 
--					  FROM CLI_DocumentosPFPJ pfj 
--					  JOIN CLI_ClientePersona cp ON pfj.NUMEROPERSONAFJ = cp.NUMEROPERSONA 
--					  WHERE pfj.NUMERODOCUMENTO in  (SELECT cuit FROM ITF_COMARB_PADRON_IMPUESTOS_AUX))
