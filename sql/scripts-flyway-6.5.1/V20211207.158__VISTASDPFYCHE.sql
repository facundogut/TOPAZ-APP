...
INNER JOIN PARAMETROSGENERALES P WITH (nolock)ON s.BANCO_GIRADO<>P.NUMERICO 
											...
											AND NOT EXISTS 
														(	SELECT * 
															...
															WHERE CMC7=s.CMC7 AND FECHAALTA = s.FECHA_ALTA)
...
CREATE VIEW [VW_DPF_OTRO_BCO_SIN_IMG] (
	...
	WHERE P.CODIGO = 2
		AND s.BANCO_GIRADO<>P.NUMERICO and NOT EXISTS 
											(	SELECT * 
												FROM CLE_IMG_CHEQUES with (nolock)
												WHERE CMC7=s.BANDA AND FECHAALTA=s.FECHA_ALTA)
...
VALUES (35346, '' '', 0, ''FECHAALTA'', ''FECHAALTA'', 8, ''F'', 0, ''F'', 0, 0, 0, 0, 0, 0, 0, 5553, ''FECHAALTA'', 0, NULL)
')
