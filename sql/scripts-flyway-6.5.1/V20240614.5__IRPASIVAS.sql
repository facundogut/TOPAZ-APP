...
----------------------------------------17
	if exists (	select a.COD_BLOQUEO 
				...
				SELECT    ''Tabla GRL_REL_BLOQUEO_SEGURIDAD no se ajusta a la integridad referencial con grl_bloqueos '',*
				from grl_bloqueos a with(nolock) 
				where a.COD_BLOQUEO not in(	select b.COD_BLOQUEO 
												from GRL_REL_BLOQUEO_SEGURIDAD B with(nolock))
	else
	print ''Comprobación OK.Tabla GRL_REL_BLOQUEO_SEGURIDAD  con grl_bloqueos ''
----------------------------------------18
----------------------------------------19
