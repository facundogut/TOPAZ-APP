...
DECLARE @cuit VARCHAR(11),@razonSocial VARCHAR(44), @periodo VARCHAR(6), @alicuota NUMERIC(4,4);

--var auxiliares
DECLARE @alicuotaAux NUMERIC(4,4), @alicuotaMax NUMERIC(4,4), @alicuotaVIEJA NUMERIC(4,4), @recaudAux VARCHAR (2), @vinoEnPadron INT;
DECLARE @personaMasGravosa NUMERIC(12), @condicionMax VARCHAR(2);
DECLARE @numeroPersona NUMERIC(20), @numeroCliente NUMERIC(20);
DECLARE @actualizaCli VARCHAR(1);
...
OPEN cursorOk
    ...
    BEGIN
    	...
    	SET @alicuotaAux = 0;
    	SELECT @alicuotaAux = CAST(substring(REGISTRO, 62,6) AS NUMERIC)/10000 FROM ITF_PADRON_IMPUESTOS_AUX WHERE SUBSTRING(REGISTRO, 1, 11) = @cuit;       
...
OPEN cursorAux
    ...
    BEGIN
    	...
    	SET @alicuota = CAST(substring(@reg, 62,6) AS NUMERIC)/10000;
		...
		IF(@cantImp <> 0)
			SELECT @vinoEnPadron = COUNT(*), @alicuotaVieja = alicuota FROM ITF_PADRON_IMPUESTOS WHERE cuit = @cuit AND CAST(CONCAT(SUBSTRING(PERIODO_ARCHIVO,3,4),SUBSTRING(PERIODO_ARCHIVO,1,2),''01'') AS DATETIME) = @periodoAcomparar GROUP BY ALICUOTA;
