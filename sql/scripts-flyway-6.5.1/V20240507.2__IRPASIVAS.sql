...
----------------------------------------13
	if exists (select a.SALDO_JTS_OID 
				...
				SELECT     ''Tabla VTA_RESERVAS no se ajusta a la integridad referencial con CI_SOLICITUD '',*
				from VTA_RESERVAS a with(nolock) 
				where a.TIPO_RESERVA= 1 
					and a.SALDO_JTS_OID not in (select b.SALDO_JTS_OID 
												from CI_SOLICITUD b  with(nolock))
...
----------------------------------------31
	if exists (select b.TARJETA 
				from PROD_RELTARJETAS b with(nolock) 
				where b.TARJETA not in (select a.c6250 
										from PRODUCTOS a with(nolock))
				)	
...
----------------------------------------33
	if exists (	select a.sector 
				...
				where a.sector not in(	select b.SECTOR 
										from PROD_RELSECTORES b with(nolock))
			)	
				SELECT   ''Tabla CLI_SECTORES no se ajusta a la integridad referencial con PROD_RELSECTORES '',*
				from CLI_SECTORES a with(nolock) 
				where a.sector not in(	select b.SECTOR 
										from PROD_RELSECTORES b with(nolock))
	else
	print ''Comprobación OK.Tabla CLI_SECTORES con PROD_RELSECTORES ''
----------------------------------------34
...
----------------------------------------37
	if exists (	select a.JTSOID 
				from GRL_CAB_ENVIO_ESTCTA a with(nolock)
				where a.JTSOID not in (	select b.JTSOID 
												from GRL_DET_ENVIO_ESTCTA b with(nolock))
			)	
				select     ''Tabla GRL_CAB_ENVIO_ESTCTA no se ajusta a la integridad referencial con GRL_DET_ENVIO_ESTCTA '',*
				from GRL_CAB_ENVIO_ESTCTA a with(nolock)
				where a.JTSOID not in (	select b.JTSOID 
												from GRL_DET_ENVIO_ESTCTA b with(nolock))
	else
	print ''Comprobación OK.Tabla GRL_CAB_ENVIO_ESTCTA con GRL_DET_ENVIO_ESTCTA ''
...
----------------------------------------40
	if exists (	select a.CODAPODERAMIENTO
				...
				select  ''Tabla PYF_APODERAMIENTO no se ajusta a la integridad referencial con pyf_apoderados '',*
				from PYF_APODERAMIENTO a with(nolock) 
				where a.CODAPODERAMIENTO not in (select b.APODERAMIENTO
												from pyf_apoderados b with(nolock))
	else
...
----------------------------------------67
	if exists (select a.codigo_tipo
				...
				SELECT   ''Tabla DPF_DOCS_recepciones no se ajusta a la integridad referencial con DPF_DOCS_solicitudes '',*
				from DPF_DOCS_recepciones a with(nolock) 
				where a.codigo_tipo not in (	select b.codigo_tipo
												from DPF_DOCS_solicitudes b with(nolock))
	else
	print ''Comprobación OK. Tabla DPF_DOCS_recepciones con DPF_DOCS_solicitudes ''
