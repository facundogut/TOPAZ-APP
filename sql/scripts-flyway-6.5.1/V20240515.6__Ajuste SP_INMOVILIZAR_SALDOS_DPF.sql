...
/* 04/01/2022 SE CONDICIONA LOS TITULOS VALOR PARA QUE SOLO PASEN A INMOVILIZADOS 
SI SON TRANSFERIBLES. NBCHDEF-3148. JMB*/
	...
	-- CARGO LAS TABLAS TEMPORALES
				...
				INSERT INTO @TMPSaldosAInmovilizarDPF
					...
					WHERE
						...
						AND S.TZ_LOCK = 0
						AND (
							(S.PRODUCTO = (SELECT NUMERICO FROM PARAMETROSGENERALES WHERE CODIGO = 708)
							AND S.TIPO_DPF = ''01'')	 
							OR (S.PRODUCTO <> (SELECT NUMERICO FROM PARAMETROSGENERALES WHERE CODIGO = 708))
							);
