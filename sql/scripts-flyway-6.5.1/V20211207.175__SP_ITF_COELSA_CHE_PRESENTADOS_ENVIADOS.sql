...
OPEN CursorUno --Abrir el CURSOR
    ...
    WHILE @@FETCH_STATUS = 0 
    					...
    					BEGIN
    					  ...
    					  -- (COD_BANCO, NUMERO_DEL_CHEQUE, NUMERO_CUENTA_GIRADORA, FECHA_DEL_CHEQUE, SUCURSAL, SERIE_DEL_CHEQUE, TIPO_DOCUMENTO)
    					   ...
    					   --Varibles para conformar @RI_ENTIDAD_DEBITAR
    						...
    						VALUES (@TICKET, @TICKET, (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)), @RI_CODIGO_TRANSAC, CAST(@RI_ENTIDAD_DEBITAR AS NUMERIC), @T_NUMERO_CUENTA_GIRADORA, @T_IMPORTE, @RI_CODIGO_POSTAL, @T_FECHA_DEL_CHEQUE, @T_FECHA_VALOR, @T_NUMERO_DEL_CHEQUE, @RI_PUNTO_INTERCAMBIO, @RI_CONTADOR_REGISTRO, ''P'', ''C'', @T_MONEDA);
    						---------------------------
        ...
        BEGIN
        	...
        	--- Grabar historial ---
    		INSERT INTO ITF_COELSA_CHEQUES_OTROS (ID_TICKET, ID_PROCESO, FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE, CODIGO_POSTAL, FECHA_PRESENTADO, FECHA_VENCIMIENTO, NRO_CHEQUE, PUNTO_INTERCAMBIO, TRACE_NUMBER, ESTADO, TIPO, MONEDA)
    		VALUES (@TICKET, @TICKET, (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)), @RI_CODIGO_TRANSAC, CAST(@RI_ENTIDAD_DEBITAR AS NUMERIC), @A_NUMERO_CUENTA, @A_IMPORTE, @A_CODIGO_POSTAL, @A_FECHA_ALTA, @A_FECHA_ALTA, @A_NUMERO_CHEQUE, @RI_PUNTO_INTERCAMBIO, @RI_CONTADOR_REGISTRO, ''P'', ''A'', @A_MONEDA);
    	    ---------------------------
	    ...
	    BEGIN
        	...
        	--- Grabar historial ---
    		INSERT INTO ITF_COELSA_CHEQUES_OTROS (ID_TICKET, ID_PROCESO, FECHAPROCESO, CODIGO_TRANSACCION, ENTIDAD_DEBITAR, CUENTA_DEBITAR, IMPORTE, CODIGO_POSTAL, FECHA_PRESENTADO, FECHA_VENCIMIENTO, NRO_CHEQUE, PUNTO_INTERCAMBIO, TRACE_NUMBER, ESTADO, TIPO, MONEDA)
    		VALUES (@TICKET, @TICKET, (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)), @RI_CODIGO_TRANSAC, CAST(@RI_ENTIDAD_DEBITAR AS NUMERIC), @D_CUENTA, @D_IMPORTE, @D_CODIGO_POSTAL, @D_FECHA, @D_FECHA, @D_NUMERO_DPF, @RI_PUNTO_INTERCAMBIO, @RI_CONTADOR_REGISTRO, ''P'', ''D'', @D_MONEDA);
    	    ---------------------------
