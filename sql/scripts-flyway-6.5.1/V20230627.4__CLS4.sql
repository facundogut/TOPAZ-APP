Execute('CREATE OR ALTER PROCEDURE [dbo].[SP_COELSA_IMAGENES_CHEQUES_PRESENTADOS_RECIBIDOS] @TICKET NUMERIC(16)
-- SP VCP
-- VCD Escribe directo en la aux
AS
BEGIN
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
BEGIN  
	  ...
	  SET @BancoDepositario = CAST(substring(@LINEA, 2, 3) AS NUMERIC);
		...
		IF(@TICKET<>0)
      BEGIN
		INSERT INTO dbo.ITF_COELSA_IMAGENES_CHEQUES_PROPIOS_AUX(ID_TICKET, ID_PROCESO, FECHA_PROCESO, ESTADO, ID_REGISTRO, BANCO_DEPOSITARIO, BANCO_GIRADO, SUCURSAL_GIRADA, CODIGO_POSTAL, NRO_CHEQUE, NRO_CUENTA, FECHA_PRESENTACION, FECHA_COMPENSACION, ESTADO_TRAN, NOMBRE_IMAGEN) 
