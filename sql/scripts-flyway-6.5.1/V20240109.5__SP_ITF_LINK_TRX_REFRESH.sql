    ...
    SET @Error = '' '';
  	...
  	BEGIN TRY
		...
		USING (
		  ...
		  FROM ITF_LINK_TRK
		  WHERE EXISTS(SELECT 1 FROM TJD_TIPOS_NOVEDADES_LINK 
		...
		WHERE EXISTS(SELECT 1 FROM ITF_LINK_TRK A 
					 WHERE SUBSTRING(A.Linea, 1, 6) IN (190000,190002,190007)
					   ...
					   AND EXISTS(SELECT 1 FROM TJD_TIPOS_NOVEDADES_LINK 
					              WHERE CODIGO_LINK = SUBSTRING(Linea, 1, 6)  
		...
		WHEN NOT MATCHED THEN
			INSERT (NRO_TARJETA, Grp_Op_Modif, Timestamp_Modif, Aud_Alta, Timestamp_Alta,
		    ...
		    VALUES(B.NRO_TARJETA, B.Grp_Op_Modif, B.Timestamp_Modif, B.Aud_Alta, B.Timestamp_Alta,
		...
		USING (SELECT ID_TARJETA = substring(LINK.Linea,731,19),
		    		  ...
		    		  CUENTA_PBF = substring(LINK.Linea, 185, 19),
		    		  --
		    		  S.JTS_OID,
		    		  S.PRODUCTO,
		    		  S.CUENTA,
		...
		WHEN MATCHED AND (substring(Linea,731,19) <> ''                   '' and  substring(Linea,731,19) <> ''0000000000000000000'') 
		  THEN
		...
		USING (SELECT ID_TARJETA = substring(Linea,731,19),
			   ...
			   WHERE EXISTS(SELECT 1 FROM TJD_TIPOS_NOVEDADES_LINK N WHERE N.ACTUALIZA_BD = ''S'' AND N.TZ_LOCK = 0 AND N.CODIGO_LINK = SUBSTRING(TRX.Linea, 1, 6))
			   )B
			 ON A.ID_TARJETA = B.ID_TARJETA
		 WHEN MATCHED AND (substring(Linea,731,19) <> ''                   '' and  substring(Linea,731,19) <> ''0000000000000000000'') 
		 	THEN
