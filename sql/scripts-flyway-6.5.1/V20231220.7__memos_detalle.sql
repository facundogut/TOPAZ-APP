EXECUTE('
CREATE PROCEDURE PA_MEMOS_DETALLE
   ...
   BEGIN
      ...
      DELETE FROM MEMO_DETALLE WHERE Fecha_Info = @FECHA_PROCESO
                ...
                SELECT DISTINCT
                 ...
                 (SELECT SUM(C2310) FROM PLANPAGOS WITH(NOLOCK) WHERE SALDO_JTS_OID = S.JTS_OID), ---INT DEVEN VENCIDO
                 (SELECT SUM(C2309) FROM PLANPAGOS WITH(NOLOCK) WHERE SALDO_JTS_OID = S.JTS_OID), ---CAPITAL VENCIDO
                 ...
                 FROM VW_DEUDA_CLIENTES DC WITH(NOLOCK)
                 INNER JOIN SALDOS S WITH(NOLOCK) ON S.JTS_OID = DC.JTS_OID AND S.TZ_LOCK = 0
                 ...
                 WHERE S.TZ_LOCK = 0 AND DC.SALDO_DEUDA <> 0
