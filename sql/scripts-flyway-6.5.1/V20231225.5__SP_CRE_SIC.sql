...
BEGIN TRY 
	...
	CASE WHEN MONTH(CLI.FECHAAPERTURA)= MONTH(@FECHAPROCESO) THEN ''S'' 
	ELSE ''N''
...
--CHEQUES RECHAZADOS - TIPO 1
   INSERT INTO dbo.CRE_SIC_BITACORA_DET (CLIENTE, FECHA, TIPO, PUNTOS_SIN_POND, PUNTOS_CON_POND, PONDERACION)
	   ...
	   P.PONDERACION
	   FROM (
		   SELECT B.CLIENTE,B.NUEVO,ISNULL(COUNT(CC.CLIENTE),0) AS CANTIDAD 
		   FROM @CLIENTES B
		   LEFT JOIN CLE_CHEQUES_CLEARING_RECIBIDO CC WITH(NOLOCK)  ON B.CLIENTE=CC.CLIENTE
		   		AND CC.ESTADO_DEVOLUCION IN (1,3) -- AND CC.FECHA >= DATEADD(MONTH, -12,@FECHAPROCESO)
		   		AND CC.TZ_LOCK=0 AND BANCO_DEPOSITANTE=311 
