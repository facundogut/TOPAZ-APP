EXECUTE('
ALTER PROCEDURE [dbo].[PA_BITACORA_EXCEPCIONES] 
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN TRY
			...
			SET @TEXTO_FINALIZADOS = N''<table>''
			    ...
			    + CAST((
					SELECT  CONCAT(p.GRUPO_CODIGO , 
							'' - '', (SELECT DESCRIPCION FROM GRUPOS WHERE GRUPO = p.GRUPO_CODIGO), ''  '') AS td,
			...
			SET @MAIL_TO  = (SELECT ALFA FROM PARAMETROSGENERALES WHERE CODIGO = 206)
			SET @MAIL_FROM = (SELECT ALFA FROM PARAMETROSGENERALES WHERE CODIGO = 205)
