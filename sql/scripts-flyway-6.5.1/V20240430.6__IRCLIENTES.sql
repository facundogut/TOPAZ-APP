...
IF (SELECT DB_NAME()) = @BD
  BEGIN
	...
	if exists (	select a.codigogrupoeconomico 
				from CLI_GruposEconomicosCliente a with(nolock) 
				where a.codigogrupoeconomico not in (	select b.CODIGOGRUPOECONOMICO
												from CLI_GruposEconomicos b with(nolock))
				)
				select  ''Tabla CLI_GruposEconomicosCliente no cumple la integridad referencial con CLI_GruposEconomicos  '',* 
				from CLI_GruposEconomicosCliente a with(nolock) 
				where a.codigogrupoeconomico not in (	select b.CODIGOGRUPOECONOMICO
												from CLI_GruposEconomicos b with(nolock))
	else
	print ''Comprobación OK. Tabla CLI_GruposEconomicosCliente con CLI_GruposEconomicos ''
	...
	if exists (	select a.numeropersonafisica 
				...
				select  ''Tabla CLI_Cliente no cumple la integridad referencial con CLI_PERSONASFISICAS  '',* 
				from CLI_HIST_FALLECIDOS a with(nolock) 
				where a.numeropersonafisica not in (
													select b.NUMEROPERSONAFISICA
													from CLI_PERSONASFISICAS b with(nolock))
	else
	print ''Comprobación OK. Tabla CLI_Cliente con CLI_PERSONASFISICAS ''

	if exists (	select a.TIPO_SOCIEDAD 
				from CLI_ROLES_OBL_CARGO a with(nolock) 
				where a.TIPO_SOCIEDAD not in (	select b.CODIGO_SOCIEDAD
												from CLI_TIPO_SOCIEDAD b with(nolock))
				)
				select  ''Tabla CLI_ROLES_OBL_CARGO no cumple la integridad referencial con CLI_TIPO_SOCIEDAD  '',* 
				from CLI_ROLES_OBL_CARGO a with(nolock) 
				where a.TIPO_SOCIEDAD not in (	select b.CODIGO_SOCIEDAD
												from CLI_TIPO_SOCIEDAD b with(nolock))
	else
	print ''Comprobación OK. Tabla CLI_ROLES_OBL_CARGO con CLI_TIPO_SOCIEDAD ''
...
  	if exists ( select a.NUMEROPERSONAFJ
        		from cli_documentos_historial a with(nolock) 
        		where a.NUMEROPERSONAFJ not in ( select b.NUMEROPERSONAFJ  
                        						 from CLI_DocumentosPFPJ b with(nolock)) 
				)
      select   ''Tabla cli_documentos_historial no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS y CLI_PERSONASFISICAS '',*
        from cli_documentos_historial a with(nolock) 
        where a.NUMEROPERSONAFJ not in (  	select b.NUMEROPERSONAFJ  
                        					from CLI_DocumentosPFPJ b with(nolock))  
  	else
    print ''Comprobación OK. Tabla cli_documentos_historial con CLI_PERSONASJURIDICAS y CLI_PERSONASFISICAS ''

	if exists (	select a.RIESGO_OBTENIDO 
				from ITF_MATRIZ_IGR a with(nolock) 
				where a.RIESGO_OBTENIDO not in (	select b.ID
												from cli_parametria_igr b with(nolock))
				)
				select  ''Tabla ITF_MATRIZ_IGR no cumple la integridad referencial con cli_parametria_igr  '',* 
				from ITF_MATRIZ_IGR a with(nolock) 
				where a.RIESGO_OBTENIDO not in (	select b.ID
												from cli_parametria_igr b with(nolock))
	else
	print ''Comprobación OK. Tabla ITF_MATRIZ_IGR con cli_parametria_igr ''

	if exists (	select a.TIPO_SUJETO_UIF 
				from CLI_PERSONASJURIDICAS a with(nolock) 
				where a.TIPO_SUJETO_UIF not in (	select b.CODIGO_TIPO_SUJETO
												from cli_tipo_sujeTo_obligado_uif b with(nolock))
				)
				select  ''Tabla CLI_PERSONASJURIDICAS no cumple la integridad referencial con cli_tipo_sujeTo_obligado_uif  '',* 
				from CLI_PERSONASJURIDICAS a with(nolock) 
				where a.TIPO_SUJETO_UIF not in (	select b.CODIGO_TIPO_SUJETO
												from cli_tipo_sujeTo_obligado_uif b with(nolock))
	else
	print ''Comprobación OK. Tabla CLI_PERSONASJURIDICAS con cli_tipo_sujeTo_obligado_uif ''
	if exists (	select a.TIPO_SUJETO_UIF 
				from CLI_PERSONASFISICAS a with(nolock) 
				where a.TIPO_SUJETO_UIF not in (	select b.CODIGO_TIPO_SUJETO
												from cli_tipo_sujeTo_obligado_uif b with(nolock))
				)
				select  ''Tabla CLI_PERSONASFISICAS no cumple la integridad referencial con cli_tipo_sujeTo_obligado_uif  '',* 
				from CLI_PERSONASFISICAS a with(nolock) 
				where a.TIPO_SUJETO_UIF not in (	select b.CODIGO_TIPO_SUJETO
												from cli_tipo_sujeTo_obligado_uif b with(nolock))
	else
	print ''Comprobación OK. Tabla CLI_PERSONASFISICAS con cli_tipo_sujeTo_obligado_uif ''

	if exists (		select a.CODIGO_PERSONA_CLIENTE
				from cli_actividad_economica a with(nolock) 
				where a.CODIGO_PERSONA_CLIENTE not in (	select b.NUMEROPERSONAJURIDICA  
												from CLI_PERSONASJURIDICAS b with(nolock)
												union all 
												select c.numeropersonafisica
												from cli_personasfisicas c with(nolock))
				)	
			select   ''Tabla cli_actividad_economica no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS y CLI_PERSONASFISICAS '',*
			from cli_actividad_economica a with(nolock) 
				where a.CODIGO_PERSONA_CLIENTE not in(	select b.NUMEROPERSONAJURIDICA  
											from CLI_PERSONASJURIDICAS b with(nolock)
											union all 
		...
		print ''Comprobación OK. Tabla cli_actividad_economica con CLI_PERSONASJURIDICAS y CLI_PERSONASFISICAS ''

	if exists (	select a.TIPO_DOC_FISICO 
				from  CLI_TIPOS_DOC_FISICOS a with(nolock) 
				where a.TIPO_DOC_FISICO not in (select b.TIPO_DOCUMENTO
												from parametros_doc_fisicos b with(nolock))
				)
				select  ''Tabla CLI_TIPOS_DOC_FISICOS no cumple la integridad referencial con parametros_doc_fisicos  '',* 
				from CLI_TIPOS_DOC_FISICOS a with(nolock) 
				where a.TIPO_DOC_FISICO not in (select b.TIPO_DOCUMENTO
												from parametros_doc_fisicos b with(nolock))
	else
	print ''Comprobación OK. Tabla CLI_TIPOS_DOC_FISICOS con parametros_doc_fisicos ''

	if exists (	select a.SUBSEGMENTOCLIENTE 
				from  cli_clientes a with(nolock) 
				where a.SUBSEGMENTOCLIENTE not in (select b.COD_SUBSEGMENTO
												from cli_subsegmentos b with(nolock))
				)
				select  ''Tabla cli_clientes no cumple la integridad referencial con cli_subsegmentos  '',* 
				from  cli_clientes a with(nolock) 
				where a.SUBSEGMENTOCLIENTE not in (select b.COD_SUBSEGMENTO
												from cli_subsegmentos b with(nolock))
	else
	print ''Comprobación OK. Tabla cli_clientes con cli_subsegmentos ''

	if exists (	select a.CODIGO_PERSONA 
				from  cli_documentos_personas a with(nolock) 
				where a.CODIGO_PERSONA not in (select b.NUMEROPERSONAFISICA
												from CLI_PERSONASFISICAS b with(nolock))
				)
				select  ''Tabla cli_documentos_personas no cumple la integridad referencial con CLI_PERSONASFISICAS  '',* 
				from  cli_documentos_personas a with(nolock) 
				where a.CODIGO_PERSONA not in (select b.NUMEROPERSONAFISICA
												from CLI_PERSONASFISICAS b with(nolock))
	else
	print ''Comprobación OK. Tabla cli_documentos_personas con CLI_PERSONASFISICAS ''

	if exists (	select a.ID
				from cli_direcciones a with(nolock) 
				where a.ID not in (	select b.NUMEROPERSONAJURIDICA  
												from CLI_PERSONASJURIDICAS b with(nolock)
												union all 
												select c.numeropersonafisica
												from cli_personasfisicas c with(nolock)
												union all
												select sucursal from sucursales with(nolock) 
												union all 
												select nro_juzgado from DJ_JUZGADOS with(nolock) 
									)
				)	
			select   ''Tabla cli_direcciones no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS y CLI_PERSONASFISICAS '',*
			from cli_direcciones a with(nolock) 
				where a.ID not in(	select b.NUMEROPERSONAJURIDICA  
											from CLI_PERSONASJURIDICAS b with(nolock)
											union all 
		...
		print ''Comprobación OK. Tabla cli_direcciones con CLI_PERSONASJURIDICAS y CLI_PERSONASFISICAS ''
	if exists (	select a.PERSONA_VINCULANTE
				from cli_vinculaciones a with(nolock) 
				where a.PERSONA_VINCULANTE not in (	select b.NUMEROPERSONAJURIDICA  
												from CLI_PERSONASJURIDICAS b with(nolock)
												union all 
												select c.numeropersonafisica
												from cli_personasfisicas c with(nolock))
				)
			select   ''Tabla cli_vinculaciones no se ajusta a la integridad referencial con CLI_PERSONASJURIDICAS y CLI_PERSONASFISICAS'',*
				from cli_vinculaciones a with(nolock) 
				where a.PERSONA_VINCULANTE not in (	select b.NUMEROPERSONAJURIDICA  
												from CLI_PERSONASJURIDICAS b with(nolock)
												union all 
		...
		print ''Comprobación OK. Tabla cli_vinculaciones con CLI_PERSONASJURIDICAS y CLI_PERSONASFISICAS Y CLI_INSTITUCIONFINANCIERA''

	---------------------------------------------------------------------------------------------------
	if exists (	select a.CALIFICACIONSUBJETIVA 
				from  cli_personasfisicas a with(nolock) 
				where a.CALIFICACIONSUBJETIVA not in (select b.CATEGORIASUB
												from CLI_CLASUBJETIVA b with(nolock))
				)
				select  ''Tabla cli_personasfisicas no cumple la integridad referencial con CLI_CLASUBJETIVA  '',* 
				from  cli_personasfisicas a with(nolock) 
				where a.CALIFICACIONSUBJETIVA not in (select b.CATEGORIASUB
												from CLI_CLASUBJETIVA b with(nolock))
	else
	print ''Comprobación OK. Tabla cli_personasfisicas con CLI_CLASUBJETIVA ''

	if exists (	select a.ENTIDAD_CALIFICADORA 
				from  cli_personasfisicas a with(nolock) 
				where a.ENTIDAD_CALIFICADORA not in (select b.ENTIDAD
												from cli_entidad_calificadora b with(nolock))
				)
				select  ''Tabla cli_personasfisicas no cumple la integridad referencial con cli_entidad_calificadora  '',* 
				from  cli_personasfisicas a with(nolock) 
				where a.ENTIDAD_CALIFICADORA not in (select b.ENTIDAD
												from cli_entidad_calificadora b with(nolock))
	else
	print ''Comprobación OK. Tabla cli_personasfisicas con cli_entidad_calificadora ''
	...
	if exists (	select a.ENTIDAD_CALIFICADORA 
				...
				select  ''Tabla cli_personasjuridicas no cumple la integridad referencial con cli_clasubjetiva  '',* 
				from  cli_personasjuridicas a with(nolock) 
				where a.CALIFICACIONSUBJETIVA not in (select b.categoriasub
												from cli_clasubjetiva b with(nolock))
	else
	print ''Comprobación OK. Tabla cli_personasjuridicas con cli_clasubjetiva ''

 	if exists (	select a.CODIGOCARGO 
				from  cli_integrantespj a with(nolock) 
				where a.CODIGOCARGO not in (select b.CODIGO
												from cli_cargos_personas b with(nolock))
				)
				select  ''Tabla cli_integrantespj no cumple la integridad referencial con cli_cargos_personas  '',* 
				from  cli_integrantespj a with(nolock) 
				where a.CODIGOCARGO not in (select b.CODIGO
												from cli_cargos_personas b with(nolock))
	else
	print ''Comprobación OK. Tabla cli_integrantespj con cli_cargos_personas ''

 	if exists (	select a.SECTOR 
				from  cli_cod_act_bcra a with(nolock) 
				where a.SECCION not in (select b.CODIGO_SECCION
												    from cli_seccion b with(nolock))
				)
				select  ''Tabla cli_cod_act_bcra no cumple la integridad referencial con cli_seccion  '',* 
				from  cli_cod_act_bcra a with(nolock) 
				where a.SECCION not in (select b.CODIGO_SECCION
												from cli_seccion b with(nolock))
	else
	print ''Comprobación OK. Tabla cli_cod_act_bcra con cli_seccion ''

 	if exists (	select a.SECTOR 
				from  cli_cod_act_bcra a with(nolock) 
				where a.SECTOR not in (select b.CODIGO_SECTOR
												    from cli_actividad_sector b with(nolock))
				)
				select  ''Tabla cli_cod_act_bcra no cumple la integridad referencial con cli_actividad_sector  '',* 
				from  cli_cod_act_bcra a with(nolock) 
				where a.SECTOR not in (select b.CODIGO_SECTOR
												from cli_actividad_sector b with(nolock))
	else
	print ''Comprobación OK. Tabla cli_cod_act_bcra con cli_actividad_sector ''

	if exists (	select a.CALIFICADORA 
				from  cre_calif_calificadoras a with(nolock) 
				where a.CALIFICADORA not in (select b.ENTIDAD
												    from cli_entidad_Calificadora b with(nolock))
				)
				select  ''Tabla cre_calif_calificadoras no cumple la integridad referencial con cli_entidad_Calificadora  '',* 
				from  cre_calif_calificadoras a with(nolock) 
				where a.CALIFICADORA not in (select b.ENTIDAD
												    from cli_entidad_Calificadora b with(nolock))
	else
	print ''Comprobación OK. Tabla cre_calif_calificadoras con cli_entidad_Calificadora ''
