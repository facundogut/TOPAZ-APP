...
[Descripcion Vinculo], [Persona Vinculada]) AS
SELECT DISTINCT v.ID_VINCULO, ISNULL(CDV.CODIGOCLIENTE, 0) AS CLIENTE_VINCULADO, ISNULL(CDV.NOMBRECLIENTE, PFJ.NOMBRE) AS NOMBRE_VINCULADO,
ISNULL(CXD.CODIGOCLIENTE,0) AS CLIENTE_VINCULANTE, ISNULL(CXD.NOMBRECLIENTE,'''') AS NOMBRE_VINCULANTE,
ISNULL(CLI.CATEGORIARESULTANTE, ''0'') AS SITUACION,
...
INNER JOIN VW_PERSONAS_FYJ PFJ WITH(NOLOCK) ON PFJ.NUMEROPERSONA = V.ID_PERSONA_VINCULADA AND PFJ.TIPOPERSONA = V.TIPO_PERSONA_VINCULADA
LEFT JOIN VW_CLI_X_DOC CDV WITH(NOLOCK) ON V.ID_PERSONA_VINCULADA = CDV.NUMEROPERSONA
LEFT JOIN CLI_CLIENTES CLI WITH(NOLOCK) ON CDV.CODIGOCLIENTE = CLI.CODIGOCLIENTE AND CLI.TZ_LOCK = 0
...
CV.TIPO_RELACION, CV.ID_VINCULO, CV.DESCRIPCION_VINCULO, OP.DESCRIPCION, CDV.NOMBRECLIENTE, CXD.NOMBRECLIENTE, PFJ.NOMBRE, PFJ.NUMEROPERSONA
')
