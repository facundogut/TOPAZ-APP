...
')
EXECUTE('
/****** Object:  StoredProcedure [dbo].[SP_MAILS_MODIFICACION_COMISIONES]    Script Date: 01/06/2021 15:09:18 ******/
...
BEGIN
	...
	SET @DescripcionCargo = (SELECT DESCRIPCION 
							FROM CI_CARGOS with (nolock) 
							WHERE [ID_CARGO] = @Cargo)
	
    DECLARE [ModificCargoCursor] CURSOR FOR
    
		SELECT DISTINCT E.EMAIL AS CORREO_DESTINO
		FROM	CLI_EMAILS AS E with (nolock) 
		INNER JOIN CLI_ClientePersona AS T with (nolock)ON E.ID = T.NUMEROPERSONA 
															AND T.TITULARIDAD = ''T''
															AND T.TZ_LOCK = 0
															AND E.TZ_LOCK = 0
		INNER JOIN GRL_ESTADOS_DE_CUENTA AS G with (nolock) ON T.CODIGOCLIENTE = G.CLIENTE
																AND G.TZ_LOCK = 0
		INNER JOIN SALDOS AS S with (nolock) ON G.JTSOID = S.JTS_OID
												AND S.C1651 <> 1 
												AND S.TZ_LOCK = 0 
