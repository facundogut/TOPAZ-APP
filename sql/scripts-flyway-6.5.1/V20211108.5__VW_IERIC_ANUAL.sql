EXECUTE('
IF OBJECT_ID (''dbo.VW_RRII_IERIC_ANUAL'') IS NOT NULL
	DROP VIEW dbo.VW_RRII_IERIC_ANUAL
')

EXECUTE('
CREATE VIEW VW_RRII_IERIC_ANUAL AS
WITH AUX AS
(
SELECT
	VS.CTA_CBU AS CBU,
...
WHERE
	HV.DEBITO_CREDITO = ''C''
	AND HV.FECHA_VALOR NOT BETWEEN DATEADD(MONTH, -12, (SELECT FECHAPROCESO FROM  PARAMETROS P  WITH (NOLOCK))) AND (SELECT FECHAPROCESO FROM  PARAMETROS P  WITH (NOLOCK))
...
WHERE
	HV.FECHA_VALOR NOT BETWEEN DATEADD(MONTH, -12, (SELECT FECHAPROCESO FROM  PARAMETROS P  WITH (NOLOCK))) AND (SELECT FECHAPROCESO FROM  PARAMETROS P  WITH (NOLOCK))
GROUP BY
	HV.SALDO_JTS_OID,
	A.MONTO_MN,
	A.CUIL_EMPLEADO,
