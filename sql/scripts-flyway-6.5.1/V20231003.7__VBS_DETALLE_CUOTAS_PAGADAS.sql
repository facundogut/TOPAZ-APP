...
SELECT 
	...
	CASE WHEN PP.TZ_LOCK <> 0 THEN ISNULL(PPO.CAPITAL,0) -- Cuota adelantada
		 WHEN PP.C2313 < PP.C2302 AND (PP.C2309+PP.C2310)=0 THEN ISNULL(PP.C2304,0) -- Cancela antes del vencimiento
...
LEFT JOIN (SELECT DISTINCT gs.SALDOS_JTS_OID,gs.NUMERO_CUOTA, gs.CARGO_ID, gs.IMPUESTO_ID, gs.IMPORTE_GASTO, gs.SALDO_GASTO,
				CASE WHEN ttl.CODIGO IS NULL THEN ci.ID_CARGO ELSE ttl.CODIGO END AS CODIGO,
				CASE WHEN ttl.DESCRIPCION IS NULL THEN ci.DESCRIPCION ELSE ttl.DESCRIPCION END AS DESCRIPCION ,op1.DESCRIPCION AS TIPO,op2.DESCRIPCION AS CATEGORIA
				FROM GASTOS_POR_CUOTA gs WITH (nolock)
				INNER JOIN CI_CARGOS ci WITH (NOLOCK)  ON ci.ID_CARGO=gs.CARGO_ID AND ci.tz_lock=0
				LEFT JOIN CRE_GASTOS_LEASING gl WITH (nolock) ON gl.SALDOS_JTS_OID=gs.SALDOS_JTS_OID AND gl.TZ_LOCK=0
				LEFT JOIN CRE_TIPO_TRAMITE_LEASING ttl WITH (nolock) ON ttl.ID_CARGO=gs.CARGO_ID AND ttl.CODIGO=gl.CODIGO_TRAMITE AND ttl.TZ_LOCK=0
				LEFT JOIN opciones  op1 WITH (nolock) ON op1.NUMERODECAMPO=43371 AND op1.OPCIONINTERNA= ttl.TIPO 
				LEFT JOIN opciones  op2 WITH (nolock) ON op2.NUMERODECAMPO=43374 AND op2.OPCIONINTERNA=ttl.CATEGORIA 
				WHERE gs.TZ_LOCK=0 
				GROUP BY gs.SALDOS_JTS_OID,gs.NUMERO_CUOTA, gs.CARGO_ID, gs.IMPUESTO_ID, gs.IMPORTE_GASTO, gs.SALDO_GASTO,
				ttl.CODIGO, ttl.DESCRIPCION,op1.DESCRIPCION,op2.DESCRIPCION,ci.DESCRIPCION,ci.ID_CARGO ) G
