EXECUTE('
---------------------------------------
--AGREGAR CAMPOS DE PRODUCTOS A VISTA--
---------------------------------------
ALTER VIEW [VW_PYF_APODERADOS] AS
...
LEFT JOIN SALDOS s with (nolock) ON 
	...
	AND s.TZ_LOCK = 0
LEFT JOIN VTA_SALDOS vs with (nolock) ON 
	vs.JTS_OID_SALDO = s.JTS_OID 
	AND vs.TZ_LOCK = 0
LEFT JOIN PRODUCTOS p with (nolock) ON 
	p.C6250 = s.PRODUCTO 
	AND p.TZ_LOCK = 0
LEFT JOIN PYF_TIPOPODERES tp with (nolock) ON 
	a.TIPO_PODER = tp.TIPO_PODER 
	AND tp.TZ_LOCK = 0
LEFT JOIN PYF_TIPOENTIDAD te with (nolock) ON 
