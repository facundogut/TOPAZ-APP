...
INNER JOIN CONV_PADRONES AS cc2 WITH (NOLOCK)
    ON cc2.CONVENIO = (
        ...
        WHERE ID_CONVREC = (
            SELECT TOP 1 CASE WHEN l.CONVENIO_PADRE = 0 THEN l.CONVENIO ELSE l.CONVENIO_PADRE END
    ...
    AND (cc2.CONVENIO = 265 AND 
            RIGHT(''00000000'' + CAST(cc2.NRO_COMPROBANTE_CLIENTE AS varchar), 8) = SUBSTRING(d.CODIGO_BARRAS, 11, 8) AND 
            cc2.TIPO_COMPROBANTE = SUBSTRING(d.CODIGO_BARRAS, 4, 2) AND 
            cc2.LETRA = CASE 
                            WHEN SUBSTRING(d.CODIGO_BARRAS, 6, 1) = ''0'' THEN ''B'' 
                            ELSE ''A'' 
                        END AND 
            cc2.PUNTO_VENTA = SUBSTRING(d.CODIGO_BARRAS, 7, 4)
        )
...
INNER JOIN CONV_PADRONES AS cc2 WITH (NOLOCK)
    ...
    AND cc2.TOTAL_CARGO_ESPECIFICO IS NOT NULL
    AND 
        (cc2.CONVENIO = 406 AND 
            RIGHT(''00000000'' + CAST(cc2.NRO_COMPROBANTE_CLIENTE AS varchar), 8) = SUBSTRING(d.CODIGO_BARRAS, 9, 8) AND 
            cc2.LETRA = CASE 
                            WHEN SUBSTRING(d.CODIGO_BARRAS, 4, 1) = ''0'' THEN ''B'' 
                            WHEN SUBSTRING(d.CODIGO_BARRAS, 4, 1) = ''1'' THEN ''A'' 
                            ELSE ''Y'' 
                        END AND 
            cc2.PUNTO_VENTA = SUBSTRING(d.CODIGO_BARRAS, 5, 4)
        )
