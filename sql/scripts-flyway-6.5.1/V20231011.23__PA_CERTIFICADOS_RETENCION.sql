EXECUTE('
ALTER PROCEDURE PA_CERTIFICADOS_RETENCION
...
BEGIN
	...
	BEGIN
		...
		    FL.MONTO_IMPONIBLE,
		    FL.IMPORTEMN,
		    CASE WHEN FL.IMPORTEMN/FL.MONTO_IMPONIBLE>0.05 THEN 10 ELSE 3 END AS ALICUOTA,
		...
		INNER JOIN SALDOS AS S WITH(nolock) ON
			...
			AND EXISTS (
					SELECT *
					FROM GRL_DETALLE_CONTABILIDAD AS DC WITH(nolock)
					WHERE
						((DC.TZ_LOCK < 100000000000000 OR DC.TZ_LOCK >= 200000000000000)
							AND (DC.TZ_LOCK < 300000000000000 OR DC.TZ_LOCK >= 400000000000000)  
						)
						AND DC.SALDO_ORIG_SUCURSAL = S.SUCURSAL
						AND DC.SALDO_ORIG_PRODUCTO = S.PRODUCTO
						AND DC.SALDO_ORIG_MONEDA = S.MONEDA
						AND DC.SALDO_ORIG_CUENTA = S.CUENTA
						AND DC.SALDO_ORIG_OPERACION = S.OPERACION
						AND DC.SALDO_ORIG_ORDINAL = S.ORDINAL
						AND DC.MOV_ASIENTO = D.MOV_ASIENTO
						AND DC.MOV_SUCURSAL = D.MOV_SUCURSAL
						AND DC.MOV_FECHA_PROCESO = D.MOV_FECHA_PROCESO
						AND DC.SALDO_ORIG_PRODUCTO = 
							CASE WHEN EXISTS (
								SELECT *
								FROM GRL_DETALLE_CONTABILIDAD AS GD WITH(nolock)
								INNER JOIN PRODUCTOS AS P WITH(nolock) ON
									GD.SALDO_ORIG_PRODUCTO = P.C6250
									AND P.C6252 IN (4)
									AND ((P.TZ_LOCK < 100000000000000 OR P.TZ_LOCK >= 200000000000000)
										AND (P.TZ_LOCK < 300000000000000 OR P.TZ_LOCK >= 400000000000000)  
									)
								WHERE
									GD.MOV_ASIENTO = DC.MOV_ASIENTO
									AND GD.MOV_SUCURSAL = DC.MOV_SUCURSAL
									AND GD.MOV_FECHA_PROCESO = DC.MOV_FECHA_PROCESO
									AND ((GD.TZ_LOCK < 100000000000000 OR GD.TZ_LOCK >= 200000000000000)
										AND (GD.TZ_LOCK < 300000000000000 OR GD.TZ_LOCK >= 400000000000000)  
									)
							) THEN (
								SELECT TOP 1 SALDO_ORIG_PRODUCTO
								FROM GRL_DETALLE_CONTABILIDAD AS GD WITH(nolock)
								INNER JOIN PRODUCTOS AS P WITH(nolock) ON
									GD.SALDO_ORIG_PRODUCTO = P.C6250
									AND P.C6252 IN (4)
								...
								WHERE 
									MOV_ASIENTO = DC.MOV_ASIENTO
									AND MOV_SUCURSAL = DC.MOV_SUCURSAL
									AND MOV_FECHA_PROCESO = DC.MOV_FECHA_PROCESO
									AND ((GD.TZ_LOCK < 100000000000000 OR GD.TZ_LOCK >= 200000000000000)
										AND (GD.TZ_LOCK < 300000000000000 OR GD.TZ_LOCK >= 400000000000000)  
									)
							) ELSE (
