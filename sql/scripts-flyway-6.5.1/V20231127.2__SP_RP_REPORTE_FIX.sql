...
SELECT DISTINCT
    ...
    FROM PARAMETROS p,
    REC_CAB_RECAUDOS_CAJA (NOLOCK) cab JOIN
    REC_LIQUIDACION (NOLOCK) liq ON liq.ID_LIQUIDACION = cab.ID_LIQUIDACION JOIN 
    REC_RENDICION (NOLOCK) ren ON ren.ID_RENDICION = liq.ID_RENDICION JOIN
    CONV_CONVENIOS_REC (NOLOCK) conv ON conv.Id_ConvRec = ren.CONVENIO OR conv.Id_ConvRec = ren.CONVENIO_PADRE LEFT JOIN 
    CONV_REL_ENTECONV (NOLOCK) relEnte ON conv.Id_ConvRec = relEnte.ID_CONVREC JOIN
    SALDOS (NOLOCK) s ON s.JTS_OID = conv.CuentaRec 
...
SELECT DISTINCT
    ...
    FROM PARAMETROS p,
    REC_CAB_RECAUDOS_CANAL (NOLOCK) cab JOIN
    REC_LIQUIDACION (NOLOCK) liq ON liq.ID_LIQUIDACION = cab.ID_LIQUIDACION JOIN 
    REC_RENDICION (NOLOCK) ren ON ren.ID_RENDICION = liq.ID_RENDICION JOIN    
    CONV_CONVENIOS_REC (NOLOCK) conv ON conv.Id_ConvRec = ren.CONVENIO OR conv.Id_ConvRec = ren.CONVENIO_PADRE LEFT JOIN 
    CONV_REL_ENTECONV (NOLOCK) relEnte ON conv.Id_ConvRec = relEnte.ID_CONVREC JOIN 
