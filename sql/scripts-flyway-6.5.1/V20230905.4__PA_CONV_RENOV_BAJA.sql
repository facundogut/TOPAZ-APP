...
EXECUTE('
CREATE OR ALTER PROCEDURE [dbo].[PA_CONV_RENOV_BAJA]
...
BEGIN
	...
	BEGIN TRY
		...
		UPDATE R 
		SET 
			R.FecVto = DATEADD(MM,T.PLAZO,T.FECHA_VTO), 
			R.FecUltAct = @FECHA
		FROM
			CONV_CONVENIOS_REC AS R 
		...
		SELECT 
			...
			@P_SUCURSAL,
			@P_CLAVE,
			CONCAT(''Se renueva automáticamente el convenio: '',T.ID_CONVENIO_REC,'' con fecha:'',DATEADD(MM,T.PLAZO,T.FECHA_VTO)),
			0
		...
		SELECT 
			...
			@P_SUCURSAL,
			@P_CLAVE,
			CONCAT(''Se da de baja el convenio: '',T.ID_CONVENIO_REC,'' por fecha de Vencimiento''),
			0
		FROM @TablaAuxiliar AS T
