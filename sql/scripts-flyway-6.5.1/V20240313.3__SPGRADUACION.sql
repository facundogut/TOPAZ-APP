...
----------------VARIABLES GENERALES--------------------------------------------------------------------------------------------- 
	...
	IF(@ES_VINCULADO = 0 OR @ES_VINCULADO IS NULL)
		BEGIN
			...
			SET @RPC_CLIENTE = ISNULL((SELECT TOP 1 RPC FROM CRE_TAM_EMP_BALANCE WITH(NOLOCK) WHERE TZ_LOCK = 0 AND ID_PERSONA = @PERSONA AND F_FINAL >= (SELECT FECHAPROCESO FROM PARAMETROS WITH(NOLOCK)) ORDER BY F_FINAL ASC),0)
...
--RESULTADO ANALISIS
	IF(@TIENE_GRUPO=0 OR @TIENE_GRUPO IS NULL)
		--GRADUACION
		BEGIN 		
			IF (@TOPE_ANALISIS>@TOT_CONTROL_GRAD)
				BEGIN
