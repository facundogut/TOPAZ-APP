execute('
CREATE or alter PROCEDURE [dbo].[PKG_DEPURACION_SEGURIDAD$proc_dep_periodica]
...
BEGIN TRY
	...
	SELECT @v_meses = PERIODO FROM PARAMETROS_DEPURACION WHERE MODULO = ''SEGURIDAD'' AND ID = ''BITACORAS'' AND TZ_LOCK = 0;		
	SET @v_fecha = DATEADD(dd, - DATEPART(dd, dateadd(mm, -1 * @v_meses, @p_dt_proceso)) + 1, dateadd(mm, -1 * @v_meses, @p_dt_proceso)); 
	
	----------------------------------------------------------------------------------
	SET @v_tabla = ''BITACORA''
	...
	SET @v_stmt = ''ALTER TABLE '' + @v_old_name + '' DROP COLUMN NUMERADOR; ALTER TABLE '' + @v_old_name + '' ADD NUMERADOR INT;''
	EXECUTE (@v_stmt);
	
	SET @v_where = ''BITACORA.FECHAHORA < @fecha''				
	EXECUTE PKG_DEPURACION$proc_dep_periodica @p_id_proceso,
													@p_dt_proceso,
													@v_tabla,
													''PKG_DEPURACION_SEGURIDAD$proc_dep_periodica'',
													'''',
													@v_where,
													''FECHAHORA'',
													@v_fecha,
	...
	-----------------------------------------------------------------------------------
	SET @v_tabla = ''BITACORA_RESTRICCIONES''
	SET @v_old_name = SUBSTRING(@v_tabla, 0, 18) + ''_OLD_'' + replace(convert(VARCHAR,getdate(), 104), ''.'' , '''');
	SET @v_stmt = ''SELECT * INTO '' + @v_old_name + '' FROM '' + @v_tabla + '' with (nolock) '' + '' WHERE 1 = 0'';
	print ''Creando tabla de BACKUP '' + @v_old_name;
	EXECUTE(@v_stmt);	
	
	SET @v_where = ''BITACORA_RESTRICCIONES.FECHA_HORA_MODIFICADO < @fecha''
	EXECUTE PKG_DEPURACION$proc_dep_periodica @p_id_proceso,
													@p_dt_proceso,
													@v_tabla,
													''PKG_DEPURACION_SEGURIDAD$proc_dep_periodica'',
	...
	-----------------------------------------------------------------------------------
	SET @v_tabla = ''BITACORA_USUARIOS''
	SET @v_old_name = SUBSTRING(@v_tabla, 0, 18) + ''_OLD_'' + replace(convert(VARCHAR,getdate(), 104), ''.'' , '''');
	SET @v_stmt = ''SELECT * INTO '' + @v_old_name + '' FROM '' + @v_tabla + '' with (nolock) '' + '' WHERE 1 = 0'';
	print ''Creando tabla de BACKUP '' + @v_old_name;
	EXECUTE(@v_stmt);	
	
	SET @v_where = ''BITACORA_USUARIOS.FECHA_HORA_MODIFICADO < @fecha''
	EXECUTE PKG_DEPURACION$proc_dep_periodica @p_id_proceso,
													@p_dt_proceso,
													@v_tabla,
													''PKG_DEPURACION_SEGURIDAD$proc_dep_periodica'',
	...
	-----------------------------------------------------------------------------------
	SET @v_tabla = ''BITACORA_GRUPOS''
	SET @v_old_name = SUBSTRING(@v_tabla, 0, 18) + ''_OLD_'' + replace(convert(VARCHAR,getdate(), 104), ''.'' , '''');
	SET @v_stmt = ''SELECT * INTO '' + @v_old_name + '' FROM '' + @v_tabla + '' with (nolock) '' + '' WHERE 1 = 0'';
	print ''Creando tabla de BACKUP '' + @v_old_name;
	EXECUTE(@v_stmt);	
	
	SET @v_where = ''BITACORA_GRUPOS.FECHA_HORA_MODIFICADO < @fecha''
	EXECUTE PKG_DEPURACION$proc_dep_periodica @p_id_proceso,
													@p_dt_proceso,
													@v_tabla,
													''PKG_DEPURACION_SEGURIDAD$proc_dep_periodica'',
...
BEGIN CATCH
    ...
    SET @p_msg_proceso = ''El proceso PKG_DEPURACION_SEGURIDAD$proc_dep_periodica produjo un error. Error original: '' + ERROR_MESSAGE() + '' Linea: '' + CAST(ERROR_LINE() AS VARCHAR(50));	
