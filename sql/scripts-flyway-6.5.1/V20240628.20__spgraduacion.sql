	...
	IF(@TIENE_GRUPO=0 OR @TIENE_GRUPO IS NULL)
		...
		BEGIN 		
			...
			ELSE IF(@LIMITE_GRAD<@TOT_CONTROL_GRAD)
				BEGIN
					SET @RESULTADO_GRADUACION = ''Exceso en Graduación: ''+ CONVERT(VARCHAR(17),FORMAT(ROUND(@TOT_CONTROL_GRAD-@LIMITE_GRAD,2), ''C'', ''es-ar'')) + ''.'';;
		...
		--GRANDES EXPOSICIONES
			...
			ELSE IF (@TRAMO_SIN_G_B>=@TRAMO_SIN_G AND @TRAMO_CON_G_B<(@TRAMO_CON_G+@TRAMO_SIN_G) )
				BEGIN
					SET @RESULTADO_FRACCIONAMIENTO = ''Encuadrado en Grandes Exposiciones sin garantía.'' +char(10) + char(13) + ''Exceso en Grandes Exposiciones con y sin garantía: '' + CONVERT(VARCHAR(17),FORMAT(ROUND((@TRAMO_CON_G+@TRAMO_SIN_G)-@TRAMO_CON_G_B,2), ''C'', ''es-ar'')) + ''.'';
					SET @EXCEDE_FRACCIONAMIENTO_O = ''S'';
				END	
			ELSE IF (@TRAMO_SIN_G_B<@TRAMO_SIN_G AND @TRAMO_CON_G_B>=(@TRAMO_CON_G+@TRAMO_SIN_G) )
				BEGIN
					SET @RESULTADO_FRACCIONAMIENTO = ''Exceso en Grandes Exposiciones sin garantía: '' + CONVERT(VARCHAR(17),FORMAT(ROUND(@TRAMO_SIN_G-@TRAMO_SIN_G_B,2), ''C'', ''es-ar'')) + ''.'' +char(10) + char(13) + ''Encuadrado en Grandes Exposiciones con y sin garantía. '';
					SET @EXCEDE_FRACCIONAMIENTO_O = ''S'';
				END
			ELSE IF (@TRAMO_SIN_G_B<@TRAMO_SIN_G AND @TRAMO_CON_G_B<(@TRAMO_CON_G+@TRAMO_SIN_G) )
				BEGIN
					SET @RESULTADO_FRACCIONAMIENTO = ''Exceso en Grandes Exposiciones sin garantía: '' + CONVERT(VARCHAR(17),FORMAT(ROUND(@TRAMO_SIN_G-@TRAMO_SIN_G_B,2), ''C'', ''es-ar'')) + ''.'' +char(10) + char(13) + ''Exceso en Grandes Exposiciones con y sin garantía: '' + CONVERT(VARCHAR(17),FORMAT(ROUND((@TRAMO_CON_G+@TRAMO_SIN_G)-@TRAMO_CON_G_B,2), ''C'', ''es-ar'')) + ''.'';
					SET @EXCEDE_FRACCIONAMIENTO_O = ''S'';
				END										
 		END
