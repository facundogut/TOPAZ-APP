...
--		PRINT LEFT(@IdClientePegador,11)
		  ...
		  IF (
			...
			len(ltrim(rtrim(@ReservadoLoteCeros)))=0 or 
			...
			isnumeric(ltrim(rtrim(@ContadorRegistros)))=0 OR
			@valMandFL=0 OR 
			@valMandFA=0
			)
		  	BEGIN
		  	INSERT INTO dbo.VTA_TRANSFERENCIAS (OP_NUMERO, NUMERO_ASIENTO,  OP_FECHA_PRES, OP_CLASE_TRANS, ESTADO, OP_MONEDA, OP_TIPO, BEN_CBU, OP_IMPORTE, BEN_JTS_OID_CUENTA, BEN_NRO_DOC, MOTIVO_RECHAZO, OP_FORMA_ING, FECHA_ESTADO, CONTABILIZADA, OP_INFO_REF,OPERATORIA_BCRA )
				VALUES ((SELECT ISNULL(MAX(OP_NUMERO),0)+1 FROM VTA_TRANSFERENCIAS WHERE OP_CLASE_TRANS=''R''),@TICKET, @FECHADATE, ''R'', ''RC'', @Moneda, @Tipo_transferencia, @CBU, @VImporte, @JTS_OID, @CUIL,(SELECT top 1 CODIGO_NACHA FROM CLE_TIPO_CAUSAL WHERE CODIGO_DE_CAUSAL = 39), ''C'' , @FECHADATE, ''N'', RTRIM(LTRIM(@ReferenciaUnivoca)),@OPERATORIA_BCRA);
	      	GOTO Final
		  	END 
		  ...
		  ----NACHA R03
		  --------------
		   	IF RIGHT(@CBU,14)=''3110001211000841057072''
			PRINT @CBU
...
IF (LEFT(@IDCLIENTEPEGADOR,1) IN (''1'',''2'',''3'') AND LEN(REPLACE(LEFT(@IDCLIENTEPEGADOR,11),'' '',''''))=11)
	SET @cuit=LEFT(@IDCLIENTEPEGADOR,11)
ELSE
begin 
...
END
		  ...
		  BEGIN
	      		INSERT INTO dbo.VTA_TRANSFERENCIAS (OP_NUMERO, NUMERO_ASIENTO,  OP_FECHA_PRES, OP_CLASE_TRANS, ESTADO, OP_MONEDA, OP_TIPO, BEN_CBU, OP_IMPORTE, BEN_JTS_OID_CUENTA, BEN_NRO_DOC, MOTIVO_RECHAZO, OP_FORMA_ING, FECHA_ESTADO, CONTABILIZADA, OP_INFO_REF,OPERATORIA_BCRA )
				VALUES ((SELECT ISNULL(MAX(OP_NUMERO),0)+1 FROM VTA_TRANSFERENCIAS WHERE OP_CLASE_TRANS=''R''),@TICKET, @FECHADATE, ''R'', ''RC'', @Moneda, @Tipo_transferencia, @CBU, @VImporte, @JTS_OID, @CUIL,40, ''C'' , @FECHADATE, ''N'', RTRIM(LTRIM(@ReferenciaUnivoca)),@OPERATORIA_BCRA);
	      	GOTO Final
	      END
...
--		  IF (convert(NUMERIC(1,0),RIGHT(@CUIT_EO,1))<>@codigoOrigen)
		  IF (@cuit_eo NOT IN (SELECT CUIT_EO FROM SNP_PRESTACIONES_EMPRESAS WHERE ENTIDAD = @IdEntidadOrigen))
	      BEGIN
	      		INSERT INTO dbo.VTA_TRANSFERENCIAS (OP_NUMERO, NUMERO_ASIENTO,  OP_FECHA_PRES, OP_CLASE_TRANS, ESTADO, OP_MONEDA, OP_TIPO, BEN_CBU, OP_IMPORTE, BEN_JTS_OID_CUENTA, BEN_NRO_DOC, MOTIVO_RECHAZO, OP_FORMA_ING, FECHA_ESTADO, CONTABILIZADA, OP_INFO_REF,OPERATORIA_BCRA )
	     ...
	     PRINT substring(@IdClientePegador,1,11) 
	      --------------
	      ----NACHA R91
	      --------------
...
--		SELECT TOP 1 numeroPersona FROM VW_CLIENTES_PERSONAS WHERE NUMERODOC=substring(@IdClientePegador,2,11)
